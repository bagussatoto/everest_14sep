...
//            matiHere("exectime :: $exectime detik");
            ...
            $arrPaymentSummary = array();
            $allPayTransIDs = array();
            $arrAllRowStepIDs = array();

            // 1. Kumpulkan seluruh id_top dari baris-baris di halaman ini
            $allTopIDs = array();
            foreach ($tmpHist[$keyPage] as $hRow) {
                if (isset($hRow->id_top) && $hRow->id_top > 0) {
                    $allTopIDs[] = $hRow->id_top;
                } elseif (isset($hRow->id) && $hRow->id > 0) {
                    $allTopIDs[] = $hRow->id;
                }
            }
            $allTopIDs = array_unique(array_filter($allTopIDs));

            // 2. Kueri seluruh ID transaksi yang satu alur/rantai (berbagi id_top yang sama)
            $chainTrans = array();
            if (sizeof($allTopIDs) > 0) {
                $this->db->select("id, id_top");
                $this->db->group_start();
                $this->db->where_in("id_top", $allTopIDs);
                $this->db->or_where_in("id", $allTopIDs);
                $this->db->group_end();
                $this->db->where("trash", 0);
                $chainTrans = $this->db->get("transaksi")->result();
            }

            // 3. Petakan seluruh ID rantai ke masing-masing baris history
            foreach ($tmpHist[$keyPage] as $hRow) {
                $rowTopID = (isset($hRow->id_top) && $hRow->id_top > 0) ? $hRow->id_top : (isset($hRow->id) ? $hRow->id : 0);

                if (isset($hRow->id) && $hRow->id > 0) {
                    $allPayTransIDs[] = $hRow->id;
                    $arrAllRowStepIDs[$hRow->id][] = $hRow->id;
                }
                if ($rowTopID > 0) {
                    $allPayTransIDs[] = $rowTopID;
                    $arrAllRowStepIDs[$hRow->id][] = $rowTopID;
                }

                if (sizeof($chainTrans) > 0) {
                    foreach ($chainTrans as $cRow) {
                        if ($cRow->id_top == $rowTopID || $cRow->id == $rowTopID) {
                            $allPayTransIDs[] = $cRow->id;
                            $arrAllRowStepIDs[$hRow->id][] = $cRow->id;
                }
                    }
                }

                if (isset($hRow->ids_his) && $hRow->ids_his != "") {
                    $hHist = blobDecode($hRow->ids_his);
                    if (is_array($hHist) && sizeof($hHist) > 0) {
                        foreach ($hHist as $hSpec) {
                            if (isset($hSpec['trID']) && $hSpec['trID'] > 0) {
                                $allPayTransIDs[] = $hSpec['trID'];
                                $arrAllRowStepIDs[$hRow->id][] = $hSpec['trID'];
                            }
                        }
                    }
                }

                if (isset($arrAllRowStepIDs[$hRow->id])) {
                    $arrAllRowStepIDs[$hRow->id] = array_unique($arrAllRowStepIDs[$hRow->id]);
            }
            }

            $allPayTransIDs = array_unique(array_filter($allPayTransIDs));
            if (sizeof($allPayTransIDs) > 0) {
                $this->db->select("transaksi_id, tagihan, terbayar, sisa");
                $this->db->where_in("transaksi_id", $allPayTransIDs);
                $this->db->where("dihapus", 0);
                $tmpPay = $this->db->get("transaksi_payment_source")->result();
                if (sizeof($tmpPay) > 0) {
                    foreach ($tmpPay as $pRow) {
                        $arrPaymentSummary[$pRow->transaksi_id] = array(
                            "tagihan"  => floatval($pRow->tagihan),
                            "terbayar" => floatval($pRow->terbayar),
                            "sisa"     => floatval($pRow->sisa),
                        );
                    }
                }
            }
...
//                                $kolomValues = isset($row->$fName) ? $row->$fName : "-";
                        ...
                        if ($fName == "jenis_master") {
                            $jCode = isset($row->jenis_master) ? $row->jenis_master : "";
                            if ($jCode == "5822" || $jCode == "582") {
                                $colValue = "Reguler";
                            } elseif ($jCode == "5823") {
                                $colValue = "POS";
                            } elseif ($jCode == "584") {
                        ...
                        } elseif ($fName == "status_bayar") {
                            $totTagihan  = 0;
                            $totTerbayar = 0;
                            $totSisa     = 0;

                            if (isset($arrAllRowStepIDs[$row->id])) {
                                foreach ($arrAllRowStepIDs[$row->id] as $stepTransID) {
                                    if (isset($arrPaymentSummary[$stepTransID])) {
                                        $totTagihan  += $arrPaymentSummary[$stepTransID]['tagihan'];
                                        $totTerbayar += $arrPaymentSummary[$stepTransID]['terbayar'];
                                        $totSisa     += $arrPaymentSummary[$stepTransID]['sisa'];
                                    }
                                }
                            }

                            if ($totTagihan == 0) {
                                if (isset($arrPaymentSummary[$row->id])) {
                                    $totTagihan  += $arrPaymentSummary[$row->id]['tagihan'];
                                    $totTerbayar += $arrPaymentSummary[$row->id]['terbayar'];
                                    $totSisa     += $arrPaymentSummary[$row->id]['sisa'];
                                } elseif (isset($arrPaymentSummary[$row->id_top])) {
                                    $totTagihan  += $arrPaymentSummary[$row->id_top]['tagihan'];
                                    $totTerbayar += $arrPaymentSummary[$row->id_top]['terbayar'];
                                    $totSisa     += $arrPaymentSummary[$row->id_top]['sisa'];
                                }
                            }

                            // START OF COMPLETE REPEATED LOGIC
                            $this->load->helper("he_finansial");
                            $payStatus = fin_evaluate_payment_status(array(
                                "tagihan"  => $totTagihan,
                                "terbayar" => $totTerbayar,
                                "sisa"     => $totSisa,
                            ), 1000.0);
                            $colValue = $payStatus['badge_sm'];
