...
class _selectorPihak extends Modul_Controller
{
    ...
    public function selectPihak()
    {
        ...
        //        cekHitam("$pihakView");
        if (sizeof($tmpO) > 0) {
            if ((sizeof($tmpO) == 1) && (is_numeric($key))) {
                //                cekHere($processor);
                echo "<script>";
                echo "top.$('#result').load('$processor?id=" . $tmpO[0]->id . "');";
                echo "</script>";
            }
            foreach ($tmpO as $row) {
                // arrPrint($row);
                ...
                if (strlen($tmpName) > 1) {
                    ...
                    $items[] = array(
                        ...
                        "kategori_nama" => isset($row->kategori_nama) ? $row->kategori_nama : "reguler",
                        "tlp_1" => isset($row->tlp_1) ? $row->tlp_1 : "",
        ...
        // arrPrint($getFields);
        foreach ($getFields as $fky => $getField) {
            $fKol = $getField['kolom'];
            $finputType = $getField['inputType'];
            $flabel = $getField['label'];

            if ($finputType != "hidden") {
                if (array_key_exists($fKol, $listedFieldsSelectItem)) {

                    #cekHijau($flabel);
                    $kolomToSearch[$fKol] = $flabel;
                }
            }
        }
        // arrPrint($kolomToSearch);

        $data = array(
            "mode"  => "view",
            "items" => $items,
            "kolomToSearch"  => $kolomToSearch,
        );
    ...
    public function selectPihak2()
    {
        ...
        $processor = base_url() . $this->configUi[$jenisTr]['pihakProcessor2'] . "/" . "$jenisTr/" . $this->configUi[$jenisTr]['pihakModel2'];
        if (sizeof($tmpO) > 0) {
            foreach ($tmpO as $row) {
    ...
    public function selectPihak3()
    {
        ...
        $processor = base_url() . $this->configUi[$jenisTr]['pihakProcessor3'] . "/" . "$jenisTr/" . $this->configUi[$jenisTr]['pihakModel3'];
        if (sizeof($tmpO) > 0) {
            foreach ($tmpO as $row) {
