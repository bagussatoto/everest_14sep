...
class Transaksi extends Modul_Controller
{
    ...
    public function viewResume()
    {
        ...
        if (sizeof($tmpEP) > 0) {
            foreach ($tmpEP as $ii => $row) {
                ...
                if (empty($row->deskripsi)) {
                    ...
                    if (isset($row->nomer2) && $row->nomer2 != "") {
                        $cancelledNomers[] = $row->nomer2;
                        $nomerCleanEx = explode("-", $row->nomer2);
                        $cancelledNomers[] = $nomerCleanEx[0];
                    }
                    if (isset($main["referenceID"]) && $main["referenceID"] > 0) {
                        $cancelledIds[$main["referenceID"]] = $main["referenceID"];
                    }
                    if (isset($main["referenceID__4"]) && $main["referenceID__4"] > 0) {
                        $cancelledIds[$main["referenceID__4"]] = $main["referenceID__4"];
                    }
                    if (isset($main["transaksi"]) && $main["transaksi"] > 0) {
                        $cancelledIds[$main["transaksi"]] = $main["transaksi"];
                    }
                    if (isset($main["currentID"]) && $main["currentID"] > 0) {
                        $cancelledIds[$main["currentID"]] = $main["currentID"];
                    }
                    if (isset($row->link_id) && $row->link_id > 0) {
                        $cancelledIds[$row->link_id] = $row->link_id;
                    }
                    $pakai_ini = 0;
                    if ($pakai_ini == 1) {
                        foreach ($main as $m_key => $mval) {
        ...
        if (sizeof($cancelledNomers) > 0) {
            $this->db->select("id");
            $this->db->from("transaksi");
            $this->db->group_start();
            $this->db->where_in("nomer", $cancelledNomers);
            $this->db->or_where_in("nomer2", $cancelledNomers);
            $this->db->group_end();
            $qCancel = $this->db->get()->result();
            if (sizeof($qCancel) > 0) {
                foreach ($qCancel as $qc) {
                    $cancelledIds[$qc->id] = $qc->id;
                }
            }
        }

        if (sizeof($cancelledIds) > 0) {
            foreach ($cancelledIds as $cId) {
                if (isset($terimaAr[$cId])) {
                    unset($terimaAr[$cId]);
                }
                if (isset($inputFaktur[$cId])) {
                    unset($inputFaktur[$cId]);
                }
            }
        }
        ...
        if (sizeof($terimaAr) > 0) {
            ...
            showLast_query("pink");
            foreach ($trepTmp as $row) {
                $subBayar = isset($row->produk_ord_hrg) ? (float)$row->produk_ord_hrg * (isset($row->produk_ord_jml) && (float)$row->produk_ord_jml > 0 ? (float)$row->produk_ord_jml : 1) : 0;
                $totalArBayar += $subBayar;
                $jenis = $row->jenis;
                $counterDecode = blobDecode($row->counters);
                $counterjenis = $jenis . "|" . $row->cabang_id;
                $counterjenismasterID = $jenis . "|" . $row->id_master;
        ...
        //------LINK KE PENERIMAAN PENJUALAN TUNAI (KAS/FINANCE) VIA TRANSAKSI_DATA
        $totalCashBayar = 0;
        $searchIDs = $trOrigIDs;
        if ($transaksiID > 0 && !in_array($transaksiID, $searchIDs)) {
            $searchIDs[] = $transaksiID;
        }
        if ($transaksiTopID > 0 && !in_array($transaksiTopID, $searchIDs)) {
            $searchIDs[] = $transaksiTopID;
        }

        if (count($searchIDs) > 0) {
            $this->db->select("t.id, t.nomer, t.dtime, t.oleh_nama, t.cabang_nama, t.jenis, t.jenis_label, t.counters, t.deskripsi, t.jenis_master, td.produk_ord_hrg, td.produk_ord_jml");
            $this->db->from("transaksi t");
            $this->db->join("transaksi_data td", "td.transaksi_id = t.id", "inner");
            $this->db->group_start();
            ...
            if ($queryPayment && $queryPayment->num_rows() > 0) {
                foreach ($queryPayment->result() as $pRow) {
                    $cPay = isset($pRow->produk_ord_hrg) ? (float)$pRow->produk_ord_hrg * (isset($pRow->produk_ord_jml) && (float)$pRow->produk_ord_jml > 0 ? (float)$pRow->produk_ord_jml : 1) : 0;
                    $totalCashBayar += $cPay;
        ...
        if (sizeof($signArray) > 0) {
            ...
            foreach ($signArray as $sItem) {
                $itemJenis = isset($sItem['jenis']) ? $sItem['jenis'] : "";
                $itemNomer = isset($sItem['nomer_orig']) ? $sItem['nomer_orig'] : (isset($sItem['nomer']) ? $sItem['nomer'] : "");

                if ($itemJenis == '4464' || strpos($itemJenis, '4464') === 0) {
                    $hasCashPayment = true;
                }
                if ($itemJenis == '749') {
                    $hasArPayment = true;
                    $arReceiptNomer = isset($sItem['nomer']) ? $sItem['nomer'] : "";
                }
                if (strpos($itemNomer, 'INV.') !== false || (isset($sItem['title']) && stripos($sItem['title'], 'INVOICE') !== false)) {
                    $hasInvoice = true;
                }
                if (strpos($itemJenis, 'spd') !== false || (isset($sItem['title']) && stripos($sItem['title'], 'PACKING LIST') !== false)) {
                    $hasPackingList = true;
                }
            }

            // START OF COMPLETE REPEATED LOGIC
            // SOT Status Pembayaran via he_finansial_helper
            $this->load->helper("he_finansial");
            if ($masterID > 0 && !in_array($masterID, $searchIDs)) {
                $searchIDs[] = $masterID;
            }
            $payStatus = fin_get_payment_status($searchIDs, 1000.0, true);

            if ($payStatus['has_record'] && $payStatus['tagihan'] > 0) {
                if ($payStatus['is_lunas']) {
                    if ($hasCashPayment) {
                        $paymentBadge = "<span class='label label-success' style='font-size:12px; margin-left:10px;'><i class='fa fa-check-circle'></i> LUNAS (Tunai Kasir)</span>";
                    } else {
                        $paymentBadge = "<span class='label label-success' style='font-size:12px; margin-left:10px;'><i class='fa fa-check-circle'></i> LUNAS (Melalui A/R Receipt)</span>";
                    }
                } elseif ($payStatus['is_cicilan']) {
                    $terbayarFmt = number_format($payStatus['terbayar'], 0, ',', '.');
                    $sisaFmt = number_format($payStatus['sisa'], 0, ',', '.');
                    $paymentBadge = "<span class='label label-warning' style='font-size:12px; margin-left:10px; color:#ffffff; background-color:#f0ad4e; border:1px solid #eea236;'><i class='fa fa-hourglass-half'></i> PELUNASAN SEBAGIAN (Terbayar: Rp $terbayarFmt / Sisa: Rp $sisaFmt)</span>";
                } elseif ($jenisMasterTrans == '5823') {
                    $paymentBadge = "<span class='label label-warning' style='font-size:12px; margin-left:10px; color:#8a6d3b; background-color:#fcf8e3; border:1px solid #faebcc;'><i class='fa fa-clock-o'></i> BELUM MENERIMA PELUNASAN (Menunggu pembayaran Cash)</span>";
                } elseif ($hasInvoice) {
                    $paymentBadge = "<span class='label label-warning' style='font-size:12px; margin-left:10px; color:#8a6d3b; background-color:#fcf8e3; border:1px solid #faebcc;'><i class='fa fa-clock-o'></i> BELUM MENERIMA PELUNASAN (Invoice Belum Lunas)</span>";
                } elseif ($hasPackingList) {
                    $paymentBadge = "<span class='label label-warning' style='font-size:12px; margin-left:10px; color:#8a6d3b; background-color:#fcf8e3; border:1px solid #faebcc;'><i class='fa fa-clock-o'></i> BELUM MENERIMA PELUNASAN (Barang Sudah Dikirim)</span>";
                } else {
                    $paymentBadge = "<span class='label label-warning' style='font-size:12px; margin-left:10px; color:#8a6d3b; background-color:#fcf8e3; border:1px solid #faebcc;'><i class='fa fa-clock-o'></i> BELUM MENERIMA PELUNASAN (Tahap Pesanan SO)</span>";
                }
            } else {
                // Fallback untuk transaksi lama yang belum terdaftar pada transaksi_payment_source
            $tagihanGrandTotal = 0;
            if (sizeof($terimaAr) > 0) {
                $qActiveSpd = $this->db->select("id")
                    ->from("transaksi")
                    ->where_in("id", $terimaAr)
                    ->where("trash", 0)
                    ->where("trash_4", 0)
                    ->where("status_edit", 0)
                    ->get()->result();
                $activeTerimaAr = array();
                if (sizeof($qActiveSpd) > 0) {
                    foreach ($qActiveSpd as $aSpd) {
                        $activeTerimaAr[] = $aSpd->id;
                    }
                }

                if (sizeof($activeTerimaAr) > 0) {
                    $qInv = $this->db->select("SUM(produk_ord_hrg * produk_ord_jml) as total_inv")
                        ->from("transaksi_data")
                        ->where_in("transaksi_id", $activeTerimaAr)
                        ->where("trash", "0")
                        ->get()->row();
                    if ($qInv && isset($qInv->total_inv) && (float)$qInv->total_inv > 0) {
                        $tagihanGrandTotal = (float)$qInv->total_inv;
                    }
                }
            }
            if ($tagihanGrandTotal == 0) {
                if (isset($main_reg['grandTotal']) && (float)$main_reg['grandTotal'] > 0) {
                    $tagihanGrandTotal = (float)$main_reg['grandTotal'];
                }
                elseif (isset($main['grandTotal']) && (float)$main['grandTotal'] > 0) {
                    $tagihanGrandTotal = (float)$main['grandTotal'];
                }
                elseif (isset($items) && is_array($items) && sizeof($items) > 0) {
                    foreach ($items as $it) {
                        if (isset($it['subtotal']) && (float)$it['subtotal'] > 0) {
                            $tagihanGrandTotal += (float)$it['subtotal'];
                        }
                        elseif (isset($it['harga']) && isset($it['jml'])) {
                            $tagihanGrandTotal += (float)$it['harga'] * (float)$it['jml'];
                        }
                    }
                }
            }

            $totalTerbayar = $totalCashBayar + $totalArBayar;
            $sisaPiutang = $tagihanGrandTotal > $totalTerbayar ? ($tagihanGrandTotal - $totalTerbayar) : 0;

            if ($totalTerbayar >= $tagihanGrandTotal && $tagihanGrandTotal > 0) {
                if ($hasCashPayment) {
                    $paymentBadge = "<span class='label label-success' style='font-size:12px; margin-left:10px;'><i class='fa fa-check-circle'></i> LUNAS (Tunai Kasir)</span>";
                }
                else {
            ...
            elseif ($totalTerbayar > 0 && $totalTerbayar < $tagihanGrandTotal) {
                $terbayarFmt = number_format($totalTerbayar, 0, ',', '.');
                $sisaFmt = number_format($sisaPiutang, 0, ',', '.');
                $paymentBadge = "<span class='label label-warning' style='font-size:12px; margin-left:10px; color:#ffffff; background-color:#f0ad4e; border:1px solid #eea236;'><i class='fa fa-hourglass-half'></i> PELUNASAN SEBAGIAN</span>";
            }
            elseif ($hasCashPayment) {
                $paymentBadge = "<span class='label label-success' style='font-size:12px; margin-left:10px;'><i class='fa fa-check-circle'></i> LUNAS (cash)</span>";
            }
            elseif ($hasArPayment) {
                $paymentBadge = "<span class='label label-success' style='font-size:12px; margin-left:10px;'><i class='fa fa-check-circle'></i> LUNAS (Melalui A/R Receipt)</span>";
            }
            ...
            $signStr .= "<div class='panel table-responsive'>";
            $signStr .= "<h4 class='text-blue'><span class='fa fa-sign-in'></span> History Entry Points </h4>";

            $signStr .= "<h5 class='text-blue'><span class='fa fa-tasks text-uppercase'></span> $resumeTitle  $resumeName $paymentBadge</h5>";
            $signStr .= "<table class='table table-condensed table-striped table-hover-color-red dataTable'>";
            $signStr .= "<tr bgcolor='#f0f0f0'>";
            $signStr .= "<td class='text-muted'>date</td>";
            $signStr .= "<td class='text-muted'>title</td>";
