...
switch ($mode) {
    case "viewHistoryOriginal":
        ...
        if (isset($customButton)) {
            if (sizeof($customButton) > 0) {
                foreach ($customButton as $ky => $preg) {
                    ...
                    $custom_button .= "{text:'$label',className: 'excel_$k',action:function(e,dt,node,config){window.open('" . base_url() . $target . $customButtonTarget . "','download');}},";
        ...
        $content .= "<style type='text/css'>
            .excel_target{
                border: 1px solid #28dbb1 !important;
                background-image: linear-gradient(to bottom, white 0%, #98e578 100%) !important;
            }
        </style>";
...
//            $content .= ("<form name='fSelect'>");
            ...
            foreach ($arrayHistoryLabels as $key => $label) {
                // arrPrint($label);
                $content .= ("<th class='text-uppercase text-center'>");
                if (is_array($label)) {
                    $content .= ($label['label']);
                }
                else {
...
//cekHere($strBg);
        ...
        if ($allow_edit) {
            $jdlBtn = "Menutup bill editor";
            $btnAllow = "<button type='button' class='btn btn-box-tool text-red' onclick=\"btn_alert_result('$jdlBtn','membuka editor bill','$link_allow_edit');\"><i class='fa fa-legal'></i></button>";
        }
        else {
            $jdlBtn = "Membuka bill editor";
        }
        $contentStr = "";
        $contentStr .= "<div class='box box-solidddd box-info'>";
        $contentStr .= "<div class='box-header with-border'>
                            ...
                            <span class='box-tools pull-right'>$strBg</span>
                        </div>";
        $contentStr .= "<div class='box-body' style='left:0px;overflow:auto;'>";
        $contentStr .= $content;
        $contentStr .= "</div>";
    ...
    case "viewHistory":
        ...
        $content .= ("<div class=\"box-body\">");
        $link_kw = base_url() . $segmenUrl . "?$kwHref&kw=";
        $btnSearch = "";
        $btnSearch .= "<ul class=\"pagination modal-1\">
                
                <li><label class='btn btn-link'>< pencarian global</label></li>
                <li><input id=\"search_ky\" type=\"text\" name=\"kw\" value=\"$kw\" placeholder=\"Tulis kata kunci\" style='height: 31px;float: left;' onkeydown=\"\"><a href=\"javascript:void(0);\" onclick=\"location.href='$link_kw' + $('#search_ky').val();\">Terapkan Filter</a></li>
                <li><a href=\"javascript:void(0);\" onclick=\"location.href='$link_kw'\">Clear Filter</a></li>
                </ul>";

        $content .= "<div class='row'><div class='col-md-4'>$btnSearch</div> <div class='col-md-8'>$btnPage</div></div>";
        if (sizeof($arrayHistory) > 0) {
//            $content .= ("<form name='fSelect'>");
            $content .= "<div class='table-responsive table_history_$segmenUriEnc'>";
            $content .= "<table id='table_history_$segmenUriEnc' class='table display no-padding'>";
...
//cekHere($strBg);
        ...
        $contentStr = "";
        $contentStr .= "<div class='box box-solidddd box-info border-cek'>";
        $contentStr .= "<div class='box-header with-border'>
                            <span class='pull-left text-uppercase'><h4 class='no-padding no-margin'>$title</h4></span>
                            <span class='box-tools pull-right'>$strBg</span>
                        </div>";
    ...
    case "showData":
        ...
        $content .= "<h4 class='box-title'>Riwayat transaksi <span cclass='meta'>($limit terakhir)</span></h4>";
...
//                                                $reValue .= "<div>$code</div>";
                                                $reValue .= "<div>" . formatField($parentKey, $code) . "</div>";
                                            }
                                        }
                                    }
                                    elseif ($key == 'nomer_top') {
                                        ...
                                        if (isset($produkCode[$parentKey][$key])) {
                                            foreach (array_flip($produkCode[$parentKey][$key]) as $code => $vval) {
//                                                $reValue .= "<div>$code</div>";
                                                $reValue .= "<div>" . formatField("nomer", $code) . "</div>";
                                            }
                                        }
                                    }
...
//                                                    $reValue .= "<div>$produk_ord_jml_total</div>";
                                                    $reValue .= "<div>" . formatField($parentKey, $produk_ord_jml_total) . "</div>";
                                                }
                                                else {
                                                    $produk_ord_jml_total = 0;
                                                    foreach ($produk_ord_jml[$parentKey][$key][$code] as $vval2) {
...
//                                                    $reValue .= "<div>$ord_sent_qty_total</div>";
                                                    $reValue .= "<div>" . formatField($parentKey, $ord_sent_qty_total) . "</div>";
                                                }
                                                else {
                                                    $ord_sent_qty_total = 0;
                                                    foreach ($ord_sent_qty[$parentKey][$key][$code] as $vval2) {
...
//                                                    $reValue .= "<div>$valid_qty_total</div>";
                                                    $reValue .= "<div>" . formatField($parentKey, $valid_qty_total) . "</div>";
                                                }
                                                else {
                                                    $valid_qty_total = 0;
                                                    foreach ($valid_qty[$parentKey][$key][$code] as $vval2) {
...
//                                                $reValue .= "<div>$value</div>";
                                                $reValue .= "<div>" . formatField($key, $value) . "</div>";
                                            }
                                        }
                                    }
                                }
...
//                                cekBiru(":: $key ::");
                                foreach ($detailsFields[$parentKey][$key] as $value) {
                                    $content .= "<div>" . formatField($key, $value) . "</div>";
                                }
                            }
                        }
                        else {
...
//                        cekHitam(":: $key ::");
                        $content .= isset($row[$key]) ? formatField($key, $row[$key]) : "";
                    }
                    $content .= ("</td>");
                }
                $content .= ("</tr>");
