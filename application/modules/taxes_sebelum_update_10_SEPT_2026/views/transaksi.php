...
//        }
        ...
        /* --------------------------------------------------
         * Cross-Tab Search Index for case "index"
         * --------------------------------------------------*/
        $otherTabsIndex_idx = array();
        if (isset($steps) && is_array($steps)) {
            foreach ($steps as $sKey => $sVal) {
                if ((string)$sKey === (string)$stepper) {
                    continue;
                }
                $itemsInStep = isset($arrayOnprogressGroup[$sKey]) ? $arrayOnprogressGroup[$sKey] : (isset($arrayOnProgress[$sKey]) ? $arrayOnProgress[$sKey] : array());
                if (!empty($itemsInStep)) {
                    $stepLabel = isset($sVal['label']) ? strtoupper($sVal['label']) : "STEP " . $sKey;
                    $undoneLinkIndex = MODUL_PATH . $this->uri->segment(2) . "/" . $this->uri->segment(3) . "/" . $this->uri->segment(4) . "?step=$sKey";

                    $otherTabsIndex_idx[$sKey] = array(
                        'stepKey' => (string)$sKey,
                        'label' => $stepLabel,
                        'linkUrl' => $undoneLinkIndex,
                        'items' => array()
                    );
                    foreach ($itemsInStep as $itemRow) {
                        $itemText = "";
                        if (is_array($itemRow) || is_object($itemRow)) {
                        ...
                        $otherTabsIndex_idx[$sKey]['items'][] = $itemText;
                    }
                }
            }
        }
        if (isset($gunggungFieldsView) && $gunggungFieldsView == true && (string)$stepper !== 'pre_gunggungan') {
            $itemsGunggung = isset($arrayOnprogressGroupNonNpwp[1]) ? $arrayOnprogressGroupNonNpwp[1] : (isset($arrayOnProgress['pre_gunggungan']) ? $arrayOnProgress['pre_gunggungan'] : array());
            if (!empty($itemsGunggung)) {
                $undoneLinkIndex = MODUL_PATH . $this->uri->segment(2) . "/" . $this->uri->segment(3) . "/" . $this->uri->segment(4) . "?step=pre_gunggungan";

                $otherTabsIndex_idx['pre_gunggungan'] = array(
                    'stepKey' => 'pre_gunggungan',
                    'label' => 'PREPARE GUNGGUNGAN',
                    'linkUrl' => $undoneLinkIndex,
                    'items' => array()
                );
                foreach ($itemsGunggung as $itemRow) {
                    $itemText = "";
                    if (is_array($itemRow) || is_object($itemRow)) {
                        foreach ($itemRow as $k => $v) {
                            if (is_scalar($v) && !empty($v)) {
                    ...
                    $otherTabsIndex_idx['pre_gunggungan']['items'][] = $itemText;
                }
            }
        }
        $jsonOtherTabsData_idx = json_encode($otherTabsIndex_idx);
                            ...
                            initComplete: function(settings, json) {
                                ...
                                var doSearch = function(searchTerm) {
                                    ...
                                    if (recordsFiltered === 0 && searchTerm !== '') {
                                        ...
                                        var otherTabsData = " . $jsonOtherTabsData_idx . ";

                                        // Perform cross-tab match scanning
                                        var termLower = searchTerm.toLowerCase();
                                        var matchedTabs = [];
                                        if (otherTabsData && typeof otherTabsData === 'object') {
                                            $.each(otherTabsData, function(sKey, tabInfo) {
                                                if (sKey !== '$stepper' && tabInfo.items && tabInfo.items.length > 0) {
                                                    var count = 0;
                                                    for (var i = 0; i < tabInfo.items.length; i++) {
                                                        if (tabInfo.items[i].toLowerCase().indexOf(termLower) !== -1) {
                                                            count++;
                                                        }
                                                    }
                                                    if (count > 0) {
                                        ...
                                        var otherTabsNoticeHtml = '';
                                        var swalConfirmText = '<i class=\"fa fa-history\"></i> Seluruh Riwayat';
                                        var swalConfirmColor = '#3085d6';
                                        var targetTabUrl = '';

                                        if (matchedTabs.length > 0) {
                                            var firstTab = matchedTabs[0];
                                            targetTabUrl = firstTab.linkUrl + (firstTab.linkUrl.indexOf('?') !== -1 ? '&' : '?') + 'kw=' + encodeURIComponent(searchTerm);
                                            
                                            otherTabsNoticeHtml = '<div style=\"background: #e8f8f5; border: 2px solid #2ecc71; border-radius: 6px; padding: 12px; margin-bottom: 15px; text-align: left;\">' +
                                                '<div style=\"color: #27ae60; font-weight: bold; font-size: 13px; margin-bottom: 5px;\"><i class=\"fa fa-info-circle\"></i> Data Ditemukan di Tab Lain:</div>';
                                            for (var m = 0; m < matchedTabs.length; m++) {
                                                otherTabsNoticeHtml += '<div style=\"font-size: 12px; color: #2c3e50; margin-left: 10px; margin-bottom: 3px;\"><b>Tab ' + matchedTabs[m].label + '</b> (' + matchedTabs[m].count + ' data)</div>';
                                            }
                                            otherTabsNoticeHtml += '</div>';

                                            swalConfirmText = '<i class=\"fa fa-arrow-right\"></i> Buka Tab ' + firstTab.label;
                                            swalConfirmColor = '#27ae60';
                                        }
...
//        $strOnprog .= "<span class=\"btn btn-sm btn-warning pull-right\" onclick=\"viewundoneList()\"><i class='fa fa-refresh'></i> reload</h4>";
        ...
        if (sizeof($steps) > 1) {
            ...
            foreach ($steps as $tStep => $stepData) {
                ...
                $strOnprog .= "<span class='text-uppercase text-bold'>$trSelesai" . $stepData['label'] . "</span>  $isiBadge </a>";
                $strOnprog .= "</li>";
                if (($tStep == 1) && ($gunggungFieldsView == true)) {
                    $badgeNonNpwpCount = isset($nonNpwpCount) ? intval($nonNpwpCount) : (isset($arrayOnprogressGroupNonNpwp[$tStep]) ? sizeof($arrayOnprogressGroupNonNpwp[$tStep]) : 0);
                    $isiBadge = $badgeNonNpwpCount > 0 ? "<span class='badge bg-red'>" . $badgeNonNpwpCount . "</span>" : "";
                    $undoneLinkIndex = MODUL_PATH . $this->uri->segment(2) . "/" . $this->uri->segment(3) . "/" . $this->uri->segment(4) . "?step=pre_gunggungan" . $filterData;
                    $varUrl = "pre_gunggungan" == $stepper ? "javascript:void(0);" : "if($('#undoneList>div>div.box-body')){ $('#undoneList>div>div.box-body').append(`<div id='overlay'><div id='text'>Loading Content...</div></div>`); document.getElementById('overlay').style.display = 'block'; $('#undoneList').load('$undoneLinkIndex'); }else{ console.log('tidak ada container undoneList'); }";
                    $actives = "pre_gunggungan" == $stepper ? "active" : "";
                    $strOnprog .= "<li class='$actives'>";
                    $strOnprog .= "<a style='padding-top: 0;padding-bottom: 0;' class='nav-link btn' onclick=\"$varUrl\"><span class='text-uppercase text-bold'>PREPARE<br>GUNGGUNGAN</span> $isiBadge</a>";
                    $strOnprog .= "</li>";
                }
            }
            $strOnprog .= "</ul>";
        ...
        /* --------------------------------------------------
         * Cross-Tab Search Index (PHP Side)
         * --------------------------------------------------*/
        $otherTabsIndex = array();
        if (isset($steps) && is_array($steps)) {
            foreach ($steps as $sKey => $sVal) {
                if ((string)$sKey === (string)$stepper) {
                    continue;
                ...
                if (!empty($itemsInStep)) {
                    $stepLabel = isset($sVal['label']) ? strtoupper($sVal['label']) : "STEP " . $sKey;
                    $filterData = ($getDate != NULL) ? "&date=$getDate" : "";
                    $undoneLinkIndex = MODUL_PATH . $this->uri->segment(2) . "/" . $this->uri->segment(3) . "/" . $this->uri->segment(4) . "?step=$sKey" . $filterData;

                    $otherTabsIndex[$sKey] = array(
                        'stepKey' => (string)$sKey,
                        'label' => $stepLabel,
                        'linkUrl' => $undoneLinkIndex,
                        'items' => array()
                    ...
                    foreach ($itemsInStep as $itemRow) {
                        ...
                        $otherTabsIndex[$sKey]['items'][] = $itemText;
                    }
                }
            }
        }
        if (isset($gunggungFieldsView) && $gunggungFieldsView == true && (string)$stepper !== 'pre_gunggungan') {
            ...
            if (!empty($itemsGunggung)) {
                ...
                $otherTabsIndex['pre_gunggungan'] = array(
                    'stepKey' => 'pre_gunggungan',
                    'label' => 'PREPARE GUNGGUNGAN',
                    'linkUrl' => $undoneLinkIndex,
                    'items' => array()
                ...
                foreach ($itemsGunggung as $itemRow) {
                    ...
                    $otherTabsIndex['pre_gunggungan']['items'][] = $itemText;
                }
            }
        }
        $jsonOtherTabsData = json_encode($otherTabsIndex);
        $urlSearchVal = isset($_GET['search']) ? trim((string)$_GET['search']) : "";
                            ...
                            initComplete: function(settings, json) {
                                ...
                                var doSearch = function(searchTerm) {
                                    ...
                                    if (recordsFiltered === 0 && searchTerm !== '') {
                                        ...
                                        var showNotFoundModal = function(matchedTabs) {
                                            var otherTabsNoticeHtml = '';
                                            var swalConfirmText = '<i class=\"fa fa-history\"></i> Seluruh Riwayat';
                                            var swalConfirmColor = '#3085d6';
                                            var targetTabUrl = urlHistory + '?search=' + encodeURIComponent(searchTerm);
                                        ...
                                        // Deteksi Real-Time apakah data ada di Tab Lain via endpoint AJAX
                                        var checkUrl = '" . MODUL_PATH . "Transaksi/searchCrossTab/" . $this->uri->segment(4) . "';
                                        $.ajax({
                                            url: checkUrl,
                                            type: 'POST',
                                            dataType: 'json',
                                            ...
                                            success: function(resp) {
                                                console.log('Cross-Tab Search Result:', resp);
                                                var matches = (resp && resp.status === 'success' && resp.matches) ? resp.matches : [];
                                                showNotFoundModal(matches);
                                            },
                                            error: function(xhr, status, err) {
                                                console.error('Cross-Tab Search Error:', status, err);
                                                showNotFoundModal([]);
                                            }
                                        });
                                    }
                                ...
                                // Auto-apply search parameter if passed from cross-tab redirect
                                var initSearchVal = " . json_encode($urlSearchVal) . ";
                                if (initSearchVal && initSearchVal !== '') {
                                    searchInput.val(initSearchVal);
                                    api.search(initSearchVal).draw();
                                }
                            },
                            fixedHeader: true,
                            lengthMenu: [ [10, 20, 50, 100, -1], [10, 20, 50, 100, 'All'] ],
...
//                                    ],
            ...
            if ($stepper == "pre_gunggungan") {
                /* --------------------------------------------------
                 * Cross-Tab Search Index for pre_gunggungan
                 * --------------------------------------------------*/
                $otherTabsIndex_gunggung = array();
                if (isset($steps) && is_array($steps)) {
                    foreach ($steps as $sKey => $sVal) {
                        $itemsInStep = isset($arrayOnprogressGroup[$sKey]) ? $arrayOnprogressGroup[$sKey] : (isset($arrayOnProgress[$sKey]) ? $arrayOnProgress[$sKey] : (isset($arrayOnprogress[$sKey]) ? $arrayOnprogress[$sKey] : array()));
                        if (!empty($itemsInStep)) {
                            $stepLabel = isset($sVal['label']) ? strtoupper($sVal['label']) : "STEP " . $sKey;
                            $filterData = ($getDate != NULL) ? "&date=$getDate" : "";
                            $undoneLinkIndex = MODUL_PATH . $this->uri->segment(2) . "/" . $this->uri->segment(3) . "/" . $this->uri->segment(4) . "?step=$sKey" . $filterData;

                            $otherTabsIndex_gunggung[$sKey] = array(
                                'stepKey' => (string)$sKey,
                                'label' => $stepLabel,
                                'linkUrl' => $undoneLinkIndex,
                                'items' => array()
                            ...
                            foreach ($itemsInStep as $itemRow) {
                                ...
                                $otherTabsIndex_gunggung[$sKey]['items'][] = $itemText;
                            }
                        }
                    }
                }
                $jsonOtherTabsData_gunggung = json_encode($otherTabsIndex_gunggung);

                cekKuning(current_url());
                ...
                $limit = 100;
                $segmenAsli = $segmen = url_segment();
                $segLima = $segmen[5]; // limit
                $segEnam = $segmen[6];
                $segTambahan = "";
                if (isset($segLima)) {
                    $segTambahan = "/$segLima/$segEnam";
                    $limit = $segLima;
                }
                // unset($segmen[3]);
                unset($segmen[5]);
                ...
                $geterTambahan = "";
                if (isset($_GET['date'])) {
                    $geterTambahan = "&date=$getDateMonth";
                }
                if (isset($_GET['kw'])) {
                    $geterTambahan = "&kw=$kw";
                }
                ...
                $link_termuda = base_url() . $segmenUrlMuda . "?$xtraHref";
                $link_reset = base_url() . $segmenUrl . "?$xtraHref";
                // cekHijau("$link_tertua");
                // cekHijau("$link_termuda");
                if ($urutanDb == "asc") {
                    $btn .= "<li><span class='current'>$limit Tertua</span></li>";
                }
                else {
                ...
                if (!isset($segLima)) {
                    $btn .= "<li><span class='current'>All</span></li>";
                }
                else {
                    $btn .= "<li><a href='javascript:void(0);' onclick=\"loadPaging('$link_reset','undoneList')\">All</a></li>";
                }
                ...
                $strOnprog .= "<div class='table-responsive'>";
                $strOnprog .= "<table id='$stepper' class='table dataTable compact table-hover'>";

                $strOnprog .= "<thead>";
                $strOnprog .= "<tr class='text-capitalize' line=" . __LINE__ . ">";
                if (sizeof($arrayProgressLabels) > 0) {
...
//                $undoneLinkIndex = MODUL_PATH . $this->uri->segment(2) . "/viewGunggunganIndex/" . $this->uri->segment(4) . "$segTambahan" . "?step=1&debuger=0" . $geterTambahan;
                cekMerah($undoneLinkIndex);
                ...
                $strOnprog .= "<script>
                                    ...
                                    var hideNpwp = 1;
//                                    $(document).ready( function(){
                                        var table = $('#$stepper').DataTable({
//                                            dom: 'lBpfr<\"toolbar\">tip',
                                            dom: 'lBpfrtip',
                                            initComplete: function(settings, json) {
                                                var api = this.api();
                                                var searchInput = $(api.table().container()).find('.dataTables_filter input');
                                                ...
                                                // Auto-apply search parameter if passed from cross-tab redirect
                                                var initSearchVal = " . json_encode(isset($_GET['search']) ? trim($_GET['search']) : '') . ";
                                                if (initSearchVal && initSearchVal !== '') {
                                                    searchInput.val(initSearchVal);
                                                    api.search(initSearchVal).draw();
                                                }
...
//                                            },
                                            ...
                                            },
                                            {
                                                ...
                                                action: function (e, dt, node, config) {
                                                    ...
                                                    if( items_check.length > 10 ){
                                                        ...
                                                        console.log('sini di bawah 10');
                                                    }
                                                }
                                            },
                                            {
                                                text: \"<i class='fa fa-check-square'></i> uncheck all\",
                                                action: function (e, dt, node, config) {
                                                    var items_check = top.$('.items_check');
                                                    jQuery.each(items_check, function(a, b){
                                                        if( $(b).is(':checked') ) {
                                                    ...
                                                    render: function (data, type, row, meta) {
                                                        if(data.dataType){
                                                            if(data.value.link){
                                                                ...
                                                                var vClass = data.value.class;
                                                                var vStyle = data.value.style;
                                                                return '<span '+ vClass +' ' + vStyle+'>' + data.value.value + '</span>'
                                                            }
                                                        }
                                                        else{
                                                            if(data.value!=''){
                                                                if(data.key=='action'){
                                                                    ...
                                                                    var is_npwp = row[3].value != '' ? 1 : 0;
                                                                    if(is_npwp){
                                                                        return \"<span style='color:red; font-size:11px;'><b>Hanya bisa diproses di<br>TAB Prepare E-Faktur</b></span>\"
                                                                    }
                                                                    else{
                                                                        return \"<input class=items_check \"+checked+\" onclick=checkBoxPost(this,'\"+data.link+\"') type='checkbox'>\"
                                                                    }
                                                                }
                                                                else{
                                                                    return data.value;
                                            ...
                                            rowCallback: function(row, data, index) {
                                                //pengenal untuk npwp, jika kolom berubah, harap merubah urut nomer saati ini urut 3
                                                if(data[3].value!=''){
                                                    $(row).css('background-color', '#dcdcdc').attr('title','anda bisa merubah data NPWP di data konsumen jika NPWP salah, jika NPWP benar, maka faktur untuk transaksi ini bisa di tindak lanjut melalui menu Prepare Gunggungan pada tab sebelah kiri.');
                                                }
                                            ...
                                            drawCallback: function(settings) {
                                                ...
                                                if (recordsFiltered === 0 && searchTerm !== '') {
                                                    ...
                                                    var urlFilterKiri = '" . MODUL_PATH . "Transaksi/viewUndoneItemsIndex2/110?step=pre_gunggungan';

                                                    var showNotFoundModal = function(matchedTabs) {
                                                        var otherTabsNoticeHtml = '';
                                                        var swalConfirmText = '<i class=\"fa fa-history\"></i> Seluruh Riwayat';
                                                        var swalConfirmColor = '#3085d6';
                                                        var targetTabUrl = urlHistory + '?kw=' + encodeURIComponent(searchTerm);
                                        ...
                                        function checkSessionData() {
                                            var statikUrl = '" . MODUL_PATH . "FollowUp/previewGunggung/110?debuger=0" . $geterTambahan . "'
...
//                                                    $('.floating-box-static-bottom').html(strInputFaktur);
                                                ...
                                                console.log('memanggil debounce');
                                                    ...
                                                    .then((confirm) => {
                                                          if (confirm) {
                                                                ...
                                                                top.$('#undoneList').load('" . base_url() . "taxes/Transaksi/viewUndoneItemsIndex2/110?step=pre_gunggungan');
                                        ...
                                        $('#pre_gunggungan_filter input').off();
                                        $('#pre_gunggungan_filter input').on('keyup', delay_v2(function() {
                                            table.search(this.value).draw();
                                        }, 700));  // Delay in milliseconds

//                                    });
//                            $('.table-responsive.$stepper').floatingScroll();
//                            $('.table-responsive.$stepper').scroll(function () {
//                                setTimeout(function () {
//                                    $('#$stepper').DataTable().fixedHeader.adjust();
//                                }, 400);
//                            });
                    </script>";
    ...
    case "viewGunggunganIndex":
        ...
        if ($itemJml > 0) {
            ...
            /* --------------------------------------------------
             * Cross-Tab Search Index for Block 4
             * --------------------------------------------------*/
            $otherTabsIndex_idx4 = array();
            if (isset($steps) && is_array($steps)) {
                foreach ($steps as $sKey => $sVal) {
                    if ((string)$sKey === (string)$stepper) {
                        continue;
                    ...
                    if (!empty($itemsInStep)) {
                        ...
                        $otherTabsIndex_idx4[$sKey] = array(
                            'stepKey' => (string)$sKey,
                            'label' => $stepLabel,
                            'linkUrl' => $undoneLinkIndex,
                            'items' => array()
                        ...
                        foreach ($itemsInStep as $itemRow) {
                            ...
                            $otherTabsIndex_idx4[$sKey]['items'][] = $itemText;
                        }
                    }
                }
            }
            if (isset($gunggungFieldsView) && $gunggungFieldsView == true && (string)$stepper !== 'pre_gunggungan') {
                ...
                if (!empty($itemsGunggung)) {
                    ...
                    $otherTabsIndex_idx4['pre_gunggungan'] = array(
                        'stepKey' => 'pre_gunggungan',
                        'label' => 'PREPARE GUNGGUNGAN',
                        'linkUrl' => $undoneLinkIndex,
                        'items' => array()
                    ...
                    foreach ($itemsGunggung as $itemRow) {
                        ...
                        $otherTabsIndex_idx4['pre_gunggungan']['items'][] = $itemText;
                    }
                }
            }
            $jsonOtherTabsData_idx4 = json_encode($otherTabsIndex_idx4);
