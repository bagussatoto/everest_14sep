...
//            "mode" => isset($this->configUi[$jenisTr]["mode"]) ? $this->configUi[$jenisTr]["mode"] : $this->uri->segment(3),
    ...
    public function viewUndoneItemsIndex2()
    {
        ...
        /** ------------------------------------------------
         * dimatikan untuk menyembunyikan data kolom status prepare gunggungan
         * req widi 9/9/2026
         * melibatkan 3 titik yang harus dimatikan seperti ini di controller ini disini baru titik ke-1
         * ------------------------------------------------*/
        if ($stepper == "pre_gunggungan") {
            // $progressFields['state'] = "status 787878";
        }
        else {
            $progressFields['state'] = "status ";
        }
...
//        matiHere();
        ...
        if ($isFilterNpwpEnabled) {
            $stepCounts[1] = intval($npwpStepOneCount);
        }
        if ($targetStep > 0) {
            $tr->addFilter("sub_step_number='" . $targetStep . "'");
        }
        if ($isFilterNpwpEnabled && $targetStep == 1 && !$isPreGunggungan) {
            // $tr->addFilter("customers_id in (select id from per_customers where ifnull(npwp,'')<>'')");
        }
...
// matiHEre();
    ...
    public function viewGunggunganIndex()
    {
         ...
         * dimatikan untuk menyembunyikan data kolom status prepare gunggungan
         * req widi 9/9/2026
         * melibatkan 3 titik yang harus dimatikan seperti ini di controller ini disini baru titik ke-2
         * ------------------------------------------------*/
        // $progressFields['state'] = "status";
        $progressFields['action'] = "action";
        ...
        $defaultDirection = (isset($urutanDb) && in_array(strtolower($urutanDb), array("asc", "desc"))) ? strtolower($urutanDb) : "asc";
        $this->db->order_by('transaksi.id', $defaultDirection);
        // ------------------------------------------------------------------------------
        // --- REVERSE MAPPING (Anti-Typo Translasi Kode) ---
        if (isset($_GET['kw'])) {
            $_GET['kw'] = str_replace(array('INVPOS.', 'INVPOS'), array('5822r.', '5822r'), $_GET['kw']);
        }
        if (isset($_POST['search']['value'])) {
            $_POST['search']['value'] = str_replace(array('INVPOS.', 'INVPOS'), array('5822r.', '5822r'), $_POST['search']['value']);
        }
        // --------------------------------------------------

        if (isset($_GET['kw']) || isset($_POST['search']['value'])) {
            $kw = isset($_POST['search']['value']) ? $_POST['search']['value'] : (isset($_GET['kw']) ? $_GET['kw'] : "");
            $tr->setKeyWord($kw);
        }
        ...
        if (isset($_GET['kw']) && trim($_GET['kw']) != "") {
            ...
            foreach ($arrKey as $k) {
                if (trim($k) != '') {
                $this->db->group_start();
                    foreach ($arrColumns as $index => $fieldName) {
                        if ($index == 0) {
        ...
        if (isset($this->keyWord)) {
            $key = isset($this->keyWord) ? $this->keyWord : (isset($_GET['kw']) ? $_GET['kw'] : "");
            $this->createSmartSearch(
                $key,
                array(
                    "transaksi.customers_nama",
                    "transaksi.oleh_nama",
...
//        $tmpHist_0_ = $tr->lookupRecentUndoneEntries_joined($sesionReplacer)->result();
//        $count_all = count($tmpHist_0_);
        ...
        $this->db->select([
            ...
            'transaksi.jenis',
            'transaksi.nomer_top',
            'transaksi.customers_id',
            'transaksi.customers_nama',
            'per_customers.npwp AS npwp',
            'transaksi.oleh_id',
            'transaksi.oleh_nama',
            'transaksi.cabang_id',
            'transaksi.ids_his',
            'transaksi_data.next_substep_num',
            'transaksi_data.next_substep_code',
            'transaksi_data.sub_step_number',
        ...
        $this->db->join('per_customers', 'transaksi.customers_id = per_customers.id');
        ...
        if (isset($_GET['date'])) {
            ...
            $getDate_ex = explode("-", $getDateMonth);
            $date_tahun = $getDate_ex[0];
            $date_bulan = $getDate_ex[1];
            $this->db->where("year(transaksi.dtime)='$date_tahun' and month(transaksi.dtime)='$date_bulan'");
        }

//        $this->db->where('per_customers.npwp', '');
        ...
        if (isset($_POST['hideNpwp']) && $_POST['hideNpwp'] == 0) {
        ...
        $this->db->where_in('next_substep_code', ['110e', '110']);
        $this->db->group_by(['transaksi_id', 'next_substep_code']);
        ...
        if (isset($_POST["order"][0]["column"]) && $_POST["order"][0]["column"] > 0) {
            ...
            $orderMap = array(
                ...
                "dpp" => "transaksi.dpp",
                "ppn" => "transaksi.ppn",
                "amount" => "transaksi.new_grand_total",
                "total" => "transaksi.new_grand_total",
                "grand_total" => "transaksi.new_grand_total",
            );
            ...
            // keep default order set from URL segment (/100/asc or /100/desc)
        }

        $query = $this->db->get();
//$this->debugShowLastQuery("kuning");
        if ($query->num_rows() > 0) {
            $tmpHist_0 = $query->result();
        }
        else {
            $tmpHist_0 = array();
        }
...
//        $tmpHist_0 = $tr->lookupRecentUndoneEntries_joined($sesionReplacer)->result();
        ...
        $arrayOnprogress = array();
        $arrayOnprogressGroup = array();
        $arrayOnprogressGroupGunggung = array();
        $arrayOnprogressPartialMark = array();
        $arrayOnprogressGroupPartialMark = array();
        ...
        foreach ($tmpHist_0 as $item) {
            ...
            $arrayOnprogressGroup[$currentStepNum][] = $item;
            if ($item->npwp == '') {
                $arrayOnprogressGroupGunggung['pre_gunggungan'][] = $item->transaksi_id;
            }
        }
        ...
        $items = isset($arrayOnprogressGroup[$step]) ? $arrayOnprogressGroup[$step] : array();
        $itemJml = count($items);
        $jmlPerhalaman = isset($_POST['length']) && $_POST['length'] != -1 ? $_POST['length'] : ($itemJml > 0 ? $itemJml : 100);
        if (empty($jmlPerhalaman)) $jmlPerhalaman = 100;
//        $jmlPerhalaman = 100;

        $this->load->library("pagination");
        $pg = new Pagination();

        $page = isset($_GET['page']) ? $_GET['page'] : (isset($_POST['start']) ? ($_POST['start'] / $jmlPerhalaman) + 1 : 1);
        ...
        if (sizeof($tmpHist_0) > 0) {
            ...
            foreach ($tmpHist as $row) {
                $arrTransID[] = $row->transaksi_id;
                $arrTransTopID[] = isset($row->id_top) ? $row->id_top : 0;
                $arrTransMasterID[] = $row->id_master;
...
//                            break;
            ...
            $tmpNp = new MdlCustomer();
            $tmpNp->addFilter("id in ('" . implode("','", $arrCust) . "')");
            $npwp = $tmpNp->lookupall()->result();

            if (!empty($npwp)) {
                foreach ($npwp as $k => $val) {
                    $arrNPWP[$val->id] = $val->npwp;
                }
            }
...
//
//                $this->debugArrPrint($row);
//                $row->customers_npwp = isset($arrNPWP[$row->customers_id]) ? $arrNPWP[$row->customers_id] : "";
//
                $step_number_val = isset($row->step_number) ? $row->step_number : (isset($row->sub_step_number) ? $row->sub_step_number : '');
                        ...
                        if (isset($row->ids_his)) {
                            if ($hisKey == "nomer") {
                                ...
                                if (isset($ids_his_decode[$hisStep][$hisKey])) {
                                    $tmp[] = array(
                                        "key" => $fName,
                                        "dataType" => is_array($ids_his_decode[$hisStep][$hisKey]),
                                        "value" => $ids_his_decode[$hisStep][$hisKey],
                                    );
                                }
                                else {
                                    $tmp[] = array(
                ...
                if (isset($row->cancel_packing_source_id) && $row->cancel_packing_source_id != '') {
//                    $this->debugArrPrint($row->cancel_packing_source_id);
                    $trx = new MdlTransaksi();
                    $trx->addFilter("id='" . $row->cancel_packing_source_id . "'");
                    $tmpTrx = $trx->lookupAll()->result();
//                    $tmp[] = $tmpTrx[0]->nomer;
                    $thisVal = formatField_he_format_json("nomer_top", $tmpTrx[0]->nomer);
                    $tmp[] = array(
                        "key" => "nomer_top",
                        "dataType" => 0,
                        "value" => $thisVal,
                 ...
                 * dimatikan untuk menyembunyikan data kolom status prepare gunggungan
                 * req widi 9/9/2026
                 * melibatkan 3 titik yang harus dimatikan seperti ini di controller ini disini baru titik ke-3
                 * ------------------------------------------------*/
//                 if ($row->sub_step_number > 0) {
// //                    $tmp[] = $this->configUi[$jenisTr]['steps'][$row->sub_step_number]['stateLabel'];
//                     $tmp[] = array(
//                         "key" => "state",
//                         "dataType" => 0,
//                         "value" => $this->configUi[$jenisTr]['steps'][$row->sub_step_number]['stateLabel'],
//                     );
//                 }
//                 else {
//                     $tmp[] = array(
...
//                }
                ...
                $currentStepNum = $row->sub_step_number;
                $currentStepCode = $row->jenis;
                $nextStepCode = isset($row->next_step_code) ? $row->next_step_code : (isset($row->next_substep_code) ? $row->next_substep_code : '');
                ...
                if ($allowFollowup) {
                    ...
                    if ($req_cancel_qty > 0 && $valid_qty == 0) {
                        $tmp[] = array(
                            'key' => "action",
                            'dataType' => 0,
                            'value' => "menuggu approve cancel",
                            'checked' => isset($_SESSION[$cCode]['items'][$row->transaksi_id]) ? 1 : 0,
                            'label' => "menuggu approve cancel",
                    ...
                    $tmp[] = array(
                        ...
                        'label' => $actionLabel,
                        'onclick' => $followupLink,
                        'link' => MODUL_PATH . "FollowUp/followupPrePreview/$jenisTr/" . $row->transaksi_id . "/$nextStepNum/" . $row->sub_step_number . "/?getmode=reject",
                        'stepLabel' => $stepLabel,
                    );
...
//                if ($row->partial == 1) {
//                    $tmpMark['style'] = "background-color:yellow;";
//                    $tmp[] = "<span style='color:red;'>transaksi diproses sebagian</span>";
//                }
//                else{
//                    $tmp[] = array(
//                        'key' => "partial",
...
//        $data = array(
//            "mode"                 => isset($this->configUi[$jenisTr]["mode"]) ? $this->configUi[$jenisTr]["mode"] : $this->uri->segment(3),
//            "arrayOnProgress"      => (isset($arrayOnprogress[1]) && sizeof($arrayOnprogress[1]) > 0) ? $arrayOnprogress[1] : array(),
//        );
//        $this->load->view("transaksi", $data);

        $json_data = [
            "draw" => isset($_POST['draw']) ? intval($_POST['draw']) : 0,
            "recordsTotal" => intval($itemJml),
            "recordsFiltered" => intval($itemJml),
            "data" => !empty($arrayOnprogress[1]) ? $arrayOnprogress[1] : array()
//            "data_order" => $keyConvert,
//            "lastQuery" => $lastQuery,
//            "tmpHist_0_" => isset($tmpHist_0[0]) ? $tmpHist_0[0] : array(),
        ];

        echo json_encode($json_data);

//        echo "<script>console.log(".json_encode($arrayOnprogress[1]).");</script>";
    ...
    public function searchCrossTab($jenisTr = "110")
    {
        // Pemeriksaan validitas sesi login (kompatibel dengan $this->session dan $_SESSION)
        $userId = isset($this->session->login['id']) ? $this->session->login['id'] : (isset($_SESSION['login']['id']) ? $_SESSION['login']['id'] : null);
        if ($userId === null) {
        ...
        $searchTerm = trim((string)$this->input->get_post('term'));
        $currentStep = trim((string)$this->input->get_post('current'));
        $getDate = $this->input->get_post('date');
        ...
        // Penyiapan term pencarian fleksibel untuk LIKE query
        $termLike = '%' . str_replace(' ', '%', $searchTerm) . '%';
        $code1 = $jenisTr;
        $code2 = $jenisTr . "r";
        $code3 = $jenisTr . "e";

        // Query database MariaDB menggunakan query binding sesuai aturan keamanan AGENTS.md
        // Menghitung jumlah transaksi belum selesai (valid_qty > 0 dan trash = 0) per sub_step dan status NPWP
        $sql = "SELECT td.sub_step_number, td.next_substep_code, (IFNULL(c.npwp, '') <> '') as has_npwp, COUNT(DISTINCT IFNULL(NULLIF(t.nomer2, ''), td.transaksi_id)) as cnt
                FROM transaksi_data td
                JOIN transaksi t ON td.transaksi_id = t.id
                LEFT JOIN per_customers c ON t.customers_id = c.id
                WHERE td.trash = '0' AND t.trash = '0' AND t.link_id = '0' AND td.valid_qty > 0 
                  AND td.next_substep_code IN (?, ?, ?)
                  AND (t.customers_nama LIKE ? OR t.nomer LIKE ? OR t.nomer2 LIKE ? OR c.nama LIKE ? OR c.npwp LIKE ?)
                GROUP BY td.sub_step_number, (IFNULL(c.npwp, '') <> '')";

        $binds = array($code1, $code2, $code3, $termLike, $termLike, $termLike, $termLike, $termLike);
        $query = $this->db->query($sql, $binds);
        $rows = $query->result_array();

        // Metadata langkah transaksi untuk modul pajak
        $stepMeta = array(
            'pre_gunggungan' => array(
                'label' => 'PREPARE GUNGGUNGAN',
                'stepParam' => 'pre_gunggungan',
            ),
            '1' => array(
        ...
        if (isset($this->configUi[$jenisTr]['steps']) && is_array($this->configUi[$jenisTr]['steps'])) {
            foreach ($this->configUi[$jenisTr]['steps'] as $sNum => $sSpec) {
                $sNumStr = (string)$sNum;
                if (isset($sSpec['label']) && !empty($sSpec['label'])) {
                    $stepMeta[$sNumStr] = array(
                        'label' => strtoupper($sSpec['label']),
                        'stepParam' => $sNumStr,
                    );
                }
            }
        ...
        $tabCounts = array();
        foreach ($rows as $r) {
            $stepNum = (string)$r['sub_step_number'];
            $hasNpwp = intval($r['has_npwp']);
            $cnt = intval($r['cnt']);

            if ($stepNum == '1') {
                if ($hasNpwp == 1) {
                    $tabCounts['1'] = isset($tabCounts['1']) ? $tabCounts['1'] + $cnt : $cnt;
                } else {
                    $tabCounts['pre_gunggungan'] = isset($tabCounts['pre_gunggungan']) ? $tabCounts['pre_gunggungan'] + $cnt : $cnt;
                }
            } else if (isset($stepMeta[$stepNum])) {
                $tabCounts[$stepNum] = isset($tabCounts[$stepNum]) ? $tabCounts[$stepNum] + $cnt : $cnt;
            }
        }

        $filterData = ($getDate != NULL) ? "&date=" . urlencode($getDate) : "";
        $matchedTabs = array();
        foreach ($tabCounts as $stepKey => $count) {
            if ((string)$stepKey !== (string)$currentStep && $count > 0 && isset($stepMeta[$stepKey])) {
                $linkUrl = MODUL_PATH . "Transaksi/viewUndoneItemsIndex2/" . $jenisTr . "?step=" . $stepMeta[$stepKey]['stepParam'] . $filterData;
                $matchedTabs[] = array(
                    'stepKey' => (string)$stepKey,
                    'label' => $stepMeta[$stepKey]['label'],
                    'linkUrl' => $linkUrl,
                    'count' => $count,
                );
            }
