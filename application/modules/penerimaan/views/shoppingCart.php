...
//                arrPrint($addRowHiddens[$k]);
        ...
        if (isset($kelebihanBayarMarketplaceParams) && (sizeof($kelebihanBayarMarketplaceParams) > 0)) {
            ...
            foreach ($kelebihanBayarMarketplaceParams["gateData"] as $iSpec) {
                ...
                $readOnly = array();
                $qtyParam = "";
                if (isset($kelebihanBayarMarketplaceParams["headers"]['jml'])) {
                    $qtyParam = "+removeCommas(document.getElementById('jml_$no').value)";
                }
                foreach ($kelebihanBayarMarketplaceParams["headers"] as $key => $label) {
                    $listMode[$key] = "input";
                    ...
                    if (array_key_exists($key, $keyUpEvents)) {
                        if (sizeof($selectedPrices) > 0) {
                            ...
                            foreach ($kelebihanBayarMarketplaceParams["headers"] as $k => $v) {
                                $nameLabel = $k . "_" . $no;
                                $keyupEvent[$key] = str_replace("{" . $k . "}", $nameLabel, $keyupEvent[$key]);
                            }
                        }
                    ...
                    if (in_array($key, $kelebihanBayarMarketplaceParams["editableFields"])) {
                        $readOnly[$key] = "";
                        if (isset($iSpec["jml"]) && $iSpec["jml"] < 1) {
                            $readOnly[$key] = "readonly_xz";
                        }
...
//                        cekHitam("validToShow: $validToShow");
                        if ($validToShow) {
                ...
                foreach ($kelebihanBayarMarketplaceParams["headers"] as $key => $label) {
                    ...
                    switch ($listMode[$key]) {
                        case "input":
                            $mplb .= "<input type='text'  min='$moq' autocomplete='off' " . $readOnly[$key] . " keyid=$keyID noid=$noID id_jml=$iID id=$colID  class='form-control text-right' style='color:$color;' value='" . niceDecimal($fieldVal) . "' onclick='this.select()' " . $keyUpStr[$key] . " ";
                            $baseInputName = isset($unionSelectors['base']) ? "document.getElementById('" . $unionSelectors['base'] . "_" . $no . "')" : "this";
                            $pemicuGerbangAsli = "onblur=\"if(this.value!=this.defaultValue){hiliteDiv(this);sendCartResultRequest('" . $iSpec['editTargetLebihBayar'] . "'$qtyParam+'$queryParams', 0);} \" $keyupData";
                            $pemicuGerbangAsli .= "*onmouseoutx=\"if(this.value!=this.defaultValue){hiliteDiv(this);sendCartResultRequest('" . $iSpec['editTargetLebihBayar'] . "'$qtyParam+'$queryParams', 0);}\" ";
                            $pemicuGerbang = "onblur=\"if($baseInputName.value!=$baseInputName.defaultValue){hiliteDiv($baseInputName);sendCartResultRequest('" . $iSpec['editTargetLebihBayar'] . "'$qtyParam+'$queryParams', 0);}\" $keyupData ";
                            $pemicuGerbang .= "*onmouseoutx=\"if($baseInputName.value!=$baseInputName.defaultValue){hiliteDiv($baseInputName);sendCartResultRequest('" . $iSpec['editTargetLebihBayar'] . "'$qtyParam+'$queryParams', 0);}\" ";
                            $pemicuGerbangUnion = "onchange=\"if($baseInputName.value!=$baseInputName.defaultValue){hiliteDiv($baseInputName);sendCartResultRequest('" . $iSpec['editTargetLebihBayar'] . "'$qtyParam+'$queryParams', 0);} \" ";

                            if (isset($unionSelectors['base'])) {
                                if ($unionSelectors['base'] == $key) {//==jadi acuan kiriman
                                    $mplb .= str_replace("this", $baseInputName, $pemicuGerbang);
                                }
                                else {
                                    if (in_array($key, $unionSelectors['members'])) {//==jadi member union, tidak memicu perubahan gerbang
                                        $mplb .= $pemicuGerbangUnion;
                                    }
                                    else {//==biasa aja, memicu perubahan gerbang
                                        $mplb .= $pemicuGerbangAsli;
                                    }
                                }
                            }
                            else {
                            ...
                            if (isset($keyupAction) && $keyupAction == true) {
                                $mplb .= "onkeyup=\"sendCartResultRequest('" . $iSpec['editTargetLebihBayar'] . "'$qtyParam+'$queryParams', 220);if(parseFloat(removeCommas(this.value))>0){ this.value=addCommas(this.value) }else{ this.value=0 }\"";
                            }
                            else {
                                $mplb .= "onkeyup=\"delay( function(){ $('#shopping_cart').trigger('change') }, 400, this );if(parseFloat(removeCommas(this.value))>0){ this.value=addCommas(this.value) }else{ this.value=0 }\"";
                            }
                            $mplb .= ">";
                        ...
                        case "text":
                            if (is_numeric($fieldVal)) {
                                $mplb .= "<span keyid=$keyID noid=$noID id=$colID class='form-control text-right' style='color:$color;background:#f0f0f0;'>" . niceDecimal($fieldVal) . "</span>";
                            }
                            else {
                                if (strlen($fieldVal) > 10) {
                                    $mplb .= "<span keyid=$keyID noid=$noID id=$colID class='' style='color:$color;border:0px;'>" . formatField($key, $fieldVal) . "</span>";
                                }
                                else {
                                    $mplb .= "<span keyid=$keyID noid=$noID id=$colID class='form-control' style='color:$color;border:0px;'>" . formatField($key, $fieldVal) . "</span>";
                                }
                            }
                            break;
//                        case "radio":
                        case "combo":
                            ...
                            $shows = "<select data-id=$iID data-style='btn btn-md btn-warning'
                                data-placeholder='pilih' data-live-search='false' data-headers='--silahkan pilih--' 
                                data-size='15' data-container='.modal' class='sselectpicker combo_select form-controlx btn btn-block btn-warning select2 show-tick'
                                onchange=\"document.getElementById('result').src='". $iSpec['editTargetLebihBayarMetode'] ."&$key='+this.value;\"
                                >";
                            $shows .= "<option value='0'> --silahkan pilih-- </option>";
                            foreach ($arrExtn2 as $kk => $pval) {
                                $selected = ($iSpec["kelebihanBayarMetode"] == $kk) ? "selected" : "";
                                $shows .= "<option $selected value='$kk'>" . strtoupper($pval) . "</option>";
                            }
                            $shows .= "</select>";
                            $mplb .= "<span sini#1 keyid=$keyID noid=$noID id=$colID class='' style='color:$color;border:0px;'>" . $shows . "</span>";
                            break;
                    }
                    $mplb .= "</td>";
