...
class Printing extends Modul_Controller
{
    ...
    public function viewReceiptReg()
    {
                        ...
                        if (isset($row->$param)) {
                            switch ($param) {
                                ...
                                case "items2_sum"://
                                    ...
                                    break;
                                case "items7_sum"://
                                    $items7_sumRegistries = unserialize(base64_decode($row->$param));
                                    break;
                                case "main"://
                                    $main = unserialize(base64_decode($row->$param));
        ...
        //----------------------
        if(isset($main["tipe_penjualan"]) && ($main["tipe_penjualan"] == 1)){
            $itemLabels = isset($this->configUi[$this->jenisTr]['shoppingCartFieldsMarketplace'][$currentStepNum]) ? $this->configUi[$this->jenisTr]['shoppingCartFieldsMarketplace'][$currentStepNum] : $itemLabels;
            $items = $itemsRegistries;
            $elements["customerDetails"] = array(
                "elementType" => "dataModel",
                "name" => "cash_account",
                "label" => "marketplace",
                "key" => "1",
                "labelSrc" => "nama",
                "labelValue" => $main["marketplaceNama"],
                "mdl_name" => "MdlCustomerTipeOnline",
                "contents" => Array
                (
                    "nama" => $main["marketplaceNama"],
                ),
            );
//            arrPrintPink($main);
        }
        ...
        //--------------------
        $opsi_cetak = isset($_GET['opsi_cetak']) ? $_GET['opsi_cetak'] : 4;
        ...
        $data = array(
            ...
            "items" => $items,
            "items2" => isset($items2_sumRegistries) ? $items2_sumRegistries : $items2,
            "items5" => isset($items7_sumRegistries) ? $items7_sumRegistries : $items5,
            "itemsRegistries" => $itemsRegistries,
    ...
    public function viewTimelinePdf()
    {
        $this->load->helper("he_misc");
        $seg4 = $this->uri->segment(4);
        $seg5 = $this->uri->segment(5);
        $transaksi_id = !empty($seg5) ? $seg5 : $seg4;

        if (empty($transaksi_id)) {
            echo "<div style='padding:20px;font-family:sans-serif;color:red;'>ID Transaksi tidak ditemukan.</div>";
            return;
        }

        $this->load->model("MdlTransaksi");
        $trModel = new MdlTransaksi();

        // 1. Fetch starting transaction
        $trModel->setFilters(array());
        if (is_numeric($transaksi_id)) {
            $trModel->addFilter("transaksi.id='" . $transaksi_id . "'");
        } else {
            $trModel->addFilter("transaksi.nomer='" . $transaksi_id . "'");
        }
        $trModel->removeFilter("transaksi.link_id='0'");
        $startTrxRaw = $trModel->lookupJoined();
        $startTrxArr = is_array($startTrxRaw) ? $startTrxRaw : (is_object($startTrxRaw) && method_exists($startTrxRaw, 'result') ? $startTrxRaw->result() : array());

        if (empty($startTrxArr)) {
            $tr2 = new MdlTransaksi();
            $tr2->setFilters(array());
            if (is_numeric($transaksi_id)) {
                $tr2->addFilter("id='" . $transaksi_id . "'");
            } else {
                $tr2->addFilter("nomer='" . $transaksi_id . "'");
            }
            $tr2->removeFilter("transaksi.link_id='0'");
            $startTrxRaw2 = $tr2->lookUpAll();
            $startTrxArr = is_array($startTrxRaw2) ? $startTrxRaw2 : (is_object($startTrxRaw2) && method_exists($startTrxRaw2, 'result') ? $startTrxRaw2->result() : array());
        }

        if (empty($startTrxArr)) {
            echo "<div style='padding:20px;font-family:sans-serif;color:red;'>Data transaksi #$transaksi_id tidak ditemukan di database.</div>";
            return;
        }

        $startTrx = $startTrxArr[0];

        // 2. Discover target project and customer ID
        $visitedIds = array((int)$startTrx->id => true);
        $targetProjectId = 0;
        $customerId = !empty($startTrx->customers_id) ? (int)$startTrx->customers_id : 0;

        if (!empty($startTrx->project_id)) {
            $targetProjectId = (int)$startTrx->project_id;
        }

        if (empty($targetProjectId)) {
            $this->db->select("project_id");
            $this->db->from("transaksi_payment_source");
            $this->db->where("transaksi_id", $startTrx->id);
            $this->db->where("project_id >", 0);
            $this->db->limit(1);
            $psQ = $this->db->get();
            $psRow = is_object($psQ) ? $psQ->row() : null;
            if (!empty($psRow) && !empty($psRow->project_id)) {
                $targetProjectId = (int)$psRow->project_id;
            }
        }
        ...
        if (empty($targetProjectId)) {
            ...
            $this->db->where("(transaksi_id = '" . (int)$startTrx->id . "' OR quot_id = '" . (int)$startTrx->id . "' OR project_start_id = '" . (int)$startTrx->id . "')", null, false);
            $this->db->limit(1);
            $ppQ = $this->db->get();
            $ppRow = is_object($ppQ) ? $ppQ->row() : null;
            if (!empty($ppRow) && !empty($ppRow->id)) {
                $targetProjectId = (int)$ppRow->id;
            }
        }

        $collectIdsFromRow = function($row) use (&$visitedIds) {
            if (empty($row)) return;

            if (isset($row->id) && is_numeric($row->id) && (int)$row->id > 0) { $visitedIds[(int)$row->id] = true; }
            if (isset($row->id_master) && is_numeric($row->id_master) && (int)$row->id_master > 0) { $visitedIds[(int)$row->id_master] = true; }
            if (isset($row->id_top) && is_numeric($row->id_top) && (int)$row->id_top > 0) { $visitedIds[(int)$row->id_top] = true; }

            if (!empty($row->ids_prev)) {
                $decodedPrev = blobDecode($row->ids_prev);
                if (is_array($decodedPrev)) {
                    foreach ($decodedPrev as $pId) {
                        if (is_numeric($pId) && (int)$pId > 0) { $visitedIds[(int)$pId] = true; }
                    }
                } else {
                    $prevArr = explode("|", str_replace(",", "|", $row->ids_prev));
                    foreach ($prevArr as $pId) {
                        $cleanPId = trim($pId);
                        if (is_numeric($cleanPId) && (int)$cleanPId > 0) { $visitedIds[(int)$cleanPId] = true; }
                    }
                }
            }

            if (!empty($row->ids_ref)) {
                $decodedRef = blobDecode($row->ids_ref);
                if (is_array($decodedRef)) {
                    foreach ($decodedRef as $rKey => $rVal) {
                        if (is_numeric($rKey) && (int)$rKey > 0) { $visitedIds[(int)$rKey] = true; }
                        if (is_numeric($rVal) && (int)$rVal > 0) { $visitedIds[(int)$rVal] = true; }
                    }
                } else if (is_numeric($row->ids_ref) && (int)$row->ids_ref > 0) {
                    $visitedIds[(int)$row->ids_ref] = true;
                }
            }

            if (!empty($row->ids_his)) {
                $decodedHis = blobDecode($row->ids_his);
                if (is_array($decodedHis)) {
                    foreach ($decodedHis as $stepKey => $stepVal) {
                        if (is_array($stepVal)) {
                            if (isset($stepVal['trID']) && is_numeric($stepVal['trID']) && (int)$stepVal['trID'] > 0) {
                                $visitedIds[(int)$stepVal['trID']] = true;
                            }
                            if (isset($stepVal['id']) && is_numeric($stepVal['id']) && (int)$stepVal['id'] > 0) {
                                $visitedIds[(int)$stepVal['id']] = true;
                            }
                        }
                    }
                }
        ...
        $collectIdsFromRow($startTrx);

        // Kumpulkan semua transaksi yang terdaftar pada project_id jika ada
        if (!empty($targetProjectId)) {
            // 1. Dari transaksi_payment_source
            $this->db->select("transaksi_id");
            $this->db->from("transaksi_payment_source");
            $this->db->where("project_id", $targetProjectId);
            $psAllQ = $this->db->get();
            $psAllRows = is_object($psAllQ) ? $psAllQ->result() : array();
            foreach ($psAllRows as $pR) {
                if (!empty($pR->transaksi_id) && is_numeric($pR->transaksi_id) && (int)$pR->transaksi_id > 0) {
                    $visitedIds[(int)$pR->transaksi_id] = true;
                }
            }

            // 2. Dari header project_produk
            $this->db->select("transaksi_id, quot_id, project_start_id");
            $this->db->from("project_produk");
            $this->db->where("id", $targetProjectId);
            $ppHQ = $this->db->get();
            $ppHRow = is_object($ppHQ) ? $ppHQ->row() : null;
            if (!empty($ppHRow)) {
                if (!empty($ppHRow->transaksi_id)) $visitedIds[(int)$ppHRow->transaksi_id] = true;
                if (!empty($ppHRow->quot_id)) $visitedIds[(int)$ppHRow->quot_id] = true;
                if (!empty($ppHRow->project_start_id)) $visitedIds[(int)$ppHRow->project_start_id] = true;
            }
        }

        // Multi-pass expansion via fast indexed queries (id, id_master, id_top)
        for ($pass = 0; $pass < 3; $pass++) {
            $keys = array_keys($visitedIds);
            if (empty($keys)) break;

            $intKeys = array();
            foreach ($keys as $k) {
                if (is_numeric($k) && (int)$k > 0) $intKeys[] = (int)$k;
            }
            if (empty($intKeys)) break;

            // Pass A: Direct rows
            $this->db->select("id, id_master, id_top, ids_prev, ids_ref, ids_his");
            $this->db->from("transaksi");
            $this->db->where_in("id", $intKeys);
            $this->db->where("trash", "0");
            $this->db->where("trash_4", "0");
            $this->db->where("link_id", "0");
            $qA = $this->db->get();
            $rowsA = is_object($qA) ? $qA->result() : array();
            foreach ($rowsA as $rA) { $collectIdsFromRow($rA); }

            // Pass B: Children by id_master
            $this->db->select("id, id_master, id_top, ids_prev, ids_ref, ids_his");
            $this->db->from("transaksi");
            $this->db->where_in("id_master", $intKeys);
            $this->db->where("trash", "0");
            $this->db->where("trash_4", "0");
            $this->db->where("link_id", "0");
            $qB = $this->db->get();
            $rowsB = is_object($qB) ? $qB->result() : array();
            foreach ($rowsB as $rB) { $collectIdsFromRow($rB); }

            // Pass C: Children by id_top
            $this->db->select("id, id_master, id_top, ids_prev, ids_ref, ids_his");
            $this->db->from("transaksi");
            $this->db->where_in("id_top", $intKeys);
            $this->db->where("trash", "0");
            $this->db->where("trash_4", "0");
            $this->db->where("link_id", "0");
            $qC = $this->db->get();
            $rowsC = is_object($qC) ? $qC->result() : array();
            foreach ($rowsC as $rC) { $collectIdsFromRow($rC); }
        }

        // Cari seluruh payment receipts (749, 4467, 583) milik customer ini
        if (!empty($customerId)) {
            $this->db->select("id, id_master, id_top, ids_prev, ids_ref, ids_his, jenis");
            $this->db->from("transaksi");
            $this->db->where("customers_id", $customerId);
            $this->db->where("(jenis IN ('749', '749_1', '4467', '4467_1', '583', '583r') OR jenis_master IN ('749', '4467', '583'))", null, false);
            $this->db->where("trash", "0");
            $this->db->where("trash_4", "0");
            $this->db->where("link_id", "0");
            $rcQ = $this->db->get();
            $rcRows = is_object($rcQ) ? $rcQ->result() : array();

            foreach ($rcRows as $rcR) {
                $isMatch = false;
                if (!empty($rcR->id_master) && isset($visitedIds[$rcR->id_master])) {
                    $isMatch = true;
                } elseif (!empty($rcR->id_top) && isset($visitedIds[$rcR->id_top])) {
                    $isMatch = true;
                } elseif (!empty($rcR->ids_ref)) {
                    $rDec = blobDecode($rcR->ids_ref);
                    if (is_array($rDec)) {
                        foreach ($rDec as $k => $v) {
                            if ((is_numeric($k) && isset($visitedIds[$k])) || (is_numeric($v) && isset($visitedIds[$v]))) {
                                $isMatch = true;
                                break;
                            }
                        }
                    } elseif (is_numeric($rcR->ids_ref) && isset($visitedIds[$rcR->ids_ref])) {
                        $isMatch = true;
                    }
                } elseif (!empty($rcR->ids_prev)) {
                    $pDec = blobDecode($rcR->ids_prev);
                    if (is_array($pDec)) {
                        foreach ($pDec as $pVal) {
                            if (is_numeric($pVal) && isset($visitedIds[$pVal])) {
                                $isMatch = true;
                                break;
                            }
                        }
                ...
                if ($isMatch) {
                    $visitedIds[(int)$rcR->id] = true;
                    $collectIdsFromRow($rcR);
                }
            }
        }

        $allTargetIds = array();
        foreach (array_keys($visitedIds) as $vK) {
            if (is_numeric($vK) && (int)$vK > 0) {
                $allTargetIds[] = (int)$vK;
            }
        }

        // 3. Query main transaction headers across all discovered IDs
        $this->db->select("transaksi.*");
        $this->db->from("transaksi");
        $this->db->where("transaksi.trash", "0");
        $this->db->where("transaksi.trash_4", "0");
        $this->db->where("(transaksi.cancel_id IS NULL OR transaksi.cancel_id = '0' OR transaksi.cancel_id = '')", null, false);
        $this->db->where("transaksi.link_id", "0");

        if (sizeof($allTargetIds) > 0) {
            $this->db->where_in("transaksi.id", $allTargetIds);
        } else {
            $this->db->where("transaksi.id", $startTrx->id);
        }
        ...
        $timelineSteps = is_object($queryResult) ? $queryResult->result() : array();

        if (empty($timelineSteps)) {
            $timelineSteps = array($startTrx);
        }

        // De-duplicate by ID while maintaining chronological order
        $uniqueSteps = array();
        $seenIds = array();
        $totalTagihan = 0;
        $totalDiterima = 0;
        ...
        $isProject = !empty($targetProjectId);
        if (!$isProject) {
            $startJm = isset($startTrx->jenis_master) ? (string)$startTrx->jenis_master : '';
            $startJ = isset($startTrx->jenis) ? (string)$startTrx->jenis : '';
            if ($startJm == '588' || $startJm == '7499' || $startJ == '7499' || strpos($startJm, 'project') !== false) {
                $isProject = true;
            }
        }

        foreach ($timelineSteps as $step) {
            if (!isset($seenIds[$step->id])) {
                $seenIds[$step->id] = true;
                $uniqueSteps[] = $step;

                $isReceiptStep = false;
                if (isset($step->jenis)) {
                    if ($step->jenis == '749' || $step->jenis == '749_1' || $step->jenis == '4467' || $step->jenis == '4467_1' || $step->jenis == '583' || $step->jenis == '583r' || (strpos($step->jenis, '749') !== false && strpos($step->jenis, '7499') === false) || strpos($step->jenis, '4467') !== false) {
                        $isReceiptStep = true;
                    }
                }
                ...
                $ppnVal = isset($step->ppn_nilai) ? (float)$step->ppn_nilai : 0;

                if ($isReceiptStep) {
                    $nomIncl = $net > 0 ? $net : $nilai;
                    $ppnStep = $ppnVal > 0 ? $ppnVal : ($nomIncl - ($nomIncl / 1.11));
                } else {
                    if ($net > 0) {
                        $nomIncl = $net;
                        $ppnStep = $ppnVal > 0 ? $ppnVal : ($net - ($net / 1.11));
                    } else if ($nilai > 0) {
                        $ppnStep = $ppnVal > 0 ? $ppnVal : ($nilai * 0.11);
                        $nomIncl = $nilai + $ppnStep;
                    } else {
                        $nomIncl = 0;
                        $ppnStep = 0;
                    }
                ...
                if ($isReceiptStep) {
                    $totalDiterima += $nomIncl;
                    $paymentReceipts[] = $step;
                } else if ($isProject) {
                    if (isset($step->jenis)) {
                        $jSt = (string)$step->jenis;
                        // Hanya akumulasikan invoice 7499 (Termin Invoice) untuk mencegah duplikasi dengan 4822
                        if ($jSt == '7499') {
                            $totalTagihan += $nomIncl;
                            $totalPpn += $ppnStep;
                        }
                    }
                } else if ($totalTagihan == 0 && $nomIncl > 0) {
                    $totalTagihan = $nomIncl;
                    $totalPpn = $ppnStep;
                }
            }
        ...
        // Fallback jika belum ada invoice 7499 tapi ada 4822
        if ($isProject && $totalTagihan == 0) {
            foreach ($uniqueSteps as $step) {
                if (isset($step->jenis) && $step->jenis == '4822') {
                    $net = isset($step->transaksi_net) ? (float)$step->transaksi_net : 0;
                    $nilai = isset($step->transaksi_nilai) ? (float)$step->transaksi_nilai : 0;
                    $ppnVal = isset($step->ppn_nilai) ? (float)$step->ppn_nilai : 0;
                    if ($net > 0) {
        ...
        if ($totalTagihan == 0 && isset($startTrx->transaksi_nilai)) {
            $sNilai = (float)$startTrx->transaksi_nilai;
            $sNet = isset($startTrx->transaksi_net) ? (float)$startTrx->transaksi_net : 0;
            $sPpn = isset($startTrx->ppn_nilai) ? (float)$startTrx->ppn_nilai : 0;
            if ($sNet > 0) {
                $totalTagihan = $sNet;
                $totalPpn = $sPpn > 0 ? $sPpn : ($sNet - ($sNet / 1.11));
            } else {
                $totalPpn = $sPpn > 0 ? $sPpn : ($sNilai * 0.11);
                $totalTagihan = $sNilai + $totalPpn;
            }
        }

        $sisaTagihan = $totalTagihan - $totalDiterima;
        if ($sisaTagihan < 0) $sisaTagihan = 0;

        $statusPembayaran = 'BELUM_LUNAS';
        if ($totalDiterima >= $totalTagihan && $totalTagihan > 0) {
        ...
        $amandemenArr = array();
        if (sizeof($uniqueSteps) > 0) {
            $uIds = array();
            foreach ($uniqueSteps as $st) {
                if (!empty($st->id)) $uIds[] = (int)$st->id;
            }
            if (sizeof($uIds) > 0) {
                $this->db->select("transaksi_amandemen_history.*, admin.nama_login as user_nama");
                $this->db->from("transaksi_amandemen_history");
                $this->db->join("admin", "admin.id = transaksi_amandemen_history.user_id", "left");
                $this->db->where_in("transaksi_amandemen_history.transaksi_id", $uIds);
                $this->db->order_by("transaksi_amandemen_history.dtime_amandemen", "ASC");
                $amQ = $this->db->get();
                $amRows = is_object($amQ) ? $amQ->result() : array();
                foreach ($amRows as $amR) {
                    $amandemenArr[(int)$amR->transaksi_id][] = $amR;
                }
            }
        }
        ...
        $data = array(
            ...
            "amandemenEvents" => $amandemenArr,
        );

        $this->load->view("view_timeline_pdf", $data);
    }
}
