...
class _shoppingCart extends Modul_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->syncPostParamsToGet();

    }

    /**
     * Kompatibilitas request mutation:
     * beberapa jalur UI mengirim parameter lewat POST (body form-urlencoded),
     * sementara controller legacy masih banyak membaca $_GET.
     * GET tetap diprioritaskan; nilai POST hanya mengisi key GET yang belum ada.
     *
     * @return void
     */
    protected function syncPostParamsToGet()
    {
        if (!isset($_SERVER['REQUEST_METHOD']) || strtoupper($_SERVER['REQUEST_METHOD']) !== 'POST') {
            return;
        }

        if (!isset($_POST) || !is_array($_POST) || sizeof($_POST) < 1) {
            return;
        }
    ...
    /**
     * Guard reload shopping cart agar tidak loop.
     * Tetap bisa baca query walau $_GET dinonaktifkan oleh konfigurasi CI.
     *
     * @return string
     */
    protected function getStopReloadFlag()
    {
        $stop1 = $this->input->get('stop1', true);
        if ($stop1 === null || $stop1 === "") {
            $requestUri = isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : "";
            if (strpos($requestUri, "stop1=1") !== false) {
                $stop1 = "1";
            }
        }
    ...
    /**
     * Bangun URL request saat ini dengan flag stop reload.
     *
     * @return string
     */
    protected function getStopReloadUrl()
    {
        ...
        return base_url() . $this->uri->uri_string() . '?' . http_build_query($query);
    }
        ...
        if (isset($this->configCore[$this->jenisTr]['externalValues'])) {
            if (sizeof($this->configCore[$this->jenisTr]['externalValues']) > 0) {
                ...
                foreach ($iterator as $vName => $vSpec) {
                    ...
                    onmouseoutx=\"if(this.value!=this.defaultValue){hiliteDiv(this);document.getElementById('result').src='" . $sumRowAction . "?key=$key&value='+this.value;}\"
                    >";
                    $sumType2[$key] = "number";
                    //endregion
...
onmouseoutx=\"if(this.value!=this.defaultValue){hiliteDiv(this);document.getElementById('result').src =$relInputTarget;}\"
>";
...
>";
                ...
                switch ($eSpec['elementType']) {
                    ...
                    BootstrapDialog.show(
                        ...
                        $selectorTarget = "'" . MODUL_PATH . get_class($this) . "/fetchElement/" . $this->jenisTr . "/$eName/$amdlName/?key='+encodeURIComponent((this && typeof this.value != 'undefined') ? this.value : '')";

                        $elStr[$eName] .= "<div class='box-body'>";

                        switch ($eSpec['inputType']) {
                            case "combo":
                                $elStr[$eName] .= "<select class='form-control' onchange=\"hiliteDiv(this);sendCartResultRequest($selectorTarget, 0);\">";
                                ...
                                if (sizeof($tmpo) > 0) {
                                    foreach ($tmpo as $row) {
                                        ...
                                        if (sizeof($ex) > 1) {
                                            ...
                                            $elPair[$amdlName][$row->$keySrc] = $labelValue;
                                            $selected = isset($_SESSION[$cCode]['main_elements'][$eName]) && $_SESSION[$cCode]['main_elements'][$eName]['key'] == $row->$keySrc ? "checked" : "";
                                            $elStr[$eName] .= "<label class='badge text-uppercase' style='padding:4px 6px 4px 6px;color:#454545;background:#e0e0e0;'><input type='radio' name='$eName' value='" . $row->$keySrc . "' $selected onclick=\"hiliteDiv(this);sendCartResultRequest($selectorTarget, 0);\">" . $labelValue . "</label>\n";
                                        }
                                        else {
                                            $elPair[$amdlName][$row->$keySrc] = isset($row->$labelSrc) ? $row->$labelSrc : "-";
                                            $selected = isset($_SESSION[$cCode]['main_elements'][$eName]) && $_SESSION[$cCode]['main_elements'][$eName]['key'] == $row->$keySrc ? "checked" : "";
                                            $elStr[$eName] .= "<label class='badge text-uppercase' style='padding:4px 6px 4px 6px;color:#454545;background:#e0e0e0;'><input type='radio' name='$eName' value='" . $row->$keySrc . "' $selected onclick=\"hiliteDiv(this);sendCartResultRequest($selectorTarget, 0);\">" . (isset($row->$labelSrc) ? $row->$labelSrc : '-') . "</label>\n";
                                        }
...
onblur=\"if(this.value!=this.defaultValue){if(this.value.length<1){this.value='$initValue'};hiliteDiv(this);sendCartResultRequest($selectorTarget, 0);}\">";
                                break;
                            case "number":
                                $maxValStr = $maxValue != "" ? " max='" . $maxValue . "''" : "";
                    ...
                    foreach ($elementItemsAutoConfigs as $eSpec) {
                        ...
                        switch ($eSpec['elementType']) {
                            case "dataModel":
                                ...
                                $elPair[$amdlName] = array();
                                $selectorTarget = "'" . MODUL_PATH . get_class($this) . "/fetchItemsElement/" . $this->jenisTr . "/$eName/$amdlName/$helpName?key='+encodeURIComponent((this && typeof this.value != 'undefined') ? this.value : '')";

                                $elStr[$eName] .= "<div class='box-body'>";
                                //                                cekHitam(": $eName :");
                                switch ($eSpec['inputType']) {
                                    case "combo":
                                        $elStr[$eName] .= "<select " . basename(__FILE__) . " " . __FUNCTION__ . " " . __LINE__ . " class='form-control' onchange=\"hiliteDiv(this);sendCartResultRequest($selectorTarget, 0);\">";
                                        $elStr[$eName] .= "<option value=''>-select-</option>";
                                        if (sizeof($tmpo) > 0) {
                                            foreach ($tmpo as $row) {
                                                //                                                cekPink2($row->$keySrc);
                                        ...
                                        if (sizeof($tmpo) > 0) {
                                            foreach ($tmpo as $row) {
                                                ...
                                                if (sizeof($ex) > 1) {
                                                    ...
                                                    $elPair[$amdlName][$row->$keySrc] = $row->$labelSrc;
                                                    $selected = isset($_SESSION[$cCode]['items'][$eName][$helpName]) && $_SESSION[$cCode]['items'][$eName][$helpName] == $row->$keySrc ? "checked" : "";
                                                    $elStr[$eName] .= "<label class='badge text-uppercase' style='padding:4px 6px 4px 6px;color:#454545;background:#e0e0e0;'><input type='radio' name='$eName' value='" . $row->$keySrc . "' $selected onclick=\"hiliteDiv(this);sendCartResultRequest($selectorTarget, 0);\">" . $row->$labelSrc . "</label>\n";
                                                }
...
//                                $tmp['background_pembayaran'] = "";
                    ...
                    $tmp["editTargetLebihBayar"] = base_url() . $iSpec['handler'] . "/selectLebihBayar/" . $this->jenisTr . "?id=" . $id;
                    $tmp["editTargetLebihBayarMetode"] = base_url() . $iSpec['handler'] . "/selectLebihBayarMetode/" . $this->jenisTr . "?id=" . $id;

                    $items[] = $tmp;

                    $total_quantity += isset($iSpec['qty_opname']) ? $iSpec['qty_opname'] : 0;

                    $_SESSION[$cCode]['items'][$id]["editTarget"] = $tmp["editTarget"];
                    $_SESSION[$cCode]['items'][$id]["editTargetLebihBayar"] = $tmp["editTargetLebihBayar"];
                    $_SESSION[$cCode]['items'][$id]["editTargetLebihBayarMetode"] = $tmp["editTargetLebihBayarMetode"];
                    $_SESSION[$cCode]['items'][$id]["removeTarget"] = $tmp["removeTarget"];
                }
            }
        }
        // arrPrintWebs($items);
            ...
            if (sizeof($this->configCore[$this->jenisTr]['externalValues']) > 0) {
                ...
                foreach ($iterator as $vName => $vSpec) {
                    ...
                    if (isset($vSpec['mdlName']) && strlen($vSpec['mdlName']) > 0) {
                        $key = $vName . "_src";
                        $sumRows2[$key] = "select " . $vSpec['label'];
                        $sumSpec2[$key] = "<select id='$key' name='$key' class='form-control' onchange=\"sendCartResultRequest('" . $sumRowAction2 . "?key=$key&value='+removeCommas(this.value), 0);\">";
                        $sumSpec2[$key] .= "<option value=''>-select-</option>";
                        $mdlName9 = $vSpec['mdlName'];
                        $this->load->model("Mdls/" . $mdlName9);
                        $o9 = new $mdlName9();
                    ...
                    $defaultValue = isset($addValues[$key]) ? $addValues[$key] : 0;
                    $sumSpec2[$key] = "<input type='text' class='form-control text-right' name=$key id=$key value='$defaultValue' 
                    onblur=\"if(this.value!=this.defaultValue){hiliteDiv(this);sendCartResultRequest('" . $sumRowAction . "?key=$key&value='+removeCommas(this.value), 0);}\"
                    onmouseoutx=\"if(this.value!=this.defaultValue){hiliteDiv(this);sendCartResultRequest('" . $sumRowAction . "?key=$key&value='+removeCommas(this.value), 0);}\"
                    >";
                    $sumType2[$key] = "number";
                    //endregion
                    if ($vSpec['taxFactor'] > 0) {
...
//                                        matiHere($defVal);
                                        ...
                                        onblur=\"if(removeCommas(this.value)!=this.defaultValue){hiliteDiv(this);if(typeof amResultRequest==='function'){amResultRequest($relInputTarget);}else if(typeof sendCartResultRequest==='function'){sendCartResultRequest($relInputTarget, 0);}else{top.$('#result').load($relInputTarget);}}\"
                                        onmouseoutx=\"if(removeCommas(this.value)!=this.defaultValue){hiliteDiv(this);if(typeof amResultRequest==='function'){amResultRequest($relInputTarget);}else if(typeof sendCartResultRequest==='function'){sendCartResultRequest($relInputTarget, 0);}else{top.$('#result').load($relInputTarget);}}\">";
...
//                                        $keyupStr .= "if(parseInt(removeCommas(this.value))>$maxValue){this.value='" . number_format($origDefValue) . "';this.select();}else{if(parseInt(removeCommas(this.value))>0){this.value=addCommas(this.value)}else{this.value=0}}";
                                    ...
                                    $blurStr = "";
                                    if ($minValue != "") {
                                        $blurStr = "if(removeCommas(this.value)!=this.defaultValue){if(parseInt(removeCommas(this.value))>=$minValue){hiliteDiv(this);if(typeof amResultRequest==='function'){amResultRequest($relInputTarget);}else if(typeof sendCartResultRequest==='function'){sendCartResultRequest($relInputTarget, 0);}else{top.$('#result').load($relInputTarget);}}else{this.value='$minValue';this.focus();}}";
                                    }
                                    else {
                                        $blurStr = "if(removeCommas(this.value)!=this.defaultValue){hiliteDiv(this);
                                        if(typeof amResultRequest==='function')
                                        {amResultRequest($relInputTarget);}
                                        else if(typeof sendCartResultRequest==='function')
                                        {sendCartResultRequest($relInputTarget, 0);}
                                        else{top.$('#result').load($relInputTarget);}}";
...
//                                        matiHEre($cek_prevals);
                                    ...
                                    if (isset($oValSpec['saldo'])) {
                                        ...
                                        $saldoVal = is_numeric($saldo) ? (float)$saldo : (is_string($saldo) ? (float)str_replace(',', '', $saldo) : 0);
                                        $addLabel = "<br>Saldo : <span class='strong' id='saldo_$oValueName'>" . number_format($saldoVal, 0, ",", ".") . "</span>";
                                    }
...
//                                            $checked = ($pihakID_parent == $_SESSION[$cCode]['main'][$pihakID_parent_key]) ? "checked" : "";
                                            ...
                                            $cekboxlink = "hiliteDiv(this);if(typeof amResultRequest==='function'){amResultRequest($relInputTargetCek);}else if(typeof sendCartResultRequest==='function'){sendCartResultRequest($relInputTargetCek, 0);}else{top.$('#result').load($relInputTargetCek);}";
                                            $cekbox = "<input type='checkbox' value='$defVal' $checked
                                                onclick=\"$cekboxlink\"
                                                >";
                                            $nilai_parent = isset($_SESSION[$cCode]["main"][$pihakID_parent_key_value]) ? $_SESSION[$cCode]["main"][$pihakID_parent_key_value] : 0;
                                            $nilai_parent_val = is_numeric($nilai_parent) ? (float)$nilai_parent : (is_string($nilai_parent) ? (float)str_replace(',', '', $nilai_parent) : 0);
                                            $nilai_parent_f = number_format($nilai_parent_val, 0, ",", ".");
                                            $addLabel .= "<br>$cekbox  <span class='meta' style='font-size:15px;'>$pihakID_parent_label <b>$nilai_parent_f</b> </span>";
                                        }
                                    }
...
onclick=\"hiliteDiv(this);sendCartResultRequest($selectorTarget, 0);\">" . $labelAccount . "</label><br>\n";
            ...
            if (isset($_SESSION[$cCode]["main"]["tipe_penjualan"]) && ($_SESSION[$cCode]["main"]["tipe_penjualan"] == 1)) {
                if (isset($kelebihanBayarMarketplace["enabled"]) && ($kelebihanBayarMarketplace["enabled"] == true)) {
                    if ($_SESSION[$cCode]["main"]["lebih_bayar"] > 0) {
                        ...
                        if (isset($_SESSION[$cCode]['items'])) {
                            foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                                $_SESSION[$cCode]["items"][$id]["kelebihanBayarMarketplace"] = 0;
                            }
                        }
                    }
                }
...
//
        ...
        //--------------------
        $label_disabled_ceklist = "";
        if (!isset($_SESSION[$cCode]["main"]["nilai_bayar"]) || ($_SESSION[$cCode]["main"]["nilai_bayar"] == 0)) {
            $label_disabled_ceklist .= "- Jumlah Penerimaan belum diisi.<br>";
        }
        if (isset($_SESSION[$cCode]['items'])) {
            foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                if ($iSpec['new_sisa'] > 0) {
...
//                    $tmp['background_pembayaran'] = $shopingCartPaymentItemsColor['colorCode'][0]['color'];
            ...
            if (sizeof($arrItemTidakDibayar) > 0) {
                $jml_tidak_dibayar = count($arrItemTidakDibayar);
                $label_disabled_ceklist .= "- Penerimaan A/R pada $jml_tidak_dibayar nota belum mencukupi (ditandai dengan warna merah).";
//                $label_disabled_ceklist .= " Silahkan hapus jika tidak dilakukan Penerimaan A/R pada $jml_tidak_dibayar nota tersebut.<br>";
                $label_disabled_ceklist .= "<br>";
            }
        }
        if ($label_disabled_ceklist != "") {
            $label_disabled_ceklist .= "Silakan perbaiki data tersebut agar transaksi dapat dilanjutkan.";
        }
        //--------------------
        ...
        if (isset($shopingCartFaktur["gateTarget"])) {
            ...
            if (isset($_SESSION[$cCode][$gateTarget])) {
                ...
                foreach ($_SESSION[$cCode][$gateTarget] as $spec) {
                    foreach ($kolom as $kol) {
                        if (!isset($_SESSION[$cCode]["main"][$kol])) {
                            $_SESSION[$cCode]["main"][$kol] = 0;
                        }
                        $_SESSION[$cCode]["main"][$kol] += isset($spec[$kol]) ? $spec[$kol] : 0;
                    }
                }
            }
        ...
        switch ($this->jenisTr){
            case "749":
                if(isset($_SESSION[$cCode]['main']['projectID']) && ($_SESSION[$cCode]['main']['projectID'] > 0)){
                    $_SESSION[$cCode]['main']['uang_muka_nppn_relasi_soproject'] = $_SESSION[$cCode]['main']['uang_muka_project_dpp_dipakai'];
                }
                else{
                    $_SESSION[$cCode]['main']['uang_muka_nppn_relasi_soproject'] = 0;
                }
//                if ($rowName == 'uang_muka_dipakai_ppn') {
//                }
                break;
        }
...
//            "sumRows2" => $sumRows2,
            ...
            //----
            "label_disabled_ceklist" => $label_disabled_ceklist,
            //----
...
//                "dtime" => "date",
        ...
        if ($shopingCartReload) {
            ...
            if ($stop1 != '1') {
                $stopReloadUrl = $this->getStopReloadUrl();
                redirect($stopReloadUrl, 'location', 302);
                return;
            }
        }
                            ...
                            foreach ($addRowsConfigs[$eName][$currentValue] as $oValueName => $oValSpec) {
                                if (isset($oValSpec['addPoints']) && in_array(1, $oValSpec['addPoints'])) {
                                    ...
                                    if ($minValue != "") {
                                        ...
                                        $blurStr = "if(removeCommas(this.value)!=this.defaultValue){hiliteDiv(this);if(typeof amResultRequest==='function'){amResultRequest($relInputTarget);}else if(typeof sendCartResultRequest==='function'){sendCartResultRequest($relInputTarget, 0);}else{top.$('#result').load($relInputTarget);}}";
...
//                                            $selected = isset($_SESSION[$cCode]['main_elements'][$eName]) && $_SESSION[$cCode]['main_elements'][$eName]['key'] == $row->$keySrc ? "checked" : "";
                                ...
                                if (sizeof($tmpo) > 0) {
                                    foreach ($tmpo as $row) {
                                        ...
                                        if (sizeof($ex) > 1) {
                                            ...
                                            $elStr[$eName] .= "<label class='badge text-uppercase' style='padding:4px 6px 4px 6px;color:#454545;background:#e0e0e0;'><input type='radio' name='$eName' value='" . $row->$keySrc . "' $selected onclick=\"hiliteDiv(this);sendCartResultRequest($selectorTarget, 0);\">" . $labelssss . "</label>\n";
                                        }
                                        else {
                                            $elPair[$amdlName][$row->$keySrc] = isset($row->$labelSrc) ? $row->$labelSrc : "-";
                                            $selected = isset($_SESSION[$cCode]['main_elements'][$eName]) && $_SESSION[$cCode]['main_elements'][$eName]['key'] == $row->$keySrc ? "checked" : "";
                            ...
                            case "table":
                                ...
                                $selectorTargetTable = "'" . MODUL_PATH . get_class($this) . "/fetchElementTable/" . $this->jenisTr . "/$eName/$amdlName/?key='+encodeURIComponent((this && typeof this.value != 'undefined') ? this.value : '')";
                                ...
                                if (isset($_SESSION[$cCode]['payment']) && count($_SESSION[$cCode]['payment']) != 0) {
                                    ...
                                    $elStr[$eName] .= "<script>
                                            top.$('#btnRemoveTable').on('click', function(){
                                                ...
                                                if(typeof sendCartResultRequest==='function'){sendCartResultRequest($closeTargetTable?key=remove&id='+encodeURIComponent(idpayment), 0);}else{top.$('#result').load($closeTargetTable?key=remove', {id: idpayment});}
                                            });
                                ...
                                $elStr[$eName] .= "<select class='form-control form-control-sm' onchange=\"hiliteDiv(this);sendCartResultRequest($selectorTargetTable, 0);\">";
                                        ...
                                        $elStr[$eName] .= "<script>
                                            ...
                                            top.$('#btnSave_$newSelectID').on('click', function(){
                                                if( $('#js_$newSelectID').val() != '' ){
                                                    ...
                                                    if(typeof sendCartResultRequest==='function'){sendCartResultRequest($saveTargetTable?key=$newSelectID&'+jQuery.param(obj), 0);}else{top.$('#result').load($saveTargetTable?key=$newSelectID', obj);}
                                                }
                                            });
...
//        cekMerah($_SESSION[$cCode]['main']['uang_muka_nppn_relasi_soproject']);
