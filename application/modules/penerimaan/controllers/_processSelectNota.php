...
//        $this->jenisTr = $this->uri->segment(4);
        ...
        $transaksi_nomor_selected = $_GET['nomer'];
        $tmp = array();
        ...
        if ((sizeof($selectorNotaLocker) > 0)) {
            if (isset($selectorNotaLocker["enabled"]) && ($selectorNotaLocker["enabled"] == true)) {
                ...
                $lt->addFilter("cabang_id='" . my_cabang_id() . "'");
                ...
                if (sizeof($ltTmp) > 0) {
                    if ($ltTmp[0]->oleh_id == my_id()) {
                        ...
                        $lastActive = strtotime($ltTmp[0]->last_access);
                        $idleSeconds = time() - $lastActive;
                        if ($idleSeconds >= 300 && isset($_GET['forceRetakeLock']) && $_GET['forceRetakeLock'] == '1') {
                            $lt->setFilters(array());
                            $lt->updateData(array("id" => $ltTmp[0]->id), array("jumlah" => "0"));
                            $allowedAction = true;
                        }
                        else {
                            $allowedAction = false;
                ...
                if ($allowedActionByHold != NULL) {
                    $ipAddr = isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : '127.0.0.1';
                    $deviceInfo = isset($ltTmp[0]->satuan) ? $ltTmp[0]->satuan : 'PC / Browser';
                    $dtimeLastActive = isset($ltTmp[0]->last_access) ? $ltTmp[0]->last_access : date('Y-m-d H:i:s');
                    $msgHtml = "<div style='text-align:center;'>".
                        "<div style='width:70px;height:70px;background:#fff8e6;border:2px solid #ffe082;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;margin:0 auto 15px auto;'>".
                        "<span style='font-size:32px;'>🔒</span></div>".
                        "<h3 style='color:#854d0e;margin-top:0;font-weight:bold;'>Transaksi Terkunci</h3>".
                        "<p style='font-size:13px;color:#4b5563;line-height:1.5;'>Transaksi/Nota dengan nomor <strong>$transaksi_nomor_selected</strong> sedang diproses oleh pengguna lain di sesi lain. Untuk mencegah bentrok data, Anda tidak dapat memprosesnya bersamaan.</p>".
                        "<table style='width:100%;background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;text-align:left;font-size:12px;margin-top:15px;border-collapse:collapse;'>".
                        "<tr><td style='padding:8px 12px;color:#6b7280;border-bottom:1px solid #f3f4f6;width:35%;'>Petugas Aktif</td><td style='padding:8px 12px;color:#111827;font-weight:bold;border-bottom:1px solid #f3f4f6;'>".htmlspecialchars($ltTmp[0]->oleh_nama)." (ID: ".htmlspecialchars($ltTmp[0]->oleh_id).")</td></tr>".
                        "<tr><td style='padding:8px 12px;color:#6b7280;border-bottom:1px solid #f3f4f6;'>IP Address</td><td style='padding:8px 12px;color:#111827;font-weight:bold;border-bottom:1px solid #f3f4f6;'>".htmlspecialchars($ipAddr)."</td></tr>".
                        "<tr><td style='padding:8px 12px;color:#6b7280;border-bottom:1px solid #f3f4f6;'>Perangkat</td><td style='padding:8px 12px;color:#111827;font-weight:bold;border-bottom:1px solid #f3f4f6;'>".htmlspecialchars($deviceInfo)."</td></tr>".
                        "<tr><td style='padding:8px 12px;color:#6b7280;'>Aktif Terakhir</td><td style='padding:8px 12px;color:#111827;font-weight:bold;'>".htmlspecialchars($dtimeLastActive)."</td></tr>".
                        "</table></div>";
                    echo swalAlert(array(
                        "title" => "",
                        "html" => $msgHtml,
