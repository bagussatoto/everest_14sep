...
class ActivityReport extends Modul_Controller
{
    ...
    public function viewMySettlement()
    {
        ...
        // arrPrint($connectedSettlementFields);
...
//        $tr->addFilter("settlement_id='3791'");
//        $tr->addFilter("settlement_id='3173'");
...
//        $this->upFileSettlement();
//        $this->delFileSettlement();
//        $this->remoteSettlement();
...
//                        foreach ($tmpReg as $regRow) {
//                            $param = $regRow->param;
//                            // arrPrint(blobDecode($regRow->values));
...
//            "sub_title" => "detail " . $this->configUi[$jn]['label'] . " by salesman $seller_nama",
    ...
    public function previewSettlement(){
        ...
        if(isset($_GET['fid'])){
            $tr->addFilter("oleh_id='".$_GET['fid']."'");
        }

        $tr->addFilter("settlement_id='1'");
        $tr->addFilter("link_id='0'");
        $this->db->where("jenis = $targetJenis");
...
//                        $sumProduk[$itemDataTmp["id"]][] = $itemDataTmp; //aslinya
                        ...
                        if( $main["jenisTr"] == "749"){
                            ...
                            $totalCash          += isset($main["grand_total"]) ? $main["grand_total"] : 0;
                            ...
                            $buildMainTmp=array(
                                ...
                                "harga" => $main['harga'],
                                "grand_total" => isset($main['grand_total']) ? $main['grand_total'] : 0,
...
//                            "paymentMethod_cash" => $main['paymentMethod_cash'],
                                ...
                                "tagihan" => isset($main['grand_total']) ? $main['grand_total'] : 0,
...
//                        cekUngu("transaksi_nilai: ". $main["grand_total"]);
            ...
            if (sizeof($trData) > 0) {
                ...
                foreach ($trData as $iix => $trDataTmp) {
                    if($trDataTmp->sub_tail_code=="749"){
                        $buildDataTr = array(
                            "id" => $trDataTmp->id,
                ...
                foreach($payment as $payID => $payNama){
                    ...
                    if(isset($arrPaymentNilai[$payID])){
                        ...
                        foreach($arrListTrxAcc as $ky => $rowsData){
                            ...
                            $toolTips .= "<td>".$rowsData['appr_code']."</td>";
                            $toolTips .= "<td class='text-right text-bold'>".number_format($rowsData['tagihan'])."</td>";
                            $toolTips .= "</tr>";
//                            $toolTips .= "<div style='whitespace: nowrap;'>".$num." | ".$rowsData['appr_code']." | ".number_format($rowsData['tagihan'])."</div>";
...
//                    echo "<span><input value='".$paymentNilai[$payID]."' style='font-size: 23px;' class='form-control text-muted text-right text-bold form-payment text-capitalize' placeholder='nilai $payNama'></span>";
                ...
                if(count($arrNonTunai)>0){
                    echo "<div class='box box-danger box-solid box-header text-center text-bold'>
                                <div class='text-red'>~ NILAI YANG TERTERA PADA LAYAR ADALAH NILAI BERDASARKAN SYSTEM POS ~</div>
                                <div class='text-red'>
...
//        $tr->addFilter("oleh_id='" . $this->session->login['id'] . "'");
        ...
        $this->db->where("jenis IN('749','982')");
...
//            arrPrintWebs($tableIn_detail_values);
