    ...
    public function select()
    {
        ...
        if (sizeof($shoppingCartEditableValidator) > 0) {
            foreach ($shoppingCartEditableValidator as $entry => $source) {
                if (isset($_GET[$entry])) {
                    $entry_nilai = $_GET[$entry];
                    if (isset($_SESSION[$cCode]['items']) && sizeof($_SESSION[$cCode]['items']) > 0) {
                        foreach ($_SESSION[$cCode]['items'] as $itemKey => $itemData) {
                            $coa_code = isset($itemData['coa_code']) && strlen($itemData['coa_code']) > 0
                                ? $itemData['coa_code']
                                : (isset($itemData['rekening']) && strlen($itemData['rekening']) > 0
                                    ? $itemData['rekening']
                                    : "1010050010");
                            if (array_key_exists($coa_code, $source)) {
                                $item_key = $source[$coa_code];
                                $source_nilai = isset($itemData[$item_key]) ? $itemData[$item_key] : 0;
                                ...
                                if ($entry_nilai > $source_nilai) {
                                    ...
                                    if ($selisih > 5) {
                                        $label_formatted = str_replace(
                                            array('{entry_nilai}', '{source_nilai}', "\r\n", "\n", "\r"),
                                            array(number_format($entry_nilai), number_format($source_nilai), '', '', ''),
                                            $label
                                        );
                                        $msg = "$label_formatted <br><span class='meta'>code: " . __LINE__ . "</span>";
                                        matiAlert($msg);
                                    }
                                }
                            }
        ...
        if (sizeof($tmpB) > 0) {
            foreach ($tmpB as $row) {
                $coa_code = isset($row->coa_code) && strlen($row->coa_code) > 0 ? $row->coa_code : (isset($row->rekening) && strlen($row->rekening) > 0 ? $row->rekening : (isset($_SESSION[$cCode]['items'][$id]['coa_code']) ? $_SESSION[$cCode]['items'][$id]['coa_code'] : ""));
                                ...
                                if (array_key_exists($coa_code, $source)) {
                                    ...
                                    if ($entry_nilai > $source_nilai) {
                                        ...
                                        if ($selisih > 5) {
                                             $label_formatted = str_replace(
                                                array('{entry_nilai}', '{source_nilai}', "\r\n", "\n", "\r"),
                                                array(formatAngkaFormat($entry_nilai), formatAngkaFormat($source_nilai), '', '', ''),
                                                $label
                                            );
                                            $msg = "$label_formatted <br><span class='meta'>code: " . __LINE__ . "</span>";
                                            matiAlert($msg);
...
//
            ...
            if (sizeof($_SESSION[$cCode]['items']) > 0) {
                ...
                foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                    ...
                    switch ($this->jenisTr){
                        case "7468":
                            $_SESSION[$cCode]['items'][$id]['nilai_biaya_usaha_entry'] = $iSpec['harga'];
                            break;
                    }
                }
            }
...
//        matiHere(__LINE__);
    ...
    public function selectReplace()
    {
        ...
        if (sizeof($tmpB) > 0) {
            foreach ($tmpB as $row) {
                $coa_code = isset($row->coa_code) && strlen($row->coa_code) > 0 ? $row->coa_code : (isset($row->rekening) && strlen($row->rekening) > 0 ? $row->rekening : "");
                $satuan = isset($row->satuan) && strlen($row->satuan) > 0 ? $row->satuan : "n/a";
                $tmpJml = 1;
                if (isset($lockerConfig['enabled']) && $lockerConfig['enabled'] == true) {
                    cekMerah("masuk locker config");
