...
class Create extends Modul_Controller
{
    ...
    public function index()
    {
        ...
        //----
        $time_line = "";
        if (isset($this->allSteps)) {
            $time_line = createStateHorizontal('-1', sizeof($this->allSteps), $jenisTr);
        }
//        echo ($time_line);
        //----
...
//        arrPrint($this->allSteps);
        if(sizeof($this->allSteps) == 1){
            $connectToNext = isset($configUi[$this->jenisTr]["connectTo"]) ? $configUi[$this->jenisTr]["connectTo"] : 0;
            if($connectToNext != 0){
                $labelTransaksi = isset($configUi[$connectToNext]["label"]) ? $configUi[$connectToNext]["label"] : NULL;
                $pic = availHakAkses($connectToNext, 2);
                $arrNextPic = array(
                    "label" => "Hak Akses " . $labelTransaksi . ": ",
                    "pic" => $pic,
                );
//                arrPrintCyan($arrNextPic);
        ...
        * ---------------------------------------------------------------------*/
        ...
        //-----------------------------------------------------
        $bookingNumber = builderKodeBooking();
...
//                arrPrint($selectorProcessor);
        ...
        //region load history
        $baseUrl = base_url();
        $link_undoneList = MODUL_PATH . "Transaksi/viewUndoneItemsIndex/$jenisTr/?gr=cGVtYmVsaWFu&ohyes=ohno";
            ...
            var viewhistoryList = function () {
                ...
                setTimeout(function () {
                    if (top.$('#historyList').find('img').length > 0) {
                        top.$('#historyList').load('$link_undoneList', function(){viewhistoryList})
                    }
                },1000);
            }
        ...
        // sementara didetek manual disini, hanya 3463 (titipan ke supplier)
        switch ($this->jenisTr) {
            case "4643":
...
//        if ($_SERVER['REMOTE_ADDR'] == "202.65.117.72") {
                    if (!isset($_SESSION[$cCode]["main"]["pihakID"]) || ($_SESSION[$cCode]["main"]["pihakID"] == 0)) {
                        $str_scriptBottom .= "<script>
                   top.$('#itemKeyword').prop('disabled', true)
                 </script>
                ";
                    }
                }
            ...
            case "7445":
                ...
                $selectorFilters = isset($this->configUi[$this->jenisTr]['selectorFilters']) ? $this->configUi[$this->jenisTr]['selectorFilters'] : array();
                $pihakMainProcessorRules = isset($this->configUi[$this->jenisTr]['selectorProcessor']) ? $this->configUi[$this->jenisTr]['selectorProcessor'] : "";
                $this->load->model("Mdls/" . $selectorSrcModel);
                $b = new $selectorSrcModel();
                makeFilter($selectorFilters, $_SESSION[$cCode]["main"], $b);
                $bTmp = $b->lookupAll()->result();
//                showLast_query("biru");
                if (sizeof($bTmp) == 1) {
                    $bid = $bTmp[0]->id;
                    $link_rules = MODUL_PATH . "$pihakMainProcessorRules/$jenisTr/$selectorSrcModel?id=$bid";
                    $str_scriptBottom .= "<script>
                   top.$('#result').load('$link_rules');
                </script>";
        ...
        $data = array(
            ...
            "showHutangKeKonsumen" => isset($this->configUi[$jenisTr]["showHutangKeKonsumen"]) ? $this->configUi[$jenisTr]["showHutangKeKonsumen"] : false,
            "hutang_ke_konsumen_ledger" => base_url() . "Ledger/viewBalances_l1/RekeningPembantuCustomerDetail/2010050?ext2_id=2010050040&main_ext2_id=2010050040&blob_ext=czoxNzoiIFJldHVybiBQZW5qdWFsYW4iOw==",
            "hutang_ke_konsumen" => base_url() . "kas/Create/hutangKeKonsumen/$jenisTr",
            //--------
            "showHutangTanpaIdentitas" => isset($this->configUi[$jenisTr]["showHutangTanpaIdentitas"]) ? $this->configUi[$jenisTr]["showHutangTanpaIdentitas"] : false,
            "hutang_tanpa_identitas" => base_url() . "kas/Create/hutangTanpaIdentitas/$jenisTr",
            //--------
            "pic_list" => $arrNextPic,
            "time_line" => $time_line,
        );
    ...
    public function save()
    {
        ...
        echo "<script>top.writeProgress('MEMULAI PROSES ....');</script>";
...
//        $this->jenisTr = $this->uri->segment(3);
        ...
        $autoCreateInv = isset($this->configUi[$this->jenisTr]['autoCreateInv']) ? $this->configUi[$this->jenisTr]['autoCreateInv'] : false;
        ...
        if (isset($_SESSION[$cCode])) {
            ...
            $cok = checkerKodeBooking($_SESSION[$cCode], $nomerBookingTransaksi);
            ...
            // Early Validation Pattern (Aturan 3.8) untuk Transaksi 4643
            if ($this->jenisTr == "4643") {
                $umRelasi = isset($_SESSION[$cCode]["main"]["um_noppn_nilai"]) ? $_SESSION[$cCode]["main"]["um_noppn_nilai"] : 0;
                $sumNonRelasi = isset($_SESSION[$cCode]["main"]["um_noppn_nonrelasi_nilai"]) ? $_SESSION[$cCode]["main"]["um_noppn_nonrelasi_nilai"] : 0;
                $optionRef = isset($_SESSION[$cCode]["main"]["optionReference"]) ? $_SESSION[$cCode]["main"]["optionReference"] : 2;
                if ($optionRef == 2) {
                    $refSo = isset($_SESSION[$cCode]["main"]["referensi_so"]) ? $_SESSION[$cCode]["main"]["referensi_so"] : 0;
                    if ($refSo <= 0) {
                        $msg = "Gagal Menyimpan Transaksi: Anda memilih opsi Pembayaran Uang Muka dengan Referensi PO, namun nomor Purchase Order (PO) belum dipilih. Silakan pilih nomor PO yang sesuai pada kolom Purchase Order sebelum melanjutkan.";
                        mati_disini($msg);
                    }

                    if ($umRelasi <= 0) {
                        $msg = "Gagal Menyimpan Transaksi: Nominal Uang Muka yang dimasukkan harus lebih besar dari Rp 0.";
                        mati_disini($msg);
                    }
                }
                else{
                ...
                $totNominal = isset($_SESSION[$cCode]["main"]["nett"]) ? $_SESSION[$cCode]["main"]["nett"] : (isset($_SESSION[$cCode]["main"]["harga"]) ? $_SESSION[$cCode]["main"]["harga"] : 0);
                if ($totNominal <= 0) {
                    $msg = "Gagal Menyimpan Transaksi: Nominal Uang Muka yang dimasukkan harus lebih besar dari Rp 0.";
                    mati_disini($msg);
                }
            ...
            // region cek referensi so, masih aktif atau nonaktif
            if (isset($_SESSION[$cCode]["main"]["referensi_so"]) && ($_SESSION[$cCode]["main"]["referensi_so"] > 0)) {
                $tr_id_cek = $_SESSION[$cCode]["main"]["referensi_so"];

                $this->load->model("MdlTransaksi");
                $tr = new MdlTransaksi();
                $tr->setFilters(array());
                $tr->addFilter("id='$tr_id_cek'");
                $tmpTr = $tr->lookupAll()->result();
                showLast_query("ungu");
                if (sizeof($tmpTr) > 0) {
                    $trash_4 = $tmpTr[0]->trash_4;
                    if ($trash_4 == 1) {
                        $cancel_dtime = $tmpTr[0]->cancel_dtime;
                        $cancel_name = $tmpTr[0]->cancel_name;
                        $nomer = $tmpTr[0]->nomer;
                        $nomer_f = formatField_he_format("nomer_nolink", $nomer);
                        $msg = "Transaksi SO $nomer_f sudah direject oleh $cancel_name pada $cancel_dtime. Silahkan refresh halaman ini. code: " . __LINE__;
                        mati_disini($msg);
                    }
                }
            }
            // endregion cek referensi so, masih aktif atau nonaktif
...
// matiHEre();
            ...
            // matiHEre(__LINE__." preproc master");
...
//            cekOrange("lolos validator @" . __LINE__ . " " . __METHOD__);
            ...
            cekUngu("========================================================== BEGIN ITERATOR COMPONEN DETAIL ==========================================================");
            ...
            cekUngu("========================================================== END-OFF ITERATOR COMPONEN DETAIL ==========================================================");
            //endregion

            //region processing main components, if in single step
            cekUngu("========================================================== BEGIN ITERATOR COMPONEN MASTER ==========================================================");
            ...
            cekUngu("========================================================== END-OFF ITERATOR COMPONEN MASTER ==========================================================");
            //endregion
            ...
            if (sizeof($iterator) > 0) {
                        ...
                        foreach ($_SESSION[$cCode][$srcGateName] as $xid => $dSpec) {
                            ...
                            if (isset($tComSpec['static'])) {
                                foreach ($tComSpec['static'] as $key => $value) {
                                    ...
                                    $realValue = makeValue($value, $_SESSION[$cCode][$srcGateName][$id], $_SESSION[$cCode][$srcGateName][$id], 0);
                                    cekHitam("gate: $srcGateName, dengan key $id | $value => $realValue");
                                    $subParams['static'][$key] = $realValue;
                                ...
                                if (isset($tComSpec['static']['transaksi_ref_id'])) {
                                    $subParams['static']['transaksi_ref_id'] = $insertID;
                                }
                                if (isset($tComSpec['static']['transaksi_ref_no'])) {
                                    $subParams['static']['transaksi_ref_no'] = $insertNum;
                                }

                                //update untuk melewati sub-postproc
                                if (isset($subParams['static']["lewati"]) && $subParams['static']["lewati"] == 1) {
                ...
                foreach ($iterator as $cCtr => $tComSpec) {
                    ...
                    if (isset($_SESSION[$cCode][$srcGateName])) {
                        ...
                        arrPrint($tmpOutParams[$cCtr]);
                        $m = new $mdlName();
                        if (!$m->pair($tmpOutParams[$cCtr])) {
//                            cekPink($this->db->last_query());
                            cekMerah("Tidak berhasil memasang  values pada post-processor: $comName | " . $this->jenisTr);
                            matiHere(__FILE__ . " | " . __CLASS__ . " | " . __FUNCTION__ . " | line: " . __LINE__);
                        }

                        if (!$m->exec()) {
//                            cekPink($this->db->last_query());
                            cekMerah("Gagal saat berusaha  exec values pada post-processor: $comName/" . $this->jenisTr . "/" . __FUNCTION__ . "/" . __LINE__);
                            matiHere(__FILE__ . " | " . __CLASS__ . " | " . __FUNCTION__ . " | line: " . __LINE__);
                        }
            ...
            if (array_key_exists($stepCode, $paymentSources)) {
                ...
                if (sizeof($payConfigs) > 0) {
                    foreach ($payConfigs[1] as $paymentSrcConfig) {
                        ...
                        $swapJenis = isset($paymentSrcConfig['swapJenis']) ? (isset($_SESSION[$cCode]['main'][$paymentSrcConfig['swapJenis']]) ? $_SESSION[$cCode]['main'][$paymentSrcConfig['swapJenis']] : $stepCode) : $stepCode;
                        if (isset($_SESSION[$cCode]['main'][$valueSrc]) && $_SESSION[$cCode]['main'][$valueSrc] > 0) {
                            ...
                            $paymentMethodFilter = isset($paymentSrcConfig['methodFilter']) ? $paymentSrcConfig['methodFilter'] : array();
                            if ($paymentMethod == "update") {
                                cekHitam(__LINE__ . " || METHOD UPDATE");
                                $filters = array(
                                    "extern_id" => "",
                                );
                                $tr->setFilters(array());
                                if (sizeof($paymentMethodFilter) > 0) {
                                    makeFilter($paymentMethodFilter, $_SESSION[$cCode]["main"], $tr);
                                }
                                $tmpData = $tr->lookupPaymentSrcByJenis($paymentSrcConfig['jenisTarget'])->result();
                                showLast_query("biru");
                                cekBiru("juml data: " . count($tmpData));
                                if (sizeof($tmpData) > 0) {
                                    //sudah ada update aja gak perlu insert
                                    $prevID = $tmpData[0]->id;
                                    $preValue = $tmpData[0]->sisa;
                                    $preTagihan = $tmpData[0]->tagihan;
                                    $prePpn = ($preValue <= 100) ? 0 : $tmpData[0]->ppn_sisa;
                                    $currValue = isset($_SESSION[$cCode]['main'][$valueSrc]) ? $_SESSION[$cCode]['main'][$valueSrc] : 0;
                                    $currValuePpn = isset($_SESSION[$cCode]['main'][$externSrc['ppn']]) ? $_SESSION[$cCode]['main'][$externSrc['ppn']] : 0;
                                    $newValue = $preValue + $currValue;
                                    $newTagihan = $preTagihan + $currValue;
                                    $newPpnSisa = $prePpn + $currValuePpn;
                                    $where = array(
                                        "id" => $prevID,
                                    );
                                    $data = array(
                                    ...
                                    cekHitam(__LINE__ . " || METHOD INSERT");
                                    //di insert baru
                                    $tr->writePaymentSrc($insertID, array(
                                        ...
                                        "ppn" => isset($_SESSION[$cCode]['main'][$externSrc['ppn']]) ? $_SESSION[$cCode]['main'][$externSrc['ppn']] : 0,
                                        ...
                                        "extern2_id" => makeValue($externSrc['extern2_id'], $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], 0),
                                        "extern2_nama" => makeValue($externSrc['extern2_nama'], $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], 0),
                                        "ppn_sisa" => makeValue($externSrc['ppn_sisa'], $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], 0),
                                        "project_id" => (isset($externSrc['project_id']) && isset($_SESSION[$cCode]['main'][$externSrc['project_id']])) ? $_SESSION[$cCode]['main'][$externSrc['project_id']] : 0,
                                        "project_nama" => (isset($externSrc['project_nama']) && isset($_SESSION[$cCode]['main'][$externSrc['project_nama']])) ? $_SESSION[$cCode]['main'][$externSrc['project_nama']] : 0,
                                    ));
                                }
                            }
                            else {
                                ...
                                $arrDataPym = array(
                                    ...
                                    "terbayar" => 0,
                                    "sisa" => isset($_SESSION[$cCode]['main'][$valueSrc]) ? $_SESSION[$cCode]['main'][$valueSrc] : 0,
                                    "dpp_ppn" => isset($_SESSION[$cCode]['main'][$externSrc['dpp_ppn']]) ? $_SESSION[$cCode]['main'][$externSrc['dpp_ppn']] : 0,
                                    "ppn" => isset($_SESSION[$cCode]['main'][$externSrc['ppn']]) ? $_SESSION[$cCode]['main'][$externSrc['ppn']] : 0,
                                    "cabang_id" => (isset($externSrc['cabang_id']) && isset($_SESSION[$cCode]['main'][$externSrc['cabang_id']])) ? $_SESSION[$cCode]['main'][$externSrc['cabang_id']] : $_SESSION[$cCode]['main']['placeID'],
                                    "cabang_nama" => (isset($externSrc['cabang_nama']) && isset($_SESSION[$cCode]['main'][$externSrc['cabang_nama']])) ? $_SESSION[$cCode]['main'][$externSrc['cabang_nama']] : $_SESSION[$cCode]['main']['placeName'],
                                    "oleh_id" => $this->session->login['id'],
                                    ...
                                    "extern3_id" => makeValue($externSrc['extern3_id'], $_SESSION[$cCode]['main'], 0),
                                    "extern3_nama" => makeValue($externSrc['extern3_nama'], $_SESSION[$cCode]['main'], 0),
                                    "ppn_sisa" => makeValue($externSrc['ppn_sisa'], $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], 0),
                                );
                            ...
                            cekHitam("tidak ada nilai untuk PAYMENT SOURCE");
                        }
                    }
                }
            }
            ...
            //endregion
//            mati_disini("CEK PAYMENT SOURCE UM PPN");
...
//                            cekMerah($this->db->last_query());
            ...
            $baseRegistries = array(
                ...
                'items4_sum' => isset($_SESSION[$cCode]['items4_sum']) ? $_SESSION[$cCode]['items4_sum'] : ( isset($_SESSION[$cCode]['items4_new']) ? $_SESSION[$cCode]['items4_new'] : array() ),
                'items5_sum' => isset($_SESSION[$cCode]['items5_sum']) ? $_SESSION[$cCode]['items5_sum'] : ( isset($_SESSION[$cCode]['items5_new']) ? $_SESSION[$cCode]['items5_new'] : array() ),
...
//            matiHere($this->cCode);
            //==========================================================================================================
            //cek duku apakah next step auto atau manual================================================================
            $pakaiinilamaversilama = 0;
            if ($pakaiinilamaversilama == 1) {
                cekHitam(":::: MULAI AUTO OTORISASI ::::");
                $autoNextStepByValue = isset($this->configUi[$this->jenisTr]["autoNextStepByValue"][1]) ? $this->configUi[$this->jenisTr]["autoNextStepByValue"][1] : array();
                if (sizeof($autoNextStepByValue) > 0) {
                    $nilai_cek_session = $_SESSION[$cCode]["main"][$autoNextStepByValue["key"]];
                    $nilai_cek = $autoNextStepByValue["nilai"];
                    if (($nilai_cek_session == $nilai_cek) && ($autoNextStepByValue["auto"] == true)) {

                        cekHijau("mulai AUTO OTORISASI");
                        $returnTransaksi = $this->autoOtorisasi($this->jenisTr, $masterID, 2, 1, $itemsReplacer = array());
//                    arrPrintHijau($returnTransaksi);
                        cekHijau("selesai AUTO OTORISASI");

                    }
                    else {
                        cekMerah("tidak memenuhi syarat untuk auto otorisasi");
                    }
                }
                else {
                    cekMerah("tidak ada config auto next step by value");
                }
            }
            ...
            $replacerConnectToItems = isset($this->configUi[$this->jenisTr]['replacerConnectToItems']) ? $this->configUi[$this->jenisTr]['replacerConnectToItems'] : array();
...
//                        matiHere("kode connektor tidak dikenali! silahkan hubungi ADMIN untuk Melakukan koneksi Modul");
//                    }
...
//                        "tableIn_detail2" => $_SESSION[$oldCode]['tableIn_detail2'],
//
...
//                    );
                    ...
                    $baseRegistries = array(
                        ...
                        "receiptDetailSrcFields" => isset($this->configLayout[$connector]['receiptDetailSrcFields'][1]) ? $this->configLayout[$connector]['receiptDetailSrcFields'][1] : array(),
                        "items_komposisi" => isset($_SESSION[$cCode]['items_komposisi']) ? $_SESSION[$cCode]['items_komposisi'] : array(),
                        "componentsBuilder" => isset($_SESSION[$cCode]['componentsBuilder']) ? $_SESSION[$cCode]['componentsBuilder'] : array(),
                        "jurnalItems" => isset($_SESSION[$cCode]['jurnalItems']) ? $_SESSION[$cCode]['jurnalItems'] : array(),
            ...
            //cek duku apakah next step auto atau manual================================================================
            $autoNextStep = isset($this->configUi[$this->jenisTr]["steps"][2]["autoNextStep"]) ? $this->configUi[$this->jenisTr]["steps"][2]["autoNextStep"] : false;
...
//                matiHere(__LINE__);
            ...
            /**
             * untuk handling autocreate invoice
             */
            if ($autoCreateInv) {
                $this->load->library("Transaksional");
                $lt = new Transaksional();
                $lt->setTransaksiId($insertID);
                $lt->setMasterId($insertID);
                $temp = $lt->buildInvoice();
            }
...
//                        cekkuning("detecting printing size");
    ...
    public function autoOtorisasi($jenisTr, $no, $stepNum, $stepNumCurrent, $itemsReplacer = array())
    {
        ...
        if (sizeof($tmpTr) > 0) {
            ...
            $tCodeTargetJenisTransaksi = $jenisTrTarget = isset($configUiMasterModulJenis["steps"][$stepNum]["target"]) ? $configUiMasterModulJenis["steps"][$stepNum]["target"] : NULL;
            ...
            $autoCreateInv = isset($configUiMasterModulJenis["autoCreateInv"]) ? $configUiMasterModulJenis["autoCreateInv"] : false;
//arrPrintWebs($configUiMasterModulJenis);
//mati_disini(__LINE__);
                    ...
                    foreach ($row as $key_reg => $val_reg) {
                        switch ($key_reg) {
                            ...
                            case "items4_sum"://
                                ...
                                break;
                            case "items5_sum"://
                                $items5_sum = $items4_sum + unserialize(base64_decode($val_reg));
                                break;
                            case "tableIn_master"://
...
//arrPrintKuning($_SESSION[$cCode]["items4_sum"]);
//arrPrintKuning($_SESSION[$cCode]["items5_sum"]);
//mati_disini(__LINE__);
            $ppnFactor = isset($_SESSION[$cCode]["main"]["ppnFactor"]) ? $_SESSION[$cCode]["main"]["ppnFactor"] : matiHere("gagal menghitung ppn silahkan refresh atau relogin");
...
//                arrPrintWebs($_SESSION[$cCode]['items5_sum']);
//matiHere(__LINE__);
                $baseRegistries = array(
                    'main' => isset($_SESSION[$cCode]['main']) ? $_SESSION[$cCode]['main'] : array(),
                    'items' => isset($_SESSION[$cCode]['items']) ? $_SESSION[$cCode]['items'] : array(),
            ...
            //endregion
//            arrPrint($iterator);
//matiHere(__LINE__." || ".$jenisTrTarget);
            //region processing main-post-processors, always
            //<editor-fold desc="----------postProc">
...
//mati_disini(__LINE__);
            ...
            if ($autoCreateInv) {
                $this->load->library("Transaksional");
                $lt = new Transaksional();
                $lt->setTransaksiId($insertTransaksiID);
                $lt->setMasterId($masterID);
                $temp = $lt->buildInvoice();
            }
    ...
    public function hutangKeKonsumen()
    {
        // https://everest.mayagrahakencana.com/ExcelWriter/row/5822spd?date1=2025-05-27&date2=2025-06-26&reqCode=czozMDoiamVuaXMgaW4gKCc1ODIyc3BkJywnNTgyM3NwZCcpIjs=
        $this->db->from('_rek_pembantu_subcustomer_cache');
        $this->db->where('periode', 'forever');
        $this->db->where('rekening', '2010050');
        $this->db->where('cabang_id', '1');
        $this->db->where('kredit >', '100');
        // $this->db->where('gudang_id >', '0');
        ...
        $jenisTr = $this->jenisTr;
        cekBiru(count($data));
        // arrPrintHijau($this->jenisTr);
        // arrPrint(array_slice($data,0,1));

        // Ambil semua extern2_id unik untuk header kolom kredit
        $extern2_ids = [];
        $extern2_namas = [];
        foreach ($data as $row) {
            $extern2_ids[$row->extern2_id] = $row->extern2_nama;
        }

        // Susun data per extern_id + extern_nama
        $result = [];
        foreach ($data as $row) {
            $key = $row->extern_id;
            if (!isset($result[$key])) {
            ...
            $result[$key]['kredit'][$row->extern2_id] = $row->kredit;
        }
        ...
        $headers = [
            ...
            "extern_nama" => array(
                "label" => "Konsumen",
                "link" => base_url() . "kas/_processPihak/select/$jenisTr/MdlCustomer_and_pre",
            ),
        ];
        $this->load->view('create', [
            "mode" => "hutangKeKonsumen",
            'jenisTr' => $jenisTr,
            'linkShoppingCart' => base_url() . "kas/_shoppingCart/reset/$jenisTr?cl=items",
            'headers' => $headers,
            'rows' => $result,
            'extern2_ids' => $extern2_ids,
        ]);

    }

    public function hutangTanpaIdentitas()
    {
        $this->load->model("Coms/ComRekening");
        $this->load->model("Mdls/MdlLockerValue");
        $rekening = "2010130";
        $locker_jenis = "uang tanpa identitas";
        $arrDataRekening = array();
        $jenisTr = $this->jenisTr;

        $c = New ComRekening();
        $c->addFilter("cabang_id=" . my_cabang_id());
        $cTmp = $c->fetchBalances($rekening);
        $arrDataRekening["nilai_buku"] = isset($cTmp[0]->kredit) ? $cTmp[0]->kredit : 0;

        $lc = New MdlLockerValue();
        $lc->addFilter("cabang_id=" . my_cabang_id());
        $lc->addFilter("jenis='$locker_jenis'");
        $lc->addFilter("nilai>'0'");
        $lcTmp = $lc->lookupAll()->result();
        if (sizeof($lcTmp) > 0) {
            foreach ($lcTmp as $ii => $lcSpec) {
                if (!isset($arrDataRekening[$lcSpec->state])) {
                    $arrDataRekening[$lcSpec->state] = 0;
                }
                $arrDataRekening[$lcSpec->state] += $lcSpec->nilai;
            }
        }
//arrPrintCyan($arrDataRekening);
        $result[0] = $arrDataRekening;
        $headers = [
            "nilai_buku" => array(
                "label" => "saldo buku",
//                "link" => "",
...
//                "link" => base_url() . "kas/_processPihak/select/$jenisTr/MdlCustomer_and_pre",
        ...
        $this->load->view('create', [
            "mode" => "hutangTanpaIdentitas",
//            "label" => "Hutang Atas Transfer Tanpa Identitas",
            "label" => "Saldo Uang Muka Tanpa Identitas",
            'jenisTr' => $jenisTr,
            'linkShoppingCart' => base_url() . "kas/_shoppingCart/reset/$jenisTr?cl=items",
            'headers' => $headers,
            'rows' => $result,
