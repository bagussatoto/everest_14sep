...
//            echo "</div>";
        ...
        if (isset($items5) && count($items5) > 0) {
            ...
            // LOGIKA MARKUP PROPORSIONAL (Opsi A)
            // Hitung modal material murni dari seluruh komponen BOM (biaya + produk) di items5
            $total_modal_bom = 0;
            if (isset($items5) && is_array($items5)) {
                foreach ($items5 as $row) {
                    if (isset($row['bahan_baku']['biaya'])) {
                        foreach ($row['bahan_baku']['biaya'] as $b) {
                            $total_modal_bom += (float)$b['saldo'];
                }
            }
                    if (isset($row['bahan_baku']['produk'])) {
                        foreach ($row['bahan_baku']['produk'] as $p_prod) {
                            $total_modal_bom += (float)$p_prod['saldo'];
                        }
                    }
                }
            }
            // Faktor markup (harga final project / total modal mentah)
            $markup_factor = ($total_modal_bom > 0) ? ($base_harga / $total_modal_bom) : 1;

            // Rasio akhir yang dikalikan ke setiap harga satuan material di Mode 4
            $rasio_markup = $rasio * $markup_factor;
            ...
            for ($i_opsi = 1; $i_opsi <= 4; $i_opsi++) {
                ...
                elseif ($i_opsi == 4) {
                    ...
                    foreach ($items5 as $row) {
                        if (isset($row['bahan_baku']['biaya'])) {
                            foreach ($row['bahan_baku']['biaya'] as $b) $jasa += ((float)$b['saldo'] * $rasio_markup);
                        }
                        if (isset($row['bahan_baku']['produk'])) {
                            foreach ($row['bahan_baku']['produk'] as $p_prod) {
                                ...
                                if ($kat == 'unit') {
                                    $unit += ((float)$p_prod['saldo'] * $rasio_markup);
                                    $exp = explode(" ", $nama_p);
                                    $m_v = isset($exp[0]) ? $exp[0] : '';
                                    if ($m_v != '' && !in_array($m_v, $merk_unit)) $merk_unit[] = $m_v;
                                }
                                else {
                                    $material += ((float)$p_prod['saldo'] * $rasio_markup);
                                }
                            }
                        }
                    }
                    ...
                    if ($total_render == 0 && $base_harga > 0) {
                        $jasa = $base_harga * $rasio;
                        $total_render = $jasa;
                        $contentStr .= '
                        <tr>
                            <td class="text-center">' . $no4++ . '.</td>
                            <td class="text-center">1 LOT</td>
                            <td class="text-left">DP ' . $persen_um . '% ' . (isset($p['nama']) ? strtoupper($p['nama']) : 'PEKERJAAN PROJECT') . '</td>
                            <td class="text-right">' . number_format($jasa) . '</td>
                            <td class="text-right">' . number_format($jasa) . '</td>
                        </tr>';
                    }
