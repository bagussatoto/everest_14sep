...
//                                        );
//                                        \" >";
//                            $strOnprog .= formatField_he_format($key, $val[$key]);
//                            $strOnprog .= "</a>";
//                            $strOnprog .= "</td>";
...
//                        $strOnprog .= formatField_he_format($key, $total[$key]);
//                        $strOnprog .= "</td>";
//                    }
//                }
//                $strOnprog .= "</tr>";
...
//            $strOnprogFooter = "<a class='btn btn-default' href='" . base_url() . $this->uri->segment(1) . "/viewIncomplete/" . $jenisTr . "'><span class='glyphicon glyphicon-time'></span> complete list ...</a>";
        ...
        $scriptBottom = "";
        if (isset($str_scriptBottom)) {
            $scriptBottom .= "";
            $scriptBottom .= $str_scriptBottom;
            $scriptBottom .= "";
        }
        ...
        //region loader hutang ke konsumen
        if ($showHutangKeKonsumen == true) {
            if (isset($hutang_ke_konsumen_ledger)) {
                $scriptBottom .= "<script>
                    top.$('#info_terbaru').load('$hutang_ke_konsumen');
                    top.$('#warning_item').append('Klik untuk melihat <a href=\"$hutang_ke_konsumen_ledger\" target=\"_blank\">Hutang ke Konsumen</a>');
                </script>";
            }
        }
        //endregion
        ...
        //region loader hutang ke konsumen
        if ($showHutangTanpaIdentitas == true) {
            if (isset($hutang_ke_konsumen_ledger)) {
                $scriptBottom .= "<script>
                    top.$('#info_terbaru').load('$hutang_tanpa_identitas');
//                    top.$('#warning_item').append('Klik untuk melihat <a href=\"$hutang_ke_konsumen_ledger\" target=\"_blank\">Hutang ke Konsumen</a>');
                </script>";
            }
        }
        ...
        if (sizeof($pic_list) > 0) {
            ...
            $strpic_list = "<div class='alert alert-info'>";
            $strpic_list .= "<h4>" . ucwords($pic_list["label"]) . "</h4>";
            $strpic_list .= "<span style='font-size:15px;'>" . ucwords($pic_list["pic"]) . "</span>";
            $strpic_list .= "</div>";
        }
...
//                arrPrint($iiSpec);
        ...
        $arrTags = array(
            ...
            "warning_item" => isset($warning_item) ? $warning_item : "",
            "pic_list" => isset($strpic_list) ? $strpic_list : "",
            "time_line" => $time_line,
        );
        if (sizeof($arrLinkMenuAdd) > 0) {
            $arrTags = $arrTags + $arrLinkMenuAdd;
...
////                    $str.=formatField_he_format($k,$pSpec[$k]);
//                        $str .= $pSpec[$k];
//                        $str .= "</td>";
//                    }
//                    $str .= "</tr>";
...
//                    $str .= "<div style='width:120px;' class='btn btn-xs btn-warning btn-block'><i class='fa fa-eye'></i> <b>".formatField_he_format('nomer',$pSpec['nomer'])."</b></div>";
//                    $str .= "</div>";
...
//        }
    ...
    case "viewKepoinItems":
        ...
        // if (sizeof($arrayOnProgress) > 0 || sizeof($arrayOnProgress2) > 0) {
        if (sizeof($kepoinFields) > 0) {

            $link_undone = base_url() . $this->modul . "/Transaksi/viewKepoinItems/$jenisTr";
            $str .= "<div class='box box-solid box-info'>";

            $str .= "<div class='box-header with-border text-uppercase'>";
            $str .= "<h4 class='box-title'><span class=\"glyphicon glyphicon-flash blink\"></span> $shortKepoinLabel</h4>";

            $str .= "<div class='box-tools pull-right'>";
            $str .= "<button type='button' class='btn btn-box-tool' onclick=\"$('#kepoinList').load('$link_undone');\"><i class='fa fa-refresh'></i></button>";
            $str .= "<button type='button' class='btn btn-box-tool' data-widget='collapse'><i class='fa fa-minus'></i></button>";
            $str .= "</div>";
            ...
            $arrayProgressLabels = $kepoinFields;
            if (sizeof($arrayOnProgressView) > 0) {
                /* -------------
                * bloking kolom state
                * untuk kembali versi lama $showState dibikin true
                * di contoler line 761-763 hidupkan scrip yg lama
                * ----------------------------------------*/
                ...
                $str .= "<tr line=" . __LINE__ . " class='bg-danger text-uppercase'>";
                foreach ($kepoinFields as $k => $label) {
                    $str .= "<th class='bg-primary'>";
                    if (is_array($label)) {
                        $str .= isset($label['label']) ? $label['label'] : "-";
                    }
                    else {
                ...
                *body table
                * ----------------------------------------*/
                foreach ($arrayOnProgressView as $key => $pSpec) {
                    $str .= "<tr line=" . __LINE__ . ">";
                    foreach ($kepoinFields as $k => $label) {
                        $str .= "<td>";
                        //                    $str.=formatField_he_format($k,$pSpec[$k]);
                    ...
                    // -----------------------------------state horisontal---------------------------
                    if ($showState != true) {
                        $strState = isset($pSpec['state']) ? $pSpec['state'] : "";
                        $str .= "<tr>";
                        $str .= "<td colspan='$jml_tampil'>$strState</td>";
                        $str .= "</tr>";
                    }
                ...
                $str .= tplNoData("Semua permintaan telah diselesaikan");
            }
...
//        }
        ...
        if (sizeof($steps) > 1) {
            ...
            foreach ($steps as $tStep => $stepData) {
                ...
                $strOnprog .= "<span class='text-uppercase text-bold' style='$visibility;'>$trSelesai" . $stepData['label'] . "</span>  $isiBadge </a>";
                $strOnprog .= "</li>";
            }
            $strOnprog .= "</ul>";
...
//                $strOnprogFooter = "";
        ...
        if (isset($arrayOnProgressToPay) && $arrayOnProgressToPay == true) {
            ...
            $strOnprog = "";
            $strOnprog .= "<div id='paymentSrc'>load..</div>";
            //top.$('#paymentSrc').load('https://demo.mayagrahakencana.com/teguh/kas/Kas/viewSelectPaymentExternSrc/759/?gr=cGVtYmVsaWFu')
            $link_payment = base_url() . $this->modul . "/Transaksi/viewSelectPaymentExternSrc/759/?gr=cGVtYmVsaWFu";
            $strOnprog .= "<script>
                top.$('#paymentSrc').load('$link_payment')
            </script>";
...
//            arrPrint($itemSrcLabels);
            ...
            if (sizeof($itemsSrc) > 0) {
                ...
                foreach ($itemsSrc as $itemsSrc_0) {
                    ...
                    $srcItems .= "<td>$n</td>";
                    foreach ($itemSrcLabels as $cKol => $cAlias) {
                        $srcItems .= "<td>" . formatField_he_format($cKol, $itemsSrc_0[$cKol]);
                        $srcItems .= "</td>";
                    }
                    $srcItems .= "</tr>";
                }
...
//                arrPrint($mainAddValues);
            ...
            if (isset($sumAddRows) && sizeof($sumAddRows) > 0) {
                ...
                foreach ($sumAddRows as $keyAdd => $label) {
                    $colspanX = sizeof($itemLabels2) > 1 ? sizeof($itemLabels2) + 1 : sizeof($itemLabels);
                    echo "<tr line=" . __LINE__ . ">";
                    echo "<td colspan='" . $colspanX . "' class='text-right'>$label</td>";
                    echo "<td class='text-right'>";
                    $val = 0;
...
//                        echo $mainAddValues[$key . "_tax"];
                        ...
                        echo formatField_he_format($key2, $val);
                        echo "</td>";
                        echo "</tr>";
                    }
                }
...
//
//
//                    echo formatField_he_format("total", $grandTotal);
//                    echo "</td>";
//                    echo "</tr>";
//                }
            }
...
//                echo "<td class='text-right'>";
//
//                echo formatField_he_format("tagihan", $main['tagihan']);
//                echo "</td>";
//                echo "</tr>";
//            }
        ...
        foreach ($mainLabels as $key => $label) {
            ...
            if (isset($main->$key)) {
                if (is_array($main->$key)) {
                    ...
                    echo formatField_he_format($key, $rslt_isi);
                }
                else {
                    echo formatField_he_format($key, $main->$key);
                }
            }
            else {
                if (isset($mainValues[$key])) {
                    echo formatField_he_format($key, $mainValues[$key]);
                }
                else {
                    echo "";
                }
...
//                                    cekHere(":: $key2 :: $new ::");
                                ...
                                if (isset($iSpec2[$key2])) {
                                    echo formatField_he_format($key2, $iSpec2[$key2]);
                                }
                                else {
                                    echo "";
                                }
...
//                            echo $mainValues[$key . "_tax"];
                            ...
                            if (in_array($key, $extEditableFields)) {
                                ...
                                echo formatField_he_format($key . "_tax", $val);
                            }
                            echo "</td>";
                            echo "</tr>";
                        }
                ...
                if (isset($sumRowsAdditional) && (sizeof($sumRowsAdditional) > 0)) {
                    foreach ($sumRowsAdditional as $key => $val) {
                        ...
                        echo formatField_he_format($key, $value);
                        echo "</td>";
                        echo "</tr>";
                    }
                }
...
//                    echo "<div class=''>Detail</div>";
                    ...
                    foreach ($addMainSourceField as $kol => $alias) {
                        if (isset($addMainSourceEdit[$kol])) {
                            ...
                            $strVal = "<input type=$inputType max='$max' name='$kol' class='form-control text-left' value='$val' onclick='this.select()' $addEvent>";
                            $tdOpt = "style='margin:0px;padding:0px;$addStyle' ";
                        }
                        else {
                            $strVal = formatField_he_format($kol, $mainValues[$kol]);
                        }
                        echo "<td $tdOpt>";
                        echo $strVal;
                        echo "</td>";
            ...
            if (isset($items) && sizeof($items) > 0) {
                if (isset($dpData) && sizeof($dpData) > 0) {
                    ...
                    foreach ($dpData['field'] as $dp_key => $dp_label) {
                        ...
                        echo "<td align='right'> " . formatField_he_format($dp_key, $dpData['value'][$dp_key]) . " </td>";
                        echo "</tr>";
                    }
                    echo "</table>";
                }
            ...
            if (isset($items) && sizeof($items) > 0) {
                ...
                if (isset($saldoLocker) && (sizeof($saldoLocker) > 0)) {
                    ...
                    foreach ($saldoLocker as $md => $mdSpec) {
                        $mdName = formatField_he_format("name", $mdSpec['name']);
                        $mdNilai = formatField_he_format("nilai", $mdSpec['nilai']);
...
//                        cekBiru("$elName");
                        if (array_key_exists($elName, $elementConfig)) {
                            cekKuning($elementEditTarget . $elName);
                            echo "<tr line=" . __LINE__ . ">";
                            echo "<td align='right'>";
                            echo "<span class='text-muted'>" . $aSpec['label'] . " &nbsp;&nbsp;&nbsp;</span>";
                            if ($elName != "cash_account" && in_array($elName, $editableElements)) {
                                $editLink = "BootstrapDialog.show(
                                   {
                                       title:'$elName',
                            ...
                            if ($elName == "cash_account" && isset($cashAccountInlineHtml) && isset($cashAccountInlineHtml['cash_account']) && strlen($cashAccountInlineHtml['cash_account']) > 0) {
                                echo "<div style='margin-bottom:2px;display:inline-block;'>";
                                echo $cashAccountInlineHtml['cash_account'];
                                echo "</div>";
                            }
                            switch ($elementConfig[$elName]['elementType']) {
                                case "dataModel":
                ...
                if (sizeof($mainElements) > 0) {
                    ...
                    foreach ($mainElements as $elName => $aSpec) {
                        ...
                        if (array_key_exists($elName, $elementConfig)) {
                            ...
                            echo "<td colspan='" . (sizeof($itemLabels)) . "' bgcolor='#ffffff'>";
                            if ($elName == "cash_account" && isset($cashAccountInlineHtml) && isset($cashAccountInlineHtml['cash_account']) && strlen($cashAccountInlineHtml['cash_account']) > 0) {
                                echo "<div style='margin-bottom:8px;display:inline-block;'>";
                                echo $cashAccountInlineHtml['cash_account'];
                                echo "</div>";
                            }
...
//                    arrPrint($items[$id]);
                    foreach ($itemLabels as $key => $label) {
                        ...
                        $contentStr .= "<td>";
                        $contentStr .= formatField_he_format($key, $val);
                        $contentStr .= "</td>";
                    }
                ...
                if (isset($extValueLabels) && sizeof($extValueLabels) > 0) {
                    ...
                    foreach ($extValueLabels as $key => $lSpec) {
                        ...
                        if (isset($lSpec['taxFactor']) && $lSpec['taxFactor'] > 0) {
                            ...
                            $contentStr .= "<td class='text-right'>";
                            $contentStr .= formatField_he_format($key . "_tax", $val);
                            $contentStr .= "</td>";
                            $contentStr .= "</tr>";
                        }
                    }
            ...
            if (isset($items2) && sizeof($items2) > 0) {
                ...
                foreach ($items2 as $id => $iSpec) {
                    ...
                    foreach ($itemLabels2 as $key => $label) {
                        ...
                        $val = isset($iSpec[$key]) ? $iSpec[$key] : "";
                        $contentStr2 .= "<td>";
                        $contentStr2 .= formatField_he_format($key, $val);
                        $contentStr2 .= "</td>";
                    }
                    $contentStr2 .= "</tr>";
                    if (isset($noteEnabled) && ($noteEnabled == true)) {
...
//                                cekHere("$key " . $mainValues[$key]);
                                ...
                                if (isset($mainValues[$key2])) {
                                    $contentStr2 .= formatField_he_format($key2, $mainValues[$key2]);
                                }
                                else {
                                    $contentStr2 .= "0";
                                }
            ...
            if (isset($items3) && sizeof($items3) > 0) {
                ...
                foreach ($items3 as $id => $iSpec) {
                    ...
                    foreach ($itemLabels3 as $key => $label) {
                        ...
                        $val = isset($iSpec[$key]) ? $iSpec[$key] : "";
                        $contentStr4 .= "<td>";
                        $contentStr4 .= formatField_he_format($key, $val);
                        $contentStr4 .= "</td>";
                    }
                    $contentStr4 .= "</tr>";
                    if (isset($noteEnabled) && ($noteEnabled == true)) {
...
//                $contenStr5 .= "<div class='panel-body'>";
                ...
                foreach ($mainData2Fields as $fieldsKey => $add_fields) {
//                    cekHitam($fieldsKey);
                    $contenStr5 .= "<td>" . formatField_he_format($fieldsKey, $mainData2[$fieldsKey]) . "</td>";
                }
                $contenStr5 .= "</tr>";
                $contenStr5 .= "<tr line=" . __LINE__ . ">";
                if (strlen($inWord2) > 5) {
                    ...
                    foreach ($sumRows as $key => $label) {
                        ...
                        if (isset($mainValues2[$key])) {
                            if ($mainValues2[$key] > 0) {
                                ...
                                $contentStr .= "<td class='text-right'>";
                                if (isset($mainValues2[$key])) {
                                    $contentStr .= formatField_he_format($key, $mainValues2[$key]);
                                }
                                else {
                                    $contentStr .= "0";
                                }
...
//                    $contentStr3 .="<div class='col-md-2 font-size-1-2'>".formatField_he_format($dp_fields,$dpValueDetils[$dp_fields])."</div>";
                }
                $contentStr3 .= "</table>";
                $contentStr3 .= "</div>";
...
//                    $contentStr .= "<td class='text-right'>";
//
//                    $contentStr .= formatField_he_format("total", $grandTotal);
//                    $contentStr .= "</td>";
//                    $contentStr .= "</tr>";
//                }
            }
...
//                                cekHere($mainValues[$key]);
                                ...
                                $contentStr .= "<td class='text-right'>";
                                if (isset($mainValues[$key])) {
                                    $contentStr .= formatField_he_format($key, round($mainValues[$key]));
                                }
                                else {
                                    $contentStr .= "0";
                                }
                ...
                foreach ($itemLabels as $key => $label) {
                    ...
                    if (isset($total_bawah[$key])) {
                        if (is_numeric($total_bawah[$key])) {
                            $contentStr .= formatField_he_format($key, $total_bawah[$key]);
                        }
                        else {
                            $contentStr .= "";
                        }
...
//        }
        ...
        if (sizeof($items) > 0) {
            ...
            $strContent .= "<table id='paymentSrc' class='table cTable-modal table-condensed table-responsive no-padding table-bordered'>";
            ...
            foreach ($items as $key => $val) {
                $nob++;
                $jenisVal = $val['jenis'];
                $modul_pathSrc = $this->config->item("heTransaksi_ui");
                $modulPathVal = isset($modul_pathSrc[$jenisVal]['modul']) ? base_url() . $modul_pathSrc[$jenisVal]['modul'] : "non";

                // arrPrintPink($modul_pathSrc);
                ...
                if (sizeof($itemLabels) > 0) {
                    ...
                    //----------------------------------------------------------------
                    $isIdleLock = isset($val['idle_lock']) && $val['idle_lock'] == true;
                    $lockHolder = isset($val['lock_holder']) ? $val['lock_holder'] : "";

                    $strContent .= "<div class='funkyradio-success'>";
                    $urut_class = "urut_$nob";
//                    if ($isIdleLock) {
//                        $strContent .= "<input class='chRadio paymentSrc' type=checkbox $checked $disabled $disabledLockerTransaksi $disabledPaymentSrc
//                        value='" . $val['transaksi_id'] . "' id='opt" . $val['transaksi_id'] . "'
//                        onclick=\"
//                            var chk = this;
//                            chk.checked = false;
//                            swal({
//                                title: 'Ambil Alih Nota?',
//                                text: 'Nota ini sedang dibuka oleh ' + '" . addslashes($lockHolder) . "' + ' yang terdeteksi tidak aktif lebih dari 10 menit. Apakah Anda yakin ingin mengambil alih?',
//                                type: 'warning',
...
//                                curUrl = curUrl.replace(/[?&]forceRetakeLock=[^&]*/g, '').replace(/[?&]retakeTrID=[^&]*/g, '');
//                                var joinChar = curUrl.indexOf('?') !== -1 ? '&' : '?';
//                                var ajaxUrl = curUrl + joinChar + 'forceRetakeLock=1&retakeTrID=" . $val['transaksi_id'] . "';
//                                $.get(ajaxUrl, function() {
//                                    chk.checked = true;
//                                    document.getElementById('result').src = '" . base_url() . $this->modul . "/$selectProcessor/$jenisTr" . "?$qstr&state=true" . "';
//                                    chk.onclick = function() {
//                                        document.getElementById('result').src = '" . base_url() . $this->modul . "/$selectProcessor/$jenisTr" . "?$qstr&state=' + this.checked;
//                                    };
//                                });
//                            });
//                        \"
//                        >";
//                    }
//                    elseif (($pembayaranBerurutan == true) && ($lanjutBerurutan == true)) {
                    if (($pembayaranBerurutan == true) && ($lanjutBerurutan == true)) {

                        $strContent .= "<input urut='$nob' class='chRadio paymentSrc $urut_class' type=checkbox $checked $disabled $disabledLockerTransaksi $disabledPaymentSrc 
                        value='" . $val['transaksi_id'] . "' id='opt" . $val['transaksi_id'] . "' 
                        ooonclick=\"document.getElementById('result').src='" . base_url() . $this->modul . "/$selectProcessor/$jenisTr" . "?$qstr&state='+this.checked;\"
                        ...
                        $strContent .= "<input class='chRadio paymentSrc' type=checkbox $checked $disabled $disabledLockerTransaksi $disabledPaymentSrc 
                        value='" . $val['transaksi_id'] . "' id='opt" . $val['transaksi_id'] . "' 
                        onclick=\"document.getElementById('result').src='" . base_url() . $this->modul . "/$selectProcessor/$jenisTr" . "?$qstr$modePym&state='+this.checked;\"
                        >";
                    }
                    ...
                    foreach ($itemLabels as $key => $label) {
                        ...
                        // arrPrint($val);
                        if (isset($val[$key])) {
                            $strContent .= strlen($val[$key]) > 0 ? formatField_he_format($key, $val[$key], $jenisVal, $modulPathVal) : "-";
                            if (is_numeric($val[$key])) {
                                if (!isset($total[$key])) {
                                    $total[$key] = 0;
                                }
...
//            $strContent .= "<script>
//                    $(document).ready( function(){
//                        var table = $('#paymentSrc').DataTable({
//                            dom: 'lBfrtip',
//                            fixedHeader: false,
//                            lengthMenu: [ [10, 20, 50, 100, -1], [10, 20, 50, 100, 'All'] ],
//                            pageLength: -1,
...
//            function check_action(c){
////                var link_enc = decodeURIComponent(link);
//                // Ketika ceklis dengan kelas 'urut_2' diubah
//                $('.urut_'+c).change(function() {
//                    // Jika ceklis urut 2 dicentang, ceklis urut 1 juga dicentang
//                    if($(this).prop('checked')) {
//                        for(let i=1;i<c;i++){
////                            console.log('ceklist checkbox '+i);
////                            console.log('link action '+link);
...
////                            console.log('un ceklist checkbox '+i);
////                            $('.urut_'+i).prop('checked', false);
//////                            $('#result').attr('src', link + '&state=false');
////                        }
////                    }
////                   
////                });
...
//                                            + \"<div class='text-right'>\"+addCommas(total5)+\"</div>\"
    ...
    case "viewSelectPaymentExternSrc":
...
//        cekHere(":: $title ::");
        $scriptBottom = "";
        $strContent = "";
        if (isset($shoppingCartPerTransaksiBtn)) {
            foreach ($shoppingCartPerTransaksiBtn as $itemBtn => $paramBtn) {
                $btnLabel = isset($paramBtn['label']) ? $paramBtn['label'] : $itemBtn;
                if (isset($paramBtn['show_default'])) {
                    $show_default_id = $paramBtn['show_default'];
                }
                $addLinkStr .= "<button type='button' class='btn btn-primary' id='$itemBtn'><i class='fa fa-thumbs-up'></i>$btnLabel</button> ";
            }
        }
        ...
        $strContent .= "<div class='box-header with-border text-uppercase'>";
        $strContent .= "<h4 class='box-title'><span class=\"glyphicon glyphicon-flash blink\"></span> on-going transactions</h4>";
        $strContent .= "<span class='pull-right'>$addLinkStr</span>";
        $strContent .= "</div>";
        $strContent .= "<div class='box-body'>";
        $strContent .= "<div class='table-responsive pembayaran'>";

        if (sizeof($items) > 0) {
            $strContent .= "<div id='prog_satu'>";
            $strContent .= "<table id='arrayOnProgressToPay' class='table cTable table-condensed table-striped table-bordered'>";
            $strContent .= "<thead>";
            $strContent .= "<tr bgcolor='#f0f0f0'>";
            if (sizeof($itemLabels) > 0) {
                $strContent .= "<td class='text-muted text-right'>";
...
//                                                cekHere("$key => $label");
                        if ($key != "button") {
                            ...
                            $strContent .= "<td data-order='' class='" . $val['class_marking'] . "'>";
                            $link_modal = "top.BootstrapDialog.show(
                                   {
                                       title:'$title - " . $val['extern_nama'] . "',
                                       message: " . '$' . "('<div></div>').load('" . $val['link'] . "'),
                                        size:top.BootstrapDialog.SIZE_WIDE,
...
//                            $strContent .= "<a href='JavaScript:void(0)' title='make a $title with " . $val['extern_nama'] . "' data-toggle='tooltip' data-placement='right'
//                                onclick=\"$link_modal\" >";
//                            $strContent .= formatField_he_format($key, $val[$key]);
                            $strContent .= "<button class='btn btn-success' onclick=\"$link_modal\">$label</button>";
//                            $strContent .= "</a>";
                            $strContent .= "</td>";
                        }
                    }
            ...
            $strContent .= "<tr bgcolor='#f0f0f0'>";
            $strContent .= "<td></td>";
            $strContent .= "<td class='text-muted' colspan='$total_col_span'>total amount of '$srcLabel'</td>";
            foreach ($itemLabels as $key => $label) {
                if (isset($total[$key])) {
                    $strContent .= "<td class='text-muted'>";
                    $strContent .= formatField_he_format($key, $total[$key]);
            ...
            /* --------------------------------------------------------------------------------------
             * tampilan kedua timeline
             * --------------------------------------------------------------------------------------*/
            if (sizeof($arrayOnProgressTransaksi) > 0) {
                $headKoloms = $arrayOnProgressTransaksiKoloms;
                // arrPrintWebs($headKoloms);
                // arrPrintWebs($shoppingCartPerTransaksiBtn);
//                foreach ($shoppingCartPerTransaksiBtn as $itemBtn => $paramBtn) {
//                    $btnLabel = isset($paramBtn['label']) ? $paramBtn['label'] : $itemBtn;
//                    if (isset($paramBtn['show_default'])) {
//                        $show_default_id = $paramBtn['show_default'];
//                    }
//
//                    $addLinkStr .= "<button type='button' class='btn btn-xs btn-danger' id='$itemBtn'><i class='fa fa-thumbs-up'></i>$btnLabel</button> ";
//                }
                // cekBiru($show_default_id);
                /* -----------------------------------------------------------------------
                 * logic penampil defauld timeline atau group
                 * -----------------------------------------------------------------------*/
                if ($show_default_id == "prog_dua") {
                    $strBottom = "$(\"#prog_satu\").css(\"display\", \"none\");
                                                    $(\"#prog_dua\").css(\"display\", \"block\");";
                }
                else {
                    $strBottom = "$(\"#prog_dua\").css(\"display\", \"none\");
                                                    $(\"#prog_satu\").css(\"display\", \"block\");
                                                    $(\"#btn_dua\").css(\"background-color\", \"#00a65a\");";
                }

                $strOnProgTransaksi .= "<div id='prog_dua'>";
                $strOnProgTransaksi .= "<table id='arrayOnProgressToPayTransaksi' class='table table-condensed table-hover'>";
                $strOnProgTransaksi .= "<thead>";
                $strOnProgTransaksi .= "<tr class='bg-orange'>";
                $strOnProgTransaksi .= "<th>no</th>";
                foreach ($headKoloms as $headKolom => $headParams) {
                    $label = isset($headParams['label']) ? $headParams['label'] : $headKolom;

                    $strOnProgTransaksi .= "<th>$label</th>";
                }
                $strOnProgTransaksi .= "</tr>";
                $strOnProgTransaksi .= "</thead>";
                // $link_exe = base_url() ."Transaksi/selectPaymentSrc/749/39";
                $nom = 0;
                $trIds = array();
                $arrTotalBawah = array();
                foreach ($arrayOnProgressTransaksi as $itemProgs) {
                    $nom++;
                    $transaksi_id = $itemProgs->transaksi_id;
                    if (isset($arrNotaINV_settlement[$transaksi_id])) {
                        foreach ($arrNotaINV_settlement[$transaksi_id] as $skey => $sval) {
                            $itemProgs->$skey = $sval;
                        }
                    }
                    ...
                    $trIds[] = $transaksi_id;
                    $strOnProgTransaksi .= "<tr>";
                    $strOnProgTransaksi .= "<th>$nom</th>";
                    foreach ($headKoloms as $headKolom => $headParams) {
                        $label = isset($headParams['label']) ? $headParams['label'] : $headKolom;

                        $nilaiProg = isset($itemProgs->$headKolom) ? $itemProgs->$headKolom : "";
                        $nilaiProg_f = isset($headParams['format']) ? $headParams['format']($headKolom, $nilaiProg) : $nilaiProg;

                        if (isset($headParams['link'])) {
                            $linkSeg_4 = isset($headParams['linkSeg_4']) ? $itemProgs->$headParams['linkSeg_4'] : "";
                            $linkSeg_5 = $itemProgs->transaksi_id;

                            $link = base_url() . $headParams['link'] . "$linkSeg_4/$linkSeg_5";

                            if (isset($headParams['linkType']) && $headParams['linkType'] == "modal") {
                                $modalHeader = isset($headParams['modalHeader']) ? $itemProgs->$headParams['modalHeader'] : "none";

                                $link_model = modalDialogBtn($modalHeader, $link);
                            }
                            else {
                                $link_model = "location.href='$link'";
                            }

                            if (isset($headParams['tipe']) && $headParams['tipe'] == "button") {
                                $nilaiProg_l = "<button class='btn btn-sm btn-success' onclick=\"$link_model\">$label</button>";
                            }
                            else {
                                $nilaiProg_l = "<a href='JavaScript:Void(0);' onclick=\"$link_model\">$nilaiProg_f</a>";
                            }
                        }
                        else {
                            $nilaiProg_l = $nilaiProg_f;
                        }

                        $strOnProgTransaksi .= "<td>$nilaiProg_l</td>";

                        if (is_numeric($nilaiProg)) {
                            if (!isset($arrTotalBawah[$headKolom])) {
                                $arrTotalBawah[$headKolom] = 0;
                            }
                            $arrTotalBawah[$headKolom] += $nilaiProg;
                        }
...
//                    $strOnProgTransaksi .= "</tr>";
                ...
                $strOnProgTransaksi .= "<tfoot>";
                $strOnProgTransaksi .= "<tr>";
                $strOnProgTransaksi .= "<th>-</th>";
                foreach ($headKoloms as $headKolom => $headParams) {

                    $nilai_bawah = isset($arrTotalBawah[$headKolom]) ? $arrTotalBawah[$headKolom] : "";
                    $nilaiProg_f = isset($headParams['format']) ? $headParams['format']($headKolom, $nilai_bawah) : $nilai_bawah;
                    $nilaiProg_f = ($headKolom == "dtime") ? "-" : $nilaiProg_f;
                    $strOnProgTransaksi .= "<th>$nilaiProg_f</th>";
                }
                $strOnProgTransaksi .= "</tr>";
                $strOnProgTransaksi .= "</tfoot>";

                $strOnProgTransaksi .= "</table>";
                $strOnProgTransaksi .= "</div>";

                $trIds_e = blobEncode($trIds);

//                $link_settlement = base_url() . "ActivityReport/viewSetorKePusat/$jenisTr?trIDS=$trIds_e";
//                $link_settlement_modial = modalDialogBtn("Settlement", $link_settlement);
//                // cekMerah($showBtnSettlement);
//                if ($showBtnSettlement) {
//                    $strOnprogFooter = "<button type='button' class='btn btn-danger' onclick=\"$link_settlement_modial\">SETTLEMENT</button>";
//                }

                $scriptBottom .= "<script>
                            $strBottom
                            $(\"#btn_satu\").click(function(){
                                    $(\"#prog_satu\").fadeOut();
                                    $(\"#prog_dua\").fadeIn();
                                    $(\"#btn_satu\").css(\"background-color\", \"#00a65a\");
                                    $(\"#btn_dua\").css(\"background-color\", \"#3c8dbc\");
                                });
                            $(\"#btn_dua\").click(function(){
                                    $(\"#prog_satu\").fadeIn();
                                    $(\"#prog_dua\").fadeOut();
                                    $(\"#btn_satu\").css(\"background-color\", \"#3c8dbc\");
                                    $(\"#btn_dua\").css(\"background-color\", \"#00a65a\");
                                });
                      </script>";
            ...
            if (sizeof($arrayOnProgressTransaksi) > 0) {
                $strContent .= $strOnProgTransaksi;
            }

            $strContent .= "<script>
                    $(document).ready( function(){
                        var table = $('#arrayOnProgressToPay').DataTable({
                            dom: 'lBfrtip',
                            fixedHeader: false,
                            lengthMenu: [ [10, 20, 50, 100, -1], [10, 20, 50, 100, 'All'] ],
                            pageLength: -1,
            ...
            $strContent .= "<script>
                    $(document).ready( function(){
                        var table = $('#arrayOnProgressToPayTransaksi').DataTable({
                            dom: 'lBfrtip',
                            fixedHeader: false,
                            lengthMenu: [ [10, 20, 50, 100, -1], [10, 20, 50, 100, 'All'] ],
                            pageLength: -1,
...
//            </script>";
            ...
            $str = "-the item you specified has no entry-";
            $strContent .= tplNoData($str);
        }

        $strContent .= $scriptBottom;
        $strContent .= "</div class='responsive'>";
        $strContent .= "</div class='box-body'>";
        $strContent .= "</div class='box box-danger'>";
        ...
        //endregion
        // <div class='box box-solid box-info'><div class='box-header with-border text-uppercase'><h4 class='box-title'><span class='glyphicon glyphicon-flash blink'></span> transaksi belum selesai</h4><div class='box-tools pull-right'><button type='button' class='btn btn-box-tool' onclick='$('#requestItems').load('https://demo.mayagrahakencana.com/teguh/kas/Transaksi/viewKepoinItems/759');'><i class='fa fa-refresh'></i></button><button type='button' class='btn btn-box-tool' data-widget='collapse'><i class='fa fa-minus'></i></button></div></div><div class='box-body'>
        $this_link = base_url() . "";
        $btn_tool = "";
        // $btn_tool .= "<button type='button' class='btn btn-box-tool' onclick='$('#requestItems').load('$this_link');'><i class='fa fa-refresh'></i></button>";
        $btn_tool .= "<button type='button' class='btn btn-box-tool' data-widget='collapse'><i class='fa fa-minus'></i></button>";
        $box_title = "setoran yang harus dilakukan";
        echo $strContent;
//        $p->setLayoutBoxBody(true);
//        $p->setLayoutBoxCss("box-danger box-solid");
//        $p->setLayoutBoxHeading($box_title, $btn_tool);
//
//        cekBiru($connectTo);
//        if (isset($connectTo) && $connectTo == true) {
//            echo $p->layout_box($strContent);
//        }
                    ...
                    if (isset($detail2_items) && sizeof($detail2_items) > 0) {
                        ...
                        foreach ($detail2_items as $iSpec) {
                            ...
                            foreach ($item2Labels as $key => $label) {
                                ...
                                if (is_numeric($iSpec[$key])) {
                                    if (!isset($arrSummary2[$key])) {
                                        $arrSummary2[$key] = 0;
                                    }
                                    $arrSummary2[$key] += $iSpec[$key];
                                }
                        ...
                        foreach ($item2Labels as $key => $label) {
                            echo "<td>";
                            if (isset($arrSummary2[$key])) {
                                $sum_val = $arrSummary2[$key];
                            }
                            else {
                                $sum_val = "-";
                            }
                            echo formatField_he_format($key, $sum_val);
                            echo "</td>";
                        }
                        echo "</tr>";
...
//                $contents .= "<td class='text-uppercase text-grey-3'>$alias</td>";
                    ...
                    $contents .= "<td>:</td>";
                    $contents .= "<td style='padding-left:10px;'>" . formatField_he_format($col, $val) . "</td>";
                    $contents .= "</tr>";
                }
            }
            $contents .= "</table>";
            ...
            if (isset($items) && sizeof($items) > 0) {
                ...
                foreach ($items as $itemsData) {
                    ...
                    $no++;
                    $contents .= "<tr line=" . __LINE__ . ">";
                    $contents .= "<td class='text-right'>" . formatField_he_format('number', $no) . "</td>";
                    foreach ($itemsLabel as $itemCol => $label) {
                        ...
                        if (isset($itemsKolomLink) && in_array($itemCol, $itemsKolomLink)) {
                            ...
                            $contents .= formatField_he_format($itemCol, $value);
                            $contents .= "</a>";
                        }
                        else {
                            $contents .= formatField_he_format($itemCol, $value);
...
//            cekHere($colspanTotal);
            if (isset($mainSumDetailsFieldsLabel) && sizeof($mainSumDetailsFieldsLabel) > 0) {
                ...
                foreach ($mainSumDetailsFieldsLabel as $sumDCol => $sum_Dalias) {
                    ...
                    $contents .= "<td>" . formatField_he_format($sumDCol, $valSum) . "</td>";
                }
                $contents .= "</tr>";
                $contents .= "<tr line=" . __LINE__ . ">";
            ...
            foreach ($mainSumFieldsLabel as $kolSum => $alias) {
                ...
                $contents .= "<td colspan='3'> " . formatField_he_format($kolSum, $val) . "</td>";
                $contents .= "</tr>";
            ...
            if (sizeof($reviewAddRows) > 0) {
                foreach ($reviewAddRows as $aKol => $aAlias) {
                    ...
                    $contents .= "<td colspan='3'> " . formatField_he_format($aKol, $val_row) . "</td>";
                    $contents .= "</tr>";
                }
            }
            //endregion
...
//                                    ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL&
                    ...
                    foreach ($arrayProgressLabels['itemsSrc'] as $key => $label) {
                        if (isset($sumFooter['itemsSrc'][$key])) {
                            ...
                            $strOnprog .= formatField_he_format($key, $sumFooter['itemsSrc'][$key]);
...
//                                    ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL&
                                    ...
                                    $strOnprog2 .= formatField_he_format($key, $val[$key]);
                                }
                                $strOnprog2 .= "</td>";
                            }
                        }
                    ...
                    foreach ($arrayProgressLabels['items'] as $key => $label) {
                        if (isset($sumFooter['itemsSrc'][$key])) {
                            ...
                            $strOnprog2 .= formatField_he_format($key, $sumFooter['items'][$key]);
//                            $strOnprog .=$sumFooter['itemsSrc'][$key];
                            $strOnprog2 .= "</td>";
                        }
                        else {
