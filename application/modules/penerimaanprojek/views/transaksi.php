...
//                            cekkuning("$elName dataField");
            ...
            if (strlen($description) > 0) {
                ...
                echo "<span class='' style='font-style:italic;font-family:Monaco, Menlo, Consolas, \"Courier New\", monospace;'>" . nl2br(str_ireplace(array('&lt;br&gt;', '&lt;br/&gt;', '&lt;br /&gt;', '<br>', '<br/>', '<br />'), "\n", (string)$description)) . "</span><br>";
                echo "</td>";
                echo "</tr>";
                echo "</table>";
            }
...
//                }
        ...
        echo "<script>$('.modal.in:last').addClass('size-wide').find('.modal-dialog:first').addClass('modal-lg modal-xl').css({'width':'95%','max-width':'1400px'});</script>";
        break;
...
//        $cPrint    .= "<SMALL>------------------------------------------";
    ...
    case "selectPaymentSrc":
        ...
        if (sizeof($items) > 0) {
            ...
            $strContent .= "<table class='table dataTable compact table-bordered'>";
            ...
            foreach ($items as $key => $val) {
                ...
                $strContent .= "<td class='" . $val['class_bg'] . "text-muted text-center'>$num</td>";
                if (sizeof($itemLabels) > 0) {
...
//                        "id_master" => "id_master",
                    ...
                    $strContent .= "<input class='chRadio' type=checkbox $checked $disabled $disabledLockerTransaksi $disabledPaymentSrc value='" . $val['transaksi_id'] . "' id='opt" . $val['transaksi_id'] . "' onclick=\"document.getElementById('result').src='" . MODUL_PATH . "$selectProcessor/$jenisTr" . "?$qstr&state='+this.checked;\">";
                    ...
                    foreach ($itemLabels as $key => $label) {
                        ...
                        if (isset($val[$key])) {
                            $strContent .= strlen($val[$key]) > 0 ? formatField_he_format($key, $val[$key], $val['jenis_master'], $val['modul_path']) : "-";
                            if (is_numeric($val[$key])) {
                                if (!isset($total[$key])) {
                                    $total[$key] = 0;
                                }
        ...
        $strContent .= "<script> top.$('.modal-dialog').toggleClass('modal-xl'); </script>";
...
//                        cekHere($key);
            ...
            $strContent .= "<tr bgcolor='#f0f0f0'>";
            $strContent .= "<td></td>";
            $strContent .= "<td class='text-muted'>total amount</td>";
...
//                                    ],
    ...
    case "viewResume":
        ...
        // view jurnal penyesuaian amandemen (jika ada mutasi amandemen pada keluarga transaksi ini)
        if (isset($amandemen_items) && sizeof($amandemen_items) > 0) {
            $adj_trx_no = isset($amandemen_info['transaksi_no']) ? $amandemen_info['transaksi_no'] : '';
            $adj_dtime = isset($amandemen_info['dtime']) ? formatField_he_format("date_time", $amandemen_info['dtime']) : '';
            $adj_delta = isset($amandemen_info['delta']) ? (float)$amandemen_info['delta'] : 0;
            $delta_badge = ($adj_delta != 0) ? " [Koreksi Delta " . ($adj_delta < 0 ? "-" : "+") . "Rp " . number_format(abs($adj_delta), 0, ',', '.') . "]" : "";

            foreach ($amandemen_items as $cabangID => $subItems) {
                if (sizeof($subItems) > 0) {
                    $cabangNama = isset($cabangData[$cabangID]) ? $cabangData[$cabangID] : "";

                    echo "<div style='margin-top:25px; border-top: 2px dashed #ffc107; padding-top:15px;'>";
                    echo "<h4 class='text-warning' style='color:#b25e00; margin-bottom: 5px;'><span class='fa fa-edit'></span> journal entries penyesuaian ($cabangNama) " . formatField_he_format("nomer", $adj_trx_no) . " <span class='badge' style='background-color:#fff3cd; color:#856404; border:1px solid #ffeeba; font-size:0.85em;'>$delta_badge</span></h4>";
                    echo "<span style='font-size:0.85em; color:#6c757d; display:block; margin-bottom:10px;'>Waktu Penyesuaian: $adj_dtime &bull; Dokumen Tagihan Diskon / Adjustment: <b>$adj_trx_no</b></span>";
        // END OF COMPLETE REPEATED LOGIC

                    echo "<div class='table-responsive'>";
                    echo "<table class='table table-condensed' style='background-color:#fffdfa;'>";

                    $tot_adj_cabang = array();
                    foreach ($subItems as $urut => $mSpec) {
                        echo "<tr bgcolor='#fdf6e7' style='border-top:1px solid #f1dfbc;'>";
                        foreach ($headers as $key => $label) {
                            echo "<td style='font-weight:bold; color:#7d5100;'>";
                            echo "$label";
                            echo "</td>";
                        }
                        echo "</tr>";
                        ...
                        foreach ($mSpec as $iSpec) {
                            ...
                            foreach ($headers as $key => $label) {
                                echo "<td>";
                                echo "<a href='" . $iSpec['link'] . "' target='_blank' style='color:#333;'>";
                                echo formatField_he_format($key, $iSpec[$key]);
                                echo "</a>";
                                echo "</td>";
                                if (is_numeric($iSpec[$key])) {
                                    if (!isset($tot_adj_cabang[$cabangID][$urut][$key])) {
                                        $tot_adj_cabang[$cabangID][$urut][$key] = 0;
                                    }
                                    $tot_adj_cabang[$cabangID][$urut][$key] += $iSpec[$key];
                                }
                            }
                            echo "</tr>";
                        }

                        echo "<tr style='font-size: 14px; font-weight: bold; background-color:#fef8eb;'>";
                        foreach ($headers as $key => $label) {
                            echo "<td>";
                            if (isset($tot_adj_cabang[$cabangID][$urut][$key])) {
                                echo formatField_he_format($key, $tot_adj_cabang[$cabangID][$urut][$key]);
                            }
                            echo "</td>";
                        }
                        echo "</tr>";
                    ...
                    // Panel Ringkasan Saldo Net Buku Besar
                    if (isset($net_ledger_summary) && !empty($net_ledger_summary)) {
                        echo "<div style='background-color:#f4f9fd; border:1px solid #b8daff; border-radius:4px; padding:12px 16px; margin-top:12px; margin-bottom:20px;'>";
                        echo "<h5 style='margin-top:0; color:#004085; font-weight:bold;'><span class='fa fa-balance-scale'></span> Ringkasan Saldo Buku Besar Berjalan (Net Ledger Position)</h5>";
                        echo "<table style='width:100%; max-width:700px; font-size:13px; line-height:24px;'>";
                        echo "<tr><td style='width:240px;'><b>Piutang Dagang Riil:</b></td><td style='color:#28a745; font-weight:bold;'>Rp " . number_format($net_ledger_summary['net_piutang'], 0, ',', '.') . " <span style='font-weight:normal; color:#666;'>(Debet)</span></td></tr>";
                        echo "<tr><td><b>Pendapatan Penjualan Riil:</b></td><td style='color:#0056b3; font-weight:bold;'>Rp " . number_format($net_ledger_summary['net_penjualan'], 0, ',', '.') . " <span style='font-weight:normal; color:#666;'>(Kredit)</span></td></tr>";
                        echo "<tr><td><b>PPN Keluaran Riil:</b></td><td style='color:#0056b3; font-weight:bold;'>Rp " . number_format($net_ledger_summary['net_ppn'], 0, ',', '.') . " <span style='font-weight:normal; color:#666;'>(Kredit)</span></td></tr>";
                        echo "<tr><td><b>Status Pelunasan Piutang:</b></td><td>";
                        if ($net_ledger_summary['is_lunas'] == 1) {
                            echo "<span class='badge' style='background-color:#d4edda; color:#155724; border:1px solid #c3e6cb; padding:4px 8px; font-size:0.9em;'>LUNAS PENUH (Sisa Rp 0)</span>";
                        } else {
                            echo "<span style='color:#d9534f; font-weight:bold;'>Sisa Rp " . number_format($net_ledger_summary['sisa'], 0, ',', '.') . "</span>";
                        }
                        echo "</td></tr>";
                        echo "</table>";
                        echo "</div>";
