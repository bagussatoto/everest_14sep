...
class Transaksi extends Modul_Controller
{
    ...
    public function validate()
    {
        ...
        if ((string)$this->jenisTr === "7499") {
            $typePayment = isset($_SESSION[$cCode]['main']['type_payment']) ? strtolower(trim($_SESSION[$cCode]['main']['type_payment'])) : "";
            if ($typePayment === "uang muka" || $typePayment === "uang_muka" || $typePayment === "downpayment") {
                $typePayment = "dp";
            }
            else if ($typePayment === "garansi") {
                $typePayment = "retensi";
            }

            if (!in_array($typePayment, array("dp", "termin", "retensi"))) {
                $errMsgs[] = "type payment wajib dipilih (DP / Termin / Retensi)";
                $errRows[] = "type_payment";
            }
        }
...
//                    $rekening_alias = isset($rekeningAlias[$row->rekening]) ? $rekeningAlias[$row->rekening] : $row->rekening;
            ...
            // Cari ayat jurnal penyesuaian amandemen yang terkait dalam keluarga transaksi ini
            $related_ids = array();
            if ($masterID > 0) {
                $this->db->select("id");
                $this->db->where("id_master", $masterID);
                $this->db->or_where("reference_id", $transaksiID);
                $rel_trxs = $this->db->get("transaksi")->result();
                if (!empty($rel_trxs)) {
                    foreach ($rel_trxs as $rt) {
                        if ((int)$rt->id != (int)$transaksiID) {
                            $related_ids[] = (int)$rt->id;
                        }
                    }
                }
            }
            ...
            if (!empty($related_ids)) {
                ...
                $this->db->where_in("j.transaksi_id", $related_ids);
                // START OF COMPLETE REPEATED LOGIC
                $this->db->group_start();
                $this->db->like("j.keterangan", "[Diskon / Adjustment");
                $this->db->or_like("j.keterangan", "[Amandemen");
                $this->db->or_where("j.koreksi_number >", 0);
                $this->db->group_end();
                // END OF COMPLETE REPEATED LOGIC
                if ($this->db->field_exists('trash', 'jurnal')) {
                    $this->db->where("j.trash", 0);
                }
                $this->db->order_by("j.dtime", "ASC");
                $this->db->order_by("j.id", "ASC");
                $tmp_adj = $this->db->get()->result();
            }
            ...
            if (!empty($tmp_adj)) {
                ...
                foreach ($tmp_adj as $arow) {
                    $d_start = backCustomDate("30", $arow->dtime);
                    $d_stop = substr($arow->dtime, 0, 10);
                    $defLink = "Ledger/viewMoveDetails_1/Rekening/" . $arow->rekening . "?o=" . $arow->cabang_id . "&trID=" . $arow->transaksi_id . "&date1=$d_start&date2=$d_stop";
                    $rek_alias = isset(fetchAccountStructureAlias()[$arow->rekening]) ? fetchAccountStructureAlias()[$arow->rekening] : (!empty($arow->rekening_2) ? $arow->rekening_2 : $arow->rekening);

                    $arow_deb = (float)$arow->debet;
                    $arow_krd = (float)$arow->kredit;
                    $adj_tot_deb += $arow_deb;
                    $adj_tot_krd += $arow_krd;

                    if (empty($adj_trx_no)) {
                        $adj_trx_no = $arow->transaksi_no;
                        $adj_dtime = $arow->dtime;
                    }

                    if ($arow->rekening == '1010020010' && $arow_krd > 0) {
                        $adj_delta = -$arow_krd;
                    }

                    $adj_items[$arow->cabang_id][$arow->urut][] = array(
                        "dtime" => $arow->dtime,
                        "rekening" => $arow_deb > 0 ? $rek_alias : "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" . $rek_alias,
                        "debet" => $arow_deb > 0 ? $arow_deb : "",
                        "kredit" => $arow_krd > 0 ? $arow_krd : "",
                        "link" => base_url() . $defLink,
                    );
                }
                ksort($adj_items);

                $adj_info = array(
                    "transaksi_no" => $adj_trx_no,
                    "dtime" => $adj_dtime,
                    "delta" => $adj_delta,
                    "total_debet" => $adj_tot_deb,
                    "total_kredit" => $adj_tot_krd
                );

                // Hitung Saldo Net Buku Besar Berjalan
                $orig_piutang = 0;
                $orig_penjualan = 0;
                $orig_ppn = 0;
                if (!empty($tmp)) {
                    foreach ($tmp as $r0) {
                        if ($r0->rekening == '1010020010' && (float)$r0->debet > 0) $orig_piutang = (float)$r0->debet;
                        if ($r0->rekening == '4010' && (float)$r0->kredit > 0) $orig_penjualan = (float)$r0->kredit;
                        if ($r0->rekening == '2030060' && (float)$r0->kredit > 0) $orig_ppn = (float)$r0->kredit;
                    }
                }
                ...
                $adj_ppn_deb = 0;
                foreach ($tmp_adj as $arow) {
                    if ($arow->rekening == '1010020010' && (float)$arow->kredit > 0) $adj_piutang_krd += (float)$arow->kredit;
                    if ($arow->rekening == '4010' && (float)$arow->debet > 0) $adj_penjualan_deb += (float)$arow->debet;
                    if ($arow->rekening == '2030060' && (float)$arow->debet > 0) $adj_ppn_deb += (float)$arow->debet;
                }

                $net_piutang = max(0, $orig_piutang - $adj_piutang_krd);
                $net_penjualan = max(0, $orig_penjualan - $adj_penjualan_deb);
                $net_ppn = max(0, $orig_ppn - $adj_ppn_deb);

                $terbayar = 0;
                $sisa = $net_piutang;
                $is_lunas = 0;
                if ($this->db->table_exists('transaksi_payment_source')) {
                    $tps_row = $this->db->where('transaksi_id', $transaksiID)
                                        ->or_where('transaksi_id', $masterID)
                                        ->order_by('id', 'DESC')
                                        ->get('transaksi_payment_source')->row_array();
                    if ($tps_row) {
                        $terbayar = (float)$tps_row['terbayar'];
                        $sisa = max(0, $net_piutang - $terbayar);
                        $is_lunas = ($sisa <= 0 && $terbayar > 0) ? 1 : 0;
                    }
                }

                $net_summary = array(
                    "piutang_awal" => $orig_piutang,
                    "delta" => $adj_delta,
                    "net_piutang" => $net_piutang,
                    "net_penjualan" => $net_penjualan,
                    "net_ppn" => $net_ppn,
                    "terbayar" => $terbayar,
                    "sisa" => $sisa,
                    "is_lunas" => $is_lunas
        ...
        $data = array(
            ...
            // Jurnal Penyesuaian Amandemen & Ringkasan Buku Besar
            "amandemen_items" => isset($adj_items) ? $adj_items : array(),
            "amandemen_info" => isset($adj_info) ? $adj_info : array(),
            "net_ledger_summary" => isset($net_summary) ? $net_summary : array(),
        );
        $this->load->view("transaksi", $data);
    }
...
\n\n
    ...
    public function selectPaymentExternSrc()
    {
        ...
        $duedateFields = (isset($this->configUi[$targetJenis]["shoppingCartReferenceExternDuedateFields"])) ? $this->configUi[$targetJenis]["shoppingCartReferenceExternDuedateFields"] : false;
        if ($duedateFields == true) {
        ...
        //ambil list project aktif
        $this->load->model("Mdls/MdlProdukProject");
        $prdprj = new MdlProdukProject();
        $prevProdukProject = $prdprj->lookUpAll()->result();

        //group by customer
        $projectByCustomer = array();
        if (!empty($prevProdukProject)) {
            foreach ($prevProdukProject as $kk => $allData) {
                $projectByCustomer[$allData->customer_id][] = $allData;
            }
        }
...
//            }
        ...
        $tmpSrc = $tr->lookupPaymentSrcByJenis_joined($targetJenis, 1)->result(); // 1 = project

        $paymentSrcDisabled = array();
        if ($targetJenis == '7499' && !empty($externID)) {
            $q_unstarted = "SELECT
                project_produk.quot_id as id,
                project_produk.quot_id as transaksi_id,
                project_produk.quot_nomer as nomer,
                project_produk.customer_id as payment_customer_id,
                project_produk.customer_nama as payment_customer_nama,
                project_produk.nama as keterangan,
                0 as tagihan,
                0 as terbayar,
                0 as sisa,
                '7499' as jenis_master,
                '7499' as jenis,
                0 as termin,
                '' as ids_prev,
                '' as ids_his,
                0 as ppn_status,
                'Belum Start' as extern_label2,
                project_produk.customer_nama as extern_nama,
                project_produk.customer_id as extern_id,
                project_produk.quot_id as id_master,
                1 as belum_start,
                project_produk.dtime as dtime,
                transaksi.nomer_top as raw_nomer_top,
                project_produk.nama as project_nama,
                project_produk.harga as extern_nilai2,
                project_produk.uang_muka_approved as uang_muka,
                project_produk.garansi as garansi
            FROM project_produk
            LEFT JOIN transaksi ON transaksi.id = project_produk.quot_id
            WHERE project_produk.customer_id = '$externID'
            AND (project_produk.project_start_id IS NULL OR project_produk.project_start_id = 0)
            AND project_produk.trash = 0 AND project_produk.status = 1";

            $res_unstarted = $this->db->query($q_unstarted)->result();
            if (!empty($res_unstarted)) {
                foreach ($res_unstarted as $row_unstarted) {
                    $tmpSrc[] = $row_unstarted;
                }
            }
        }
...
//        showLast_query("kuning");
        ...
        $arrTransID = array();
        if (sizeof($tmpSrc) > 0) {
            foreach($tmpSrc as $preTmp){
                if (!empty($preTmp->id) && !empty($preTmp->id_master)) {
                    $arrTransID[$preTmp->id] = $preTmp->id_master;
                }
            }
        }
        ...
        if (!empty($arrTransID)) {
            $tr->setFilters(array());
            $tr->addFilter("transaksi_id in (" . implode(",", $arrTransID) . ")");
            $tr->setJointSelectFields("items2, main, transaksi_id");
            $tmpReg = $tr->lookupDataRegistries()->result();

            $pairRegistriesResult = array();
            if (sizeof($tmpReg) > 0) {
                foreach ($tmpReg as $regRow) {
                    $pairRegistriesResult[$regRow->transaksi_id] = blobDecode($regRow->items2);
                }
            }
        }


//        arrPrint($tmpTerminReg);
//        matiHere(__LINE__);
        ...
        if (sizeof($tmpSrc) > 0) {
            foreach ($tmpSrc as $row) {
                ...
                //informasi DP, GARANSI dan TERMIN
                if (isset($pairRegistriesResult[$arrTransID[$transaksi_id]]) && !empty($pairRegistriesResult[$arrTransID[$transaksi_id]])) {
                    $tmp['data_dp'] = $pairRegistriesResult[$arrTransID[$transaksi_id]][0]['dp'];
                    $tmp['data_termin'] = $pairRegistriesResult[$arrTransID[$transaksi_id]][0]['termin'];
                    $tmp['data_garansi'] = $pairRegistriesResult[$arrTransID[$transaksi_id]][0]['garansi'];
                }
//arrPrint(blobDecode($row->nomers_prev));
                if ($readerDueDate) {
                ...
                if (isset($row->belum_start) && $row->belum_start == 1) {
                    $paymentSrcDisabled[] = $row->id;
                    $row->project_nama = $row->project_nama . " <br><span class='label label-danger'>Project Belum Start</span> <span class='text-danger text-sm'>Silahkan start project dulu.</span>";
                    $tmp['class_bg'] = "bg-warning ";
                }
                ...
                if ($targetJenis == "7499") {
                    $soa_code = blobDecode($row->ids_his);
//                    arrPrintWebs($soa_code);
//                    cekHitam($targetJenis);
                    unset($row->nomer_top);
                    $tmp['nomer_top'] = isset($soa_code[2]) ? $soa_code[2]["nomer"] : "";
                }
...
//arrPrintWebs($tmp);
                ...
                $ids_his_intext = blobDecode($row->ids_his);

                $intext_ref = "";
                if (is_array($ids_his_intext)) {
                    foreach ($ids_his_intext as $k => $intextData) {
                        if ($intext_ref == "") {
                            $intext_ref .= formatField_he_format("nomer", $intextData['nomer']);
                            $intext_ref .= ", " . $intextData['nomer'];
                        }
                        else {
                            $intext_ref .= ", " . formatField_he_format("nomer", $intextData['nomer']);
                            $intext_ref .= ", " . $intextData['nomer'];
                ...
                if (isset($row->belum_start) && $row->belum_start == 1) {
                    $tmp['nomer_ref_so'] = formatField_he_format("nomer", $row->raw_nomer_top) . ", " . $row->nomer;
                }
                else {
                    $tmp['nomer_ref_so'] = $intext_ref;
                }
...
//        arrPrint($this->configUi[$targetJenis]["shoppingCartReferenceFields"]);
//        arrPrint($targetJenis);
//arrPrint($items);
//        matiHere(__LINE__);
        ...
        $data = array(
            ...
            "paymentSrcDisabled" => $paymentSrcDisabled,
...
// matiHEre();
    ...
    public function viewResumeDetails()
    {
        ...
        $d_stop = substr($joinedTrans->fulldate, 0, 10);
