...
class _processSelectProductException extends Modul_Controller
{
...
//        $this->jenisTr = $this->uri->segment(4);
    ...
    public function select()
    {
        ...
        $compareFieldsItems = isset($this->configUi[$this->jenisTr]['shopingCartCompareFields'][1]) ? $this->configUi[$this->jenisTr]['shopingCartCompareFields'][1] : array();
        $shopingCartParamForceEditable = isset($this->configUi[$this->jenisTr]['shopingCartParamForceEditable'][1]) ? $this->configUi[$this->jenisTr]['shopingCartParamForceEditable'][1] : array();
        //---------------------------------
...
//                        matiHEre("else nya ini");
            ...
            //-- membuat session items2 sebagai subitems dari items
            if (sizeof($_SESSION[$cCode]['items']) > 0) {
                ...
                foreach ($_SESSION[$cCode]['items'] as $iSpec) {
                    ...
                    if (!isset($_SESSION[$cCode]['items2'][$iID])) {
                        $_SESSION[$cCode]['items2'][$iID] = array();
                        $_SESSION[$cCode]['items2'][$iID][0]['jml'] = "1";
                        $_SESSION[$cCode]['items2'][$iID][0]['qty'] = "1";
                        $_SESSION[$cCode]['items2'][$iID][0]['pph_persen'] = $_SESSION[$cCode]['main']['pihakPph23'];
                        $_SESSION[$cCode]['items2'][$iID][0]['ppnFactor'] = $this->ppnFactor;
                        if (sizeof($advanceSubItemsLabel) > 0) {
                            foreach ($advanceSubItemsLabel as $skey => $sval) {
                                $_SESSION[$cCode]['items2'][$iID][0][$skey] = "";
                            }
                        }
...
//mati_disini("$key => $val");
        if (isset($_SESSION[$cCode]['items2']) && (sizeof($_SESSION[$cCode]['items2']) > 0)) {
            switch ($key) {
                case "dpp_pph_persen":
                    if($val > 100){
                        mati_disini("Prosentase DPP PPh yang anda isikan salah ($val). Silahkan dikoreksi. code: " . __LINE__);
                    }
                    elseif($val < 0){
                        mati_disini("Prosentase DPP PPh yang anda isikan salah ($val). Silahkan dikoreksi. code: " . __LINE__);
                    }
                ...
                case "dpp_ppn_persen":
                    if($val > 100){
                        mati_disini("Prosentase DPP PPN yang anda isikan salah ($val). Silahkan dikoreksi. code: " . __LINE__);
                    }
                    elseif($val < 0){
                        mati_disini("Prosentase DPP PPN yang anda isikan salah ($val). Silahkan dikoreksi. code: " . __LINE__);
                    }
            ...
            $_SESSION[$cCode]['items2'][$iID][$subNo]['ppnFactor'] = $this->ppnFactor;
        }
...
//arrPrintPink($advanceSubNumItemsLabel);
        ...
        if (sizeof($advanceSubItemsLabel) > 0) {
            ...
            $_SESSION[$cCode]['items2'][$iID][$subNo]['pph_persen'] = $_SESSION[$cCode]['main']['pihakPph23'];
            $_SESSION[$cCode]['items2'][$iID][$subNo]['ppnFactor'] = $this->ppnFactor;
        }
        if (sizeof($advanceSubNumItemsLabel) > 0) {
            foreach ($advanceSubNumItemsLabel as $skey => $sval) {
