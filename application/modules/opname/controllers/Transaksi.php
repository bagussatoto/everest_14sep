...
class Transaksi extends Modul_Controller
{
    ...
    public function validate()
    {
        ...
        $checkOpnameValidate = isset($this->configUi[$this->jenisTr]["checkOpnameValidate"]) ? $this->configUi[$this->jenisTr]["checkOpnameValidate"] : false;
        ...
        if ($checkOpnameValidate == true) {
            if (isset($_SESSION[$cCode]['items']) && sizeof($_SESSION[$cCode]['items']) > 0) {
                ...
                foreach ($_SESSION[$cCode]['items'] as $pID => $iSpec) {
                    ...
                    $label = $iSpec['label'];
                    $nama = $iSpec['nama'];
                    if (!isset($_SESSION[$cCode]['items'][$pID]['ceklist_opname'])) {
                        $msg = "Produk $label $kode $nama belum diceklist. Silahkan diperiksa baris nomer $no.";
                        $errMsgs[] = $msg;
                    }
                    elseif ($_SESSION[$cCode]['items'][$pID]['ceklist_opname'] == null) {
                        $msg = "Produk $label $kode $nama belum diceklist. Silahkan diperiksa baris nomer $no.";
                        $errMsgs[] = $msg;
                    }
                }
                if (!isset($_SESSION[$cCode]['main']['opnameNoteCeklist_1'])) {
                    $msg = "Pastikan sudah ceklist pada baris Data yang diinput.";
                    $errMsgs[] = $msg;
                }
                elseif ($_SESSION[$cCode]['main']['opnameNoteCeklist_1'] == null) {
                    $msg = "Pastikan sudah ceklist pada baris Data yang diinput.";
                    $errMsgs[] = $msg;
                }
                if (!isset($_SESSION[$cCode]['main']['opnameNoteCeklist_2'])) {
                    $msg = "Pastikan sudah ceklist pada baris Total Quantity.";
                    $errMsgs[] = $msg;
                }
                elseif ($_SESSION[$cCode]['main']['opnameNoteCeklist_2'] == null) {
                    $msg = "Pastikan sudah ceklist pada baris Total Quantity.";
                    $errMsgs[] = $msg;
                }
            }
                ...
                foreach ($tmpReg as $regRow) {
                    ...
                    foreach ($regRow as $key_reg => $val_reg) {
                        if ($key_reg != "transaksi_id") {
                            $tmpReg_result[$regRow->transaksi_id][$key_reg] = ($val_reg != NULL) ? blobDecode($val_reg) : array();
                        }
                    }
            ...
            if (sizeof($steps) > 1) {
                ...
                if (sizeof($tmpHist) > 0) {
                    ...
                    if (sizeof($tmpReg) > 0) {
                        foreach ($tmpReg as $regRow) {
                            $param = $regRow->param;
                            $tmpReg_result[$regRow->transaksi_id][$param] = ($regRow->values != NULL) ? blobDecode($regRow->values) : array();
                        }
                    }
...
//                    }
        ...
        if (sizeof($tmpHist) > 0) {
            ...
            $stagingStatusMap = array();
            if (count($arrTransID) > 0) {
                $this->db->select('transaksi_id, status');
                $this->db->where_in('transaksi_id', $arrTransID);
                $qStaging = $this->db->get('so_headers_staging');
                if ($qStaging && $qStaging->num_rows() > 0) {
                    foreach ($qStaging->result() as $stgRow) {
                        $stagingStatusMap[$stgRow->transaksi_id] = $stgRow->status;
                    }
                }
            }
            ...
            foreach ($tmpHist as $row) {
                ...
                if ($row->sub_step_number > 0) {
                    $stateColor = isset($this->configUi[$jenisTr]['steps'][$row->sub_step_number]['stateColor']) ? $this->configUi[$jenisTr]['steps'][$row->sub_step_number]['stateColor'] : "#333333";
                    $stateLabel = isset($this->configUi[$jenisTr]['steps'][$row->sub_step_number]['stateLabel']) ? $this->configUi[$jenisTr]['steps'][$row->sub_step_number]['stateLabel'] : "step " . $row->sub_step_number;

                    if ($row->sub_step_number == 3 && isset($stagingStatusMap[$row->transaksi_id]) && $stagingStatusMap[$row->transaksi_id] != 'COMPLETED') {
                        $tmp['state'] = "<span class='label label-warning' style='font-size:11px;'><i class='fa fa-spinner fa-spin'></i> Menunggu RabbitMQ</span>";
                    }
                    else {
                        $tmp['state'] = "<span style='color:" . $stateColor . "'>" . $stateLabel . "</span>";
                    }
...
//                                $link_reject = MODUL_PATH . "FollowUp/doRevert/" . $this->jenisTr . "/$transaksiID_reject/$masterRevertStep/$childRevertStep/$currentStepNum?mode=autoPrePreview";
                                ...
                                onclick =\"this.disabled=true;top.open_holdon();document.getElementById('result').src='$link_reject'\">" . $actionLabel_edit . " </button>";
...
//                                $link_reject_all = MODUL_PATH . "FollowUp/doRevertAll/" . $this->jenisTr . "/$transaksiID_reject/$currentStepNum?mode=autoPrePreview";
                                ...
                                $disabled_hold $disabled_button_reject
                                onclick =\"this.disabled=true;top.open_holdon();document.getElementById('result').src='$link_reject_all'\">" . $actionLabel_edit . " </button>";
...
//                        }
        ...
        // START OF COMPLETE REPEATED LOGIC
        //region tampilkan transaksi step 3 yang masih antrean latar belakang (RabbitMQ / Staging belum selesai / belum ada jurnal)
        $finalStepNum = count($steps);
        if ((!isset($arrayOnprogressGroup[$finalStepNum]) || sizeof($arrayOnprogressGroup[$finalStepNum]) == 0) && $this->db->table_exists('so_headers_staging')) {
            $this->db->select("transaksi.*, s.status as staging_status, s.retry_count as staging_retry_count, s.total_item as staging_total_items, s.created_at as staging_created_at");
            $this->db->from("transaksi");
            $this->db->join("so_headers_staging s", "s.transaksi_id = transaksi.id", "inner");
            $this->db->where("transaksi.trash_4", "0");
            $this->db->where("transaksi.id_top >", "0");
            $this->db->where("transaksi.jenis", $this->jenisTr);
            $this->db->where("transaksi.jenis_master", $this->jenisTr);
            $this->db->where("transaksi.step_number", $finalStepNum);
            $this->db->where("transaksi.next_step_code", "");
            $this->db->where_in("s.status", array("PENDING", "PROCESSING", "FAILED"));
            if (isset($this->session->login['cabang_id']) && $this->session->login['cabang_id'] != CB_ID_PUSAT) {
                $this->db->where("transaksi.cabang_id", $this->session->login['cabang_id']);
            }
            $this->db->order_by("transaksi.id", "DESC");
            $tmpPending = $this->db->get()->result();

            if (sizeof($tmpPending) > 0) {
                $numbDone = 0;
                foreach ($tmpPending as $rowDone) {
                    $numbDone++;
                    $tmpD = array();
                    foreach ($historyFields as $fName => $fLabel) {
                        $tmpD[$fName] = isset($rowDone->$fName) ? formatField_he_format($fName, $rowDone->$fName) : "-";
                        if ($fName == "no") {
                            $tmpD[$fName] = formatField_he_format($fName, $numbDone);
                        }
                    }

                    if ($rowDone->staging_status == "PROCESSING") {
                        $tmpD['state'] = "<span class='label label-primary' style='font-size:11px;'><i class='fa fa-spinner fa-spin'></i> Diproses RabbitMQ</span><br><small class='text-muted'><i class='fa fa-clock-o'></i> Sedang membuat jurnal & kartu stok</small>";
                    }
                    elseif ($rowDone->staging_status == "FAILED") {
                        $tmpD['state'] = "<span class='label label-danger' style='font-size:11px;'><i class='fa fa-exclamation-triangle'></i> Gagal RabbitMQ</span><br><small class='text-danger'>Terjadi kendala antrean latar belakang</small>";
                    }
                    else {
                        $tmpD['state'] = "<span class='label label-warning' style='font-size:11px;'><i class='fa fa-hourglass-half fa-spin'></i> Antrean RabbitMQ</span><br><small class='text-muted'><i class='fa fa-clock-o'></i> Menunggu proses latar belakang (jurnal & stok)</small>";
                    }

                    $tmpD['next_pic'] = "<span class='text-muted'><i class='fa fa-cogs'></i> Background Worker</span>";
                    $tmpD['action'] = "<div class='btn-group btn-group-justified' role='group'>";
                    $tmpD['action'] .= "<a class='btn btn-info btn-xs' href='" . base_url() . "opname/Transaksi/viewResume/" . $this->jenisTr . "/" . $rowDone->nomer . "' target='_blank' title='Lihat Resume Transaksi'><i class='fa fa-eye'></i> Resume</a>";
                    $tmpD['action'] .= "<a class='btn btn-default btn-xs' href='" . base_url() . "opname/Printing/cetak/" . $this->jenisTr . "/" . $rowDone->id . "' target='_blank' title='Cetak Transaksi'><i class='fa fa-print'></i> Cetak</a>";
                    $tmpD['action'] .= "</div>";

                    $arrayOnprogressGroup[$finalStepNum][] = $tmpD;
                }
            }
        }
        //endregion
            ...
            if (sizeof($tmpReg) > 0) {
                foreach ($tmpReg as $eParams) {
                    foreach ($pairRegistry as $param) {
                        $$param = ($eParams->$param != NULL) ? blobDecode($eParams->$param) : array();
                    }
                }
            }
        }
                    ...
                    foreach ($tmpReg as $row) {
                        foreach ($regfields as $params) {
                            ...
                            if (isset($row->$params) && ($params != "transaksi_id")) {
                                ...
                                $var = $params . "_reg";
                                $$var = ($row->$params != NULL) ? unserialize(base64_decode($row->$params)) : array();
                            }
                    ...
                    foreach ($tmpRegRef as $refRow) {
                        foreach ($regfields as $params) {
                            if (isset($refRow->$params)) {
                                $varR = $params . "_ref";
                                $$varR = ($refRow->$params != NULL) ? unserialize(base64_decode($refRow->$params)) : array();
                            }
                        }
                    }
...
//            cekHere(":: $jenisMasterID ::");
                $tr = new MdlTransaksi();
                $tr->addFilter("id_master='$transaksiTopID'");
                $tr->addFilter("jenis='1119ro'");
                $tmp0 = $tr->lookupAll()->result();
                $trid_pkd = $tmp0[0]->id;
                $tr->setFilters(array());
                $tr->setJointSelectFields(array());//select fields jika ada, atau biarkana array untuk reset
    ...
    public function index()
    {
        ...
        if (placeCanMakeTrans_he_menu($this->session->login['membership'], $this->session->login['cabang_id'], $this->session->login['gudang_id'], $this->jenisTr, $this->configUiJenis)) {
            ...
            if (array_key_exists($this->jenisTr, $createIndexes)) {
                ...
                $targetUrl = MODUL_PATH . "Create/index/" . $this->jenisTr;
                // $targetUrl = MODUL_PATH . "Opname/index/Produk/" . $this->jenisTr;
            }
    ...
    public function viewResumeDetails()
    {
        ...
        foreach ($tmpReg as $regData) {
            foreach ($regData as $key_reg => $val_reg) {
                switch ($key_reg) {
                    case "main":
                        $mainData = ($val_reg != NULL) ? blobDecode($val_reg) : array();
                        break;
                    case "items":
                        $masterItems_0 = ($val_reg != NULL) ? blobDecode($val_reg) : array();
                        ...
                        break;
                    case "items2_sum":
                        $masterItems2_sum = ($val_reg != NULL) ? blobDecode($val_reg) : array();
                        //                    foreach ($masterItems_0 as $pId => $masterItem) {
                        //                        $ppn_key = $masterItem['ppn'] / $masterItem['nett1'];
                        //                        $addItem['harganppn'] = $masterItem['harga'] + ($masterItem['harga'] * $ppn_key);
                        //                        $masterItems[$pId] = $masterItem + $addItem;
                        ...
                        break;
                    case "main_elements"://
                        $mainElements = ($val_reg != NULL) ? blobDecode($val_reg) : array();
                        break;
                    case "receiptDetailFields"://
                        $receiptDetailFields = ($val_reg != NULL) ? blobDecode($val_reg) : array();
                        break;
                    case "receiptSumFields"://
                        $receiptSumFields = ($val_reg != NULL) ? blobDecode($val_reg) : array();
                        break;
                }
            }
