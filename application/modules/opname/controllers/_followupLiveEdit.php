...
//        $_SESSION[$cCode]['tableIn_master_values']['partial'] = 1;
        ...
        fillValues_he_value_builder($this->jenisTr, $fromStep, $intoStep, $this->configCoreJenis, $this->configUiJenis, $this->configValuesJenis);
...
//        echo "</script>";
        ...
        $this->load->helper("he_value_builder");
        fillValues_he_value_builder($jenisTr, $fromStep, $intoStep, $this->configCoreJenis, $this->configUiJenis, $this->configValuesJenis);
...
//        mati_disini();
    ...
    public function updateItemFieldOpname()
    {
        ...
        $val = isset($_GET['val']) ? $_GET['val'] : (isset($_POST['val']) ? $_POST['val'] : "");
        $isSilent = (isset($_GET['silent']) && $_GET['silent'] == 1) || (isset($_POST['silent']) && $_POST['silent'] == 1);

        if (isset($_SESSION[$cCode]['items'][$id][$key])) {

            $oldVal = $_SESSION[$cCode]['items'][$id][$key];
            $_SESSION[$cCode]['items'][$id][$key] = $val;

            // Audit Trail Logging
            if ((string)$oldVal !== (string)$val) {
                $this->load->model('Coms/ComOpnameAuditTrail');
                $audit = new ComOpnameAuditTrail();
                $itemNama = isset($_SESSION[$cCode]['items'][$id]['nama']) ? $_SESSION[$cCode]['items'][$id]['nama'] : (isset($_SESSION[$cCode]['items'][$id]['produk_nama']) ? $_SESSION[$cCode]['items'][$id]['produk_nama'] : '');
                $itemKode = isset($_SESSION[$cCode]['items'][$id]['produk_kode']) ? $_SESSION[$cCode]['items'][$id]['produk_kode'] : '';

                $actionType = ($key == 'qty_opname' || $key == 'qty' || $key == 'jml') ? 'EDIT_QTY' : (($key == 'note' || $key == 'keterangan') ? 'EDIT_NOTE' : 'EDIT_FIELD');
                $diff = ($key == 'qty_opname' || $key == 'qty' || $key == 'jml') ? ((float)$val - (float)$oldVal) : '';

                $audit->recordSessionEvent($cCode, $actionType, array(
                    'item_id' => $id,
                    'item_kode' => $itemKode,
                    'item_nama' => $itemNama,
                    'field' => $key,
                    ...
                    'new_val' => $val,
                    'diff' => $diff,
                    'note' => 'Penyesuaian ' . $key
                ));
            }

            if ($key == "qty_opname") {

                $stok = isset($_SESSION[$cCode]['items'][$id]['stok']) ? $_SESSION[$cCode]['items'][$id]['stok'] : 0;
                $selisih = $val - $stok;
                if ($selisih > 0) {
...
//                        matiHere(__LINE__."::".$produk_jenis);
                            if (!isset($_SESSION[$cCode]['items2'][$idp])) {
//                            if($_SESSION[$cCode]['items'][$idp]["qty_kredit"]>0){
                                $_SESSION[$cCode]['items2'][$idp] = array();
//                            }

                            }
                            foreach ($fieldSrcs as $fkey => $src) {
                                //                        cekHitam("$fkey => $src");
                                if (is_array($src) && sizeof($src) > 0) {
                                    //cekHitam("masuk disini " . __LINE__);
                                    foreach ($src as $srcSpec) {
                                        if (isset($tmp[$srcSpec]) || isset($rows->$srcSpec)) {
                                            $_SESSION[$cCode]['items'][$idp][$fkey] = makeValue($srcSpec, $tmp, $tmp, isset($rows->$srcSpec) ? $rows->$srcSpec : "-");
                                        }
                                    }
                                } else {
                                    //                            cekUngu("masuk disini [$fkey => $src] " . __LINE__);
                                    $_SESSION[$cCode]['items'][$idp][$fkey] = makeValue($src, $tmp, $tmp, isset($rows->$src) ? $rows->$src : 0);
                                }
                            }
                            ...
                            // START OF COMPLETE REPEATED LOGIC
                                ...
                                $selisih_opname = abs($_SESSION[$cCode]['items'][$idp]["qty_selisih"]);
                                if ($jml_serial * 1 == 1) {
                                    $d_kode = $rows->kode;
                                    $_SESSION[$cCode]['items'][$idp][$d_kode] = $selisih_opname;
                                    if ($_SESSION[$cCode]['items'][$idp]["qty_kredit"] > 0) {
                                        $_SESSION[$cCode]['items4'][$idp][$d_kode] = $selisih_opname;
                                    } else {
                                        if (isset($_SESSION[$cCode]['items4'][$idp])) {
                                            unset($_SESSION[$cCode]['items4'][$idp]);
                                        }
                                    }
                                }
                            }
//                            arrPrint($arrCode);
                            ...
                            if (!empty($arrCode)) {
                                foreach ($arrCode as $kcat => $vcat) {
                                    ...
                                    if ($_SESSION[$cCode]['items'][$idp]["qty_kredit"] > 0) {
                                        $_SESSION[$cCode]['items4'][$idp][$kcat] = $new_vcat;
                                    } else {
                                        if (isset($_SESSION[$cCode]['items4'][$idp])) {
                                            unset($_SESSION[$cCode]['items4'][$idp]);
                                        }
                            ...
                            $_SESSION[$cCode]['items'][$idp]['jenis_barang'] = $jenis_barang;

                        }
                    }
                    // endregion reload data produk sesuai config dari shoppingcart-----------------------------------
                }
//                arrPrint($_SESSION[$cCode]["itesm4"]);
//matiHere();
                // START OF COMPLETE REPEATED LOGIC
                // Bersihkan sesi serial dari produk yang diganti opnamenya
                if (isset($_SESSION[$cCode]["items5"][$id])) {
                    unset($_SESSION[$cCode]["items5"][$id]);
                }
                if (isset($_SESSION[$cCode]["items4"][$id]) && isset($_SESSION[$cCode]['items'][$id]['qty_kredit']) && $_SESSION[$cCode]['items'][$id]['qty_kredit'] <= 0) {
                    unset($_SESSION[$cCode]["items4"][$id]);
                }
                if (isset($_SESSION[$cCode]["items3"][$id]) && isset($_SESSION[$cCode]['items'][$id]['qty_debet']) && $_SESSION[$cCode]['items'][$id]['qty_debet'] <= 0) {
                    unset($_SESSION[$cCode]["items3"][$id]);
                }
                if (isset($_SESSION[$cCode]["items2"][$id]) && isset($_SESSION[$cCode]['items'][$id]['qty_selisih']) && $_SESSION[$cCode]['items'][$id]['qty_selisih'] == 0) {
                ...
                if (isset($_SESSION[$cCode]["items5"]) && is_array($_SESSION[$cCode]["items5"])) {
                    foreach ($_SESSION[$cCode]["items5"] as $id_x => $tempDatas) {
                        if (is_array($tempDatas)) {
                            foreach ($tempDatas as $pSku => $serialData) {
                                if (is_array($serialData)) {
                                    foreach ($serialData as $serial_list => $serial_jml) {
                                        $tempSerial = array();
                                        foreach ($tempKey as $key_target => $key_src) {
                                            if (isset($_SESSION[$cCode]["items"][$id_x][$key_src])) {
                                                $tempSerial[$key_target] = $_SESSION[$cCode]["items"][$id_x][$key_src];
                                            }
                                        }
                                        $tempSerial["serial"] = $serial_list;
                                        $tempSerial["serial_number"] = $serial_list;
                                        $tempSerial["jml"] = $serial_jml;
                                        $tempSerial["qty"] = $serial_jml;
                                        $tempSerial["produk_sku_part_nama"] = $pSku;
                                        $_SESSION[$cCode]["items5_sum"][] = $tempSerial;
                                    }
                                }
                            }
                        }
...
//mati_disini("qty debet: " . $_SESSION[$cCode]['items'][$id]['qty_debet'] . ", qty_kredit: " . $_SESSION[$cCode]['items'][$id]['qty_kredit']);


            foreach ($itemNumLabels as $numKey => $label) {
                if (isset($_SESSION[$cCode]['items'][$id][$numKey])) {
                    $_SESSION[$cCode]['items'][$id]["sub_" . $numKey] = ($_SESSION[$cCode]['items'][$id][$numKey] * $_SESSION[$cCode]['items'][$id]["jml"]);
                }
...
//            echo(lgShowAlert("NOT replacing $key with $val"));
        ...
        $_SESSION[$cCode]['main']['harga'] = 0;
        if (sizeof($_SESSION[$cCode]['items']) > 0) {
            foreach ($_SESSION[$cCode]['items'] as $itId => $iSpec) {
                $_SESSION[$cCode]['main']['harga'] += ($iSpec['jml'] * $iSpec['harga']);
            }
        }

        // Jika request dalam mode silent (AJAX in-place sync dari frontend), kirim JSON response instan (< 20ms)
        if ($isSilent) {
            $totalStokBuku = 0;
            $totalDebet = 0;
            $totalKredit = 0;
            ...
            if (isset($_SESSION[$cCode]['items']) && is_array($_SESSION[$cCode]['items'])) {
                foreach ($_SESSION[$cCode]['items'] as $itId => $iSpec) {
                    $stokRow = isset($iSpec['stok']) ? (float)$iSpec['stok'] : 0;
                    $qtyOpnameRow = isset($iSpec['qty_opname']) ? (float)$iSpec['qty_opname'] : $stokRow;
                    $selisihRow = isset($iSpec['qty_selisih']) ? (float)$iSpec['qty_selisih'] : ($qtyOpnameRow - $stokRow);
                    $qtyDebetRow = isset($iSpec['qty_debet']) ? (float)$iSpec['qty_debet'] : ($selisihRow > 0 ? $selisihRow : 0);
                    $qtyKreditRow = isset($iSpec['qty_kredit']) ? (float)$iSpec['qty_kredit'] : ($selisihRow < 0 ? abs($selisihRow) : 0);
                    $subtotalRow = isset($iSpec['subtotal']) ? (float)$iSpec['subtotal'] : 0;

                    $totalStokBuku += $stokRow;
                    $totalDebet += $qtyDebetRow;
                    $totalKredit += $qtyKreditRow;
                    $totalSubtotal += $subtotalRow;
            ...
            $serialCompliance = $this->_calcSerialCompliance($cCode);
            $itemIsSerial = ((isset($_SESSION[$cCode]['items'][$id]['jml_serial']) && (int)$_SESSION[$cCode]['items'][$id]['jml_serial'] > 0) || (isset($_SESSION[$cCode]['items'][$id]['scan_mode']) && $_SESSION[$cCode]['items'][$id]['scan_mode'] == 'serial'));

            header('Content-Type: application/json');
            $resPayload = array(
                "status" => 200,
                "id" => $id,
                "selisih" => isset($_SESSION[$cCode]['items'][$id]['qty_selisih']) ? (float)$_SESSION[$cCode]['items'][$id]['qty_selisih'] : 0,
                "qty_debet" => isset($_SESSION[$cCode]['items'][$id]['qty_debet']) ? (float)$_SESSION[$cCode]['items'][$id]['qty_debet'] : 0,
                "qty_kredit" => isset($_SESSION[$cCode]['items'][$id]['qty_kredit']) ? (float)$_SESSION[$cCode]['items'][$id]['qty_kredit'] : 0,
                "subtotal" => isset($_SESSION[$cCode]['items'][$id]['subtotal']) ? (float)$_SESSION[$cCode]['items'][$id]['subtotal'] : 0,
                "has_serial" => $itemIsSerial ? 1 : 0,
                "total_stok_buku" => $totalStokBuku,
                ...
                "total_harga_master" => isset($_SESSION[$cCode]['main']['harga']) ? (float)$_SESSION[$cCode]['main']['harga'] : 0,
                "serial_mismatch_count" => $serialCompliance['serial_mismatch_count'],
                "serial_total_sku" => $serialCompliance['serial_total_sku'],
                "serial_balanced_sku" => $serialCompliance['serial_balanced_sku'],
            );
        ...
        //region kembalikan ke followupPreview (Mode Legacy)
        $refresh = 0;
        $rawBuilderURL = isset($_GET['rawBuilderURL']) ? $_GET['rawBuilderURL'] : "";
        if ($refresh == 1) {
            $actionTarget = "top.BootstrapDialog.closeAll();top.BootstrapDialog.show(
...
//        echo "\n<script>top.location.reload();</script>";
    ...
    {
        //update tgl 30 agustus 2026
        //produk serial dari opname yang menambah stok, hasil generate serial ditampung pada sesi items7 dan items7_sum
        //produk serial dari opname yang mengurangi stok, serial hasil ceklist tampungan pada sesi items5 dan items5_sum (sebelumnya items7 -> tercampur dengan hasil gnerate).

        //updateserial items5
        //updateserial items5_sum

        $this->jenisTr = $this->uri->segment(4);
        $cCode = "_TR_" . $this->jenisTr;
        $state = isset($_GET["state"]) ? $_GET["state"] : (isset($_POST["state"]) ? $_POST["state"] : "");
        $pid = isset($_GET["id"]) ? $_GET["id"] : (isset($_POST["id"]) ? $_POST["id"] : "");
        $sku = isset($_GET["sku"]) ? $_GET["sku"] : (isset($_POST["sku"]) ? $_POST["sku"] : "");
        $serial = isset($_GET["serial"]) ? $_GET["serial"] : (isset($_POST["serial"]) ? $_POST["serial"] : "");

        unset($_SESSION[$cCode]["items8_sum"]);

        $this->load->model('Coms/ComOpnameAuditTrail');
        $audit = new ComOpnameAuditTrail();
        $itemNama = isset($_SESSION[$cCode]['items'][$pid]['nama']) ? $_SESSION[$cCode]['items'][$pid]['nama'] : (isset($_SESSION[$cCode]['items'][$pid]['produk_nama']) ? $_SESSION[$cCode]['items'][$pid]['produk_nama'] : '');
        $itemKode = isset($_SESSION[$cCode]['items'][$pid]['produk_kode']) ? $_SESSION[$cCode]['items'][$pid]['produk_kode'] : $sku;

        if ($state == "true" || $state == "1") {
            if (!isset($_SESSION[$cCode]["items5"][$pid][$sku])) {
                $_SESSION[$cCode]["items5"][$pid][$sku] = array();
            }
            $_SESSION[$cCode]["items5"][$pid][$sku][$serial] = 1;

            $audit->recordSessionEvent($cCode, 'CHECK_SERIAL', array(
                'item_id' => $pid,
                'item_kode' => $itemKode,
                'item_nama' => $itemNama,
                'serial_number' => $serial,
                'field' => 'serial_number',
                'old_val' => 'Uncheck (Tidak Ada)',
                'new_val' => 'Centang (Ada)',
                'note' => 'Serial number diverifikasi fisik ada'
            ));
        } else {
            if (isset($_SESSION[$cCode]["items5"][$pid][$sku][$serial])) {
                unset($_SESSION[$cCode]["items5"][$pid][$sku][$serial]);
            }
            if (isset($_SESSION[$cCode]["items5"][$pid][$sku]) && empty($_SESSION[$cCode]["items5"][$pid][$sku])) {
                unset($_SESSION[$cCode]["items5"][$pid][$sku]);
            }
            if (isset($_SESSION[$cCode]["items5"][$pid]) && empty($_SESSION[$cCode]["items5"][$pid])) {
                unset($_SESSION[$cCode]["items5"][$pid]);
            }

            $audit->recordSessionEvent($cCode, 'UNCHECK_SERIAL', array(
                'item_id' => $pid,
                'item_kode' => $itemKode,
                'item_nama' => $itemNama,
                'serial_number' => $serial,
                'field' => 'serial_number',
                'old_val' => 'Centang (Ada)',
                'new_val' => 'Uncheck (Tidak Ada)',
                'note' => 'Serial number dibatalkan / tidak ditemukan'
            ));
        }
        //rebuild key dari items
        $tempKey = array(
            "id" => "id",
            "nama" => "nama",
            "name" => "nama",
        ...
        unset($_SESSION[$cCode]["items5_sum"]);
        $_SESSION[$cCode]["items5_sum"] = array();
        if (isset($_SESSION[$cCode]["items5"]) && is_array($_SESSION[$cCode]["items5"]) && count($_SESSION[$cCode]["items5"]) > 0) {
            foreach ($_SESSION[$cCode]["items5"] as $id => $tempDatas) {
                if (is_array($tempDatas)) {
                    foreach ($tempDatas as $pSku => $serialData) {
                        if (is_array($serialData)) {
                            foreach ($serialData as $serial_list => $serial_jml) {
                                $tempSerial = array();
                                foreach ($tempKey as $key_target => $key_src) {
                                    if (isset($_SESSION[$cCode]["items"][$id][$key_src])) {
                                        $tempSerial[$key_target] = $_SESSION[$cCode]["items"][$id][$key_src];
                                    }
                                }
                                $tempSerial["serial"] = $serial_list;
                                $tempSerial["serial_number"] = $serial_list;
        ...
        $stokItem = isset($items[$pid]['stok']) ? (float)$items[$pid]['stok'] : 0;
        $qtyOpnameItem = isset($items[$pid]['qty_opname']) ? (float)$items[$pid]['qty_opname'] : $stokItem;
        $selisihItem = isset($items[$pid]['qty_selisih']) ? (float)$items[$pid]['qty_selisih'] : ($qtyOpnameItem - $stokItem);
        $targetQtyItem = $selisihItem < 0 ? abs($selisihItem) : 0;
        $checkedForCurrentPart = isset($_SESSION[$cCode]["items5"][$pid][$sku]) ? count($_SESSION[$cCode]["items5"][$pid][$sku]) : 0;
        ...
        echo json_encode(array(
            ...
            'serial' => $serial,
            'state' => $state,
            'checked_count' => $checkedForCurrentPart,
            'target_qty' => $targetQtyItem,
            'serial_mismatch_count' => $serialCompliance['serial_mismatch_count'],
            'serial_total_sku' => $serialCompliance['serial_total_sku'],
            'serial_balanced_sku' => $serialCompliance['serial_balanced_sku'],
        ));
    ...
    private function _calcSerialCompliance($cCode)
    {
        $selectedSerial = isset($_SESSION[$cCode]['items5']) ? $_SESSION[$cCode]['items5'] : array();
        $items = isset($_SESSION[$cCode]['items']) ? $_SESSION[$cCode]['items'] : array();
        $totalSerialMinusSku = 0;
        $serialBalancedSkuCount = 0;
        $serialMismatchCount = 0;
        ...
        if (is_array($items)) {
            foreach ($items as $id => $iSpec) {
                ...
                $qtyOpnameRow = isset($iSpec['qty_opname']) ? (float)$iSpec['qty_opname'] : $stokRow;
                $selisihRow = isset($iSpec['qty_selisih']) ? (float)$iSpec['qty_selisih'] : ($qtyOpnameRow - $stokRow);
                $isSerial = ((isset($iSpec['jml_serial']) && (int)$iSpec['jml_serial'] > 0) || (isset($iSpec['scan_mode']) && $iSpec['scan_mode'] == 'serial'));

                if ($selisihRow < 0 && $isSerial) {
                    $totalSerialMinusSku++;
                    $requiredPerPart = abs($selisihRow);
                    $isItemBalanced = true;

                    $skuParts = array();
                    if (isset($selectedSerial[$id]) && is_array($selectedSerial[$id]) && count($selectedSerial[$id]) > 0) {
                        $skuParts = array_keys($selectedSerial[$id]);
                    }
                    if (empty($skuParts)) {
                        $pKode = isset($iSpec['produk_kode']) ? $iSpec['produk_kode'] : (isset($iSpec['kode']) ? $iSpec['kode'] : (isset($iSpec['nama']) ? $iSpec['nama'] : 'DEFAULT'));
                        $skuParts = array($pKode);
                    }

                    foreach ($skuParts as $pSku) {
                        $checkedInPart = 0;
                        if (isset($selectedSerial[$id][$pSku]) && is_array($selectedSerial[$id][$pSku])) {
                            $checkedInPart = count($selectedSerial[$id][$pSku]);
                        }
                        if ($checkedInPart != $requiredPerPart) {
                            $isItemBalanced = false;
                            break;
                        }
                    }
        ...
        return array(
            'serial_mismatch_count' => $serialMismatchCount,
            'serial_total_sku' => $totalSerialMinusSku,
            'serial_balanced_sku' => $serialBalancedSkuCount,
        );
    }
    // END OF COMPLETE REPEATED LOGIC
}