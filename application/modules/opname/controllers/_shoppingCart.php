...
//        $this->jenisTr = $this->uri->segment(3);
        ...
        // Proteksi Opsi 1: Jika sesi yang tersisa adalah sesi preview transaksi lama, bersihkan agar keranjang create tidak tercemar!
        if (isset($_SESSION[$cCode]['session_context']) && $_SESSION[$cCode]['session_context'] === 'preview_followup') {
        ...
        if (isset($_SESSION[$cCode])) {
            ...
            if (isset($_SESSION[$cCode]['items'])) {
                ...
                foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                    ...
                    $tmp["editTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "opname/", $iSpec['handler']) : '0') . "/" . $editHandlerMethod . "/" . $this->jenisTr . "?id=" . $id . "&newQty=";
                    $tmp["removeTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "opname/", $iSpec['handler']) : '0') . "/remove/" . $this->jenisTr . "?id=" . $id;
...
//            "sumRows2" => $sumRows2,
            ...
            "checkOpnamePaired" => MODUL_PATH . get_class($this) . "/checklistOpname/" . $this->jenisTr,
            //--------------
...
//        );
    ...
    public function checklistOpname()
    {
        ...
        if (isset($_SESSION[$cCode]['items'][$pID])) {
            $oldStatus = isset($_SESSION[$cCode]['items'][$pID]['ceklist_opname']) ? $_SESSION[$cCode]['items'][$pID]['ceklist_opname'] : 0;
            ...
            if ($oldStatus != 1) {
                ...
                $itemNama = isset($_SESSION[$cCode]['items'][$pID]['nama']) ? $_SESSION[$cCode]['items'][$pID]['nama'] : '';
                $itemKode = isset($_SESSION[$cCode]['items'][$pID]['produk_kode']) ? $_SESSION[$cCode]['items'][$pID]['produk_kode'] : '';

                $audit->recordSessionEvent($cCode, 'CHECKLIST_OPNAME', array(
                    'item_id' => $pID,
                    'item_kode' => $itemKode,
                    'item_nama' => $itemNama,
                    'field' => 'ceklist_opname',
                    'old_val' => 'Belum Dicek',
                    'new_val' => 'Sudah Dicek (Fisik OK)',
                    'note' => 'Checklist fisik barang'
                ));
            }
        }
...
//        arrPrint($_GET);
        ...
        if (isset($_SESSION[$cCode]['main'])) {
            ...
            if (isset($_GET['note1'])) {
                $oldNote1 = isset($_SESSION[$cCode]['main']['opnameNote_1']) ? $_SESSION[$cCode]['main']['opnameNote_1'] : '';
                $_SESSION[$cCode]['main']['opnameNote_1'] = $note1;
                $_SESSION[$cCode]['main']['opnameNoteCeklist_1'] = 1;

                if ((string)$oldNote1 !== (string)$note1) {
                    $audit->recordSessionEvent($cCode, 'EDIT_NOTE', array(
                        'field' => 'opnameNote_1',
                        'old_val' => $oldNote1,
                        'new_val' => $note1,
                        'note' => 'Catatan verifikasi opname 1'
                    ));
                }
            }
            if (isset($_GET['note2'])) {
                $oldNote2 = isset($_SESSION[$cCode]['main']['opnameNote_2']) ? $_SESSION[$cCode]['main']['opnameNote_2'] : '';
                $_SESSION[$cCode]['main']['opnameNote_2'] = $note2;
                $_SESSION[$cCode]['main']['opnameNoteCeklist_2'] = 1;

                if ((string)$oldNote2 !== (string)$note2) {
                    $audit->recordSessionEvent($cCode, 'EDIT_NOTE', array(
                        'field' => 'opnameNote_2',
                        'old_val' => $oldNote2,
                        'new_val' => $note2,
                        'note' => 'Catatan verifikasi opname 2'
                    ));
                }
            }
        }
