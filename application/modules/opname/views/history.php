    ...
    case "showData":
        ...
        if (isset($stepLabels) && sizeof($stepLabels) > 0) {
            ...
            foreach ($stepLabels as $step => $label) {
                ...
                $varUrl = $steps[$step]['target'] == $currentState ? "javascript:void(0);" : "if($('#historyList').length > 0){ $('#historyList').append(`<div id='overlay'><div id='text'>Loading Content...</div></div>`); document.getElementById('overlay').style.display = 'block'; $('#historyList').load('$linkStep'); }else{ console.log('tidak ada container historyList'); }";
...
//            $content .= "<form name='fSelect'>";
        ...
        $content .= "</div>"; // close box-body
        $content .= "</div>"; // close box
