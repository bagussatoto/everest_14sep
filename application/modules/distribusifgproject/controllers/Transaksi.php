...
//        arrPrint($this->session->login);
        if ($this->session->login["cabang_id"] == CB_ID_PUSAT) {
            ...
            $mg = New MdlGudang();
            $mg->addFilter("cabang_id=" . $this->session->login["cabang_id"]);
            $mg->addFilter("jenis='gudang project'");
            $mgTmp = $mg->lookupAll()->result();
            $arrGudangIDs = array();
            ...
            if (sizeof($mgTmp) > 0) {
                foreach ($mgTmp as $mgSpec) {
                    $arrGudangIDs[$mgSpec->id] = $mgSpec->id;
                }
            }
            if (in_array($this->session->login["gudang_id"], $arrGudangIDs)) {

            }
//            else if($gudang_id==-999 && $cabang_id==-999){
            else {
                if ($gudang_id == "-999" && $cabang_id == CB_ID_PUSAT) {

                }
                else {
                    $msg = "Anda tidak memiliki hak akses untuk transaksi ini (hanya pihak Gudang Project di DC/Pusat). Silahkan hubungi admin untuk mengatur hak akses anda.<br>cabang: $cabang_nama || gudang: $gudang_nama";
                    $arrAlert["html"] = $msg;
                    $link_to = base_url();
                    $alert = swalAlertGoToBlock($arrAlert, $link_to, "OK");
//                    $alert = swalAlertGoTo($arrAlert, $link_to, "OK");
//                die();
                    $this->alert = $alert;
                }
            }
    ...
    public function validate()
    {
        ...
        $previewCtr = isset($this->configUi[$this->jenisTr]["previewCtr"]) ? $this->configUi[$this->jenisTr]["previewCtr"] : mati_disini('controller preview belum diset');
                    ...
                    if ((isset($iSpec['disabled']) && $iSpec['disabled'] == "0") || !isset($iSpec['disabled'])) {
                        ...
                        foreach ($fieldValidatorRules as $field => $label) {
                            ...
                            if (!is_numeric($iSpec[$field])) {
                                $errMsgs[] = "$label wajib berupa angka.";
                                $errLines[] = $id;
                                $errFields[$id][] = $field;
                            }
                            if ((int)$iSpec[$field] < 0.5) {
                                $errMsgs[] = "$label wajib diisi dan lebih dari 0.";
                                $errLines[] = $id;
                                $errFields[$id][] = $field;
                            }
                        }
                        ...
                        foreach ($fieldOpnameValidatorRules as $field => $label) {
                            if (isset($iSpec['qty_selisih']) && ($iSpec['qty_selisih'] > 0)) {
                                if ((int)$iSpec[$field] <= 0) {
                                    $errMsgs[] = "$label $name wajib diisi dan lebih dari 0";
                                    $errLines[] = $id;
                                    $errFields[$id][] = $field;
                                }
                            }
        ...
        if (!isset($_SESSION[$cCode][$gateNameDetail]) || sizeof($_SESSION[$cCode][$gateNameDetail]) == 0) {
            if( $cCode != "_TR_9833" ){
            $msg = "[$cCode] Transaksi gagal dilanjutkan karena rincian transaksi kosong.<br>Silahkan merubah qty/unit price dahulu dan masukkan data yang benar supaya transaksi bisa dilanjutkan. @" . __LINE__ . __METHOD__;
            mati_disini($msg);
        }
        }
        else {
            foreach ($_SESSION[$cCode][$gateNameDetail] as $spec) {
                if (!isset($spec['valid_qty'])) {
                    ...
                    $msg = "[$cCode] Transaksi gagal dilanjutkan karena rincian transaksi kosong.<br>Silahkan merubah qty/unit price dahulu dan masukkan data yang benar supaya transaksi bisa dilanjutkan. $hidden_msg";
                    mati_disini($msg);
                }
                else {
//                    if ($spec['valid_qty'] == 0) {
//                        $hidden_msg = "<br><span style='color:#ffffff;'>gerbang valid_qty bernilai 0 (gateNameDetail: $gateNameDetail)</span>";
...
//                    }
    ...
    public function viewUndoneItems()
    {
        ...
        if (sizeof($tmpHist) > 0) {
            ...
            foreach ($tmpHist as $row) {
                ...
                resolveReturnProjectRowFields($row);
                $tmp = array();
                $numb++;
    ...
    public function viewUndoneItemsIndex()
    {
        ...
        //TEMBAK CABANG ADN GUDANG ID UNTUK NGETEST---------------
        switch ($jenisTr) {
            case "5855":
            case "9833":
            case "9855":
...
//                    "gudang_id" => "-710",
        ...
        $pakai_ini = 0;// mengikuti hak aksesnya
...
//                    }
        ...
        $tr->addFilter("transaksi.jenis_top='" . $steps[1]['target'] . "'");
...
//        $tr->addFilter("transaksi_data.valid_qty>0");
        if ($this->session->login['employee_type'] == "employee_freelance") {
            $tr->addFilter("transaksi.seller_id='" . $this->session->login['id'] . "'");
        }
                ...
                if (isset($this->configUi[$jenisTr]['steps'][$nextStepNum])) {
                    if (isset($this->accessList[$jenisTr])) {
                        if (isset($this->accessList[$jenisTr][$nextStepNum][$nextStepCode]["allowFollowUp"])) {
                            ...
                            $allowFollowup = false;
                            $actionLabel = "Anda tidak memiliki hak akses<br>" . $this->configUi[$jenisTr]['steps'][$nextStepNum]['label'];
//                            cekHitam(__LINE__ . " [$allowFollowup] " . $row->nomer);
                        }
                    }
                    else {
...
//                die();
        ...
        //------------------------------
        $hakAkses = loadCreateTransaksi($jenisTr, my_id());
        if (sizeof($hakAkses) == 1) {
//            arrPrintWebs($hakAkses);
            $hakAksesStep = $hakAkses[0]->steps;
            if ($hakAksesStep == 1) {
//                cekHere(current_url());
                $link = str_replace("index.php/", "", current_url());
                $link = str_replace("Transaksi/", "Create/", $link);
                redirect($link);
            }
        }
        //------------------------------
...
//                                $trReg->setJointSelectFields("transaksi_id,main,items");
                                ...
                                if (sizeof($tmpReg) > 0) {
                                    foreach ($tmpReg as $regRow) {
//                                        arrPrint($regRow);
                                        foreach ($regRow as $r_key => $r_val) {
                                            if ($r_key != "transaksi_id") {
                                                $tmpReg_result[$regRow->transaksi_id][$r_key] = ($r_val != null) ? blobDecode($r_val) : array();
                                            }
                                        }
//                                        $param = $regRow->param;
//                                        $tmpReg_result[$regRow->transaksi_id][$param] = blobDecode($regRow->values);
...
//                                    arrPrint($tmpReg_result[$tmpSpec->transaksi_id]);
                                    foreach ($tmpReg_result[$tmpSpec->transaksi_id] as $param => $eReg) {
//                                        arrPrintPink($eReg);
                                        if ($param == "main") {
                                            foreach ($eReg as $k => $v) {
                                                if (($k != null) && !isset($tmpSpec->$k)) {
//                                                    cekHere(":: $k => $v ::");
                                                    $tmpSpec->$k = $v;
                                                }
                                            }
...
//arrPrintPink($tmpSpec);
    ...
    public function viewRequestItemsIndex2()
    {
...
//        arrPrint($sesionReplacer);
        $jenisTr = $this->jenisTr;
        $cCode = "_TR_" . $this->jenisTr;
        $historyFields = isset($this->configUi[$jenisTr]['shortRequestFields2']) ? $this->configUi[$jenisTr]['shortRequestFields2'] : array();
        $pairRegistries = isset($this->configUi[$jenisTr]['pairRegistries']) ? $this->configUi[$jenisTr]['pairRegistries'] : array();
        $arrExtHistoryFields2 = isset($this->configUi[$jenisTr]["extHistoryFields2"]) ? $this->configUi[$jenisTr]["extHistoryFields2"] : array();
        $arrItemShow = isset($this->configUi[$jenisTr]["shortItemsFields"]) ? $this->configUi[$jenisTr]["shortItemsFields"] : array();
        $shortRequestFields2Config = isset($this->configUi[$jenisTr]["shortRequestFields2Config"]) ? $this->configUi[$jenisTr]["shortRequestFields2Config"] : array();
...
//arrPrintKuning($stepHistoryFields);
        $prePreFields = $historyFields;
//        $prePreFields['state'] = "status";
        $prePreFields['action'] = "action";
        $swapJenisTr = isset($this->configUi[$jenisTr]['requestCode']['masterCode']) ? $this->configUi[$jenisTr]['requestCode']['masterCode'] : array();
        ...
        $tmpHist = array();
        $link_swap = "";
        if (isset($shortRequestFields2Config["enabled"]) && ($shortRequestFields2Config["enabled"] == true)) {
            $link_swap = $shortRequestFields2Config["linkSwapper"];
            $tr->addFilter("transaksi.div_id='" . $this->session->login['div_id'] . "'");
            $tr->addFilter("transaksi_data.valid_qty>0");
...
//                arrPrint($row);
                ...
                $modul = isset(loadConfigUiMaster()[$jenisMaster]['modul']) ? loadConfigUiMaster()[$jenisMaster]['modul'] : NULL;
                $modul_path = base_url() . "$modul/";
                $tmp['jenis_master'] = $row->jenis_master;
                $tmp['modul'] = "$modul";
                $tmp['modul_path'] = "$modul_path";
                ...
                foreach ($historyFields as $fName => $fLabel) {
                    ...
                    if (is_array($fLabel)) {
                        ...
                        $tmp[$fName] = isset($row->$fName) ? formatField_he_format($fName, $row->$fName, $jenisMaster, $modul_path) : formatField_he_format($fName, 0, $jenisMaster, $modul_path);
                    }

                    if ($fName == "no") {
                        $tmp[$fName] = formatField_he_format($fName, $numb, $jenisMaster, $modul_path);
                    }
...
//                }
                ...
                $tmp['action'] = "<div class='input-group'>";
                $targetFollowupLink = $isCancelPacking == true ? "followupCancelPackingPrePreview" : "followupPrePreview";
                $followupLink = "top.$('#result').load('" . MODUL_PATH . "$link_swap" . "$jenisTr/" . $row->transaksi_id . "');";
//                $followupLink = "";
                $tmp['action'] .= "<button class='btn btn-primary btn-block' title='turn this entry into $stepLabel' href='javascript:void(0)' $disabled_button
                                onClick =\"$followupLink\">tentukan jumlah yang dikirim</button>";
                $tmp['action'] .= "</div class='input-group'>";
                $tmp['action'] .= $add_note;
...
//            "mode" => isset($this->configUi[$jenisTr]["mode"]) ? $this->configUi[$jenisTr]["mode"] : $this->uri->segment(3),
            ...
            "availDbTable" => $availDbTable,
            "arrayHistory" => isset($arrayHistory) ? $arrayHistory : array(),
            "onprogressTitle" => "<span class='glyphicon glyphicon-alert'></span> BARANG YANG PERLU DIKIRIM DARI GUDANG DC/PUSAT KE KONSUMEN ",
            "arrayProgressLabels" => $progressFields,
            "arrayOnProgressToPay" => $paymentConfig,
            "itemLabels" => isset($itemLabels) ? $itemLabels : "",
            "srcLabel" => isset($srcLabel) ? $srcLabel : "",
    ...
    public function viewRequestItemsIndexDirect()
    {
...
//        arrPrint($sesionReplacer);
        $jenisTr = $this->jenisTr;
        $cCode = "_TR_" . $this->jenisTr;
        $historyFields = isset($this->configUi[$jenisTr]['shortDirectFields2']) ? $this->configUi[$jenisTr]['shortDirectFields2'] : array();
        $pairRegistries = isset($this->configUi[$jenisTr]['pairRegistries']) ? $this->configUi[$jenisTr]['pairRegistries'] : array();
        $arrExtHistoryFields2 = isset($this->configUi[$jenisTr]["extHistoryFields2"]) ? $this->configUi[$jenisTr]["extHistoryFields2"] : array();
        $arrItemShow = isset($this->configUi[$jenisTr]["shortItemsFields"]) ? $this->configUi[$jenisTr]["shortItemsFields"] : array();
        $shortRequestFields2Config = isset($this->configUi[$jenisTr]["shortDirectFields2Config"]) ? $this->configUi[$jenisTr]["shortDirectFields2Config"] : array();
...
//            cekHijau(sizeof($tmpHist));
        ...
        if (sizeof($tmpHist) > 0) {
            ...
            $psrc = New MdlTransaksi();
            $psrc->setFilters(array());
            $psrc->addFilter("transaksi_id in ('" . implode("','", $arrTransSoID) . "')");
//            $psrc->addFilter("transaksi_id in ('" . implode("','", $arrTransID) . "')");
            $psrc->addFilter("terbayar=0");
            $psrcTmp = $psrc->lookUpAllPaymentSrc()->result();
//            showLast_query("biru");
...
//arrPrintPink($row);
                ...
                if ($pakai_ini == 1) {
                    if (sizeof($arrItemShow) > 0) {
                        $tblItems .= "<table class='table table-bordered' style='margin:0px;'>";
                        $tblItems .= "<tr>";
                        $tblItems .= "<th>No</th>";
                        foreach ($arrItemShow as $k => $iLabel) {
                            if (is_array($iLabel)) {
...
//            "mode" => isset($this->configUi[$jenisTr]["mode"]) ? $this->configUi[$jenisTr]["mode"] : $this->uri->segment(3),
            ...
            "availDbTable" => $availDbTable,
            "arrayHistory" => isset($arrayHistory) ? $arrayHistory : array(),
            "onprogressTitle" => "<span class='glyphicon glyphicon-alert'></span> BARANG DROPSHIP ",
            "arrayProgressLabels" => $progressFields,
            "arrayOnProgressToPay" => $paymentConfig,
            "itemLabels" => isset($itemLabels) ? $itemLabels : "",
            "srcLabel" => isset($srcLabel) ? $srcLabel : "",
