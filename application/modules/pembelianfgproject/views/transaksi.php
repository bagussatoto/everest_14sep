...
//            pageResize: true,
        ...
        if (isset($arrWarningPlaceTrans) && (sizeof($arrWarningPlaceTrans) > 0)) {
            $alerts = $arrWarningPlaceTrans["alert"];
            $link = $arrWarningPlaceTrans["link"];
            $btn_label = $arrWarningPlaceTrans["btn_label"];
            $btn_cancel = $arrWarningPlaceTrans["btn_cancel"];
            $trName .= swalAlertGoTo3($alerts, $link, $btn_label, $btn_cancel);
        }
            ...
            <div class="col-md-5 col-sm-6">
                ...
                <div class="input-group">
                    ...
                    <span class="input-group-btn">
                        <button class="btn btn-default btn-sm" type="button" onclick="resetFilterProject();" title="Reset Filter Proyek"><i class="fa fa-times text-red"></i></button>
                    </span>
                </div>
            </div>
            <div class="col-md-5 col-sm-6">
                ...
                <div class="input-group">
                    ...
                    <span class="input-group-btn">
                        <button class="btn btn-default btn-sm" type="button" onclick="resetFilterSearch();" title="Reset Teks Pencarian"><i class="fa fa-times text-red"></i></button>
            ...
            <table class="table table-bordered table-striped table-condensed" style="margin-bottom: 0;">
                <thead>
                    <tr class="bg-gray-active" style="background-color: #e0e0e0;">
                        ...
                        <th style="width: 20%;">Proyek & No. Kontrak</th>
                        <th style="width: 20%;">Kode & Nama Produk</th>
                        <th style="width: 8%; text-align: right;">Qty RAB</th>
                        <th style="width: 7%; text-align: right;">Qty PO</th>
                        <th style="width: 8%; text-align: right;">Qty Sisa</th>
                        <th style="width: 10%; text-align: right;">Stok Project DC</th>
                        <th style="width: 12%; text-align: right;">Stok Project Supplier</th>
                ...
                <tbody id="project_pool_tbody">
                    <tr>
                        <td colspan="10" class="text-center text-muted"><i class="fa fa-spinner fa-spin"></i> Memuat data kebutuhan proyek...</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
<script>
function resetFilterProject() {
    $("#filter_project_id").val("0");
    loadProjectPoolInline();
}

function resetFilterSearch() {
    $("#filter_project_search").val("");
    loadProjectPoolInline();
}

function cancelPoolItem(poolId, itemKode) {
    if (confirm("Apakah Anda yakin ingin membatalkan/menghapus kebutuhan barang [" + itemKode + "] dari Autopurchase Pool?")) {
        var targetUrl = "' . base_url() . 'pembelianfgproject/_shoppingCart/cancelProjectPoolItemAjax/1466?pool_id=" + poolId;
        $.ajax({
            url: targetUrl,
            type: "GET",
            dataType: "json",
            success: function(res) {
                if (res.status === "success") {
                    ...
                    alert(res.message || "Gagal membatalkan item.");
                }
            },
            error: function() {
...
function loadProjectPoolInline() {
    ...
    $("#project_pool_tbody").html("<tr><td colspan=\'10\' class=\'text-center text-muted\' style=\'padding:15px;\'><i class=\'fa fa-spinner fa-spin\'></i> Memuat data kebutuhan proyek...</td></tr>");
        ...
        success: function(res) {
            if (res.status === "success") {
                ...
                if (!res.data || res.data.length === 0) {
                    html = "<tr><td colspan=\'10\' class=\'text-center text-muted\' style=\'padding:15px;\'>Tidak ada kebutuhan produk proyek yang ditemukan.</td></tr>";
                } else {
    ...
    $(document).on("change", "select[name*=\'pihakID\'], #pihakID, select[name*=\'pihak_id\']", function() {
        ...
        $p->addTags(
            array(
                ...
                "pihak_extern_caller" => $pihakExternCaller,
                ...
                "pihak_extern_caller_delete" => $pihakExternCallerDelete,
                ...
                "pihak_extern_label" => $pihakExternLabel,
                ...
                "pihak_nama" => isset($pihak_nama) ? $pihak_nama : "",
                "disabled_pihak" => isset($disabled_pihak) ? $disabled_pihak : "",
...
//            $anu = "no data found.<br>";
//            $anu .= "to go back to index, you can click BACK button<br>";
            $str .= tplNoData();
        }
        $str .= "</div class='box box-danger'>";
...
//        }
        ...
        if (sizeof($steps) > 1) {
            ...
            foreach ($steps as $tStep => $stepData) {
                ...
                $actives = $tStep == $stepper ? "active" : "";
                $trSelesai = count($steps) == $tStep ? "SELESAI<br>" : "";
                $visibility = count($steps) == $tStep ? "visibility:hidden;" : "";

                /*---penanda tab ada print qr----*/
                $print_qr = "";
                if (isset($stepData["allowPrintQr"]) && $stepData["allowPrintQr"] == true) {
                    $print_qr .= "<i class='fa fa-print text-red' title='QR Print'>&nbsp;</i>";
                    $print_qr .= "<i class='fa fa-qrcode text-red' title='QR Print'></i>";
                }
                ...
                $strOnprog .= "<span class='text-uppercase text-bold' style='$visibility;'>$print_qr $trSelesai" . $stepData['label'] . "</span>  $isiBadge </a>";
                $strOnprog .= "</li>";
            }
            $strOnprog .= "</ul>";
        ...
        if (sizeof($arrayOnProgress) > 0) {
            if ($isMobile == true & $stepper == 2) {
                // arrPrintPink($arrayProgressLabels);
                $strOnprogMbl = "<div>";
                foreach ($arrayOnProgress as $key => $val) {
                    // arrPrintKuning($val);
                    $strOnprogMbl .= "<div class='box box-danger'>";
                    $strOnprogMbl .= "<div class='box-header with-border'>";
                    $strOnprogMbl .= "Tanggal: " . $val['dtime'];
                    $strOnprogMbl .= "</div>";

                    $strOnprogMbl .= "<div class='box-body'>";
                    $strOnprogMbl .= "<p>Nomer PO: " . $val['nomer_po'] . "</p>";
                    $strOnprogMbl .= "<p>Supplier: " . $val['suppliers_nama'] . "</p>";
                    $strOnprogMbl .= "</div>";

                    $strOnprogMbl .= "<div class='box-footer text-center text-uppercase'>";
                    $strOnprogMbl .= $val['action'];
                    $strOnprogMbl .= "</div>";
...
//                                    ],
        ...
        if ($isMobile == true & $stepper == 2) {
            ...
            $str .= "<div style='float:left;' class='text-uppercase'><h3>GRN CEK</h3> </div>";
            $str .= "<div style='float: right;font-size: 1.7em;margin-right: 10px;'><a href='JavaScript:void(0);' onclick=\"getScanTugas();\" ><i class='fa fa-qrcode'></i></a></div>";
            $str .= "</div>";
            $str .= $strOnprogMbl;
        }
        echo $str;
...
//        cekHere(":: HAHAHA ::");
        ...
        if (sizeof($stepLabels) > 0) {
            ...
            echo createStateHorizontalMap_he_misc($currentStep, sizeof($stepLabels), $stepLabels, $jenisTr, $configUi);
            echo "</div class=''>";
        }
            ...
            foreach ($items as $iSpec) {
                ...
                foreach ($itemLabels as $key => $label) {
                    ...
                    if (substr($key, 0, 1) == "*") {
                        ...
                        if (isset($iSpec[$key])) {
                            if (isset($notFormatedFields) && in_array($key, $notFormatedFields)) {
                                $fieldVal = $iSpec[$key];
                            }
                            else {
                                $fieldVal = formatField($key, $iSpec[$key]);
                            }
                        }
                        else {
                            $fieldVal = "";
...
//                            cekkuning("$elName dataField");
    ...
    case "followupPreview":
        ...
        if (sizeof($warning_cek) > 0) {
            $list_data = "<div class='alert alert-danger text-center' style='font-size:20px;margin-top:10px;'>";
            foreach ($warning_cek as $warning_label) {
                $list_data .= "$warning_label<br>";
            }
            $list_data .= "</div>";
            echo $list_data;
        }
...
//                            cekKuning(":: $key editable ::");
                            ...
                            if (isset($notFormatedFields) && in_array($key, $notFormatedFields)) {
                                // tidak diformat tampilannya....
                                $val_ff = $val;
                            }
                            else {
//                                cekKuning(":: $key ::");
                                $val_ff = number_format($val);
                            }
                            $strVal = "<input type=$inputType name='$key" . "_" . "$id' class='form-control text-right' value='" . $val_ff . "' onclick='this.select()' $addEvent>";
...
//                            $strVal = formatField($key, $val);
                        ...
                        echo "<td $tdOpt >$strVal $add_data_val";
//                        if (isset($arrSubDetailDataKolom[$key][$id])) {
//                            echo "<br>";
//                            echo formatField_he_format("serial", $arrSubDetailDataKolom[$key][$id]);
//                        }
                        echo "</td>";
                    }
                    ...
                    if (isset($arrSubDetailDataKolom["nama"][$id])) {
                        echo "<tr line=" . __LINE__ . ">";
                        echo "<td>&nbsp;</td>";
                        echo "<td colspan='" . sizeof($itemLabels) . "' style='font-size:12px;'>";
                        echo "<div style='white-space:break-spaces;'>";
                        foreach ($arrSubDetailDataKolom["nama"][$id] as $sku => $sku_data) {
                            echo "<b>$sku</b> : " . formatField_he_format("serial", $sku_data) . "<br>";
                        }
                        echo "</td>";
                        echo "</div>";
                        echo "</tr>";
...
//                            cekBiru("BAWAH");
                        ...
                        echo "<tr line=" . __LINE__ . ">";
                        if (isset($shopingcartAddDpp[$key])) {
                            $colspan_i = sizeof($itemLabels) - 2;
                            foreach ($shopingcartAddDpp[$key] as $ik => $ik_label) {
                                if (isset($sumDppTax[$key][$ik])) {
                                    $colspan_r = "1";
                                    echo "<td colspan='$colspan_i' class='text-right bottom_borderless valign-m text-uppercase'>$ik_label</td>";
                                    echo "<td colspan='1' class='text-right bottom_borderless valign-m'>" . number_format($sumDppTax[$key][$ik]) . "</td>";
                                }

                            }
                        }
                        else {
                            $colspan_r = sizeof($colspanSum);
                        }
//                        echo "<td colspan='" . sizeof($colspanSum) . "' class='text-right'>$label</td>";
                        echo "<td colspan='" . $colspan_r . "' class='text-right'>$label</td>";
                        echo "<td class='text-right'>";
...
//                    echo "<div class=''>Detail</div>";
            ...
            if (isset($warning_sku) && sizeof($warning_sku)) {
                echo "<div class='alert alert-danger text-center' style='font-size:20px;'>";
                foreach ($warning_sku as $msgSpec) {
                    echo $msgSpec . "<br>";
                }
                echo "</div class='alert alert-warning'>";
            }

            //region bagian freeproduk pembelian
            if (isset($addProdukDisc) && count($addProdukDisc) > 0) {
                echo "<div class='panel-header table-responsive no-padding no-border' style='margin-top:15px;'><h5 class='text-left text-bold'>PROMO SUPPLIER</h5></div>";
                echo "<div class='panel-body table-responsive no-padding no-border'>";
                echo "<table class='table table-condensed no-padding table-bordered no-margin'>";
                echo "<tr>";
                echo "<th>no</th>";
                foreach ($addProdukDiscLabel as $xKey => $x_label) {
                    echo "<th>" . $x_label . "</th>";
                }
                echo "</tr>";
                $ix = 0;
                foreach ($addProdukDisc as $produk_id => $addProdukDisc_0) {
                    $ix++;
                    echo "<tr>";
                    echo "<td>$ix</td>";
                    foreach ($addProdukDiscLabel as $xKey => $x_label) {
                        echo "<td>";
                        echo formatField_he_format($xKey, $addProdukDisc_0[$xKey]);
                        echo "</td>";
                    }
                    echo "</tr>";
                }
            ...
            if (sizeof($connected_diskon_jual) > 0) {
                if (isset($connected_diskon_jual_visibility) && ($connected_diskon_jual_visibility != null)) {
                    $visibility = "$connected_diskon_jual_visibility";
                }
                echo "<div class='$visibility'>";// isinya hidden
                echo $connected_diskon_jual;
                echo "</div>";
            }
            ...
            if (isset($followupDiskonSupplierConfig) && sizeof($followupDiskonSupplierConfig) > 0) {

                if (isset($followupDiskonSupplierConfig["formKey"]) && (sizeof($followupDiskonSupplierConfig["formKey"]) > 0)) {
                    $alert_theme = "alert-danger";
                    if ($followupDiskonSupplierSelected > 0) {
                        $alert_theme = "alert-info";
                    }

                    echo "<div class='alert $alert_theme' style='font-size:15px;'>";// isinya hidden
                    echo "<h4>" . $followupDiskonSupplierConfig["label"] . "</h4>";
                    foreach ($followupDiskonSupplierConfig["formKey"] as $kk => $kk_label) {
                        $checked = ($kk == $followupDiskonSupplierSelected) ? "checked" : "";
                        $key_nama = $followupDiskonSupplierConfig["key"];
                        echo "<input $checked type='radio' name='$key_nama' value='$kk'
                        onclick=\"document.getElementById('result').src='$followupDiskonSupplierAction?key=$key_nama&val='+encodeURIComponent(this.value);\"
                        >";
                        echo " &nbsp; $kk_label";
                        echo "<br>";
                    }
...
//arrPrint($itemLabelsByGudang);
//arrPrintCyan($itemLabelsByGudangViewOnlyData);
            if (isset($itemLabelsByGudang) && (sizeof($itemLabelsByGudang) > 0)) {
                $str = "<div class='alert alert-warning text-center' style='font-size:20px;'>";
                $str .= "<h4>$itemLabelsByGudangTitle</h4>";
                if (isset($itemLabelsByGudangViewOnly) && ($itemLabelsByGudangViewOnly == true)) {
                    $str .= "<div>";
                    $str .= "Anda akan terima barang di <b>" . $itemLabelsByGudangViewOnlyData["gudangTujuanLabel"] . "</b>.<br>";
                    $str .= "Jika akan ganti Gudang, Silahkan Reject 1 Step transaksi ini.";
                    $str .= "</div>";
                }
                else {
                    $disabled = ($disabled_radio == true) ? "disabled" : "";
                    if ($label_exception != NULL) {
                        $str .= "<h4>$label_exception</h4>";
                    }
                    $key_nama = "gantiGudangTujuan";
                    foreach ($itemLabelsByGudang as $gid => $glabel) {
                        $checked = ($gid == $itemLabelsByGudangSelected) ? "checked" : "";
                        $str .= "<input $checked type='radio' name='$key_nama' value='$gid' $disabled
                        onclick=\"document.getElementById('result').src='$itemLabelsByGudangAction?key=$key_nama&label=$glabel&val='+encodeURIComponent(this.value);\"
                        > <span style='text-transform: uppercase;'>$glabel</span> ";
                        $str .= "&nbsp;&nbsp;&nbsp;";
                    }
                }
                $str .= "</div class='alert alert-warning'>";
...
//                    cekHere(sizeof($descriptionAdditionalPreviews));
                ...
                if (sizeof($descriptionMainFollowupRule) > 0) {
                    if (isset($descriptionMainFollowupRule['enabled']) && ($descriptionMainFollowupRule['enabled'] == true)) {
                        $maximal_karakter = $descriptionMainFollowupRule['maximal_karakter'];
                        $maximal_karakter_label = str_replace("maximal_karakter", $maximal_karakter, $descriptionMainFollowupRule['maximal_karakter_label']);
                        $maximal_karakter_label_ff = ($maximal_karakter_label != NULL) ? " &nbsp;&nbsp;<span class='meta'>$maximal_karakter_label</span>" : "";
                        echo "<table class='table table-bordered table-condensed'>";
                        echo "<tr line=" . __LINE__ . ">";
                        echo "<td colspan='" . (sizeof($itemLabels) + 1) . "'>";
                        echo "<span class='text-muted'>" . $descriptionMainFollowupRule['label'] . "</span>$maximal_karakter_label_ff<br>";
                        echo "<span class='' style='font-style:italic;font-family:Monaco, Menlo, Consolas, \"Courier New\", monospace;'>";
...
//                }
            ...
            if (isset($items) && sizeof($items) > 0) {
                ...
                if (isset($deleteSpec['targetUrl']) != "" && $deleteSpec['targetUrl'] != "") {
                    ...
                    $disabled_button_kreditlimit $disabled_button 
                ...
                if (isset($editSpec['targetUrl']) != "" && $editSpec['targetUrl'] != "") {
                    ...
                    class='btn btn-default margin' style='border:1px #ff7700 solid;color:#ff7700;' 
                    onclick=\"if(confirm('" . $editSpec['warning'] . "')==1){document.getElementById('f1').action='" . $editSpec['targetUrl'] . "';document.getElementById('f1').submit();}\">
                    <span class='fa fa-pencil'></span> " . $editSpec['label'] . "</button>";
                }
                else {
                ...
                if ((isset($extBtns) && sizeof($extBtns) > 0) || (isset($payBtns) && sizeof($payBtns) > 0)) {
                    ...
                    document.getElementById('f1').submit();}\"><span class='fa fa-remove'></span> " . $xShipmentBtn['label'] . "</button>";
                        ...
                        echo "<button button type='button' disabled class='btn btn-danger' style='border:1px #dd3300 solid;color:#dcdcdc;'><span class='glyphicon glyphicon-ban-circle'></span> " . $xShipmentBtn['label'] . "</button>&nbsp;&nbsp;&nbsp;";
                    }
                    // -------------------------------------------------------------
                    if (isset($rejectionSpec['targetUrl']) != "" && $rejectionSpec['targetUrl'] != "") {
                        echo "<button type='button' class='btn btn-danger margin' style='border:1px #dd3300 solid;ccolor:#dd3300;'
                    ...
                    if (isset($approvalSpec['targetUrl']) != "" && $approvalSpec['targetUrl'] != "") {
                        echo "<button button type='button' class='btn btn-success margin' style='border:1px #008800 solid;color:#ffffff;' 
                        $disabled_button_kreditlimit $disabled_button $disabled_checker
                        onclick=\"if(confirm('" . $approvalSpec['warning'] . "')==1){this.disabled=true;document.getElementById('f1').action='" . $approvalSpec['targetUrl'] . "';document.getElementById('f1').submit();top.open_holdon();}\"><span class='glyphicon glyphicon-ok'></span> " . $approvalSpec['label'] . "</button>";
                    }
                    else {
                        echo "&nbsp;";
                    }
...
//                                            + \"<div class='text-right'>\"+addCommas(total5)+\"</div>\"
    ...
    case "viewResume":
        ...
        if (isset($addMainSourceField) && sizeof($addMainSourceField) > 0) {
//            arrPrintPink($main);
            echo "<h4 class='text-blue'><span class='fa fa-book'>" . $main["jenisTrName"] . "</span></h4>";
            echo "<div class='table-responsive'>";
            echo "<table class='table ttable-bordered table-condensed'>";
            echo "<tr bgcolor='#f5f5f5'>";
            echo "<th class='text-muted' style='font-weight:bold;' align='right' width='5'>no.</th>";
            foreach ($addMainSourceField as $key => $label) {
                echo "<th class='text-muted' style='font-weight:bold;'>";
            ...
            foreach ($addMainSourceField as $kol => $alias) {
                ...
                $strVal = formatField($kol, $main[$kol]);
                echo "<td $tdOpt>";
                echo $strVal;
                echo "</td>";
        ...
        if (isset($arrDiskonLabel) && (sizeof($arrDiskonLabel) > 0)) {
            echo "<h4 class='text-blue'><span class='fa fa-book'>$diskonTitle</span></h4>";
            echo "<div class='table-responsive'>";
            echo "<table class='table table-condensed'>";
            echo "<tr bgcolor='#f0f0f0'>";
            echo "<td>no.</td>";
            foreach ($arrDiskonLabel as $key => $label) {
                echo "<td>";
                echo "$label";
                echo "</td>";
            }
            echo "</tr>";
            $no = 0;
            foreach ($arrDiskonData as $iSpec) {
                $no++;
                echo "<tr line=" . __LINE__ . ">";
...
//                    echo "</a>";
                    echo "</td>";
                    if (is_numeric($iSpec[$key])) {
                        if (!isset($totalDiskon[$key])) {
                            $totalDiskon[$key] = 0;
                        }
                        $totalDiskon[$key] += $iSpec[$key];
                    }
                }
                echo "</tr>";
            }
            ...
            foreach ($arrDiskonLabel as $key => $label) {
                echo "<td>";
                if (isset($totalDiskon[$key])) {
                    echo formatField_he_format($key, $totalDiskon[$key]);
                }
                echo "</td>";
            }
            echo "</tr>";
...
//cekHere($supplier_id);
        //        arrPrint($tabFieldsItems);
//arrPrintKuning($arrayOnProgress2);
//arrPrintPink($tabHistoryFields);
        $str = "";
        if (sizeof($arrayOnProgress) > 0 || sizeof($arrayOnProgress2) > 0) {
            if (sizeof($tabHistoryFields) > 0) {
                $link = MODUL_PATH . "Transaksi/viewRequestItems/1466";
                $str .= "<div class=\"title\"><h3><a href='JavaScript:void(0)' onclick=\"$('#requestItems').load('$link')\">Purchasing Request</a> $supplier_nama</h3></div>";
                $str .= "<div class=\"nav-tabs-custom\">";
                $str .= "<ul class=\"nav nav-tabs\">";
                $no1 = 1;
                foreach ($tabHistoryFields as $ky => $arrLab) {
...
//                    cekHere("::: $ky :::");
                    switch ($ky) {
                        case "produk_id":
                             ...
                             * --------------------------------------------------------------------------*/
                            $str .= "<thead style='background: lightgrey;'>";
                            $str .= "<tr line=" . __LINE__ . " class='text-uppercase'>";
                            foreach ($tabFieldsItems[$ky] as $kyLabel => $label) {
                                if ($kyLabel == 'select') {
                                    if (isset($allowMultiSelect) && $allowMultiSelect == true) {
                                        $selectAll = "<input type='checkbox' id='selectAll_$ky'>";
                            ...
                            $str .= "</thead>";
                            /* --------------------------------------------------------------------------
                             * body; format konten diatur dr coTransaksiUi tabFieldsItems (...attr)
                             * --------------------------------------------------------------------------*/
                            $str .= "<tbody>";
                            // arrPrint($arrayOnProgress2);
                            foreach ($arrayOnProgress2[$ky] as $row) {
                                // arrPrint($row);
                                $tabFieldsAttr = isset($tabFieldsItems[$ky . "_attr"]) ? $tabFieldsItems[$ky . "_attr"] : array();
                                // arrPrintPink($tabFieldsAttr);
                                $str .= "<tr line=" . __LINE__ . ">";
                                foreach ($tabFieldsItems[$ky] as $kyLabel => $row_label) {
                                    if (isset($row[$kyLabel])) {
                                        $nilai = $row[$kyLabel];
                                        $sup_id = $row['id'];

                                        $format_key = isset($tabFieldsAttr[$kyLabel]['format_key']) ? $tabFieldsAttr[$kyLabel]['format_key'] : $kyLabel;
                                        $nilai_f = isset($tabFieldsAttr[$kyLabel]['format']) ? $tabFieldsAttr[$kyLabel]['format']($format_key, $nilai) : $nilai;
                                        $attr = isset($tabFieldsAttr[$kyLabel]['attr']) ? $tabFieldsAttr[$kyLabel]['attr'] : "";
                                        // --------------------------------------------------------------------------------------link
                                        // pengaturan link ada di tabFieldsItems/con ui
                                        // --------------------------------------------------------------------------------------link
                                        $head_link = isset($tabFieldsAttr[$kyLabel]['link_head']) ? $tabFieldsAttr[$kyLabel]['link_head'] : "";
                                        $str_head = isset($row[$head_link]) ? $row_label . " " . $row[$head_link] : "";

                                        $link = isset($tabFieldsAttr[$kyLabel]['link']) ? base_url() . $tabFieldsAttr[$kyLabel]['link'] . "?id=$sup_id" : "";
                                        $btl_link = modalDialogBtn($str_head, $link);
                                        $nilai_l = isset($tabFieldsAttr[$kyLabel]['link']) ? "<a href='JavaScript:Void(0);' onclick=\"$btl_link\">$nilai_f</a>" : $nilai_f;
                            ...
                            foreach ($tabFieldsItems[$ky] as $kyLabel => $rows) {
                                ...
                                $nilai_footer = $angka[$ky] > 0 ? $angka[$ky] : "";
                                $format_key = isset($tabFieldsAttr[$kyLabel]['format_key']) ? $tabFieldsAttr[$kyLabel]['format_key'] : $kyLabel;
                                $nilai_footer_f = isset($tabFieldsAttr[$kyLabel]['format']) ? $tabFieldsAttr[$kyLabel]['format']($format_key, $nilai_footer) : $nilai_footer;
...
//                            cekHere($arrLab['allowFollowup']);
                                    if (isset($arrLab['allowFollowup']) && $arrLab['allowFollowup'] == true) {
//                                 cekHijau($ky);
                                        $str .= "<div class='col-sm-6 text-left'>";
                                        // $str .= "<button id='btnConnect$ky' name='btnConnect$ky' class='btn btn-primary' href=# oonclick=\"this.disabledxx=true;this.innerHTML='clear the list to connect another one';document.getElementById('$ky').submit()\"><span class='fa fa-external-link'></span> Followup " . $arrLab['label'] . "</button>";
                                        $str .= "<button type='button' id='btnCreateShoppingCart' name='btnConnect$ky' class='btn btn-primary btn-block' href=# oonclick=\"this.disabledxx=true;this.innerHTML='clear the list to connect another one';document.getElementById('$ky').submit()\"><span class='fa fa-external-link'></span> Followup " . $arrLab['label'] . "</button>";
                                        $str .= "</div>";
                                        $str .= "<div class='col-sm-6'><span class='meta'>Pilih dengan memberikan tic pada tiap bahan baku kemudian KLIK button [Followup] untuk memasukannya bahan baku tersebut ke shoping cart</span></div>";

                                        $str .= "\n\n<script>
                                            
                                            function initBtn(){
                                                var arrCheck = $(\"input[name='prdID\[\]']\");
                                                var arrValid = {}
                        ...
                        case "supplier_id_":
                            $no2 = 1;
//                            $str .= "SUPPLIER";
                            $getSupplierID = isset($supplier_id) ? $supplier_id : 0;
                            $active = $no2 == 1 ? " active" : "";
                            $str .= "<div class=\"tab-pane$active\" id=\"tab_$ky\">";
                            //-------
                            if (isset($supplier_data)) {
                                foreach ($supplier_data as $supplier_id => $supplier_nama) {
                                    $btn = $getSupplierID == $supplier_id ? "btn btn-success" : "btn btn-primary";
                                    $str .= "<button class='$btn' href='JavaScript:void(0)' onclick=\"localStorage.setItem('selectedSupplier',$supplier_id);$('#requestItems').load('$link?supplier_id=$supplier_id')\">";
                                    $str .= "$supplier_nama";
                                    $str .= "</button>&nbsp&nbsp";
                                }
                                $str .= "</br>";
                            }
                            //-------
//                            $str .= "<form method='post' id='$ky' name='$ky' target='result' action='$reqFormTarget?supplier_id=$getSupplierID'>";
                            $str .= "<form method='post' id='$ky' name='$ky' target='result' aaction='$reqFormTarget?supplier_id=$getSupplierID'>";
                            $str .= "<table class='table $ky table-hover' >";
                            /* --------------------------------------------------------------------------
                             * header
                             * --------------------------------------------------------------------------*/
                             ...
                             * --------------------------------------------------------------------------*/
                            $str .= "<tbody>";
//                            arrPrint($arrayOnProgress2[$ky][1]);
                            foreach ($arrayOnProgress2[$ky][$getSupplierID] as $row) {
...
//                                 cekHijau($ky);
                                        ...
                                        $str .= "\n\n<script>
                                            var selectedSupplier = localStorage.selectedSupplier==undefined ? 0 : localStorage.selectedSupplier;
                                            function initBtn(){
                                                var arrCheck = $(\"input[name='prdID\[\]']\");
                                                var arrValid = {}
                                                if(arrCheck.length > 0){
                                                   ...
                                                   $('#btnCreateShoppingCart')
                                                    ...
                                                    .on('click', function(){
                                                        $.ajax({
                                                            type: 'POST',
                                                            url: '$btnToShoppingCart',
                                                            data: { items: btoa(JSON.stringify(arrValid)), supplier: selectedSupplier },
                                                            success: function(data) {
                                                                var arrData = JSON.parse(data)
//                                                                console.log(data);
//                                                                console.log(arrData.status);
...
//                            $str .= "<form method='post' id='$ky' name='$ky' target='result' action='$reqFormTarget?supplier_id=$getSupplierID'>";
                            $str .= "<form method='post' id='$ky' name='$ky' target='result' aaction='$reqFormTarget?supplier_id=$getSupplierID'>";
                            $str .= "<table class='table $ky table-hover' id='table_request'>";
                            /* --------------------------------------------------------------------------
                             * header
                             * --------------------------------------------------------------------------*/
                            $str .= "<thead style='background: lightgrey;'>";
                            ...
                            if (isset($supplier_data)) {
                                foreach ($supplier_data as $supplier_id => $supplier_nama) {
//                                    cekHere(":: $supplier_id => $supplier_nama ::");
                                    foreach ($arrayOnProgress2[$ky][$supplier_id] as $row) {
                                        $num++;
                                        $tabFieldsAttr = isset($tabFieldsItems[$ky . "_attr"]) ? $tabFieldsItems[$ky . "_attr"] : array();
                                        // arrPrintPink($tabFieldsAttr);
                                        $str .= "<tr line=" . __LINE__ . ">";
...
//arrPrint($row);
                                                ...
                                                $link_modal = isset($tabFieldsAttr[$kyLabel]['link_modal']) && ($tabFieldsAttr[$kyLabel]['link_modal'] == false) ? $tabFieldsAttr[$kyLabel]['link_modal'] : true;
                                                if ($link_modal == true) {
                                                    ...
                                                    if (isset($tabFieldsAttr[$kyLabel]['link'])) {
                                                        if (isset($selectProduk) && ($selectProduk == true)) {
                                                            $blobItems = blobEncode(array($sup_id => $new_order));
                                                        }
                                                        else {
                                                            $blobItems = isset($requestSupplier[$supplier_id]) ? blobEncode($requestSupplier[$supplier_id]) : blobEncode(array());
                                                        }
                                                        $btl_link = "top.$('#result').load('" . MODUL_PATH . $tabFieldsAttr[$kyLabel]['link'] . "/?supplier=$supplier_id&items=$blobItems')";
                                                        $nilai_l = "<a href='JavaScript:Void(0);' onclick=\"$btl_link\">$nilai_f</a>";
                                                    }
                                                    else {
                                                        $nilai_l = $nilai_f;
                                                    }
...
//                                        },
                            ...
                            footerCallback: function ( row, data, start, end, display ) {
                                        ...
                                        var arrayFooter = $('#table_request>tfoot>tr>th');
                                        var dpageTotal = [];
                                        jQuery.each(arrayFooter, function(i,d){
                                            ...
                                            jQuery.each( $(api.column(id_n_index, { page: 'current'}).data() ), function(ii, obj){
                                                ...
                                                var hr = obj.indexOf('<hr>');
                                                var br = obj.indexOf('<br>');
                                                if(pos!==-1&&hr==-1&&id_n_index>0&&br==-1){
                                                    dpageTotal[id_n_index] += intVal( $(obj).html() );
                                                }
                                                else{
                                                }
                                ...
                                //new $.fn.dataTable.FixedHeader( table );
                                $('.table-responsive.table_request').floatingScroll();
                                $('.table-responsive.table_request').scroll(function() {
                                    setTimeout(function () {
                                        $('#table_request').DataTable().fixedHeader.adjust();
                                    }, 100);
                            });
                            });
                    </script>";
