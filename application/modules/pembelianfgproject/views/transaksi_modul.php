    ...
    case "index":
        ...
        if (isset($arrayOnProgressToPay) && $arrayOnProgressToPay == true) {
            ...
            if (sizeof($arrayOnProgress) > 0) {
                ...
                $strOnprog .= "<div class='alert alert-info overflow-h col-md-6' style='margin: -110px -32px 10px 32%; position: fixed;z-index: 999;'>";
                $strOnprog .= "<a href='#' class='close' data-dismiss='alert' aria-label='close' style='text-decoration: none;'>&times;</a>";
                $strOnprog .= createStateHorizontal($stepper, sizeof($steps), $jenisTr, false);
                $strOnprog .= "</div>";
                $strOnprog .= "</div>";
                ...
                if (sizeof($arrayProgressLabels) > 0) {
                    ...
                    foreach ($arrayProgressLabels_2 as $key => $label) {
                        $strOnprog .= "<th class=''>";
                        if (is_array($label)) {
                ...
                if (count($steps) == $stepper) {
                    ...
                    foreach ($arrayOnProgress as $key => $val) {
                        ...
                        // $strState = $val['state'];
                        // $strOnprog .= "<tr>";
                        // $strOnprog .= "<td colspan='$jml_tampil'>$strState</td>";
                        // $strOnprog .= "</tr>";
                    }
                }
        ...
        //region ongoing AI Order
        $str_AI_ongoing = "";
        $str_AI_ongoing .= "<div id='text_kata_kata' class='text-center text-bold text-red lead'>{order_dalam_kata}</div>";
        $str_AI_ongoing .= "<div id='div_ongoing_table_AI' class='stripe row-border order-column'></div>";
        $str_AI_ongoing .= "
            <script>
                function ongoing_stok(){

                    var periode_omset_value     = 30
                    var periode_stok_value      = 7
                    var index_value             = 100
                    
                    $('#text_kata_kata').html('*value order berdasarkan omset rata² selama '+periode_omset_value+' hari dan persiapan stok untuk '+periode_stok_value+' hari kedepan.') 
                    
                    $.ajax({
                        type: 'GET',
                        url: $('#baseURL').html() + 'eusvc/Dashboard/stok_control/' + $('#toko_id').html() + '/' + periode_omset_value + '/' + periode_stok_value + '/' + index_value,
                        success: function(data) {
                            if(data.status==1){
                                var arrDatas = data.datas
                                var arrParams = data.params
        
                                var table = ''
                                    table += \"<table style='margin-top: 20px;' id='ongoing_table_AI' class='table table-hover table-striped compact table-responsive'>\"
                                if($('#ongoing_table_AI')){
                                        var lebar_device = $(window).width();
                                        var tinggi_device = $(window).height();
                                        var now = new Date;
                                        var dateNow = now.customFormat('#YYYY#-#MM#-#DD# #hhhh#:#mm#:#ss#');
                                        //header
                                        table += '<thead>'
                                        table += '<tr class=text-uppercase>'
                                        table += '<th>no</th>'
                                        table += '<th>nama bahan</th>'
                                        table += '<th>omset</th>'
                                        table += '<th>rata2 pemakaian</th>'
                                        table += '<th>stok buffer</th>'
                                        table += '<th>stok riil</th>'
                                        table += '<th>stok ideal</th>'
                                        table += '<th>MOQ</th>'
                                        table += '<th>stok order</th>'

//                                        table += \"<th><input onclick='toggleCheckAll(this)' id='checkAll' type='checkbox'></th>\"

                                        table += '</tr>'
                                        table += '</thead>'
                                        //body
                                        table += '<tbody>'
                                        arrDatas = Object.keys(arrDatas).map(function(k){
                                            return arrDatas[k];
                                        });
                                        ...
                                        var num = 0
                                        var arrPushBahan={}
                                        jQuery.each(arrDatas, function(i, dt){
                                            num++;
                                            var stok_out = dt.stok_out*1 > 0 ?          \"<span class='text-default text-bold'>\" + addCommas( (dt.stok_out*1).toFixed(2) )        + '</span>' + \" <span class='text-info'><sup>\" + dt.stok_satuan +'</sup></span>' : 0
                                            var stok_out_avg = dt.stok_out_avg*1 > 0 ?  \"<span class='text-default text-bold'>\" + addCommas( (dt.stok_out_avg*1).toFixed(2) )    + '</span>' + \" <span class='text-info'><sup>\" + dt.stok_satuan +'</sup></span>' : 0
                                            var stok_buffer = dt.stok_buffer*1 > 0 ?    \"<span class='text-default text-bold'>\" + addCommas( (dt.stok_buffer*1).toFixed(2) )     + '</span>' + \" <span class='text-info'><sup>\" + dt.stok_satuan +'</sup></span>' : 0
                                            var stok_real = dt.stok_real*1 > 0 ?        \"<span class='text-default text-bold'>\" + addCommas( (dt.stok_real*1).toFixed(2) )       + '</span>' + \" <span class='text-info'><sup>\" + dt.stok_satuan +'</sup></span>' : 0
                                            var stok_ideal = dt.stok_ideal*1 > 0 ?      \"<span class='text-default text-bold'>\" + addCommas( (dt.stok_ideal*1).toFixed(2) )      + '</span>' + \" <span class='text-info'><sup>\" + dt.stok_satuan +'</sup></span>' : 0
                                            var moq = dt.moq*1 > 0 ?                    \"<span class='text-default text-bold'>\" + addCommas( (dt.moq*1).toFixed(2) )             + '</span>' + \" <span class='text-info'><sup>\" + dt.stok_satuan +'</sup></span>' : 0
                                            var stok_order = dt.stok_order*1 > 0 ?      \"<span class='text-default text-bold'>\" + addCommas( (dt.stok_order*1).toFixed(2) )      + '</span>' + \" <span class='text-info'><sup>\" + dt.stok_satuan +'</sup></span>' : 0
                                            
                                            if( dt.stok_order*1>0 ){
                                                table += '<tr>'
                                                table += '<td>'+num+'</td>'
                                                table += '<td>'+dt.nama+'</td>'
                                                table += \"<td data-order='\"+dt.stok_out+\"'>\"+stok_out+\"</td>\"
                                                table += \"<td data-order='\"+(dt.stok_out_avg*1>0?dt.stok_out_avg*1:0)+\"'>\"+stok_out_avg+\"</td>\"
                                                table += \"<td data-order='\"+dt.stok_buffer+\"'>\"+stok_buffer+\"</td>\"
                                                table += \"<td data-order='\"+dt.stok_real+\"'>\"+stok_real+\"</td>\"
                                                table += \"<td data-order='\"+dt.stok_ideal+\"'>\"+stok_ideal+\"</td>\"
                                                table += \"<td data-order='\"+dt.moq+\"'>\"+moq+\"</td>\"
                                                table += \"<td data-order='\"+(dt.stok_order*1<0?0:dt.stok_order*1)+\"'>\"+stok_order+\"</td>\"
                                                table += '</tr>'
                                                arrPushBahan[dt.id] = (dt.stok_order*1)
                                            }
                                        ...
                                        console.log('arrPushBahan', arrPushBahan);
                                        console.log('countObj', countObj(arrPushBahan));
                                        
                                        if(countObj(arrPushBahan)>0){
                                            
                                            localStorage.sesCartBahan = JSON.stringify(arrPushBahan)
                                            
                                            var totalBahan = countObj(arrPushBahan);
                                            
                                            table += \"<div style='margin-bottom: 10px;' class='row'>\"
                                            table += \"<div class='col-md-4'>\"
                                            table += \"</div>\"
                                            table += \"<div class='col-md-4'>\"
                                            table += \"</div>\"
                                            table += \"<div id='btnActionTable' class='col-md-4 text-right'>\"
                                            table += \"<span onclick='sendToCart()' class='btn btn-sm btn-success'>Lanjut Input/Pembelian Bahan (\"+totalBahan+\")</span>\"
                                            table += \"<span onclick='javascript:void(0)' class='btn btn-sm btn-default'><i class=fa fa-trash></i> Input/Pembelian Tanpa AI</span>\"
                                            table += \"</div>\"
                                            table += \"</div>\"
                                        }
                                        
                                    $('#div_ongoing_table_AI').html(table);
                                    
                                    var tablesAI = $('#ongoing_table_AI').DataTable({ });

                                    $('#last_update_ui').attr('time-data', dateNow)
                                    $('#lebar_device').html('width: '+ lebar_device + ' height: '+tinggi_device+' ||  ');
                                    $('#btnRefreshKalkulator').addClass('btn-success').children().removeClass('fa-spin');
                ...
                function sendToCart(){
                    ...
                    if(sesCartBahan!=0&&countObj(sesCartBahan)>0){
                        $.ajax({
                            ...
                            success: function(data) {
                                //location.href = baseUrl + 'Transaksi/createForm/461'
                                $.ajax({
                                    type: 'GET',
                                    async: false,
        ...
        //endregion ongoing AI Order

        //region produk request from AI stok
        $strStokOnCalc = "";
        $strStokOnCalc .= "
            <div style='margin-bottom: 10px;' class='row'>
                <div class='col-md-4'>
                    <span id='' class=''>{jenisTR} : " . $jenisTr . "</span>
                    <span id='lebar_device' class=''>{lebar}</span>
                    <span id='last_update_ui' time-data='' class=''>{time_last_update}</span>
                </div>
            </div>
            <div style='margin-bottom: 25px;' class='row'>
                <div class='col-md-2'>
                    <label for='index_'>Index</label>
                    <input onclick='select()' id='index_' class='form-control' >
                </div>
                <div class='col-md-2'>
                    <label for='periode_omset_'>Periode Omset (hari)</label>
                    <input onclick='select()' id='periode_omset_' class='form-control' >
                </div>
                <div class='col-md-2'>
                    <label for='periode_stok_'>Periode Stok (hari)</label>
                    <input onclick='select()' id='periode_stok_' class='form-control' >
                </div>
                <div class='col-md-2'>
                ...
                <div class='col-md-2'>
                    <span id='btnRefreshKalkulator' onclick=\"hitung_stok_beli();$(this).removeClass('btn-success').children().addClass('fa-spin')\" class='btn btn-success pull-right'><i class='fa fa-refresh'></i></span>
                </div>
            </div>
        ";
        $strStokOnCalc .= "<div id='div_table_AI' class='stripe row-border order-column'></div>";

        $strStokOnCalc .= "
            <script>
                function hitung_stok_beli(){
                    var index_ = $('#index_');
                    var periode_omset_ = $('#periode_omset_');
                    var periode_stok_ = $('#periode_stok_');
                    var periode_omset_value = $(periode_omset_).val() != '' ? $(periode_omset_).val() : '';
                    var periode_stok_value = $(periode_stok_).val() != '' ? $(periode_stok_).val() : '';
                    var index_value = $(index_).val() != '' ? $(index_).val() : '';
                    var seg_1 = periode_omset_value != '' ? '/' + periode_omset_value : ''
                    var seg_2 = periode_stok_value != '' ? '/' + periode_stok_value : ''
                    var seg_3 = index_value != '' ? '/' + index_value : ''
                    $.ajax({
                        type: 'GET',
                        url: $('#baseURL').html() + 'eusvc/Dashboard/stok_control/' + $('#toko_id').html() + seg_1 + seg_2 + seg_3,
                        success: function(data) {
                            if(data.status==1){
                                var arrDatas = data.datas
                                var arrParams = data.params
                                if($(index_)){
                                    $(index_).val( arrParams.index )
                                }
                                if($(periode_omset_)){
                                    $(periode_omset_).val( arrParams.periode_omset )
                                }
                                if($(periode_stok_)){
                                    $(periode_stok_).val( arrParams.periode_stok )
                                }
                                var table = ''
                                    table += \"<table style='margin-top: 20px;' id='table_AI' class='table table-hover table-striped compact table-responsive'>\"
                                if($('#table_AI')){
                                        var lebar_device = $(window).width();
                                        var tinggi_device = $(window).height();
                                        var now = new Date;
                                        ...
                                        jQuery.each(arrDatas, function(i, dt){
                                            num++;
                                            var stok_out = dt.stok_out*1 > 0 ? addCommas(dt.stok_out*1) + ' ' + dt.stok_satuan : 0
                                            var stok_out_avg = dt.stok_out_avg*1 > 0 ? addCommas(dt.stok_out_avg*1) + ' ' + dt.stok_satuan : 0
                                            var stok_buffer = dt.stok_buffer*1 > 0 ? addCommas(dt.stok_buffer*1) + ' ' + dt.stok_satuan : 0
                                            var stok_real = dt.stok_real*1 > 0 ? addCommas(dt.stok_real*1) + ' ' + dt.stok_satuan : 0
                                            var stok_ideal = dt.stok_ideal*1 > 0 ? addCommas(dt.stok_ideal*1) + ' ' + dt.stok_satuan : 0
                                            var stok_order = dt.stok_order*1 > 0 ? addCommas(dt.stok_order*1) + ' ' + dt.stok_satuan : 0
                                            ...
                                            table += \"<td data-order='\"+dt.stok_ideal+\"'>\"+stok_ideal+\"</td>\"
                                            table += \"<td data-order='\"+(dt.stok_order*1<0?0:dt.stok_order*1)+\"'>\"+stok_order+\"</td>\"
                                            table += '<td><input data-id=\"'+dt.id+'\" data-order=\"0\" data-stok-order=\"'+dt.stok_order+'\" name=\"stok_order[]\" class=\"custom-control-input\" type=\"checkbox\" id=\"stok_order_'+dt.id+'\"></td>'
                                            table += '</tr>'
                                        })
                                        table += '</tbody>'
                                        table += '</table>'
                                        table += \"<div style='margin-bottom: 10px;' class='row'>\"
                                            ...
                                            table += \"<div class='col-md-4'>\"
                                            table += \"</div>\"
                                            table += \"<div id='btnActionTable' class='col-md-4 text-right hidden'>\"
                                                table += \"<span onclick='sendToCart()' class='btn btn-sm btn-success'>Lanjut Pembelian Bahan</span>\"
                                                table += \"<span onclick='clearSesBahan()' class='btn btn-sm btn-default'><i class=fa fa-trash></i> Clear Check Bahan</span>\"
                                            table += \"</div>\"
                                        table += \"</div>\"
                                    $('#div_table_AI').html(table);
                                    var tablesAI = $('#table_AI').DataTable({
                                                        dom: 'Blftrip',
...
//                                                                        );
                                    ...
                                    $('#table_AI').on( 'page.dt', delay_v2(function () {
                                        updateSesCart(),setClickAction()
                                    }, 500) );
                                    $('#table_AI').on( 'order.dt', delay_v2(function () {
                                        updateSesCart(),setClickAction()
                                    }, 500) );
                                    $('#last_update_ui').attr('time-data', dateNow)
                                    $('#lebar_device').html('width: '+ lebar_device + ' height: '+tinggi_device+' ||  ');
                ...
                hitung_stok_beli()
                $('input#index_,input#periode_omset_,input#periode_stok_').on('keyup', delay_v2( function(){ hitung_stok_beli() }, 500))
                function timeLast(){
                    var strTimeLast = $('#last_update_ui').attr('time-data')
                    var daysAgo = moment( strTimeLast + '+07:00', 'YYYY-MM-DD HH:mm:ssZ').fromNow();
                    $('#last_update_ui').html('<b>last update: </b>' + daysAgo);
                }
                setInterval(function(){
                    timeLast();
                }, 5000)
                timeLast();
                function updateSesCart(source=0){
                    var sesCart = localStorage.sesCartBahan!=undefined?JSON.parse(localStorage.sesCartBahan):0;
                    var arrStokOrder = $(\"input[name='stok_order[]']\")
                        if(sesCart==0){
                            sesCart = {}
                        }
                    jQuery.each(arrStokOrder, function(i, dk){
                            var id_bahan = $(dk).attr('data-id')
                            var stok_order = $(dk).attr('data-stok-order')

                        if( $(dk).is(':checked') ){
                            if(source==1){
                                if(sesCart[id_bahan]==undefined){
                                    sesCart[id_bahan]={}
                                }
                                sesCart[id_bahan] = (stok_order*1<0 ? 0 : stok_order*1)
                            }
                        }
                        else{
                            if(sesCart[id_bahan]!=undefined && source==1){
                                delete sesCart[id_bahan]
                            }
                        }
                        if(source==0){
                            if(sesCart[id_bahan]!=undefined){
                                $(dk).prop('checked', true)
                                $(dk).parent().attr('data-order', 1)
                            }
                            else{
                    ...
                    localStorage.sesCartBahan = JSON.stringify(sesCart)
                    if(countObj(sesCart)>0){
                        $('#btnActionTable').removeClass('hidden')
                    }
                    else{
                        $('#btnActionTable').addClass('hidden')
                    }
                }
                function setClickAction(){
                    $(\"input[name='stok_order[]']\").on('click', delay_v2(function(){
                        updateSesCart(1)
                    }, 500))
                }
                function sendToCart(){
                ...
                function clearSesBahan(){
                     localStorage.removeItem('sesCartBahan')
                     var arrStokOrder = $(\"input[name='stok_order[]']\")
                     jQuery.each(arrStokOrder, function(i, dk){
                         $(dk).prop('checked', false)
                     })
                ...
                function toggleCheckAll(e){
                    var arrStokOrder = $(\"input[name='stok_order[]']\")
                    var checkOr = $(e).is(':checked')
                    jQuery.each(arrStokOrder, function(i, dk){
                         $(dk).prop('checked', checkOr)
                    })
                    updateSesCart(1)
                }
        ...
        ";
        //endregion produk request from AI stok
...
//            pageResize: true,
        ...
        $p->addTags(
            array(
                ...
                //  -----------------------
                "ongoing_AI_display"      => $jenisTr == 461 ? "block" : "none",
                "ongoing_AI_title"        => "PEMBELIAN BAHAN",
                "ongoing_AI_content"      => $str_AI_ongoing,
                "ongoing_AI_footer"       => "",
                //  -----------------------
                "prog_AI_display"         => $jenisTr == 461 ? "block" : "none",
                "onprogress_AI_title"     => "PERENCANAAN DAN PENGAWASAN PENGGUNAAN BAHAN",
                "onprogress_AI_content"   => $strStokOnCalc,
                "onprogress_AI_footer"    => "",
                //  -----------------------
    ...
    case "createForm":
        ...
        if (isset($allSteps)) {
            // cekBiru($allSteps);
            $jml_steps = sizeof($allSteps);
            $strAllStep .= "<div class='row'>";
            $strAllStep .= "<div class='col-md-12 pull-right text-center'>";
            $strAllStep .= createStateHorizontal('0', $jml_steps, $jenisTransaksi);
            $strAllStep .= "</div>";
            $strAllStep .= "</div>";
            // cekHere($jenisTransaksi);
        }
        // cekHijau($strAllStep);
        if (strlen($errMsg) > 0) {
            $error = "<div class='alert alert-danger-dot text-center'><span>$errMsg</span></div>";
        ...
        $submitLabel = "Lanjutkan " . $subTitle;
        //endregion baca atribut, keterangan dari config
        // cekKuning($template . " " . __FILE__);
...
$('#uplodXls').on('submit',function() {
    ...
    var setInt= setInterval(function() {
      var arrProduk = JSON.parse(localStorage.getItem('items'));
        ...
        //region histories
        if (sizeof($arrayHistory) > 0) {
            $strHist .= "<table class='table table-condensed table-bordered table-hover no-padding no-margin'>";

            $strHist .= "<tr bgcolor='#f0f0f0' class='text-uppercase'>";
            if (sizeof($arrayHistoryLabels) > 0) {
                foreach ($arrayHistoryLabels as $key => $label) {
                    $strHist .= "<td class='text-muted'>";
                    if (is_array($label)) {
        ...
        $p->addTags(
            array(
                ...
                "submit_button_target"           => $submit_button_target,
    ...
    case "editForm":
        ...
        $p->addTags(
            array(
                ...
                "selector_label2"      => isset($selectorLabel2) ? $selectorLabel2 : "",
                "submit_button"        => $submitLabel,
                "submit_button_target" => "opname/Transaksi/validate/",
                // dibawa ke controle rlo
                //                "clear_shopping_cart" => $setClearShoppingCart,
                //                "action_shopping_cart" => $setActionShoppingCart,
                "onprogress_content"   => $strOnprog,
                "onprogress_footer"    => $strOnprogFooter,
    ...
    case "viewUndoneItems":
        ...
        if (sizeof($arrayOnProgress) > 0 || sizeof($arrayOnProgress2) > 0) {
            $str .= "<div class='box box-solid box-danger'>";

            $str .= "<div class='box-header with-border text-uppercase'>";
            $str .= "<h4 class='box-title'><span class=\"glyphicon glyphicon-flash blink\"></span> on-going transactions</h4>";
            $str .= "</div>";
            ...
            if (sizeof($arrayOnProgress) > 0) {
                ...
                * ----------------------------------------*/
                ...
                if (isset($sumFooter) && sizeof($sumFooter) > 0) {
                    $str .= "<tr line=" . __LINE__ . " class='bg-grey-1'>";
                    if (sizeof($arrayProgressLabels) > 0) {
                        //                        $str .= "<td>-</td>";
                        foreach ($arrayProgressLabels as $key => $label) {
                            $str .= "<td>";
    ...
    case "preview":
        ...
        if (sizeof($stepLabels) > 0) {
            ...
            echo createStateMap($currentStep, sizeof($stepLabels), $stepLabels, $jenisTr, $configUi);
            echo "</div class=''>";
        }
        ...
        if (isset($items) && sizeof($items) > 0) {
                                ...
                                if (sizeof($elContents) > 0) {
                                    ...
                                    foreach ($elContents as $label => $val) {
                                        ...
                                        if ($val != "") {
                                            ...
                                            $val_f = is_numeric($val) ? formatField($strLabel, $val) : $val;
                                            echo "<td align='left'>$val_f</td>";
                                            echo "</tr>";
                                        }
            ...
            <span class='glyphicon glyphicon-ok'></span> $buttonLabel</button>";
    ...
    case "followupPreview":
        ...
        // arrPrintPink($stepLabels);
        if (sizeof($stepLabels) > 0) {
            echo "<div class='text-center alert alert-info-dot text-grey text-center' style='overflow: hidden'>";
            // echo createStateMap($currentStep, sizeof($stepLabels), $stepLabels, $jenisTr);
            echo createStateHorizontalMap($currentStep, sizeof($stepLabels), $stepLabels, $jenisTr, $configUi);
            echo "</div>";
        }

        /* ------------------------------------------------------------
         * penampil para pihak terkait
         * ------------------------------------------------------------*/
        echo "<ul class='list-group' style='margin-bottom: 0;'>";
        foreach ($mainLabels as $key => $label) {
            echo "<li class='list-group-item borderless' style='padding: 0 10px;'>";
            echo "<div class='row'>";
            echo "<div class='col-md-2 text-muted text-uppercase'>";
            echo $label;
            echo "</div>";
            echo "<div class='col-md-6'>";
            echo ":&nbsp;";
        ...
        if ((isset($items) && sizeof($items) > 0) || (isset($items2) && sizeof($items2) > 0)) {
            ...
            $colspans = 1;
            // if(sizeof($items2_detail) > 0){
            //     $colspans = 2;
            // }

            echo "<table class='table table-bordered table-condensed table-striped table-hover compact nowarp'>";
            $no = 0;
            if (isset($items) && sizeof($items) > 0) {

                echo "<thead>";
                echo "<tr line='" . __LINE__ . "' bgcolor='#f0f0f0' class='text-uppercase'>";
                echo "<th class='text-muted text-center' style='font-weight:bold;' align='right' width='5'>no.</th>";
                $xx1 = 0;
                foreach ($itemLabels as $key => $label) {
                    $xx1++;
                    // if($xx1 == 1){
                    //     $strColspan = "colspan='$colspans'";
                    // }
                    // else{
                    //     $strColspan = "";
                    // }

                    echo "<th class='text-muted text-center' style='font-weight:bold;'>";
                    echo $label;
                    echo "</th>";
                }
                echo "</tr>";
                ...
                foreach ($items as $id => $iSpec) {
                    ...
                    $no++;
                    $rowspan = 1;
                    $trBg = $no % 2 == 0 ? "background-color:#FFE4E1;" : "";
                    $trBg = $no % 2 == 0 ? "" : "";
                    $trBgCls = $no % 2 == 0 ? "active" : "";
                    if (isset($items2_detail[$id]) && (sizeof($items2_detail[$id]) > 0)) {
                        $rowspan = $rowspan + sizeof($items2_detail[$id]);
                    }
                    ...
                    if ($allowRemove == false) {
                        ...
                        // echo "<td>";
                        // echo "<a href='javascript:void(0);' onclick=\"document.getElementById('result').src='$removeItemTarget?id=$id&ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL';\">
                        //         <span class='glyphicon glyphicon-remove text-danger'>**</span></a>";
                        // echo "</td>";

                        $rowRemove = "<a href='javascript:void(0);' onclick=\"document.getElementById('result').src='$removeItemTarget?id=$id&ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL';\"><span class='glyphicon glyphicon-remove text-danger'></span></a>";
                    }

                    echo "<tr line=" . __LINE__ . ">";

                    echo "<td align='right' style='vertical-align: middle; $addStyle' rowspan='$rowspan'>";
                    echo $no;
                    echo "</td>";
                    ...
                    foreach ($itemLabels as $key => $label) {
                        ...
                        if ($allowEdit == true && in_array($key, $editableFields)) {
                            if (is_numeric($val)) {
                                ...
                                $addEvent = "";
                                if (!$allowIncrement) {
                                    $addEvent = " oninput=\"if(parseInt(removeCommas(this.value))<1 || parseInt(removeCommas(this.value))>$maxVal){this.value='" . number_format($maxVal) . "';}\" onkeyup=\"this.value=addCommas(this.value);\" onblur=\"top.$('#result').load('$updateItemFieldTarget?id=$id&key=$key&val='+removeCommas(this.value)+'&spc=1')\" ";
                                }
                                else {
                                    $addEvent = " onkeyup=\"this.value=addCommas(this.value);\"  onblur=\"top.$('#result').load('$updateItemFieldTarget?ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL&id=$id&key=$key&val='+removeCommas(this.value)+'&spc=1')\" ";
                                }
                            }
                            else {
                                $inputType = isset($shoppingCartInputType[$key]) ? $shoppingCartInputType[$key] : "text";
                                $addEvent = "";
                            }
                            // cekLime($val);
                            $strVal = "<input type=$inputType name='$key" . "_" . "$id' class='form-control text-center text-bold text-green' value='" . ($val > 900 ? number_format($val) : $val) . "' onclick='this.select()' $addEvent>";
                            $tdOpt = "style='vertical-align: middle; margin:0px;padding:0px;$addStyle' ";
                        }
                        else {
                            $strVal = formatField($key, $val);
                            $tdOpt = "style='vertical-align: middle; $addStyle'";
                        }
                    ...
                    if ($allowEdit == true) {//==delete item
                        if ($allowSubTambah == false) {
                        }
                        else {
                            //                            echo "<td>";
                            //                            echo "<a href='javascript:void(0);' onclick=\"document.getElementById('result').src='$tambahSubItemTarget?id=$id&ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL';\">
                            //                                <span class='glyphicon glyphicon-plus text-success'></span></a>";
                            //                            echo "</td>";
                        }
                        // if ($allowRemove == false) {
                        // }
                    ...
                    if (isset($items2_detail[$id]) && (sizeof($items2_detail[$id]) > 0)) {
                        $rowspan = $rowspan + sizeof($items2_detail[$id]);
                        foreach ($items2_detail[$id] as $iid => $iiSpec) {

                            $removeItem2 = "";
                            if ($allowRemove == true) {
                                $removeItem2 = "<a href='javascript:void(0)' onclick=\"document.getElementById('result').src='$removeSubItemTarget?id=$id&iid=$iid&ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL';\"><span class='glyphicon glyphicon-remove text-danger'></span></a>";
                            }
                            echo "<tr style='$trBg'>";
                            $xx2 = 0;
                            foreach ($itemsSubLabels as $key => $label) {
                                $xx2++;
                                $iiVal = isset($iiSpec[$key]) ? $iiSpec[$key] : "";
                                if ($allowEdit == true && in_array($key, $editableSubFields)) {
                                    if (isset($pair_ext[$id][$key])) {
                                        $iiVal = "<select class='form-control' onchange=\"document.getElementById('result').src='$itemsEditableLockerDateConfig?id=$id&iid=$iid&key=$key&val='+this.value+'&spc=1'\">";
                                        $iiVal .= "<option> -- please select -- </option>";
                                        foreach ($pair_ext[$id][$key] as $tmpPair) {
                                            $rowID = $tmpPair['id'];
                                            $batch_no_data = $tmpPair['batch_number'] != NULL ? "Batch: " . $tmpPair['batch_number'] . "&nbsp;&nbsp;&nbsp;" : "&nbsp;";
                                            $expired_date_data = $tmpPair['expired_date'] != NULL ? "ED: " . $tmpPair['expired_date'] . "&nbsp;&nbsp;&nbsp;" : "&nbsp;";
                                            $jumlah_data = $tmpPair['jumlah'] != NULL ? "Stok: " . $tmpPair['jumlah'] : "&nbsp;";
                                            $selected = (isset($selectedSubItems[$id][$iid]) && ($selectedSubItems[$id][$iid] == $rowID)) ? "selected" : "";
                                            $batch_no = "<button style='font-size:1.5em;color:#0056cd;border:0px solid red;' class='btn btn-default text-left'>" . $batch_no_data . "</button>";
                                            $expired_date = "<button style='font-size:1.5em;color:#0056cd;border:0px solid blue;' class='btn btn-default text-center'>" . $expired_date_data . "</button>";
                                            $jumlah = "<button style='font-size:1.5em;color:#ff7700;border:0px solid black;' class='btn btn-default text-right'>" . $jumlah_data . "</button>";
                                            $iiVal .= "<option class='form-control' $selected value='$rowID' style='font-size:15px;height:35px;'>";
                                            $iiVal .= "$batch_no";
                                            $iiVal .= "$expired_date";
                                            $iiVal .= "$jumlah";
                                            $iiVal .= "</option>";
                                        ...
                                        if (is_numeric($iiVal)) {
                                            $val += 0;
                                            $maxVal = isset($iSpec["max_" . $key]) ? $iSpec["max_" . $key] : $iiSpec[$key];
                                            $inputType = "number";
                                            $addEvent = "";
                                            if (!$allowSubIncrement) {
                                                $addEvent = " oninput=\"if(parseInt(this.value)<1 || parseInt(this.value)>$maxVal){this.value='$maxVal';}\" onblur=\"if($iiVal!=this.value){ document.getElementById('result').src='$itemsEditableLockerDateConfig?iid=$iid&id=$id&key=$key&val='+this.value+'&spc=1' }else{ return false; }\" ";
                                            }
                                            else {
                                                $addEvent = " onblur=\"if('$iiVal'!=this.value){ document.getElementById('result').src='$itemsEditableLockerDateConfig?ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL&iid=$iid&id=$id&key=$key&val='+this.value+'&spcxxxxxx=1' }else{ return false; }\" ";
                                            }
                                        }
                                        else {
                                            $inputType = isset($shoppingCartInputType[$key]) ? $shoppingCartInputType[$key] : "text";
                                            $addEvent = " onblur=\" if('$iiVal'!=this.value){ document.getElementById('result').src='$itemsEditableLockerDateConfig?ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL&iid=$iid&id=$id&key=$key&val='+this.value+'&spc=1' }else{ return false; }\" ";
                                        }
                                        $iiVal = "<input type=$inputType keyname='$key" . "_" . "$id' keyid='$key" . "_" . $id . "_" . "$iid' name='$key" . "_" . "$iid' class='form-control text-right' value='$iiVal' onclick='this.select()' $addEvent> ";
                                    }
                                }
                                else {
                                    $iiVal = formatField($key, $iiVal);
                                }
                                ...
                                //                                $js_adds .= "\nvar arrQty_$key"."_".$id." = $('input[keyname=$key"."_".$id."]'); ";
                            }
                            // if ($allowEdit == true) {//==delete item
                            //     if ($allowRemove == false) {
                            //     }
                            //     else {
                            //         echo "<td>";
                            //         echo "<a href='javascript:void(0)' onclick=\"document.getElementById('result').src='$removeSubItemTarget?id=$id&iid=$iid&ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL';\"><span class='glyphicon glyphicon-remove text-danger'></span></a>";
                            //         echo "</td>";
                            //         // $removeItem2 = "<a href='javascript:void(0)' onclick=\"document.getElementById('result').src='$removeSubItemTarget?id=$id&iid=$iid&ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL';\"><span class='glyphicon glyphicon-remove text-danger'></span></a>";
                            //     }
                            // }
                            echo "</tr>";
                            //                            $js_adds .= "\nvar arrJml_$key"."_".$id." = $('input[keyname=$key"."_".$id."]'); ";
                        }

                        $js_adds .= "\n  ";
                        $js_adds .= "\nfunction itemsHandler_$id(){
                                        var batch_no_$id = $('input[keyname=batch_no_$id]');
                                        var produksi_date_$id = $('input[keyname=produksi_date_$id]');
                                        var expired_$id = $('input[keyname=expired_$id]');
                                        var qty_$id = $('input[keyname=qty_$id]');
                                        jQuery.each(qty_$id, function(i,thiiiss){

                                            var noUrutSubItems = $(thiiiss).attr('name').replace('qty_', '');

                                            //validasi batch no
                                            if( $('input[keyid=batch_no_" . $id . "_'+noUrutSubItems+']').val()=='' ){
                                                $('input[keyid=batch_no_" . $id . "_'+noUrutSubItems+']').parent().addClass('bg-red-gradient');
                                                $('input[keyid=batch_no_" . $id . "_'+noUrutSubItems+']').on('keyup', function(){
                                                    $(this).parent().removeClass('bg-red-gradient');
                                                })
                                                error_total +=1;
                                            }
                                            else{ $('input[keyid=batch_no_" . $id . "_'+noUrutSubItems+']').parent().removeClass('bg-red-gradient'); }
                                            //validasi produksi date
                                            if( $('input[keyid=produksi_date_" . $id . "_'+noUrutSubItems+']').val()=='' ){
                                                $('input[keyid=produksi_date_" . $id . "_'+noUrutSubItems+']').parent().addClass('bg-red-gradient');
                                                $('input[keyid=produksi_date_" . $id . "_'+noUrutSubItems+']').on('input', function(){
                                                    $(this).parent().removeClass('bg-red-gradient');
                                                })
                                                error_total +=1;
                                            }
                                            else{ $('input[keyid=produksi_date_" . $id . "_'+noUrutSubItems+']').parent().removeClass('bg-red-gradient'); }
                                            //validasi expired
                                            if( $('input[keyid=expired_" . $id . "_'+noUrutSubItems+']').val()=='' ){
                                                $('input[keyid=expired_" . $id . "_'+noUrutSubItems+']').parent().addClass('bg-red-gradient');
                                                $('input[keyid=expired_" . $id . "_'+noUrutSubItems+']').on('input', function(){
                                                    $(this).parent().removeClass('bg-red-gradient');
                                                })
                                                error_total +=1;
                                            }
                                            else{ $('input[keyid=expired_" . $id . "_'+noUrutSubItems+']').parent().removeClass('bg-red-gradient'); }
                                            //validasi qty
                                            if( $(thiiiss).val()==0 ){
                                                $(thiiiss).parent().addClass('bg-red-gradient');
                                                $(thiiiss).on('keyup', function(){
                                                    $(this).parent().removeClass('bg-red-gradient');
                                                })

                                                error_total +=1;
                                            }
                                            else{ $(thiiiss).parent().removeClass('bg-red'); }
                                        })
                                   }\n setTimeout( function(){ itemsHandler_$id(); }, 500); ";
                    }

                    $js_adds .= "\n
                                setTimeout( function(){
                                    if(parseFloat(error_total)>0){
                                        $('.btn.btn-success').attr('onclick', 'javascript:void(0)').prop('disabled', true);
                                    }
                                    console.log('error_total: '  + error_total);
                                }, 1000)
                ...
                foreach ($sumRows as $key => $label) {
                    ...
                    if (isset($items) && sizeof($items2) > 0) {
                        //                            cekKuning("ATAS");
                        $colspanSum = sizeof($itemLabels) > 1 ? $itemLabels : $itemLabels;
                    }
                    else {
                        //                            cekBiru("BAWAH");
                    ...
                    echo "<tr line=" . __LINE__ . ">";
                    echo "<td colspan='" . sizeof($colspanSum) . "' class='text-right bottom-borderless valign-m'>$label</td>";
                    echo "<td class='text-right valign-m text-bold'>";
                ...
                //         echo "<tr line=" . __LINE__ . ">";
                //         echo "<td colspan='" . sizeof($colspanSum) . "' class='text-right bottom-borderless valign-m'>$label</td>";
                //         echo "<td class='text-right valign-m'>";
                //
                //         if (isset($mainValues[$key])) {
                //
                //             echo formatField($key, $mainValues[$key]);
            ...
            if (isset($items) && sizeof($items) > 0) {
                ...
                if (sizeof($mainElements) > 0) {
                    ...
                    echo "<h4 line='" . __LINE__ . "' class='text-uppercase no-margin'>$title details</h4>";
                    echo "<div class='row'>";
                    foreach ($mainElements as $elName => $aSpec) {
                        echo "<div class='col-md-6'>";
                        echo "<div class='panel panel-default' style='background:#f0f0f0;'>";
                        // echo "<table classs='ttable table-bordered table-condensed no-margin' border='1'>";
                        // foreach ($mainElements as $elName => $aSpec) {
                        //                        cekBiru("$elName");
                        if (array_key_exists($elName, $elementConfig)) {
                            ...
                            // echo "<tr line=" . __LINE__ . ">";
                            // echo "<td align='left'>";
                            echo "<div class='panel-heading text-capitalize'>";
                            echo "<span class='text-red'>" . $aSpec['label'] . " &nbsp;&nbsp;&nbsp;</span>";
                            if (in_array($elName, $editableElements)) {
                                ...
                                echo "<a href='javascript:void(0)' class='pull-right text-muted' onclick=\"$editLink\">";
                            ...
                            switch ($elementConfig[$elName]['elementType']) {
                                case "dataModel":
                                    ...
                                    if (sizeof($elContents) > 0) {
                                        echo "<table class='table table-condensed table-borderedd no-margin'>";
                                        foreach ($elContents as $label => $val) {
                                            if ($val != "") {
                                                echo "<tr line=" . __LINE__ . ">";
                                                ...
                                                echo "<td colspan='2' align='left' class='text-black'>$titik2 $val</td>";
                                                echo "</tr>";
                                            }
                                        ...
                                        echo "<table class='tables table-condensed no-margin'>";
                                        echo "<tr line=" . __LINE__ . ">";
                                        echo "<td align='left' class='text-danger'>$msg</td>";
                                        echo "</tr>";
                                        echo "</table>";
                                    }
                                    break;
                ...
                if (isset($description)) { // mendeteksi gerbang catatan (main), bila ada maka ditampilkan. berlaku semua transaksi.
                    ...
                    if (isset($noteEditabled) && ($noteEditabled == true)) {
                        ...
                        $addEvent_description = " onblur=\"document.getElementById('result').src='$updateMainFieldTarget?ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL&key=$key_note&val='+encodeURIComponent(this.value) + '&spc=1';\"";
            ...
            echo "<div onclick=\"bukaFollowupPreview()\" class='kalkulasi_modal btn btn-sm btn-warning btn-block btn-flat text-bold hidden'><span class='kalkulasi_modal_timer'></span> KLIK DISINI UNTUK UPDATE HASIL AKHIR <span class=''><i class='fa fa-question-circle'></i></span><BR><i class='fa fa-refresh fa-spin'></i>  <i>(popup akan direfresh)</i></div>";
            echo "<div class='clearfix'>&nbsp;</div>";
            echo "<script>$js_adds</script>";
            echo "
            <script>
                    function bukaFollowupPreview(){
                        top.BootstrapDialog.closeAll();
                        top.BootstrapDialog.show(
                            {
                                title:'Followup preview',
                                message: " . 'top.$' . "('<div></div>').load('$followupPreviewTarget?rawBuilderURL=$rawBuilderURL'),
                                size:top.BootstrapDialog.SIZE_WIDE,
                                draggable:false,
                                animate:false,
                                closable:false,
            ...
            if (isset($items) && sizeof($items) > 0) {
                ...
                if (isset($deleteSpec['targetUrl']) != "" && $deleteSpec['targetUrl'] != "") {
                    ...
                    $btnStyle = "class='btn btn-danger' style='border:1px #ff7700 solid;ccolor:#ff7700;'";
                    $btnWarning = $deleteSpec['warning'];
                    $actionTarget = $deleteSpec['targetUrl'];
                    $btnLabel = "<span class='fa fa-undo'></span> " .$deleteSpec['label'];
                    // ----------------------------------
                    echo "<button type='button' $btnStyle onclick=\"confirm_alert_result('Peringatan','$btnWarning','$actionTarget');this.disabled=true;\"> $btnLabel</button>";
                }
                else {
                ...
                if (isset($undoSpec['targetUrl']) != "" && $undoSpec['targetUrl'] != "") {
                    ...
                    $btnStyle = "class='btn btn-default' style='border:1px #ff7700 solid;color:#ff7700;'";
                    $btnWarning = $undoSpec['warning'];
                    $actionTarget = $undoSpec['targetUrl'];
                    $btnLabel = "<span class='fa fa-undo'></span> " .$undoSpec['label'];
                    // ----------------------------------
                    echo "<button type='button' $btnStyle onclick=\"confirm_alert_result('Peringatan','$btnWarning','$actionTarget');this.disabled=true;\"> $btnLabel</button>";
                }
                else {
                ...
                if (isset($editSpec['targetUrl']) != "" && $editSpec['targetUrl'] != "") {
                    ...
                    $btnStyle = "class='btn btn-default' style='border:1px #ff7700 solid;color:#ff7700;'";
                    $btnWarning = $editSpec['warning'];
                    $actionTarget = $editSpec['targetUrl'];
                    $btnLabel = "<span class='fa fa-pencil'></span> " .$editSpec['label'];
                    // ----------------------------------
                    echo "<button type='button' $btnStyle onclick=\"confirm_alert_result('Peringatan','$btnWarning','$actionTarget');this.disabled=true;\"> $btnLabel</button>";
                }
                else {
                ...
                if ((isset($extBtns) && sizeof($extBtns) > 0) || (isset($payBtns) && sizeof($payBtns) > 0)) {
                    ...
                    if (isset($rejectionSpec['targetUrl']) != "" && $rejectionSpec['targetUrl'] != "") {
                        ...
                        $btnStyle = "class='btn btn-danger' style='border:1px #dd3300 solid;ccolor:#dd3300;'";
                        $btnWarning = $rejectionSpec['warning'];
                        $actionTarget = $rejectionSpec['targetUrl'];
                        $btnLabel = "<span class='glyphicon glyphicon-ban-circle'></span> " .$rejectionSpec['label'];
                        // ----------------------------------
                        echo "<button type='button' $btnStyle onclick=\"confirm_alert_result('Peringatan','$btnWarning','$actionTarget');this.disabled=true;\"> $btnLabel</button>&nbsp;&nbsp;&nbsp;";
                    }
                    else {
                    ...
                    if (isset($rejectionSpecAll['targetUrl']) != "" && $rejectionSpecAll['targetUrl'] != "") {
                        ...
                        $btnStyle = "class='btn btn-danger' style='border:1px #000000 solid;color:#ffffff;background-color:#000000;'";
                        $btnWarning = $rejectionSpecAll['warning'];
                        $actionTarget = $rejectionSpecAll['targetUrl'];
                        $btnLabel = "<span class='glyphicon glyphicon-alert'></span> " .$rejectionSpecAll['label'];
                        // ----------------------------------
                        echo "<button type='button' $btnStyle onclick=\"confirm_alert_result('Peringatan','$btnWarning','$actionTarget');this.disabled=true;\"> $btnLabel</button>&nbsp;&nbsp;&nbsp;";
                    }
                    else {
                    ...
                    if (isset($rejectionSpecAll['targetUrl']) != "" && $rejectionSpecAll['targetUrl'] != "") {
                        ...
                        $btnWarning = $rejectionSpecAll['warning'];
                        $actionTarget = $rejectionSpecAll['targetUrl'];
                        $btnLabel = $rejectionSpecAll['label'];
                        echo "<button type='button' class='btn btn-danger' style='border:1px #000000 solid;color:#ffffff;background-color:#000000;' onclick=\"confirm_alert_result('Peringatan','$btnWarning','$actionTarget');this.disabled=true;\"><span class='glyphicon glyphicon-alert'></span> $btnLabel</button>&nbsp;&nbsp;&nbsp;";
                    }
                    else {
                        echo "<button type='button' disabled class='btn btn-danger' style='border:1px #000000 solid;color:#dcdcdc;background-color:#000000;'><span class='glyphicon glyphicon-alert'></span>&nbsp;&nbsp; " . $rejectionSpecAll['label'] . "</button>&nbsp;&nbsp;&nbsp;";
                    }
                    // -------------------------------------------------------------
                    // cekKuning($approvalSpec, __FILE__ . " @" . __LINE__);
                    if (isset($approvalSpec['targetUrl']) != "" && $approvalSpec['targetUrl'] != "") {
                        ...
                        $btnStyle = "class='btn btn-success' style='border:1px #008800 solid;color:#ffffff;'";
                        $btnWarning = $approvalSpec['warning'];
                        $actionTarget = $approvalSpec['targetUrl'];
                        $btnLabel = "<span class='glyphicon glyphicon-ok'></span> " .$approvalSpec['label'];
                        // ----------------------------------
                        echo "<button type='button' $btnStyle onclick=\"confirm_alert_result('Peringatan','$btnWarning','$actionTarget');this.disabled=true;\">$btnLabel</button>";

                        // echo "<button class='btn btn-primary' onclick=\"confirm_alert_result('Peringatan','Transaksi akan disimpan setelah mengeklik OK','$actionTarget');this.disabled=true;\"><span class='glyphicon glyphicon-ok'></span> ".$approvalSpec['label']."</button>";
                    }
                    else {
                        echo "&nbsp;";
                    }
...
src='$updateMainFieldTarget?ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL&key=$key_note&val='+this.value;\"";
    ...
    case "showDetails":
        ...
        if (sizeof($dataProduk) > 0) {
            ...
            foreach ($arrayTablesHeader as $kolom => $alias) {
                $header_result_f = array('data'  => $alias,
                                         'class' => 'text-center'
                );
    ...
    /* ------------------------------------------------------
     * print nota pindah ke viewer printing
     * ------------------------------------------------------*/
...
//                                            + \"<div class='text-right'>\"+addCommas(total5)+\"</div>\"
    ...
    case "viewResume":
        ...
        // data utama ada di sini (diatur dr controler Transaksi/viewResume)
        echo $signStr;
        ...
        if (sizeof($items) > 0) {
            foreach ($items as $cabangID => $subItems) {
                ...
                if (sizeof($subItems) > 0) {
                    ...
                    echo "<h4 class='text-blue text-uppercase'><span class='fa fa-book'></span> journal entries ($cabangNama) " . formatField("nomer", $title) . "$urutCounter</h4>";
                    ...
                    foreach ($subItems as $urut => $mSpec) {
                        ...
                        echo "<tr bgcolor='#f0f0f0' class='text-capitalize' line='" . __LINE__ . "'>";
                        ...
                        echo "<tr bgcolor='#f0f0f0' style='font-size: 15px;font-weight: bold;'>";
                        foreach ($headers as $key => $label) {
                            echo "<td>";
                            if (isset($total[$cabangID][$urut][$key])) {
        ...
        if (isset($detail_items) && sizeof($detail_items) > 0) {
            if (isset($itemLabels) && (sizeof($itemLabels) > 1)) {
                ...
                echo "<h4 class='text-blue text-uppercase'><span class='fa fa-book'>$detail_title</span></h4>";

                echo "<div class='table-responsive'>";
                echo "<table class='table table-bordered table-condensed no-margin' style='background:#ffffff;'>";

                //region header tabel bawah
                echo "<tr bgcolor='#f5f5f5' class='text-capitalize'>";
                echo "<th class='text-muted' style='font-weight:bold;' align='right' width='5'>no</th>";
                foreach ($itemLabels as $key => $label) {
                    echo "<th class='text-muted' style='font-weight:bold;'>";
                    echo $label;
                ...
                foreach ($detail_items as $iSpec) {
                    ...
                    echo "</tr>";
                    // arrPrintPink($detail2_items);
                    // arrPrint($detail3_items);
                    if (($noteEnabled == true) || ((isset($imageEnabled)) && ($imageEnabled == true))) {
                        if (isset($iSpec['note']) || (isset($imageEnabled))) {
                            echo "<tr line=" . __LINE__ . ">";
                            echo "<td>&nbsp;</td>";
                            echo "<td colspan='" . sizeof($itemLabels) . "' style='font-style:italic;font-family:Monaco, Menlo, Consolas, \'Courier New\', monospace;'>";
                            if (isset($iSpec['note']) && (strlen($iSpec['note']) > 1)) {
                ...
                if (isset($detail_sumRows) && sizeof($detail_sumRows) > 0) {
                    foreach ($detail_sumRows as $key => $label) {
                        ...
                        echo "<tr line=" . __LINE__ . " class='text-capitalize'>";
                        echo "<td colspan='" . $colspanX . "' class='text-right border-none'>$label</td>";
                        echo "<td class='text-right'>";
                ...
                if (isset($addRows) && sizeof($addRows) > 0) {
                    ...
                    foreach ($addRows as $key => $label) {
                        $listAddRows .= "<tr line='" . __LINE__ . "'>";
                        $valAddRows = isset($detail_main[$key]) ? $detail_main[$key] : "0";
                        $listAddRows .= "<td colspan='$colspanRows' align='right' class='border-none text-capitalize'>$label</td>";
        ...
        if (sizeof($main) > 0) {
            ...
            if (isset($main['accuracy']) && isset($main['lattitude']) && isset($main['longitude'])) {
                ...
                echo "<div class='panel margin-top-10'>";
        ...
        echo "<button name='close' type='button' class='btn pull-left' data-dismiss='modal'>Close</button>";
...
src='$updateMainFieldTarget?ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL&key=$key_note&val='+this.value;\"";
    ...
    case "index_multi":
        ...
        $strProjectPoolInline = "";
        $checkTr = isset($jenisTr) ? $jenisTr : (isset($this->jenisTr) ? $this->jenisTr : "1466");
        if (true) {
            $strProjectPoolInline = '
<div class="panel panel-info" style="margin-top: 15px; border-color: #3c8dbc;">
    <div class="panel-heading" style="background-color: #3c8dbc; color: #ffffff; padding: 10px 15px;">
        <h3 class="panel-title" style="font-weight: bold; font-size: 15px;">
            <i class="fa fa-boxes"></i> DAFTAR KEBUTUHAN PROJECT (Autopurchase Pool)
        </h3>
    </div>
    <div class="panel-body" style="padding: 15px;">
        <div id="project_pool_alert" class="alert alert-warning" style="display: none; margin-bottom: 12px;">
            <i class="fa fa-exclamation-triangle"></i> <span id="project_pool_alert_msg">Silakan pilih Supplier/Vendor pada form header terlebih dahulu.</span>
        </div>

        <div id="project_pool_controls" class="row" style="margin-bottom: 15px;">
            <div class="col-md-5 col-sm-6">
                <label style="font-size: 12px; margin-bottom: 4px;">Filter Proyek:</label>
                <select id="filter_project_id" class="form-control input-sm" onchange="loadProjectPoolInline();">
                    <option value="0">-- Semua Proyek Aktif --</option>
                </select>
            </div>
            <div class="col-md-5 col-sm-6">
                <label style="font-size: 12px; margin-bottom: 4px;">Cari Barang / Kode / Proyek:</label>
                <div class="input-group">
                    <input type="text" id="filter_project_search" class="form-control input-sm" placeholder="Ketik kata kunci..." onkeyup="if(event.keyCode==13){ loadProjectPoolInline(); }">
                    <span class="input-group-btn">
                        <button class="btn btn-default btn-sm" type="button" onclick="loadProjectPoolInline();"><i class="fa fa-search"></i> Cari</button>
                    </span>
                </div>
            </div>
            <div class="col-md-2 col-sm-12 text-right" style="padding-top: 23px;">
                <button class="btn btn-sm btn-default" type="button" onclick="loadProjectPoolInline();" title="Refresh Data"><i class="fa fa-refresh"></i> Refresh</button>
            </div>
        </div>
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-striped table-condensed" style="margin-bottom: 0;">
                <thead>
                    <tr class="bg-gray-active" style="background-color: #e0e0e0;">
                        <th style="width: 5%; text-align: center;">ID Pool</th>
                        <th style="width: 25%;">Proyek & No. Kontrak</th>
                        <th style="width: 25%;">Kode & Nama Produk</th>
                        <th style="width: 10%; text-align: right;">Qty RAB</th>
                        <th style="width: 10%; text-align: right;">Qty PO</th>
                        <th style="width: 10%; text-align: right;">Qty Sisa</th>
                        <th style="width: 5%; text-align: center;">Status</th>
                        <th style="width: 10%; text-align: center;">Action</th>
                    </tr>
                </thead>
                <tbody id="project_pool_tbody">
                    <tr>
                        <td colspan="8" class="text-center text-muted"><i class="fa fa-spinner fa-spin"></i> Memuat data kebutuhan proyek...</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
<script>
function loadProjectPoolInline() {
    var supplierId = 0;
    if ($("select[name*=\'pihakID\']").length > 0) {
        supplierId = $("select[name*=\'pihakID\']").val();
    } else if ($("#pihakID").length > 0) {
        supplierId = $("#pihakID").val();
    } else if ($("select[name*=\'pihak_id\']").length > 0) {
        supplierId = $("select[name*=\'pihak_id\']").val();
    } else if ($("input[name*=\'pihakID\']").length > 0) {
        supplierId = $("input[name*=\'pihakID\']").val();
    } else if ($("#pihak_id").length > 0) {
        supplierId = $("#pihak_id").val();
    }

    var projectId = $("#filter_project_id").val() || 0;
    var search = $("#filter_project_search").val() || "";

    $("#project_pool_alert").hide();
    $("#project_pool_tbody").html("<tr><td colspan=\'8\' class=\'text-center text-muted\' style=\'padding:15px;\'><i class=\'fa fa-spinner fa-spin\'></i> Memuat data kebutuhan proyek...</td></tr>");

    var targetUrl = "' . base_url() . 'pembelianfgproject/_shoppingCart/getProjectPoolItemsAjax/1466?supplier_id=" + supplierId + "&project_id=" + projectId + "&search=" + encodeURIComponent(search);
        ...
        success: function(res) {
            if (res.status === "success") {
                ...
                if (!res.data || res.data.length === 0) {
                    html = "<tr><td colspan=\'8\' class=\'text-center text-muted\' style=\'padding:15px;\'>Tidak ada kebutuhan produk proyek yang ditemukan.</td></tr>";
                } else {
                    $.each(res.data, function(idx, item) {
                        if (item.project_id && item.project_nama) {
                            projectsMap[item.project_id] = item.project_nama;
                        }

                        var badgeClass = (item.status === "OPEN") ? "label-warning" : "label-info";
                        var supplierTag = (item.supplier_nama && item.supplier_nama !== "") ? "<br><small class=\'text-blue\'><i class=\'fa fa-truck\'></i> " + item.supplier_nama + "</small>" : "";
                        var cleanQtyRAB   = parseFloat(item.qty_kebutuhan);
                        var cleanQtyPO    = parseFloat(item.qty_po);
                        var cleanQtySisa  = parseFloat(item.qty_sisa);
                        var cleanStokDc   = parseFloat(item.stok_dc || 0);
                        var cleanStokSupp = parseFloat(item.stok_supplier || 0);
                        var safeSupplierNama = item.supplier_nama ? item.supplier_nama.split(\'\').join("") : "";
                        var safeProdukKode   = item.produk_kode ? item.produk_kode.split(\'\').join("") : "";

                        var dcBadge   = (cleanStokDc > 0) ? "<b class=\'text-green\'>" + cleanStokDc.toLocaleString() + "</b>" : "<span class=\'text-muted\'>0</span>";
                        var suppBadge = (cleanStokSupp > 0) ? "<b class=\'text-blue\'>" + cleanStokSupp.toLocaleString() + "</b>" : "<span class=\'text-muted\'>0</span>";

                        html += "<tr>";
                        html += "<td class=\'text-center\'><b>" + item.id + "</b></td>";
                        html += "<td><b>" + item.project_nama + "</b><br><small class=\'text-muted\'>" + (item.project_start_nomer || "") + "</small></td>";
                        html += "<td><b>[" + item.produk_kode + "]</b> " + item.produk_nama + supplierTag + "<br><small class=\'text-muted\'>Satuan: " + item.satuan_nama + "</small></td>";
                        html += "<td class=\'text-right\'>" + cleanQtyRAB.toLocaleString() + " " + item.satuan_nama + "</td>";
                        html += "<td class=\'text-right\'>" + cleanQtyPO.toLocaleString() + "</td>";
                        html += "<td class=\'text-right\'><b class=\'text-green\'>" + cleanQtySisa.toLocaleString() + "</b></td>";
                        html += "<td class=\'text-right\'>" + dcBadge + "</td>";
                        html += "<td class=\'text-right\'>" + suppBadge + "</td>";
                        html += "<td class=\'text-center\'><span class=\'label " + badgeClass + "\'>" + item.status + "</span></td>";
                        html += "<td class=\'text-center\'>";
                        html += "<div class=\'input-group input-group-sm\' style=\'width: 165px; margin: 0 auto;\'>";
                        html += "<input type=\'number\' id=\'qty_pool_" + item.id + "\' class=\'form-control text-right\' value=\'" + cleanQtySisa + "\' min=\'1\' max=\'" + cleanQtySisa + "\'>";
                        html += "<span class=\'input-group-btn\'>";
                        html += "<button class=\'btn btn-success btn-sm\' type=\'button\' onclick=\'addPoolItemToCart(" + item.produk_id + ", " + item.id + ", " + (item.supplier_id || 0) + ", \"" + safeSupplierNama + "\");\' title=\'Tambah ke Cart\'><i class=\'fa fa-plus\'></i></button>";
                        html += "<button class=\'btn btn-danger btn-sm\' type=\'button\' onclick=\'cancelPoolItem(" + item.id + ", \"" + safeProdukKode + "\");\' title=\'Batalkan/Hapus Item Pool\'><i class=\'fa fa-trash\'></i></button>";
                        html += "</span>";
                        html += "</div>";
                        html += "</td>";
                        html += "</tr>";
                ...
                $("#project_pool_tbody").html(html);

                var currentOptCount = $("#filter_project_id option").length;
                if (currentOptCount <= 1) {
                    $.each(projectsMap, function(pId, pNama) {
                        $("#filter_project_id").append("<option value=\'" + pId + "\'>" + pNama + "</option>");
                    });
                }
            } else {
                $("#project_pool_alert").show();
                $("#project_pool_alert_msg").html(res.message || "Gagal memuat data pool.");
                $("#project_pool_tbody").html("<tr><td colspan=\'10\' class=\'text-center text-danger\' style=\'padding:15px;\'>" + (res.message || "Error") + "</td></tr>");
            }
        },
        error: function(err) {
            $("#project_pool_tbody").html("<tr><td colspan=\'10\' class=\'text-center text-danger\' style=\'padding:15px;\'>Gagal menghubungi server.</td></tr>");
        }
    });
}

function addPoolItemToCart(produkId, poolId, itemSupplierId, itemSupplierNama) {
    var qty = $("#qty_pool_" + poolId).val();
    if (!qty || parseFloat(qty) <= 0) {
        alert("Masukkan Qty Order yang valid.");
        return;
    }

    var currentSupplierId = 0;
    if ($("select[name*=\'pihakID\']").length > 0 && $("select[name*=\'pihakID\']").val()) {
        currentSupplierId = parseInt($("select[name*=\'pihakID\']").val());
    } else if ($("#pihakID").length > 0 && $("#pihakID").val()) {
        currentSupplierId = parseInt($("#pihakID").val());
    } else if ($("input[name*=\'pihakID\']").length > 0 && $("input[name*=\'pihakID\']").val()) {
        currentSupplierId = parseInt($("input[name*=\'pihakID\']").val());
    }

    itemSupplierId = parseInt(itemSupplierId) || 0;

    var proceedAdd = function(forceSupplier) {
        var forceParam = forceSupplier ? "&force_supplier_id=1" : "";
        var targetUrl = "' . base_url() . 'pembelianfgproject/_processSelectProduct/select/1466?id=" + produkId + "&project_purchase_pool_id=" + poolId + "&jml=" + qty + forceParam;
        if (document.getElementById("result")) {
            document.getElementById("result").src = targetUrl;
        } else if (window.top && window.top.document.getElementById("result")) {
            window.top.document.getElementById("result").src = targetUrl;
        } else {
            $.get(targetUrl, function() {
                if ($("#shopping_cart").length > 0) {
                    $("#shopping_cart").load("' . base_url() . 'pembelianfgproject/_shoppingCart/viewCart/1466");
                }
            });
        }
    };

    if (currentSupplierId > 0 && itemSupplierId > 0 && currentSupplierId !== itemSupplierId) {
        var confirmMsg = "Header transaksi saat ini ditujukan untuk Supplier lain.\n\nApakah Anda ingin mengganti Supplier Header menjadi [" + (itemSupplierNama || "Supplier Baru") + "] untuk membuat PO ini?";
...
$(document).ready(function() {
    loadProjectPoolInline();
    $(document).on("change", "select[name*=\'pihakID\'], #pihakID, select[name*=\'pihak_id\']", function() {
        loadProjectPoolInline();
    });
});
</script>';
        ...
        $p->addTags(
            array(
                ...
                "newTrDisp"               => isset($addLink['link']) ? "inline-table" : "none",
                "scriptBottom"            => isset($scriptBottom) ? $scriptBottom : "",
                "project_pool_inline"     => $strProjectPoolInline,
            )
        );
