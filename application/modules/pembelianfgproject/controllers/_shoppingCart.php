        ...
        if (isset($_SESSION[$cCode])) {
            ...
            if (isset($_SESSION[$cCode]['items'])) {
                ...
                foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                    ...
                    $tmp["editTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "pembelian/", $iSpec['handler']) : '0') . "/" . $editHandlerMethod . "/" . $this->jenisTr . "?id=" . $id . "&newQty=";
                    $tmp["removeTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "pembelian/", $iSpec['handler']) : '0') . "/remove/" . $this->jenisTr . "?id=" . $id;
...
//        mati_disini("kakakaak");
    ...
    public function recordAddRow()
    {
        ...
        echo "<script>";
        //        echo "top.$('#result').load('" . base_url() . "ValueGate/buildValues/" . $this->jenisTr . "/" . $_SESSION[$cCode]["main"]['stepNumber'] . "/?epreketek=yes');";
        echo "top.$('#shopping_cart').load('" . base_url() . $this->modul . "/" . "_shoppingCart/viewCart/" . $this->jenisTr . "?epreketek=yes');";
        echo "</script>";
    }
...
//        arrPrint($_GET);
    ...
    public function getProjectPoolItemsAjax()
    {
        ...
        } elseif (isset($_SESSION[$cCode]['main']['pihakID']) && (int)$_SESSION[$cCode]['main']['pihakID'] > 0) {
            $supplierId = (int)$_SESSION[$cCode]['main']['pihakID'];
        }

        $projectId = isset($_GET['project_id']) ? (int)$_GET['project_id'] : 0;
        $search = isset($_GET['search']) ? trim($_GET['search']) : "";

        $this->db->select("project_purchase_pool.*");
        $this->db->from("project_purchase_pool");

        if ($supplierId > 0) {
            $this->db->join("produk_per_supplier", "project_purchase_pool.produk_id = produk_per_supplier.produk_id", "left");
            $this->db->group_start();
            $this->db->where("project_purchase_pool.supplier_id", $supplierId);
            $this->db->or_where("produk_per_supplier.suppliers_id", $supplierId);
            $this->db->group_end();
        }

        $this->db->where_in("project_purchase_pool.status", array('OPEN', 'PARTIAL'));
        $this->db->where("project_purchase_pool.qty_sisa >", 0);

        if ($projectId > 0) {
            $this->db->where("project_purchase_pool.project_id", $projectId);
        }

        if (strlen($search) > 0) {
            $this->db->group_start();
            $this->db->like("project_purchase_pool.produk_nama", $search);
            $this->db->or_like("project_purchase_pool.produk_kode", $search);
            $this->db->or_like("project_purchase_pool.project_nama", $search);
            $this->db->or_like("project_purchase_pool.supplier_nama", $search);
            $this->db->group_end();
        }

        $this->db->group_by("project_purchase_pool.id");
        $this->db->order_by("project_purchase_pool.id", "ASC");
        $items = $this->db->get()->result_array();

        // 1. Lookup ID Gudang Project DC / Pusat Internal (supplier_id IS NULL OR supplier_id = 0)
        $this->db->select("gudang.id");
        $this->db->from("gudang");
        $this->db->join("gudang_kategori", "gudang.kategori_gudang_id = gudang_kategori.id", "left");
        $this->db->where("gudang.status", "1");
        $this->db->where("(gudang.supplier_id IS NULL OR gudang.supplier_id = 0)", null, false);
        $this->db->group_start();
        $this->db->like("gudang.nama", "project");
        $this->db->or_like("gudang_kategori.nama", "project");
        $this->db->group_end();
        $gudangDcRows = $this->db->get()->result_array();
        $gudangDcIds = array();
        if (!empty($gudangDcRows)) {
            foreach ($gudangDcRows as $gRow) {
                $gudangDcIds[] = (int)$gRow['id'];
            }
        }

        // 2. Lookup Mapping ID Gudang Project Supplier (supplier_id > 0)
        $this->db->select("gudang.id, gudang.supplier_id");
        $this->db->from("gudang");
        $this->db->join("gudang_kategori", "gudang.kategori_gudang_id = gudang_kategori.id", "left");
        $this->db->where("gudang.status", "1");
        $this->db->where("gudang.supplier_id >", 0);
        $this->db->group_start();
        $this->db->like("gudang.nama", "project");
        $this->db->or_like("gudang_kategori.nama", "project");
        $this->db->group_end();
        $gudangSuppRows = $this->db->get()->result_array();
        $gudangSuppMap = array();
        if (!empty($gudangSuppRows)) {
            foreach ($gudangSuppRows as $sRow) {
                $sId = (int)$sRow['supplier_id'];
                if (!isset($gudangSuppMap[$sId])) {
                    $gudangSuppMap[$sId] = array();
                }
                $gudangSuppMap[$sId][] = (int)$sRow['id'];
            }
        }

        if (is_array($items) && count($items) > 0) {
            foreach ($items as &$itemRow) {
                $itemRow['qty_kebutuhan'] = ((float)$itemRow['qty_kebutuhan'] == (int)$itemRow['qty_kebutuhan']) ? (int)$itemRow['qty_kebutuhan'] : (float)$itemRow['qty_kebutuhan'];
                $itemRow['qty_po']        = ((float)$itemRow['qty_po'] == (int)$itemRow['qty_po']) ? (int)$itemRow['qty_po'] : (float)$itemRow['qty_po'];
                $itemRow['qty_sisa']      = ((float)$itemRow['qty_sisa'] == (int)$itemRow['qty_sisa']) ? (int)$itemRow['qty_sisa'] : (float)$itemRow['qty_sisa'];

                $produkId = (int)$itemRow['produk_id'];
                $itemSupplierId = (int)$itemRow['supplier_id'];

                // Hitung Stok Project DC / Pusat Internal
                $stokDcVal = 0;
                if (!empty($gudangDcIds)) {
                    $this->db->select_sum("jumlah", "total_stok");
                    $this->db->where("produk_id", $produkId);
                    $this->db->where_in("gudang_id", $gudangDcIds);
                    $this->db->where("state", "active");
                    $qStok = $this->db->get("stock_locker")->row();
                    if (!empty($qStok) && $qStok->total_stok > 0) {
                        $stokDcVal = (float)$qStok->total_stok;
                    }
                }

                // Hitung Stok Project Supplier
                $stokSuppVal = 0;
                $suppGudangIds = isset($gudangSuppMap[$itemSupplierId]) ? $gudangSuppMap[$itemSupplierId] : array();
                if (!empty($suppGudangIds)) {
                    $this->db->select_sum("jumlah", "total_stok");
                    $this->db->where("produk_id", $produkId);
                    $this->db->where_in("gudang_id", $suppGudangIds);
                    $this->db->where("state", "active");
                    $qStokS = $this->db->get("stock_locker")->row();
                    if (!empty($qStokS) && $qStokS->total_stok > 0) {
                        $stokSuppVal = (float)$qStokS->total_stok;
                    }
                }

                $itemRow['stok_dc']       = ((float)$stokDcVal == (int)$stokDcVal) ? (int)$stokDcVal : (float)$stokDcVal;
                $itemRow['stok_supplier'] = ((float)$stokSuppVal == (int)$stokSuppVal) ? (int)$stokSuppVal : (float)$stokSuppVal;
            }
            unset($itemRow);
        }
    ...
    public function cancelProjectPoolItemAjax()
    {
        $poolId = isset($_GET['pool_id']) ? (int)$_GET['pool_id'] : (isset($_POST['pool_id']) ? (int)$_POST['pool_id'] : 0);
        if ($poolId > 0) {
            ...
            if (!empty($pool)) {
                ...
                echo json_encode(array(
                    "status"  => "success",
                    "message" => "Item kebutuhan proyek berhasil dibatalkan dari Autopurchase Pool."
                ));
                return;
            }
        }
        ...
        echo json_encode(array(
            "status"  => "error",
            "message" => "Item pool tidak ditemukan."
        ));
    }
