...
//        showLast_query("lime");
        ...
        if (sizeof($tmpB) > 0) {
            foreach ($tmpB as $row) {
                ...
                // !isset($_SESSION[$cCode]['items'])  ? $_SESSION[$cCode]['items'] : "";
//                arrPrintPink($_SESSION[$cCode]['items'], __LINE__ . " " . __METHOD__);
                // arrPrintBlue($_SESSION[$cCode]);
                // matiHere(__LINE__ . " " .__METHOD__);
                ...
                if (!isset($_SESSION[$cCode]['items']) || !array_key_exists($id, $_SESSION[$cCode]['items'])) {
                    ...
                    if (isset($_GET['jml']) && (float)$_GET['jml'] > 0) {
                        $tmp['jml'] = (float)$_GET['jml'];
                        $tmp['qty'] = (float)$_GET['jml'];
                    }
                    else {
                        $tmp['jml'] = (float)$jml;
                        $tmp['qty'] = (float)$jml;
                    ...
                    if ($subAmountConfig != null) {
                        ...
                        $tmp['subtotal'] = ($tmp['jml'] * (isset($tmp['harga']) ? $tmp['harga'] : 0));
                    }

                    $_SESSION[$cCode]['items'][$id] = $tmp;

                    if (isset($_GET['project_purchase_pool_id']) && (int)$_GET['project_purchase_pool_id'] > 0) {
                        $poolId = (int)$_GET['project_purchase_pool_id'];
                        $poolRow = $this->db->get_where("project_purchase_pool", array("id" => $poolId))->row();
                        if (!empty($poolRow)) {
                            $_SESSION[$cCode]['items'][$id]['project_purchase_pool_id'] = $poolId;
                            $_SESSION[$cCode]['items'][$id]['project_id']               = $poolRow->project_id;
                            $_SESSION[$cCode]['items'][$id]['project_nama']             = $poolRow->project_nama;

                            if ((!isset($_SESSION[$cCode]['main']['pihakID']) || (int)$_SESSION[$cCode]['main']['pihakID'] <= 0 || isset($_GET['force_supplier_id'])) && (int)$poolRow->supplier_id > 0) {
                                $_SESSION[$cCode]['main']['pihakID']   = (int)$poolRow->supplier_id;
                                $_SESSION[$cCode]['main']['pihakName'] = $poolRow->supplier_nama;
                            }
                        }
                    }
                    ...
                    if (isset($_GET['project_purchase_pool_id']) && (int)$_GET['project_purchase_pool_id'] > 0) {
                        if (isset($_GET['jml']) && (float)$_GET['jml'] > 0) {
                            $_SESSION[$cCode]['items'][$id]['jml'] = (float)$_GET['jml'];
                            $_SESSION[$cCode]['items'][$id]['qty'] = (float)$_GET['jml'];
                        }
                        ...
                        $_SESSION[$cCode]['items'][$id]['qty'] += $jml;
                        $_SESSION[$cCode]['items'][$id]['subtotal'] = ($_SESSION[$cCode]['items'][$id]['jml'] * ($_SESSION[$cCode]['items'][$id]['harga'] + (isset($_SESSION[$cCode]['items'][$id]['ppn']) ? $_SESSION[$cCode]['items'][$id]['ppn'] : 0)));
                    }
                ...
                $tmp['kategori_id'] = $rows->kategori_id;
                $tmp['kategori_nama'] = $rows->kategori_nama;
...
//        }
    ...
    public function remove()
    {
        ...
        if (isset($_SESSION[$cCode]['items'][$id])) {
            ...
            unset($_SESSION[$cCode]['items5_sum'][$id]);
        }
        if (isset($_SESSION[$cCode]['items2'][$id])) {
    ...
    public function multiSelectBi__()
    {
        // unset($_SESSION["Bi"]);
        // arrPrint($this->uri->segment_array());
        // arrprint($_POST);
        // arrprint($_GET);
        // arrPrintWebs($_SESSION["Bi"]);
        // //
        ...
        //region auto select multi produk
        $arrItems = $_SESSION["Bi"][$id];
...
//        matiHEre();
        if (sizeof($arrItems) > 0) {
            foreach ($arrItems as $pid => $jmlParam_0) {
                // ceklime($pid);
                $jmlParam = $jmlParam_0["new_order"];
...
//         matiHEre();
        ...
        $vg->buildValue($this->jenisTr, $id, $initMasterValues, $this->modul);
        if (sizeof($arrItems) > 0) {
            unset($_SESSION["Bi"][$id]);//reset session yang tidak lagi relevan
            echo "<script>";
            echo "top.window.location.href='" . base_url() . $this->modul . "/Create/index/" . $this->jenisTr . "'";
            echo "</script>";
        }
        //endregion
    }
