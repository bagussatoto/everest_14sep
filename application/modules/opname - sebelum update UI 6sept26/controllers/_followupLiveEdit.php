...
//matiHere();
                //tambahan untuk reset semua serial dari produk yang diganti opnamenya
                if(isset($_SESSION[$cCode]["items7"][$id])){
                    unset($_SESSION[$cCode]["items7"][$id]);
                }
                if(isset($_SESSION[$cCode]['items5_sum'])){
                    $tempKey = array(
                        "id" => "id",
                        "nama" => "nama",
                        "name" => "nama",
                    ...
                    foreach ($_SESSION[$cCode]["items7"] as $id_x => $tempDatas) {
                        foreach ($tempDatas as $sku => $serialData) {
                            $tempSerial = array();
                            foreach ($serialData as $serial_list => $serial_jml) {
                                ...
                                $tempSerial["produk_sku_part_nama"] = $sku;
                                $_SESSION[$cCode]["items5_sum"][] = $tempSerial;
                            }

//                    arrprint(array_keys($serialData));
...
//            echo(lgShowAlert("NOT replacing $key with $val"));
        ...
        if ($isSilent) {
            ...
            $resPayload = array(
                "status" => 200,
                "id" => $id,
                "selisih" => isset($_SESSION[$cCode]['items'][$id]['qty_selisih']) ? $_SESSION[$cCode]['items'][$id]['qty_selisih'] : 0,
                "qty_debet" => isset($_SESSION[$cCode]['items'][$id]['qty_debet']) ? $_SESSION[$cCode]['items'][$id]['qty_debet'] : 0,
                "qty_kredit" => isset($_SESSION[$cCode]['items'][$id]['qty_kredit']) ? $_SESSION[$cCode]['items'][$id]['qty_kredit'] : 0,
                "subtotal" => isset($_SESSION[$cCode]['items'][$id]['subtotal']) ? $_SESSION[$cCode]['items'][$id]['subtotal'] : 0,
            );
...
//        echo "\n<script>top.location.reload();</script>";
    ...
    public function updateSerial()
    {
        ...
        $serial = $_GET["serial"];
        if ($state=="true") {
            if (!isset($_SESSION[$cCode]["items7"][$pid][$sku][$serial])) {
                $_SESSION[$cCode]["items7"][$pid][$sku][$serial] = 1;
            }
        } else {
            unset($_SESSION[$cCode]["items7"][$pid][$sku][$serial]);
        }
        ...
        unset($_SESSION[$cCode]["items5_sum"]);
        if (count($_SESSION[$cCode]["items7"]) > 0) {
            foreach ($_SESSION[$cCode]["items7"] as $id => $tempDatas) {
                foreach ($tempDatas as $sku => $serialData) {
                    $tempSerial = array();
                    foreach ($serialData as $serial_list => $serial_jml) {
                        foreach ($tempKey as $key_target => $key_src) {
