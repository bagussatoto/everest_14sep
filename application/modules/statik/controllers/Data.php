...
class Data extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $className = "Mdl" . $segment4;
        if (!empty($segment4) && file_exists(APPPATH . "models/Mdls/" . $className . ".php")) {
            $this->load->model("Mdls/" . $className);
            $o = new $className;
    ...
    private function buildGudangListLabelMaps($rows)
    {
        $maps = array(
            ...
            "kategori_gudang_id" => array(),
            "supplier_id" => array(),
            "_cabang_pusat_label" => "100 pusat (dc)",
        );

        foreach (array("cabang_id", "tipe_gudang_id", "kategori_gudang_id", "supplier_id") as $fcol) {
        ...
        if (is_array($rows) && sizeof($rows) > 0) {
            foreach ($rows as $row) {
                ...
                if (isset($row->kategori_gudang_id)) {
                    $val = trim((string)$row->kategori_gudang_id);
                    if ($val !== "" && $val !== "0") {
                        $idsKategori[$val] = $val;
                    }
                }
        ...
        if (sizeof($idsKategori) > 0) {
            $this->db->select(array("id", "nama"));
            $this->db->where_in("id", array_values($idsKategori));
            $tmpKategoris = $this->db->get("gudang_kategori")->result();
            if (sizeof($tmpKategoris) > 0) {
                foreach ($tmpKategoris as $tipe) {
                    $id = isset($tipe->id) ? trim((string)$tipe->id) : "";
                    $nama = isset($tipe->nama) ? trim((string)$tipe->nama) : "";
                    if ($id !== "" && $nama !== "") {
                        $maps["kategori_gudang_id"][$id] = $nama;
                    }
                }
            }
        }
    ...
    {
        if (!in_array($kolom, array("cabang_id", "tipe_gudang_id", "kategori_gudang_id", "supplier_id"))) {
            return null;
        }
...
//                            top.$('iframe#result2').attr('src', urlBack);
    ...
    public function delete()
    {
        ...
        if ($className == "MdlCustomer" && method_exists($o, "isCustomerDeletable")) {
            if (!$o->isCustomerDeletable($selectedID)) {
                $this->session->errMsg = "Penghapusan ditolak: Konsumen masih memiliki saldo piutang berjalan. Gunakan opsi Non-Aktifkan/Bekukan.";
                redirect(MODUL_PATH . get_class($this) . "/viewdt/$ctrlName");
                return;
            }
        }
    ...
    public function viewHistories()
    {
        ...
        if (sizeof($tmpH) > 0) {
            $content .= "<div class='table-responsive' style='max-height: 480px; overflow-y: auto; overflow-x: auto; border: 1px solid #d2d6de;'>";
            $content .= "<table class='table table-condensed table-bordered table-striped table-hover' style='margin-bottom: 0;'>";
            $content .= "<thead class='text-uppercase' style='background: #2c3b41; color: #fff;'>";
            $content .= "<tr>";
            $content .= "<th class='text-center' style='width: 35px;'>No</th>";
            $content .= "<th style='width: 135px;'>Waktu</th>";
            $content .= "<th style='width: 130px;'>Aktivitas</th>";
            $content .= "<th style='min-width: 200px;'>Uraian / Catatan Tata Kelola</th>";
            foreach ($listedFields as $fName => $label) {
                $content .= "<th title='" . htmlspecialchars($fName) . "'>" . htmlspecialchars($label) . "</th>";
            }
            $content .= "<th style='width: 120px;'>Petugas</th>";
            $content .= "</tr>";
            $content .= "</thead>";
            $content .= "<tbody>";
            ...
            foreach ($tmpH as $row) {
                $no++;
                $oldContents = array();
                if (!empty($row->old_content)) {
                    $decodedOld = blobDecode($row->old_content);
                    if (empty($decodedOld)) {
                        $rawB64 = @base64_decode($row->old_content);
                        if ($rawB64 !== false) {
                            $decodedOld = @unserialize($rawB64);
                        }
                    }
                    if (is_object($decodedOld)) {
                        $oldContents = (array)$decodedOld;
                    } elseif (is_array($decodedOld)) {
                        $oldContents = isset($decodedOld[0]) ? (array)$decodedOld[0] : $decodedOld;
                    }
                }

                $newContents = array();
                if (!empty($row->new_content)) {
                    $decodedNew = blobDecode($row->new_content);
                    if (empty($decodedNew)) {
                        $rawB64 = @base64_decode($row->new_content);
                        if ($rawB64 !== false) {
                            $decodedNew = @unserialize($rawB64);
                        }
                    }
                    if (is_object($decodedNew)) {
                        $newContents = (array)$decodedNew;
                    } elseif (is_array($decodedNew)) {
                        $newContents = isset($decodedNew[0]) ? (array)$decodedNew[0] : $decodedNew;
                    }
                }

                $content .= "<tr>";
                $content .= "<td class='text-center'>" . $no . "</td>";
                $content .= "<td><small>" . htmlspecialchars($row->dtime) . "</small></td>";

                // Pemeriksaan Aksi Tata Kelola Status Master Data (ISO 8000)
                $action = isset($newContents['action']) ? $newContents['action'] : '';
                if ($action == 'deactivate') {
                    $content .= "<td><span class='label label-warning' style='background:#e67e22;'><i class='fa fa-lock'></i> Non-Aktifkan</span></td>";
                    $alasanKat = isset($newContents['alasan_kategori']) ? $newContents['alasan_kategori'] : '-';
                    $alasanCat = isset($newContents['alasan_catatan']) ? $newContents['alasan_catatan'] : '-';
                    $nomorMemo = (isset($newContents['nomor_memo']) && strlen(trim($newContents['nomor_memo'])) > 0) ? " | Memo: " . $newContents['nomor_memo'] : "";
                    $content .= "<td><b>[" . htmlspecialchars($alasanKat) . "]</b> " . htmlspecialchars($alasanCat) . "<br><small class='text-muted'>" . htmlspecialchars($nomorMemo) . "</small></td>";
                } elseif ($action == 'activate') {
                    $content .= "<td><span class='label label-success' style='background:#27ae60;'><i class='fa fa-unlock'></i> Aktifkan</span></td>";
                    $alasanCat = isset($newContents['alasan_catatan']) ? $newContents['alasan_catatan'] : 'Diaktifkan kembali';
                    $content .= "<td>" . htmlspecialchars($alasanCat) . "</td>";
                } else {
                    $content .= "<td><span class='label label-info'><i class='fa fa-edit'></i> Update</span></td>";
                    $content .= "<td><small class='text-muted'>Perubahan entri data</small></td>";
                }

                foreach ($listedFields as $fName => $label) {
                    $type_conten = isset($fields[$fName]['type']) ? $fields[$fName]['type'] : "";
                    $fColName = $fName;
                    $valNew = isset($newContents[$fColName]) ? $newContents[$fColName] : null;
                    $valOld = isset($oldContents[$fColName]) ? $oldContents[$fColName] : null;
                    $valDisplay = !is_null($valNew) ? $valNew : (!is_null($valOld) ? $valOld : "-");

                    if ($fName == 'status') {
                        if (isset($newContents['new_status'])) {
                            $valDisplay = $newContents['new_status'] == 'nonaktif' ? "<span class='label label-warning'>Non-Aktif</span>" : "<span class='label label-success'>Aktif</span>";
                        } elseif (!is_null($valOld)) {
                            $trashVal = isset($oldContents['trash']) ? intval($oldContents['trash']) : 0;
                            if (intval($valOld) == 1 && $trashVal == 1) {
                                $valDisplay = "<span class='label label-warning'>Non-Aktif</span>";
                            } elseif (intval($valOld) == 1 && $trashVal == 0) {
                                $valDisplay = "<span class='label label-success'>Aktif</span>";
                            } else {
                                $valDisplay = "<span class='label label-danger'>Trash</span>";
                            }
                        }
                    } elseif ($type_conten == "blob" || $type_conten == "image") {
                        if ($valDisplay != "-") {
                            $valDisplay = "<img src='" . htmlspecialchars($valDisplay) . "' class='img-responsive' width='85px'>";
                        }
                    }

                    $content .= "<td>" . $valDisplay . "</td>";
                }

                $content .= "<td>" . htmlspecialchars($row->oleh_name) . "</td>";
                $content .= "</tr>";
            }
            ...
            $content .= "<div class='alert alert-warning text-center' style='margin-bottom:0;'><i class='fa fa-info-circle'></i> Belum ada rekaman riwayat perubahan untuk data ini.</div>";
        }
...
//                                        size: BootstrapDialog.SIZE_WIDE,
    ...
    function viewdt()
    {
        ...
        $data = array(
            ...
            "maximumData" => $maximumData,
            "jmlDataNow" => $jmlDataNow,
            "statusCounts" => method_exists($o, 'getStatusCounts') ? $o->getStatusCounts() : (method_exists($o, 'getCustomerStatusCounts') ? $o->getCustomerStatusCounts() : array()),
            "customerStatusCounts" => method_exists($o, 'getStatusCounts') ? $o->getStatusCounts() : (method_exists($o, 'getCustomerStatusCounts') ? $o->getCustomerStatusCounts() : array()),
    ...
    function fetch_data()
    {
        ...
        $statusState = isset($_POST['status_state']) ? $_POST['status_state'] : (isset($_GET['status_state']) ? $_GET['status_state'] : 'aktif');
        $modelTable = method_exists($this->$mdlName, 'getTableName') ? $this->$mdlName->getTableName() : '';
        $hasStatusCol = !empty($modelTable) && $this->db->table_exists($modelTable) && $this->db->field_exists('status', $modelTable);
        $hasTrashCol = !empty($modelTable) && $this->db->table_exists($modelTable) && $this->db->field_exists('trash', $modelTable);

        if ($hasStatusCol) {
            if ($hasTrashCol) {
                if ($statusState == 'aktif') {
                    $this->$mdlName->setFilters(array("status='1'", "trash='0'"));
                } elseif ($statusState == 'nonaktif') {
                    $this->$mdlName->setFilters(array("status='1'", "trash='1'"));
                } elseif ($statusState == 'trash') {
                    $this->$mdlName->setFilters(array("status='0'", "trash='1'"));
                } else {
                    $this->$mdlName->setFilters(array("status='1'"));
                }
            } else {
                if ($statusState == 'aktif') {
                    $this->$mdlName->setFilters(array("status='1'"));
                } elseif ($statusState == 'nonaktif') {
                    $this->$mdlName->setFilters(array("status='0'"));
                }
            }
        }
        ...
        $customerArList = array();
        if ($mdlName == "MdlCustomer" && sizeof($dataIds) > 0 && method_exists($this->$mdlName, "getCustomerOutstandingAr")) {
            $customerArList = $this->$mdlName->getCustomerOutstandingAr($dataIds);
        }
        ...
        foreach ($fetch_data as $row) {
            ...
            foreach ($listedFields as $kolom => $label) {
                ...
                elseif ($kolom == 'status') {
                    $rowTrash = isset($row->trash) ? intval($row->trash) : 0;
                    $rowStatus = isset($row->status) ? intval($row->status) : 0;
                    if ($rowStatus == 0 && $rowTrash == 1) {
                        $sub_array[] = "<span class='label label-danger' style='background:#c0392b;'><i class='fa fa-trash'></i> Trash</span>";
                    } elseif ($rowStatus == 1 && $rowTrash == 1) {
                        $sub_array[] = "<span class='label label-warning' style='background:#e67e22;'><i class='fa fa-ban'></i> Non-Aktif</span>";
                    } elseif ($rowStatus == 1 && $rowTrash == 0) {
                        $sub_array[] = "<span class='label label-success' style='background:#27ae60;'><i class='fa fa-check-circle'></i> Aktif</span>";
                    } else {
                        $sub_array[] = "<span class='label label-default'>-</span>";
                    }
                }
                elseif ($kolom == 'nama' && $mdlName == "MdlCustomer") {
                    $valNama = isset($row->nama) ? htmlspecialchars($row->nama) : "-";
                    $cAr = isset($customerArList[$rowId]) ? $customerArList[$rowId]['saldo_piutang'] : 0;
                    if ($cAr > 0) {
                        $strAr = number_format($cAr, 0, ',', '.');
                        $sub_array[] = "<b>$valNama</b><br><span class='label label-danger' style='font-size:10px; cursor:pointer;' title='Sisa Piutang Berjalan: Rp $strAr'><i class='fa fa-exclamation-triangle'></i> Piutang: Rp $strAr</span>";
                    } else {
                        $sub_array[] = "$valNama";
                    }
                }
                elseif ($kolom == 'kredit_limit' && $mdlName == "MdlCustomer") {
                    $krLimit = isset($row->kredit_limit) ? ($row->kredit_limit * 1) : 0;
                    $cAr = isset($customerArList[$rowId]) ? $customerArList[$rowId]['saldo_piutang'] : 0;
                    $sisaLimit = $krLimit - $cAr;
                    $strLimit = number_format($krLimit, 0, ',', '.');
                    $strSisa = number_format($sisaLimit, 0, ',', '.');
                    $strAr = number_format($cAr, 0, ',', '.');
                    $titleTooltip = "Limit: Rp $strLimit | Terpakai: Rp $strAr | Sisa: Rp $strSisa";
                    $sub_array[] = "<span title='$titleTooltip' data-toggle='tooltip'>" . $strLimit . "</span>";
                }
                elseif (($kolom == 'tlp_1' || $kolom == 'no_ktp' || $kolom == 'npwp') && $mdlName == "MdlCustomer") {
                    $valPii = isset($row->$kolom) ? trim($row->$kolom) : "";
                    if (strlen($valPii) > 4) {
                        $masked = substr($valPii, 0, 4) . str_repeat("*", max(4, strlen($valPii) - 8)) . substr($valPii, -4);
                        $escapedVal = htmlspecialchars($valPii, ENT_QUOTES);
                        $escapedMasked = htmlspecialchars($masked, ENT_QUOTES);
                        $sub_array[] = "<span class='pii-mask-wrapper' data-real='$escapedVal' data-masked='$escapedMasked'><span>$escapedMasked</span> <a href='javascript:void(0);' onclick='togglePiiMask(this);' class='text-muted' title='Buka/Tutup Masking' style='margin-left:4px;'><i class='fa fa-eye'></i></a></span>";
                    } else {
                        $sub_array[] = strlen($valPii) > 0 ? htmlspecialchars($valPii) : "-";
                    }
                }
                else {
            ...
            if ($this->allowEdit && $objState != "1") {
                ...
                // Tombol Non-Aktifkan / Aktifkan Kembali (Tata Kelola Status Master Data - ISO 8000)
                if (isset($row->status)) {
                    $rowTrash = isset($row->trash) ? $row->trash : 0;
                    $rowStatus = $row->status;
                    if ($rowStatus == 1 && $rowTrash == 0) {
                        $btn_actions .= " <button type='button' class='btn btn-warning btn-xs' data-toggle='tooltip' data-placement='left' title='Non-Aktifkan Data' onclick=\"openDeactivateMasterDataModal('$rowId', '" . addslashes($title_f) . "', '$mdlName');\"><i class='fa fa-lock'></i></button>";
                    } elseif ($rowStatus == 1 && $rowTrash == 1) {
                        $btn_actions .= " <button type='button' class='btn btn-success btn-xs' data-toggle='tooltip' data-placement='left' title='Aktifkan Kembali Data' onclick=\"openActivateMasterDataModal('$rowId', '" . addslashes($title_f) . "', '$mdlName');\"><i class='fa fa-unlock'></i></button>";
                    } elseif ($rowStatus == 0 && $rowTrash == 0) {
                        // Untuk entitas yang hanya memiliki kolom status tanpa trash
                        $btn_actions .= " <button type='button' class='btn btn-success btn-xs' data-toggle='tooltip' data-placement='left' title='Aktifkan Kembali Data' onclick=\"openActivateMasterDataModal('$rowId', '" . addslashes($title_f) . "', '$mdlName');\"><i class='fa fa-unlock'></i></button>";
                    }
                }
                ...
                if (array_key_exists($idDeleted, $data_aktif)) {
                    ...
                    if (array_key_exists($idDeleted, $data_aktif_locker_filter)) {
                        ...
                        if (isset($dataOutstanding[$rowId])) {
                            ...
                            if ($mdlName == "MdlCustomer" && isset($customerArList[$rowId]) && $customerArList[$rowId]['saldo_piutang'] > 0) {
                                $strArAlert = number_format($customerArList[$rowId]['saldo_piutang'], 0, ',', '.');
                                $btn_actions .= "<button type='button' class='btn btn-danger btn-xs hidden-print' data-toggle='tooltip' data-placement='left' title='Hapus tidak diizinkan: Masih ada saldo piutang' onClick=\"swal('Peringatan Penghapusan', 'Konsumen ini masih memiliki saldo piutang berjalan sebesar Rp $strArAlert dan dilarang untuk dihapus. Silakan gunakan opsi Non-Aktifkan/Bekukan.', 'warning');\"><span class='glyphicon glyphicon-remove'></span></button>";
                            }
                            else {
            ...
            if ($this->allowViewHistory) {
                $btn_actions .= " <button type='button' class='btn btn-default btn-xs' data-toggle='tooltip' data-placement='left' title='Lihat Riwayat Perubahan' onclick=\"openMasterDataHistoryModal('$ctrlName', '$rowId', '" . addslashes($title_f) . "');\"><span class='glyphicon glyphicon-time'></span></button>";
            }
        ...
        $statusCounts = array();
        if (method_exists($this->$mdlName, 'getStatusCounts')) {
            $statusCounts = $this->$mdlName->getStatusCounts();
        } elseif (method_exists($this->$mdlName, 'getCustomerStatusCounts')) {
            $statusCounts = $this->$mdlName->getCustomerStatusCounts();
        }
...
//         matiHere("belum commit @" . __LINE__);
    ...
    public function deactivateCustomerAjax()
    {
        // Wrapper delegasi ke deactivateMasterDataAjax untuk kompatibilitas
        $_POST['id'] = isset($_POST['customer_id']) ? $_POST['customer_id'] : (isset($_POST['id']) ? $_POST['id'] : 0);
        $_POST['mdl'] = 'MdlCustomer';
        $this->deactivateMasterDataAjax();
    }

    public function activateCustomerAjax()
    {
        // Wrapper delegasi ke activateMasterDataAjax untuk kompatibilitas
        $_POST['id'] = isset($_POST['customer_id']) ? $_POST['customer_id'] : (isset($_POST['id']) ? $_POST['id'] : 0);
        $_POST['mdl'] = 'MdlCustomer';
        $this->activateMasterDataAjax();
    }

    public function checkMasterDataDependencyAjax()
    {
        header('Content-Type: application/json');
        if (!isset($this->session->login['id'])) {
            echo json_encode(array('status' => 'error', 'message' => 'Sesi login telah berakhir.'));
            return;
        }

        $id = isset($_GET['id']) ? intval($_GET['id']) : (isset($_POST['id']) ? intval($_POST['id']) : 0);
        $mdl = isset($_GET['mdl']) ? trim($_GET['mdl']) : (isset($_POST['mdl']) ? trim($_POST['mdl']) : '');
        ...
        $this->load->model('Mdls/' . $mdl);
        if (!isset($this->$mdl)) {
            echo json_encode(array('status' => 'error', 'message' => 'Model ' . $mdl . ' tidak ditemukan.'));
            return;
        }

        $obj = $this->$mdl;
        $reasons = method_exists($obj, 'getDeactivationReasons') ? $obj->getDeactivationReasons() : array(
            "Duplikasi Data / Koreksi Input",
            "Data Tidak Digunakan / Usang",
            "Perubahan Kebijakan Operasional",
            "Lainnya"
        );

        $warningData = method_exists($obj, 'checkDeactivationWarning') ? $obj->checkDeactivationWarning($id) : array(
            'safe' => true,
            'has_warning' => false,
            'warnings' => array()
        );

        // Ambil label entitas dari konfigurasi heDataBehaviour jika ada
        $dataBehavior = $this->config->item('heDataBehaviour');
        $label = isset($dataBehavior[$mdl]['label']) ? $dataBehavior[$mdl]['label'] : $mdl;

        echo json_encode(array(
            'status' => 'success',
            'id' => $id,
            'mdl' => $mdl,
            'entity_label' => ucfirst($label),
            'reasons' => $reasons,
            'has_warning' => isset($warningData['has_warning']) ? $warningData['has_warning'] : false,
            'warnings' => isset($warningData['warnings']) ? $warningData['warnings'] : array(),
            'safe' => isset($warningData['safe']) ? $warningData['safe'] : true,
        ));
    }

    public function deactivateMasterDataAjax()
    {
        header('Content-Type: application/json');
        if (!isset($this->session->login['id'])) {
            echo json_encode(array('status' => 'error', 'message' => 'Sesi login telah berakhir.'));
        ...
        $id = isset($_POST['id']) ? intval($_POST['id']) : (isset($_POST['customer_id']) ? intval($_POST['customer_id']) : 0);
        $mdl = isset($_POST['mdl']) ? trim($_POST['mdl']) : 'MdlCustomer';
        $alasanKategori = isset($_POST['alasan_kategori']) ? trim($_POST['alasan_kategori']) : '';
        $alasanCatatan = isset($_POST['alasan_catatan']) ? trim($_POST['alasan_catatan']) : '';
        $nomorMemo = isset($_POST['nomor_memo']) ? trim($_POST['nomor_memo']) : '';

        if ($id <= 0) {
            echo json_encode(array('status' => 'error', 'message' => 'ID data tidak valid.'));
            return;
        }

        if (empty($alasanKategori)) {
            echo json_encode(array('status' => 'error', 'message' => 'Kategori alasan penonaktifan wajib dipilih.'));
            return;
        }

        if (strlen($alasanCatatan) < 10) {
            echo json_encode(array('status' => 'error', 'message' => 'Uraian alasan penonaktifan wajib diisi minimal 10 karakter.'));
            return;
        }
        ...
        $obj = $this->$mdl;
        $rowLookup = $obj->lookupByID($id);
        $rowData = $rowLookup ? $rowLookup->row() : null;
        if (!$rowData) {
            echo json_encode(array('status' => 'error', 'message' => 'Data tidak ditemukan.'));
            return;
        }

        $tableName = method_exists($obj, 'getTableName') ? $obj->getTableName() : '';
        if (empty($tableName) || !$this->db->table_exists($tableName)) {
            echo json_encode(array('status' => 'error', 'message' => 'Tabel database untuk model ini tidak ditemukan.'));
            return;
        }

        $hasTrash = $this->db->field_exists('trash', $tableName);
        $rowTitle = isset($rowData->nama) ? $rowData->nama : (isset($rowData->name) ? $rowData->name : (isset($rowData->kode) ? $rowData->kode : "ID $id"));
        ...
        if ($this->db->field_exists('last_update', $tableName)) {
        ...
        // Catat jejak audit ke data__history
        $dataBehavior = $this->config->item('heDataBehaviour');
        $label = isset($dataBehavior[$mdl]['label']) ? $dataBehavior[$mdl]['label'] : $mdl;
        ...
        $auditContent = array(
            ...
            'alasan_catatan' => $alasanCatatan,
            'nomor_memo' => $nomorMemo,
            'entity_title' => $rowTitle,
            'ip_address' => $clientIp,
        );

        $actionLabel = 'Deactivate ' . ucfirst($label);
        if (strlen($actionLabel) > 32) {
            $actionLabel = substr($actionLabel, 0, 32);
        }

        $olehName = isset($this->session->login['nama']) ? $this->session->login['nama'] : (isset($this->session->login['nama_login']) ? $this->session->login['nama_login'] : 'admin');
        if (strlen($olehName) > 32) {
            $olehName = substr($olehName, 0, 32);
        }

        $tmpHData = array(
            'orig_id' => $id,
            'mdl_name' => $mdl,
            'mdl_label' => ucfirst($label),
            'old_content' => base64_encode(serialize($rowData)),
            'new_content' => base64_encode(serialize($auditContent)),
            'label' => $actionLabel,
            'oleh_id' => isset($this->session->login['id']) ? $this->session->login['id'] : 0,
            'oleh_name' => $olehName,
        );
        $hTmp->addData($tmpHData);

        $this->db->trans_complete();

        if ($this->db->trans_status() === false) {
            echo json_encode(array('status' => 'error', 'message' => 'Gagal memperbarui status data di database.'));
        } else {
            echo json_encode(array('status' => 'success', 'message' => 'Data ' . ucfirst($label) . ' berhasil dinonaktifkan.'));
        }
    }

    public function activateMasterDataAjax()
    {
        header('Content-Type: application/json');
        if (!isset($this->session->login['id'])) {
            echo json_encode(array('status' => 'error', 'message' => 'Sesi login telah berakhir.'));
        ...
        $id = isset($_POST['id']) ? intval($_POST['id']) : (isset($_POST['customer_id']) ? intval($_POST['customer_id']) : 0);
        $mdl = isset($_POST['mdl']) ? trim($_POST['mdl']) : 'MdlCustomer';
        $alasanCatatan = isset($_POST['alasan_catatan']) ? trim($_POST['alasan_catatan']) : 'Aktivasi kembali data master';
        ...
        $actionLabel = 'Reactivate ' . ucfirst($label);
        if (strlen($actionLabel) > 32) {
            $actionLabel = substr($actionLabel, 0, 32);
        }
        ...
        if ($this->db->trans_status() === false) {
            echo json_encode(array('status' => 'error', 'message' => 'Gagal mengaktifkan kembali data di database.'));
        } else {
            echo json_encode(array('status' => 'success', 'message' => 'Data ' . ucfirst($label) . ' berhasil diaktifkan kembali.'));
        }
    }

    public function checkCustomerArAjax()
    {
        header('Content-Type: application/json');
        $customerId = isset($_GET['customer_id']) ? intval($_GET['customer_id']) : (isset($_POST['customer_id']) ? intval($_POST['customer_id']) : 0);

        if ($customerId <= 0) {
            echo json_encode(array('status' => 'error', 'message' => 'ID Konsumen tidak valid.'));
            return;
        }

        $this->load->model('Mdls/MdlCustomer');
        $cst = new MdlCustomer();
        $arInfo = $cst->getCustomerOutstandingAr($customerId);
        $cstData = $cst->lookupByID($customerId)->row();

        $saldoPiutang = isset($arInfo[$customerId]['saldo_piutang']) ? $arInfo[$customerId]['saldo_piutang'] : 0;
        $fakturCount = isset($arInfo[$customerId]['faktur_count']) ? $arInfo[$customerId]['faktur_count'] : 0;
        $overdueCount = isset($arInfo[$customerId]['overdue_count']) ? $arInfo[$customerId]['overdue_count'] : 0;

        echo json_encode(array(
            'status' => 'success',
            'customer_id' => $customerId,
            'nama' => isset($cstData->nama) ? $cstData->nama : '',
            'kredit_limit' => isset($cstData->kredit_limit) ? ($cstData->kredit_limit * 1) : 0,
            'saldo_piutang' => $saldoPiutang,
            'faktur_count' => $fakturCount,
