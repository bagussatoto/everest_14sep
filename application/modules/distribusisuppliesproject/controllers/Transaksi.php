...
//            arrprint($_SESSION[$cCode]);
    ...
    public function validate()
    {
        ...
        $field2ValidatorRules = isset($this->configUi[$this->jenisTr]["shoppingCartField2Validators"]) ? $this->configUi[$this->jenisTr]["shoppingCartField2Validators"] : array();
...
//                            mati_disini("[$field => $label] || " . $iSpec[$field]);
                            if (!isset($iSpec[$field])) {
                                $errMsgs[] = "$label wajib diisi dengan benar.";
                                $errLines[] = $id;
                                $errFields[$id][] = $field;
                            }
                            if (!is_numeric($iSpec[$field])) {
                                $errMsgs[] = "$label harus berisi angka.";
                                $errLines[] = $id;
                                $errFields[$id][] = $field;
                            }
        ...
        if (sizeof($field2ValidatorRules) > 0) {
            if (isset($_SESSION[$cCode]['items2']) && sizeof($_SESSION[$cCode]['items2']) > 0) {
                foreach ($_SESSION[$cCode]['items2'] as $xid => $xSpec) {
                    foreach ($xSpec as $xxid => $iSpec) {
                        $id = $iSpec['id'];
                        $nama = htmlspecialchars($iSpec['biaya_dasar_nama']);
                        if ((isset($iSpec['disabled']) && $iSpec['disabled'] == "0") || !isset($iSpec['disabled'])) {
                            if (!isset($errFields[$id])) {
                                $errFields[$id] = array();
                            }
                            foreach ($field2ValidatorRules as $field => $label) {
                                if (!isset($iSpec[$field])) {
                                    $errMsgs[] = "$label $nama wajib diisi dengan benar.";
                                    $errLines[] = $id;
                                    $errFields[$id][] = $field;
                                }
                                if (!is_numeric($iSpec[$field])) {
                                    $errMsgs[] = "$label $nama harus berisi angka.";
                                    $errLines[] = $id;
                                    $errFields[$id][] = $field;
                                }
                                if ((int)$iSpec[$field] < 0.5) {
                                    $errMsgs[] = "$label $nama harus lebih dari 0.";
                                    $errLines[] = $id;
                                    $errFields[$id][] = $field;
                                }
                            }
        ...
        if (!isset($_SESSION[$cCode][$gateNameDetail]) || sizeof($_SESSION[$cCode][$gateNameDetail]) == 0) {
            if ($cCode != "_TR_9834") {
                $msg = "[$cCode] #1 Transaksi gagal dilanjutkan karena rincian transaksi kosong.<br>Silahkan merubah qty/unit price dahulu dan masukkan data yang benar supaya transaksi bisa dilanjutkan. @" . __LINE__ . __METHOD__;
                mati_disini($msg);
            }
        }
        else {
            foreach ($_SESSION[$cCode][$gateNameDetail] as $spec) {
                if (!isset($spec['valid_qty'])) {
                    ...
                    $msg = "[$cCode] #2 Transaksi gagal dilanjutkan karena rincian transaksi kosong.<br>Silahkan merubah qty/unit price dahulu dan masukkan data yang benar supaya transaksi bisa dilanjutkan. $hidden_msg";
                    mati_disini($msg);
                }
                else {
                    if ($spec['valid_qty'] == 0) {
...
//                    "gudang_id" => "-710",
        ...
        $arrExtHistoryFields2 = isset($this->configUi[$jenisTr]["extHistoryFields2"]) ? $this->configUi[$jenisTr]["extHistoryFields2"] : array();
        $arrExtHistoryFields2_items2 = isset($this->configUi[$jenisTr]["extHistoryFields2_items2"]) ? $this->configUi[$jenisTr]["extHistoryFields2_items2"] : array();
...
//            matiHere(__LINE__);
            ...
            foreach ($tmpHist as $row) {
                //arrPrint($row);
                $extHistoryFields2 = isset($arrExtHistoryFields2[$row->step_number]) ? $arrExtHistoryFields2[$row->step_number] : array();
                $extHistoryFields2_items2 = isset($arrExtHistoryFields2_items2[$row->step_number]) ? $arrExtHistoryFields2_items2[$row->step_number] : array();
                        ...
                        foreach ($tmpReg_result[$row->transaksi_id] as $param => $eReg) {
                            switch ($param) {
                                ...
                                case "items":
                                    ...
                                    break;
                                case "items2":
                                    if (sizeof($extHistoryFields2_items2) > 0) {
                                        foreach ($extHistoryFields2_items2 as $k1 => $v1) {
                                            if (is_array($v1)) {
                                                $kolom = $v1['kolom'];
                                                $format = $v1['format'];
                                                if (($k1 != null) && !isset($row->$k1)) {
                        ...
                        foreach ($tabHistoryFields as $key => $row) {
                            ...
                            switch ($key) {
                                ...
                                case "transaksi_id":
                                    ...
                                    $tmp .= "<a class='btn btn-danger btn-block' title='' href='javascript:void(0)' onClick =\"$followupLink\">" . $actionLabel . "</a>";
                                    $tmp .= "</div class='input-group'>";
