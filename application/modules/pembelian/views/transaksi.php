...
$('#uplodXls').on('submit',function() {
    ...
    var arrProduk = JSON.parse(localStorage.getItem('items'));
        ...
        $strDiskon = "";
        $strDiskon .= "<div class='text-left' id='container_information'>";
        $strDiskon .= "</div>";

        $uploadData .= $strDiskon;
        //region onprogressView Doank
        $strOnprogView = "";
        if (is_array($arrayOnProgressView) && sizeof($arrayOnProgressView) > 0) {
            $strOnprogView .= "<table class='table table-condensed table-bordered no-padding'>";
...
//            arrPrint($steps[$_GET['step']]);
        ...
        $strOnprog .= "<div class='box-tools pull-right'>";
        $varUrl = "if($('#undoneList>div>div.box-body')){ $('#undoneList>div>div.box-body').append(`<div id='overlay'><div id='text'>Loading Content...</div></div>`); document.getElementById('overlay').style.display = 'block'; $('#undoneList').load('$reloadLink'); }else{ console.log('tidak ada container undoneList'); }";
        $strOnprog .= "<button type='button' class='btn btn-warning btn-box-tool' title='refresh' 
                style='color:#ffffff;'
                onclick=\"$varUrl;\">
        ...
        if (sizeof($steps) > 1) {
            ...
            foreach ($steps as $tStep => $stepData) {
                ...
                $cssSelesai = count($steps) == $tStep ? "style='padding-top: 0;padding-bottom: 0;'" : "";
                $strOnprog .= "<li id='nav_$tStep' class='none $actives'>";
                ...
                $strOnprog .= "<a $cssSelesai id='a_$tStep' class='nav-link btn' onclick=\"$varUrl;localStorage.setItem('stepJenis', '$jenisStep');localStorage.setItem('step', '$tStep');\">";
                $strOnprog .= "<span class='text-uppercase text-bold' style='$visibility;'>$print_qr $trSelesai" . $stepData['label'] . "</span>  $isiBadge </a>";
                $strOnprog .= "</li>";
            }
            $strOnprog .= "</ul>";
            $strOnprog .= "<script>
                var jenisStep = '$jenisStep';
                var stepNow = localStorage.getItem('step');
                var stepJenisNow = localStorage.getItem('stepJenis');
                // console.log(stepNow);
                
                if(document.getElementById('nav_' + stepNow) && stepJenisNow == jenisStep){
                    $('#nav_'+ stepNow).addClass('active').trigger('click');                    
                    $('#a_'+ stepNow).trigger('click');                    
                }
            </script>";
...
//                $strOnprog .= "<div class='alert alert-warning'>";
                $anu = "- $stepName item you specified has no entry -";
//                $strOnprog .= "</div>";
                $strOnprog .= tplNoData($anu);
            }
        }
...
//                    arrPrint($shopingcartAddDpp[$k]);
                        ...
                        foreach ($shopingcartAddDpp[$key] as $ik => $ik_label) {
                            if (isset($main[$ik])) {
                                ...
                                echo "<td colspan='1'>" . number_format($main[$ik]) . "</td>";
                            }
...
//region bagian free produk pembelian /mendapatkan produk x ketika beli x kuantitas
            //contoh beli promos 5 unit gratis 1 x holder senilai 2000
...
//                            cekkuning("$elName dataField");
            ...
            if ($show_titipan) {
                $msg_fulfill = "<div class='row'>";
                $msg_fulfill .= "<div class='alert alert-danger text-center font-size-1-5'>$notif_titipan</div>";
                $msg_fulfill .= "</div>";
                echo $msg_fulfill;
            }
            echo "</div class='table-responsive'>";
            ...
            if (isset($items) && sizeof($items) > 0) {
                ...
                foreach ($items as $id => $iSpec) {
                    ...
                    foreach ($itemLabels as $key => $label) {
                        ...
                        if ($key == "qty_alokasi" || $key == "qty_sisa_alokasi") {
                            $qtyPo = isset($detailValues[$id]["produk_ord_jml"]) ? $detailValues[$id]["produk_ord_jml"] : (isset($detailValues[$id]["jml"]) ? $detailValues[$id]["jml"] : (isset($iSpec["produk_ord_jml"]) ? $iSpec["produk_ord_jml"] : (isset($iSpec["jml"]) ? $iSpec["jml"] : 0)));
                            $qtyReguler = isset($detailValues[$id]["qty_reguler"]) ? $detailValues[$id]["qty_reguler"] : (isset($iSpec["qty_reguler"]) ? $iSpec["qty_reguler"] : 0);
                            $qtyDaikin = isset($detailValues[$id]["qty_daikin"]) ? $detailValues[$id]["qty_daikin"] : (isset($iSpec["qty_daikin"]) ? $iSpec["qty_daikin"] : 0);

                            $qtyPo = (float)str_replace(",", "", $qtyPo);
                            $qtyReguler = (float)str_replace(",", "", $qtyReguler);
                            $qtyDaikin = (float)str_replace(",", "", $qtyDaikin);

                            $qtyAlokasi = $qtyReguler + $qtyDaikin;
                            $qtySisaAlokasi = $qtyPo - $qtyAlokasi;

                            if ($key == "qty_alokasi") {
                                if ($qtyAlokasi > $qtyPo) {
                                    $iClass = "text-danger";
                                }
                                elseif ($qtyAlokasi == $qtyPo) {
                                    $iClass = "text-success";
                                }
                                else {
                                    $iClass = "text-primary";
                                }
                                $strVal = "<span class='$iClass text-bold'>" . number_format($qtyAlokasi, 0) . "</span>";
                            }
                            else {
                                if ($qtySisaAlokasi < 0) {
                                    $iClass = "text-danger";
                                }
                                elseif ($qtySisaAlokasi == 0) {
                                    $iClass = "text-success";
                                }
                                else {
                                    $iClass = "text-warning";
                                }
                                $strVal = "<span class='$iClass text-bold'>" . number_format($qtySisaAlokasi, 0) . "</span>";
                            }
                            $tdOpt = "style='text-align:right;$addStyle'";
                        }
                        elseif ($allowEdit == true && in_array($key, $editableFields)) {
                            ...
                            if (is_numeric($val)) {
                                ...
                                $addEvent = "";
                                if (!$allowIncrement) {
                                    if (in_array($key, array("qty_reguler", "qty_daikin"))) {
                                        $addEvent = " oninput=\"var v=parseInt(removeCommas(this.value));if(isNaN(v)||v<0){v=0;}if(v>$maxVal){v=$maxVal;}this.value=addCommas(v);\" onkeyup=\"this.value=addCommas(this.value);\" onblur=\"top.$('#result').load('$updateItemFieldTarget?id=$id&key=$key&val='+removeCommas(this.value))\" ";
                                    }
                                    else {
                                    ...
                                    if (in_array($key, array("qty_reguler", "qty_daikin"))) {
                                        $addEvent = " oninput=\"var v=parseInt(removeCommas(this.value));if(isNaN(v)||v<0){v=0;}if(v>$maxVal){v=$maxVal;}this.value=addCommas(v);\" onkeyup=\"this.value=addCommas(this.value);\"  onblur=\"top.$('#result').load('$updateItemFieldTarget?ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL&id=$id&key=$key&val='+removeCommas(this.value))\" ";
                                    }
                                    else {
...
//                    echo "<div class=''>Detail</div>";
            ...
            if (isset($itemLabelsByGudang) && (sizeof($itemLabelsByGudang) > 0)) {
                ...
                if (isset($itemLabelsByGudangViewOnly) && ($itemLabelsByGudangViewOnly == true)) {
                    $str .= "<div>";
                    $str .= "Anda akan terima barang di " . $itemLabelsByGudangViewOnlyData["gudangTujuanLabel"] . ".<br>";
...
//                                            + \"<div class='text-right'>\"+addCommas(total5)+\"</div>\"
    ...
    case "viewResume":
        ...
        if (sizeof($itemsShadow) > 0) {
            foreach ($itemsShadow as $cabangID => $subItems) {
                if (sizeof($subItems) > 0) {
                    $cabangNama = isset($cabangData[$cabangID]) ? $cabangData[$cabangID] : "";
...
//            arrPrintPink($main);
            ...
            foreach ($addMainSourceField as $kol => $alias) {
                ...
                if (isset($main[$kol])) {
                    if (is_numeric($main[$kol])) {
                        $vall = ($main[$kol] > 0) ? $main[$kol] : 0;
                    }
                    else {
                        $vall = strlen($main[$kol]) > 3 ? $main[$kol] : 0;
                    }
                    $strVal = formatField_he_format($kol, $vall);
//                    $strVal = formatField_he_format($kol, $main[$kol]);
        ...
        if (isset($arrDiskonLabel) && (sizeof($arrDiskonLabel) > 0)) {
            ...
            if (sizeof($arrDiskonData) > 0) {
                ...
                echo "<div class='alert alert-warning' style='margin-top:0px;font-size:15px;'>";
                echo "tidak ada diskon dari supplier.";
                echo "</div>";
            }
        ...
        $str = "<div>";
        $str .= "<h4 sstyle='font-size:15px;ffont-weight:bold;'><span class='fa fa-book'> Promo Free Produk:</span></h4>";
        $str .= "<div>";
        if (isset($free_produk) && ($free_produk != NULL)) {
            $str .= $free_produk;
        }
        else {
            $str .= "<div class='alert alert-warning' style='font-size:15px;'>tidak ada promo free produk.</div>";
        }
        $str .= "</div>";
        $str .= "</div>";
        echo $str;
...
//                            cekKuning(":: $key editable ::");
                            if (is_numeric($val)) {
                                ...
                                if (!$allowIncrement) {
                                    if (in_array($key, array("qty_reguler", "qty_daikin"))) {
                                        $addEvent = " oninput=\"if(parseInt(this.value)<0){this.value='0';} if(parseInt(this.value)>$maxVal){this.value='$maxVal';}\" onblur=\"document.getElementById('result').src='$updateItemFieldTarget?id=$id&key=$key&val='+this.value\" ";
                                    }
                                    else {
                                        $addEvent = " oninput=\"if(parseInt(this.value)<1 || parseInt(this.value)>$maxVal){this.value='$maxVal';}\" onblur=\"document.getElementById('result').src='$updateItemFieldTarget?id=$id&key=$key&val='+this.value\" ";
                                    }
