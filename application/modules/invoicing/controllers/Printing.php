...
class Printing extends Modul_Controller
{
    ...
    public function viewReceiptReg_ori()
    {
        ...
        if (sizeof($tmpTr) > 0) {
            ...
            * -------------------------------------------------*/
            ...
            if (($mainJenis == "paket") && ($tmpTr[0]->jenis_master == "1582" || $tmpTr[0]->jenis_master == "4822")) {
                // cekBiru("MASUK ATAS");
                //                arrPrint($itemsRegistries);
                $rawItems = array();
    ...
    public function viewReceiptReg()
    {
        ...
        $masterID = isset($tmp1[0]->id_master) ? $tmp1[0]->id_master : 0;
        $statusInvoice = isset($tmp1[0]->trash_4) ? $tmp1[0]->trash_4 : 0;
        $tmpTr = $tr->lookupJoined();

        /* -----------------------------------
         * membacaca log printer
         * -----------------------------------*/
        $logPrint = $this->pl->lookupSelected($indTransksiID)->row_array();
//        arrPrintHijau($logPrint);
         ...
         * ------------------------------*/
        $datasLog = array(
            "transaksi_id" => $indTransksiID,
            "oleh_id" => my_id(),
            "oleh_nama" => my_name(),
        );
        $this->pl->createLog($datasLog);
        //--------------------------------------
...
//            showLast_query("biru");
                        ...
                        if (isset($row->$param)) {
                            switch ($param) {
                                ...
                                case "items5_sum"://
                                    ...
                                    break;
                                case "main"://
                                    $mainRegistries = $main = unserialize(base64_decode($row->$param));
            ...
            // Auto-hydration khusus untuk invoice project (4822 / termin 7499) yang snapshot items5_sum-nya kosong di DB
            $is_project_invoice = ($tmpTr[0]->jenis_master == "4822"
                || (isset($tmpTr[0]->transaksi_jenis) && $tmpTr[0]->transaksi_jenis == "4822")
                || (isset($tmpTr[0]->transaksi_jenis2) && $tmpTr[0]->transaksi_jenis2 == "project")
                || (isset($tmpTr[0]->nomer) && substr($tmpTr[0]->nomer, 0, 4) == "4822")
                || (isset($main['projectID']) && $main['projectID'] > 0)
                || (isset($main['project_id']) && $main['project_id'] > 0));

            if ($is_project_invoice && empty($items5_sumRegistries)) {
                $proj_id = isset($main['projectID']) && $main['projectID'] > 0 ? (int)$main['projectID'] : (isset($main['project_id']) && (int)$main['project_id'] > 0 ? (int)$main['project_id'] : 0);
                $ref_id = isset($main['refID']) && $main['refID'] > 0 ? (int)$main['refID'] : (isset($main['referenceID_1']) ? (int)$main['referenceID_1'] : 0);

                if ($proj_id == 0 && $ref_id > 0) {
                    $spk_q = $this->db->query("SELECT project_id, project_nama FROM transaksi WHERE id = ?", array($ref_id))->row_array();
                    if (!empty($spk_q) && isset($spk_q['project_id']) && (int)$spk_q['project_id'] > 0) {
                        $proj_id = (int)$spk_q['project_id'];
                    }
                }

                $proj_nama = isset($main['projectName']) && !empty($main['projectName']) ? $main['projectName'] : (isset($main['project_nama']) && !empty($main['project_nama']) ? $main['project_nama'] : '');
                if (empty($proj_nama) && $proj_id > 0) {
                    $p_q = $this->db->query("SELECT project_nama FROM transaksi WHERE project_id = ? AND trash = '0' LIMIT 1", array($proj_id))->row_array();
                    if (!empty($p_q) && isset($p_q['project_nama']) && !empty($p_q['project_nama'])) {
                        $proj_nama = $p_q['project_nama'];
                    }
                }
                if (empty($proj_nama)) {
                    $proj_nama = "Proyek " . $tmpTr[0]->nomer;
                }

                $target_dpp = isset($main['dpp_ppn']) && (float)$main['dpp_ppn'] > 0 ? (float)$main['dpp_ppn'] : ((float)$tmpTr[0]->transaksi_nilai > 0 ? (float)$tmpTr[0]->transaksi_nilai : (isset($main['harus_bayar']) ? (float)$main['harus_bayar'] : 0));
                $target_ppn = isset($main['grand_ppn']) && (float)$main['grand_ppn'] > 0 ? (float)$main['grand_ppn'] : (isset($main['ppn']) && (float)$main['ppn'] > 0 ? (float)$main['ppn'] : round($target_dpp * 0.11));

                $main['dpp_ppn'] = $target_dpp;
                $main['grand_ppn'] = $target_ppn;
                $main['nett1'] = $target_dpp;
                $main['ppn_out_bulat'] = $target_ppn;
                $main['grand_pembulatan'] = $target_dpp + $target_ppn;
                $main['grand_total'] = $target_dpp + $target_ppn;

                $komposisi_rows = array();
                if ($proj_id > 0) {
                    $komposisi_rows = $this->db->query("SELECT * FROM project_sub_tasklist_komposisi WHERE fase_id = ? AND trash = '0' ORDER BY id ASC", array($proj_id))->result_array();
                }

                $sub_fase_id = $proj_id > 0 ? $proj_id : $trID;
                $items5_synthetic = array();

                if (!empty($komposisi_rows)) {
                    $total_orig_saldo = 0;
                    foreach ($komposisi_rows as $kr) {
                        $total_orig_saldo += (float)$kr['saldo'];
                    }

                    $produk_list = array();
                    $biaya_list = array();
                    $allocated_sum = 0;
                    $count_items = count($komposisi_rows);
                    $idx = 0;

                    foreach ($komposisi_rows as $kr) {
                        $idx++;
                        $orig_saldo = (float)$kr['saldo'];
                        if ($total_orig_saldo > 0) {
                            if ($idx == $count_items) {
                                $scaled_saldo = $target_dpp - $allocated_sum;
                            } else {
                                $scaled_saldo = round(($orig_saldo / $total_orig_saldo) * $target_dpp);
                                $allocated_sum += $scaled_saldo;
                            }
                        } else {
                            $scaled_saldo = round($target_dpp / $count_items);
                            $allocated_sum += $scaled_saldo;
                        }

                        $qty = (float)$kr['jml'] > 0 ? (float)$kr['jml'] : 1;
                        $scaled_unit_price = round($scaled_saldo / $qty);

                        $entry = array(
                            'id' => $kr['id'],
                            'produk_dasar_id' => $kr['produk_dasar_id'],
                            'produk_dasar_nama' => $kr['produk_dasar_nama'],
                            'jml' => $qty,
                            'satuan' => !empty($kr['satuan']) ? $kr['satuan'] : 'Unit',
                            'harga' => $scaled_unit_price,
                            'saldo' => $scaled_saldo,
                            'jenis' => $kr['jenis'],
                            'cat_nama' => $kr['cat_nama'],
                        );
                    ...
                    $items5_synthetic[$sub_fase_id] = array(
                        'id' => $sub_fase_id,
                        'produk_nama' => $proj_nama,
                        'harga' => $target_dpp,
                        'bahan_baku' => array(
                            'produk' => $produk_list,
                            'biaya' => $biaya_list
                        ...
                        'bahan_baku' => array(
                            ...
                            'biaya' => array(
                                array(
                                    'id' => $sub_fase_id,
                                    'produk_dasar_id' => 0,
                                    'produk_dasar_nama' => "Termin Proyek " . $proj_nama,
                                    'jml' => 1,
                                    'satuan' => 'LOT',
                                    'harga' => $target_dpp,
                                    'saldo' => $target_dpp,
                ...
                $items5_sumRegistries = $items5_synthetic;
            }
            // END OF COMPLETE REPEATED LOGIC

            //jika project
            if (!empty($items5_sumRegistries)) {
                $main['nett1'] = isset($main['dpp_ppn']) ? $main['dpp_ppn'] : 0;
                if (!isset($main['ppn_out_bulat'])) {
                    $main['ppn_out_bulat'] = isset($main['grand_ppn']) ? $main['grand_ppn'] : (isset($main['ppn']) ? $main['ppn'] : 0);
                }
                if (!isset($main['grand_pembulatan']) || empty($main['grand_pembulatan'])) {
                    if (isset($main['grand_total']) && (float)$main['grand_total'] > 0) {
                        $main['grand_pembulatan'] = $main['grand_total'];
                    }
                    elseif (isset($main['new_net3']) && (float)$main['new_net3'] > 0) {
                        $main['grand_pembulatan'] = $main['new_net3'];
                    }
                    elseif (isset($main['piutang_dagang']) && (float)$main['piutang_dagang'] > 0) {
                        $main['grand_pembulatan'] = $main['piutang_dagang'];
                    }
                    elseif (isset($main['piutang_usaha']) && (float)$main['piutang_usaha'] > 0) {
                        $main['grand_pembulatan'] = $main['piutang_usaha'];
                    }
                    else {
                        $main['grand_pembulatan'] = (isset($main['nett1']) ? (float)$main['nett1'] : 0) + (isset($main['ppn_out_bulat']) ? (float)$main['ppn_out_bulat'] : 0);
                    }
                }
            }
            ...
            if (!isset($main['grand_pembulatan']) || empty($main['grand_pembulatan'])) {
                ...
                elseif (isset($main['piutang_usaha']) && (float)$main['piutang_usaha'] > 0) {
                    ...
                    $dpp_v = isset($main['nett1']) ? (float)$main['nett1'] : (isset($main['dpp_ppn']) ? (float)$main['dpp_ppn'] : 0);
                    $ppn_v = isset($main['ppn_out_bulat']) ? (float)$main['ppn_out_bulat'] : (isset($main['grand_ppn']) ? (float)$main['grand_ppn'] : (isset($main['ppn']) ? (float)$main['ppn'] : 0));
                    if ($dpp_v > 0) {
                        $main['grand_pembulatan'] = $dpp_v + $ppn_v;
                    }
                }
            }

//            cekMerah("items5_sumRegistries");
//            arrPrintWebs($items5_sumRegistries);
            // matiHEre("stopppdulu brooo");
            //cekBiru(count($tmpTr));
//            cekBiru($mainElements);
//            arrPrintPink(blobDecode($mainElements['customerDetails']['contents']));
...
//            arrPrintPink($itemsRegistries);
            ...
            * -------------------------------------------------*/
            ...
            // arrPrintHijau($receiptSumFields);
            // arrPrint($elementConfigs);
            // $itemLabels = $receiptDetailFields;
            $itemLabels2 = $receiptDetailFields2;
            $itemSrcLabel = $receiptDetailSrcFields;
            if (($mainJenis == "paket") && ($tmpTr[0]->jenis_master == "1582" || $tmpTr[0]->jenis_master == "4822")) {
                ...
                foreach ($tmpTr as $row) {
                    ...
                    if (isset($itemsRegistries[$id])) {
                        ...
                        $kategori_id_items = isset($itemsRegistries[$id]["kategori_id"]) ? $itemsRegistries[$id]["kategori_id"] : 0;
                    }
//                     arrPrint($tmp);
                    ...
                    $rawItems[$row->produk_id] = array_replace(array_filter($childTableInValueParams[$row->produk_id]), array_filter($rawItems[$row->produk_id]));
                    // arrPrint($rawItems);
                    $rawItems[$row->produk_id]["kategori_id"] = $kategori_id_items;
...
//                    arrprint($rawItems);
        ...
        foreach ($rawItems as $pids => $item) {
            ...
            foreach ($arrKombinasi as $old_ky => $new_items) {
                ...
                $old_nilai1 = isset($item[$old_ky2]) ? $item[$old_ky2] : 0;
                $old_qty = isset($item['jml']) ? $item['jml'] : 0;
        ...
        $arrCompanyProfile = array();
        if (!empty($arrTmpCompany)) {
            foreach ($arrTmpCompany as $key => $val) {
                if (is_array($val) || is_object($val)) {
                    foreach ($val as $subKey => $subVal) {
                        $arrCompanyProfile['companyProfile_' . $subKey] = $subVal;
                    }
                } else {
                    $arrCompanyProfile['companyProfile_' . $key] = $val;
                }
        ...
        if (sizeof($elementFixedConfig) > 0) {
            ...
            if (sizeof($elementHideFixedConfig) > 0) {
                foreach ($elementHideFixedConfig as $hideSpec) {
                    if (!is_array($hideSpec) || !isset($hideSpec['key']) || !isset($hideSpec['keyResult']) || !is_array($hideSpec['keyResult'])) {
                        continue;
                    }
                    $keyResult = isset($globalVars[$hideSpec['key']]) ? $globalVars[$hideSpec['key']] : "";
                    if (in_array($keyResult, $hideSpec['keyResult'])) {
...
//        arrPrintPink($receiptInWordConfig);
//cekHere("$in_word");
        $in_word2 = "";
...
// arrPrint($itemLabels);
        ...
        if ($referenceNumber_current_explode == "4464") {
            $jenis_source = $main["jenis_source"];
            $jenis_source = $referenceNumber_current_explode;
//            cekHere("[$jenis_source]");
//            arrPrintHitam($main);
            // $itemLabels = isset($this->configLayout[$this->jenisTr]['receiptReference'][$jenis_source]['receiptDetailFields'][$currentStepNum]) ? $this->configLayout[$this->jenisTr]['receiptReference'][$jenis_source]['receiptDetailFields'][$currentStepNum] : array();
            // $itemNumLabels = isset($this->configLayout[$this->jenisTr]['receiptReference'][$jenis_source]['receiptNumFields'][$currentStepNum]) ? $this->configLayout[$this->jenisTr]['receiptReference'][$jenis_source]['receiptNumFields'][$currentStepNum] : array();
            $receiptSumFields = $receiptSumFieldsConf = isset($this->configLayout[$this->jenisTr]['receiptReference'][$jenis_source]['receiptSumFields'][1]) ? $this->configLayout[$this->jenisTr]['receiptReference'][$jenis_source]['receiptSumFields'][1] : array();
            // $itemLabels = $itemLabels + $itemNumLabels + array("nett" => "subtotal");
            $itemLabels = $itemLabels + $itemNumLabels;
//
...
//            arrPrintWebs($receiptSumFields);
            ...
            $itemFinals = array();
            $items = array();
            foreach ($items_00 as $sub_ref_id => $item_params) {
//                arrPrintCyan($item_params);
                if (isset($item_params["kategori_id"]) && ($item_params["kategori_id"] == 0)) {
//                    cekHitam("referensi nota");
                    $tmpReg_00 = $tr->lookupDataRegistriesByMasterID($sub_ref_id)->result();
//                    showLast_query("hijau");
                    if (sizeof($tmpReg) > 0) {
                    ...
                    $receiptSumFields_reference = $receiptSumFieldsConf;
                    $mainRegistries_reference = $mainRegistries;

                }
                else {
                    cekHitam("referensi barang");
                    $receiptSumFields_reference = array();
                    $mainRegistries_reference = array();
                }


                $mainFinals[$sub_ref_id] = $main;
                // arrPrintKuning($itemsRegistries);
                //--------
                $rawItems = array();
                foreach ($itemsRegistries as $row_00) {
                    $row = (object)$row_00;
                    // arrPrintHijau($row);
                    $id = $row->produk_id;
                    //                    cekBiru($id);
                    ...
                    $tmp["produk_ord_jml"] = $row->jml;
                    $tmp["produk_nama"] = $row->nama;
                    $tmp["subtotal"] = $subtotal;

                    $rawItems[$row->id] = $tmp;
                    $rawItems[$row->id] = array_replace(array_filter($childTableInValueParams[$row->id]), array_filter($rawItems[$row->id]));
                    // arrPrint($rawItems);
                    ...
                    if ((is_array($receiptDetailFieldsReplacerConfig) && (sizeof($receiptDetailFieldsReplacerConfig) > 0))) {
                        foreach ($receiptDetailFieldsReplacerConfig as $key => $vSpec) {
                            if (array_key_exists($key, $rawItems[$row->id])) {
                                $rawItems[$row->id][$key] = isset($vSpec[$rawItems[$row->id][$key]]) ? $vSpec[$rawItems[$row->id][$key]] : "";
                            }
                        }
                    }
                    ...
                    foreach ($availValueKeys as $ak) {
                        //                        echo "checking $ak: ";
                        if (isset($rawItems[$row->id][$ak]) && is_numeric($rawItems[$row->id][$ak])) {
                            //                        echo "ada ";
                            $valueKey = $ak;
                        }
                        else {
...
//            cekHitam();
        ...
        // arrPrintKuning($items_00);
        // arrPrintPink($items);
        // arrPrintWebs($itemFinals);
//        arrPrintWebs($receiptSumFields);
//        cekMerah(__LINE__);
        ...
        $phones = array();
        if (!empty($decoded['tlp_1'])) $phones[] = $decoded['tlp_1'];
        if (!empty($decoded['tlp'])) $phones[] = $decoded['tlp'];
        if (!empty($decoded['tlp_2'])) $phones[] = $decoded['tlp_2'];
        $no_tujuan = implode(',', $phones);
        $path = defined('DOC_ROOT') ? DOC_ROOT . "/ctemp" : FCPATH . "ctemp";
        $cookie_file_path = $path . "/cookie.txt";
        $https_url = "https://cdn.mayagrahakencana.com/";
        $modulPathParts = explode('/', rtrim(parse_url(MODUL_PATH, PHP_URL_PATH), '/'));
        $nota_prefix = end($modulPathParts);
        $postValues = array(
            'no' => $no,
            'baseUrl_' => $_SERVER['HTTP_HOST'],
            'folder_prefix' => "",
            'nota_prefix' => $nota_prefix,
...
//arrPrint($main);
        $data = array(
            ...
            "items5" => isset($items5_sumRegistries) ? $items5_sumRegistries : $items5,
            "confLayout" => $confLayout,
            "template" => $this->configLayout[$jenisTr]["receiptTemplate"][$currentStepNum],
            "title" => $this->configUi[$jenisTr]["steps"][$currentStepNum]["label_nota"],
            "jenisTr" => $jenisTr,
            "companyProfile" => $arrTmpCompany,
            "srckBillDatas" => $srckBillDatas,
            "pihakLabel" => $this->configUi[$jenisTr]["pihakLabel"],
            "mainLabels" => $this->configLayout[$jenisTr]["receiptMainFields"],
            "main" => $mainProp,
            "mainFinals" => isset($mainFinals) ? $mainFinals : array(),
            "mainValues" => $main,
            "pembayaran_status" => $pembayaran_status,
            "shippingMethod" => isset($main['shippingMethod']) ? $main['shippingMethod'] : "",
            "itemLabels" => $itemLabels,
            "items" => $items,
            "itemFinals" => $itemFinals,
            "itemsRegistries" => $itemsRegistries,
            ...
            "logPrint" => $logPrint,
            "receiptSumFields_reference" => isset($receiptSumFields_reference) ? $receiptSumFields_reference : array(),
            "mainRegistries_reference" => isset($mainRegistries_reference) ? $mainRegistries_reference : array(),
            "statusInvoice" => ($statusInvoice == 1) ? watermarkRejected(1, $statusInvoice) : "",
