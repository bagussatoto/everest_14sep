...
//            pageResize: true,
    ...
    case "createForm":
        ...
        $pihakLabelNote = isset($pihakLabelNote) ? $pihakLabelNote : 'pilih';
        $pihakLabel2Note = isset($pihakLabel2Note) ? $pihakLabel2Note : 'pilih';
...
//            $strOnprogFooter = "<a class='btn btn-default' href='" . base_url() . $this->uri->segment(1) . "/viewIncomplete/" . $jenisTr . "'><span class='glyphicon glyphicon-time'></span> complete list ...</a>";
        ...
        $p->addTags(
            array(
                ...
                "pihak_label_note" => $pihakLabelNote,
                "pihak_label2" => isset($pihakLabel2) ? $pihakLabel2 : "",
                "pihak_label2_note" => isset($pihakLabel2Note) ? $pihakLabel2Note : "",
                ...
                //---------------
                "pihak_projek_label" => isset($pihakProjekLabel) ? $pihakProjekLabel : "",
                "pihak_projek_caller" => isset($pihakProjekCaller) ? $pihakProjekCaller : "",
                "pihak_projek_caller_delete" => isset($pihakProjekCallerDelete) ? $pihakProjekCallerDelete : "",
                //---------------
                "requestReferenceDisabled" => $requestReferenceDisabled,
                //----
                "time_line" => $time_line,
            )
        );
...
//        }
    ...
    case "viewUndoneItemsIndex":
        ...
        $stepper = isset($_GET['step']) ? ($isMobile == true ? 1 : $_GET['step']) : 1;
        if (isset($_GET['step'])) {
//            arrPrint($steps[$_GET['step']]);
        }
...
//        }
        ...
        //region onprogress
        // cekHere("$isMobile");
        $class_hidden = $isMobile == true ? "hidden-xs hidden-sm" : "";
        $strOnprog = "";

        $strOnprog .= "<div class=\"box box-danger $cslass_hidden\">";
...
//            cekBiru("BAWAH");
        ...
        /*---transaksi perlu action-----*/
//        cekHere("$isMobile === $stepper");
//    arrPrintPink($arrayOnProgress);
        if (sizeof($arrayOnProgress) > 0) {
            // $isMobile = true;
            // arrPrintWebs($arrayOnProgress);

            if (($isMobile == true) & ($stepper == 1)) {
...
//                     arrPrintKuning($val);
                    ...
                    $strOnprogMbl .= "<p> Nomer: " . $val['nomer'] . "</p>";
                    $strOnprogMbl .= "<p>Tujuan Distribusi: " . $val['cabang2_nama'] . "</p>";
//                    $strOnprogMbl .= "<p>Pengirim: " . $val['pengirim_nama'] . "</p>";
                    $strOnprogMbl .= "</div>";
...
//                $strOnprogFooter = "";
        ...
        //endregion
//cekHere($strOnprogMbl);
//cekhere("::: [$isMobile] $stepper :::");
        $str .= $strOnprog;
        if ($isMobile == true & $stepper == 1) {
            $link = "";
            if (isset($link_scan_mobile) && ($link_scan_mobile != NULL)) {
                $link = "<button class='btn btn-warning' type='button' 
                    onclick=\"location.href='$link_scan_mobile'\">";
                $link .= "<span class='glyphicon glyphicon-qrcode'></span> Scan QR ";
                $link .= "</button>";
            }
            $str = "<div class='text-uppercase'><h3>SCAN PRODUK <span class='pull-right'>$link</span></h3></div>";
            $str .= $strOnprogMbl;
        }
        echo $str;
        break;
...
//arrPrint($items);
//arrPrint($arrSubDetailDataKolom["nama"]);
...
//                        }
                    ...
                    if (isset($arrSubDetailDataKolom["nama"][$id])) {
                        ...
                        foreach ($arrSubDetailDataKolom["nama"][$id] as $sku => $sku_data) {
                            ...
                            echo "$sku : " . formatField_he_format("serial", $sku_data) . "<br>";
                        }
                        echo "</td>";
                        echo "</tr>";
                    }
...
//                    }
                ...
                if ((isset($itemLabels3)) && (sizeof($itemLabels3) > 0)) {
                    if (isset($items3) && sizeof($items3) > 0) {
                        echo "<div class='table-responsive'>";
                        echo "<table class='table table-bordered table-condensed'>";
                        echo "<tr bgcolor='#f5f5f5'>";
...
//arrPrintPink($tabHistoryFields);
        ...
        if (sizeof($arrayOnProgress) > 0 || sizeof($arrayOnProgress2) > 0) {
            if (sizeof($tabHistoryFields) > 0) {
                $link = MODUL_PATH . "Transaksi/viewRequestItems/2587";
                $str .= "<div class=\"title\"><h3><a href='JavaScript:void(0)' onclick=\"$('#requestItems').load('$link')\">Pengembalian Barang</a></h3></div>";
                $str .= "<div class=\"nav-tabs-custom\">";
                $str .= "<ul class=\"nav nav-tabs\">";
                $no1 = 1;
                foreach ($tabHistoryFields as $ky => $arrLab) {
...
//                                        $nilai_l = isset($tabFieldsAttr[$kyLabel]['link']) ? "<a href='JavaScript:Void(0);' onclick=\"$btl_link\">$nilai_f</a>" : $nilai_f;
                                        ...
                                        if ($link_modal == true) {
                                            ...
                                            if (isset($tabFieldsAttr[$kyLabel]['link'])) {
                                                $btl_link = "document.getElementById('result').src='" . MODUL_PATH . $tabFieldsAttr[$kyLabel]['link'] . "?selector&id=$sup_id'";
                                        ...
                                        foreach ($tabFieldsItems[$ky] as $kyLabel => $row_label) {
                                            if (isset($row[$kyLabel])) {
                                                ...
                                                if ($link_modal == true) {
                                                    ...
                                                    if (isset($tabFieldsAttr[$kyLabel]['link'])) {
//                                                        cekHere($tabFieldsAttr[$kyLabel]['link']);
