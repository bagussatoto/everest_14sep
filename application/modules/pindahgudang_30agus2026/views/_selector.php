...
//                            cekUngu("bawah");
                            ...
                            switch ($cCode) {
                                ...
                                case "_TR_461":
                                    ...
                                    break;
                                default:
                                    if (isset($iSpec['no_pinjam']) && ($iSpec['no_pinjam'] == 1)) {
                                        $itemNamaSafe = addslashes(isset($iSpec['nama']) ? strip_tags($iSpec['nama']) : "Produk");
                                        $strContent .= "<a href='javascript:void(0);' class='text-muted' onclick=\" BootstrapDialog.show({
                                                            title: 'Perhatian !',
                                                            message: 'Produk <b class=\'text-red\'>" . $itemNamaSafe . "</b> tidak memiliki saldo pinjaman (Qty Pinjam: 0). Tidak dapat dimasukkan ke daftar pengembalian.',
                                                            type: BootstrapDialog.TYPE_WARNING
                                                        }); \">";
                                    }
                                    elseif (isset($iSpec['qty_pinjam']) && $iSpec['qty_pinjam'] > 0 && isset($iSpec['stok_fisik']) && $iSpec['stok_fisik'] <= 0) {
                                        $itemNamaSafe = addslashes(isset($iSpec['nama']) ? strip_tags($iSpec['nama']) : "Produk");
                                        $satuanSafe = isset($iSpec['satuan']) ? $iSpec['satuan'] : '';
                                        $strContent .= "<a href='javascript:void(0);' class='text-muted' onclick=\" BootstrapDialog.show({
                                                            title: 'Stok Fisik Kosong !',
                                                            message: 'Produk <b class=\'text-red\'>" . $itemNamaSafe . "</b> tercatat memiliki saldo pinjaman (<b>" . $iSpec['qty_pinjam'] . " " . $satuanSafe . "</b>), namun <b>stok fisik di gudang saat ini 0</b>.<br>Tidak dapat diproses pengembalian.',
