                ...
                foreach ($itemLabels as $key => $label) {
                    ...
                    switch ($listMode[$key]) {
                        ...
                        case "text":
                            $add_data_val = "";
                            if (is_array($label)) {
                                $add_data_val = isset($iSpec[$label["addKey"]]) ? "<br><small class='text-muted'>" . $iSpec[$label["addKey"]] . "</small>" : "";
                            }
                            if (is_numeric($fieldVal)) {
                                echo "<span keyid=$keyID noid=$noID id=$colID class='text-right' style='color:$color;display:block;padding:6px 12px;background:#f0f0f0;border-radius:4px;'>" . niceDecimal($fieldVal) . "$add_data_val</span>";
                            }
                            else {
                                echo "<span keyid=$keyID noid=$noID id=$colID style='color:$color;display:inline-block;padding:6px 0;'>" . formatField($key, $fieldVal) . "$add_data_val</span>";
                            }
                            break;
                    }
                    if (sizeof($shoppingCartFieldsLink) > 0) {
                        if (isset($shoppingCartFieldsLink[$key]) && ($iSpec["jml_intransit"] > 0)) {
                            $p_nama = $iSpec['nama'];
                            $p_link = $shoppingCartFieldsLink[$key] . "&id=$iID";
                            $anu = "<button type=\"button\" class=\"btn btn-link\" onclick=\"top.BootstrapDialog.closeAll();
                            top.BootstrapDialog.show(
                                   {
                                       title:'$p_nama',
                                       message: $('&lt;div&gt;&lt;/div&gt;').load('$p_link'),
                    ...
                    if (sizeof($pairedItemField) > 0) {
                        foreach ($pairedItemField as $key => $label) {
                            ...
                            switch ($listMode[$key]) {
                                ...
                                case "text":
                                    if (is_numeric($fieldVal)) {
                                        echo "<span class='text-right' style='color:$color;display:block;padding:6px 12px;background:#f0f0f0;border-radius:4px;'>" . niceDecimal($fieldVal) . "</span>";
                                    }
                                    else {
                                        echo "<span class='text-left' style='color:$color;display:inline-block;padding:6px 0;'>" . str_replace(" ", "&nbsp;", $fieldVal) . "</span>";
                                    }
                                    break;
                            }
                            echo "</td>";
...
//                arrPrint($addRowHiddens[$k]);
        ...
        if (isset($label_info) && ($label_info != NULL)) {
            echo "<div class='alert alert-info' style='margin-top: 10px;font-size: 20px;'>";
            echo "<span>$label_info</span>";
            echo "</div>";
        }
...
//
//        echo "<script>
//            $('#pilihan_kiri').load(' #pilihan_kiri > *');
//        </script>";
    }
    else {
