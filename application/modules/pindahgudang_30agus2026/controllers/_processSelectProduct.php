                ...
                if (isset($lockerConfig['enabled']) && $lockerConfig['enabled'] == true) {
                    ...
                    if (sizeof($tmpC) > 0) {
                        ...
                        foreach ($tmpC as $row) {
                            ...
                            // region validasi saldo pinjaman (khusus transaksi pengembalian pinjam barang)
                            if (isset($this->configUi[$this->jenisTr]['pairMakers'][1]['stokPinjam']) || $this->jenisTr == "2587") {
                                ...
                                $csPinjam->addFilter("extern_id='$id'");
                                $csPinjam->addFilter("cabang_id=" . $pre__cabang_id);
                                $csPinjam->addFilter("gudang_id=" . $pre__gudang_id);
                                $csPinjam->addFilter("periode='forever'");
                                $tmpPinjam = $csPinjam->lookupAll()->result();
                                $qtyPinjam = 0;
                                if (sizeof($tmpPinjam) > 0) {
                                    $qtyPinjam = isset($tmpPinjam[0]->qty_kredit) ? (float)$tmpPinjam[0]->qty_kredit : 0;
                                }

                                if ($qtyPinjam <= 0) {
                                    $msg = "Produk <b class='text-red'>" . htmlspecialchars($kode . " " . $nama) . "</b> tidak memiliki kuantitas pinjaman (Qty Pinjam: 0).<br>Produk tidak dapat dimasukkan ke daftar pengembalian.";
                                    $alerts = array(
                                        "type" => "warning",
                                        "title" => "TIDAK ADA SALDO PINJAMAN",
                                        "html" => $msg,
                                    );
                                    echo swalAlert($alerts);
                                    die($msg);
                                ...
                                if ($jml_diperlukan > $qtyPinjam) {
                                    $msg = "Jumlah pengembalian ($jml_diperlukan $satuan) melebihi kuantitas pinjaman ($qtyPinjam $satuan) untuk produk <b class='text-red'>" . htmlspecialchars($kode . " " . $nama) . "</b>.";
                                    $alerts = array(
                                        "type" => "warning",
                                        "title" => "MELEBIHI SALDO PINJAMAN",
                                        "html" => $msg,
                                    );
                                    echo swalAlert($alerts);
                                    die($msg);
                            ...
                            // endregion validasi saldo pinjaman
...
//                            cekKuning("[pre__cabang_id: $pre__cabang_id] [pre__gudang_id: $pre__gudang_id]");
