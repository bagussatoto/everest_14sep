...
//             mati_disini("LOLOS, tidak kena VALIDATE :: $prevAction");
    ...
    public function viewUndoneItemsIndex()
    {
        ...
        $authAllowedTR = array(
            "1587a" => array("gudang_id" => "gudang_nama"),
            "1587" => array("gudang2_id" => "gudang2_nama"),
        );
        // arrPrint($authAllowed);
...
//            matiHere(__LINE__);
            ...
            foreach ($tmpHist as $row) {
                ...
                //region tambahan pindah antar gudang puat supaya tidak overwrite hak akses
                if (isset($authAllowedTR[$nextStepCode])) {
                    // arrPrint($authAllowedTR[$nextStepCode]);
                    foreach ($authAllowedTR[$nextStepCode] as $keyGudangID => $gudangIDVal) {
                        if ($this->session->login["gudang_id"] == $row->$keyGudangID) {
                            $allowFollowup = true;
                            $btnColor = "btn-primary";
                            $actionLabel = $this->configUi[$jenisTr]['steps'][$nextStepNum]['actionLabel'];
                        }
                            ...
                            $actionLabel = "waiting to receive on " . $row->$gudangIDVal;
                        }
                    }
                }
                //endregion
                ...
                if ($allowFollowup) {
                    ...
                    cekHere(__LINE__ . " $allowScaner // $nextStepNum");
                    $tmp['action'] = "<div class='input-group'>";
                    if ($allowScaner == false || $isMob == 1) {
                        ...
                        // $targetFollowupLink = "followupPrePreview";
                        $followupLink = "top.$('#result').load('" . MODUL_PATH . "FollowUp/$targetFollowupLink/$jenisTr/" . $row->transaksi_id . "/$nextStepNum/" . $row->sub_step_number . "');";
    ...
    public function viewRequestItems()
    {
        ...
        if (isset($this->configUi[$this->uri->segment(4)]['tabRequestCode'])) {
            ...
            if ($pakai_ini == 1) {
                ...
                $rs = New ComRekeningPembantuPinjamBarang();
                $rs->addFilter("periode='forever'");
                $rs->addFilter("cabang_id=" . my_cabang_id());
                $rs->addFilter("gudang_id=" . my_gudang_id());
                $rsTmp = $rs->lookupAll()->result();
                showLast_query("biru");
                cekHitam("data ada " . count($rsTmp));
            }
...
//arrPrintKuning($tabHistoryFields);

            if (sizeof($rsTmp) > 0) {
                $arrIds = array();
                $arrProdukData = array();
                foreach ($rsTmp as $row) {
                    $arrIds[] = $row->extern_id;
                }
                $sp = New MdlProduk();
                $sp->setFilters(array());
                $sp->addFilter("id in ('" . implode("','", $arrIds) . "')");
                $spTmp = $sp->lookupAll()->result();
                foreach ($spTmp as $spSpec) {
                    $arrProdukData[$spSpec->id] = $spSpec;
                }
...
//                                    $qty_stock = isset($stockSupplies[$rows->id]) ? $stockSupplies[$rows->id] : 0;
//                                    $qty_sales_order = isset($arrDataRequest['h582so'][$rows->id]['qty_kredit']) ? $arrDataRequest['h582so'][$rows->id]['qty_kredit'] : 0;
//                                    $qty_purchase = isset($arrDataRequest['1467'][$rows->id]['qty_kredit']) ? $arrDataRequest['1467'][$rows->id]['qty_kredit'] : 0;
//                                    $qty_purchase_request = isset($arrDataRequest['1466'][$rows->id]['qty_kredit']) ? $arrDataRequest['1466'][$rows->id]['qty_kredit'] : 0;
//                                    $qty_buffer = 0;
//                                    // $qty_new_order = ($qty_sales_order - $qty_stock + $qty_buffer) - $qty_purchase;
//                                    $qty_new_order = $qty_stock - $qty_sales_order + $qty_purchase - $qty_buffer + $qty_purchase_request;
//                                    if ($qty_new_order > 0) {
//                                        $qty_new_order = 0;// tidak perlu order bahan baku
//                                    }
//                                    else {
//                                        $qty_new_order = $qty_new_order * -1;// perlu order bahan baku
//                                    }
//
//
//                                    $supplier_id = isset($arrProdukSupplier[$rows->id]) ? $arrProdukSupplier[$rows->id] : 0;
//                                    $supplier_nama = isset($arrSupplier[$supplier_id]) ? $arrSupplier[$supplier_id] : 0;
//                                    $po_link = base_url() . "laporan/Pembelian";

//                                    $qty_outstanding = $qty_purchase - $qty_stock;
                                    $link_data = base_url() . "fff";
                                    $btn_po = modalDialogBtn('hahaah', $link_data);


                                    $clickEventMulti = "onclick=\"initBtn();resetCalcCheckAll()\"";
                                    $id = $rows->extern_id;
                                    $vendorID = "";
                                    $link_ceklist = base_url() . "Bi/checklistBi/$vendorID/?mode=item&pid=$id";
                                    $produk_id = array(
                                        "select" => "<input type='$inputType' povalue='$qty_new_order' id='$id' name='prdID[]' class='' 
                                        onclick='initBtn();resetCalcCheckAll()' 
                                        onclicks=\"document.getElementById('result').src='$link_ceklist'\">",
                                        "id" => $id,
                                        "barcode" => isset($arrProdukData[$id]->barcode) ? $arrProdukData[$id]->barcode : "-",
                                        "nama" => isset($arrProdukData[$id]->nama) ? $arrProdukData[$id]->nama : "-",
                                        "satuan" => isset($arrProdukData[$id]->satuan) ? $arrProdukData[$id]->satuan : "-",
//                                        "stock" => $qty_stock,
//                                        "sales_order" => $qty_sales_order,
//                                        "stok_buffer" => $qty_buffer,
//                                        "purchase_request" => $qty_purchase_request,
//                                        "purchase_order" => $qty_purchase,
//                                        "new_order" => $qty_new_order,
//                                        "supplier_id" => $supplier_id,
//                                        "supplier_nama" => $supplier_nama,
                                        "qty_debet" => isset($rows->qty_debet) ? $rows->qty_debet : 0,
                                        "qty_kredit" => isset($rows->qty_kredit) ? $rows->qty_kredit : 0,
                                    );

                                    $tmpTabData[$key][$rows->extern_id] = $produk_id;
                                    if (isset($rows->qty_kredit) && ($rows->qty_kredit > 0)) {
                                        $requestProduk[$rows->extern_id] = isset($rows->qty_kredit) ? $rows->qty_kredit : 0;
                                    }
                                    break;
                                case "supplier_id":
                                    $inputType = $allowMultiSelect == true ? "checkbox" : "radio";
...
//                                $qty_stock = 0;
                                    ...
                                    $produk_id = array(
                                        "select" => "<input type='$inputType' povalue='$qty_new_order' id='$id' name='prdID[]' class='' 
                                        onclick='localStorage.setItem(\"selectedSupplier\",$supplier_id);initBtn();resetCalcCheckAll()' 
                                        onclicks=\"document.getElementById('result').src='$link_ceklist'\">
                                        <input type='hidden' value='$supplier_id' name='nsupplier'>",
                                        "id" => $rows->id,
                                        "nama" => isset($rows->nama) ? $rows->nama : "-",
...
//                                cekHere($rows->id . " :: $supplier_id ::");
                                    $tmpTabData[$key][$supplier_id][$rows->id] = $produk_id;
                                    $tmpTabData[$key][-100][$rows->id] = $produk_id;
                                    $requestSupplier[$supplier_id][$rows->id] = $qty_new_order;
                                    break;
                            }
                        }
                    }
...
//        arrPrintWebs($requestProduk);

        $btnShopCrt = MODUL_PATH . "_processSelectProduct/multiSelectBi/$jenisTr";
        $data = array(
            ...
            "clearCartTarget" => MODUL_PATH . "_shoppingCart/reset/" . $this->jenisTr,
            "scriptBottom" => "",
            "btnToShoppingCart" => $btnShopCrt,
            "requestProduk" => $requestProduk,
        );
    ...
    public function viewResume()
    {
        ...
        $supplierNama = $tmp1[0]->suppliers_nama;
        $customerNama = $tmp1[0]->customers_nama;
        $jenisMasterTrans = $tmp1[0]->jenis_master;
        $indTransksiID = $transaksiID = $tmp1[0]->id;
        ...
        if (sizeof($tmpReg) > 0) {
            if (sizeof($tmpReg) > 0) {
                foreach ($tmpReg as $eParams) {
                    foreach ($eParams as $param => $paramVal) {
                        if ($param != "transaksi_id") {
                            $$param = blobDecode($paramVal);
                        }
                    }
                }
            }
...
//        arrPrintWebs($items3_sum);
        ...
        if (array_intersect($allowedView, $mems)) {
            ...
            //----------------------------------------------
//            arrPrintPink($items4_sum);
            if (sizeof($items4_sum) > 0) {
                $this->load->model("Mdls/MdlSupplierDiskon");
                $ds = New MdlSupplierDiskon();
                $dsTmp = $ds->lookupAll()->result();
                foreach ($dsTmp as $spec) {
                    $dsData[$spec->id] = $spec->label;
                }
//                arrPrint($dsData);
                $arrDiskonLabel = array(
                    "alias" => "nama diskon",
                    "sub_diskon_nilai" => "nilai diskon",
                );
                $arrDiskonData = array();
                foreach ($items4_sum as $spec) {
//                    arrPrintPink($spec);
                    $arrDiskonData[$spec["diskon_id"]]["id"] = $spec["diskon_id"];
                    $arrDiskonData[$spec["diskon_id"]]["nama"] = $spec["diskon_nama"];
                    $arrDiskonData[$spec["diskon_id"]]["alias"] = isset($dsData[$spec["diskon_id"]]) ? $dsData[$spec["diskon_id"]] : $spec["diskon_nama"];
                    $arrDiskonData[$spec["diskon_id"]]["diskon_nilai"] = $spec["diskon_nilai"];
                    if (!isset($arrDiskonData[$spec["diskon_id"]]["sub_diskon_nilai"])) {
                        $arrDiskonData[$spec["diskon_id"]]["sub_diskon_nilai"] = 0;
                    }
                    $arrDiskonData[$spec["diskon_id"]]["sub_diskon_nilai"] += $spec["sub_diskon_nilai"];
                }
            }
//            arrPrintHijau($arrDiskonData);
            //----------------------------------------------
...
//                    $dTmp = $m->lookupDetailTransaksi($transaksiTopID)->result();
                    ...
                    if (sizeof($notApprove) > 0) {
                        ...
                        foreach ($notApprove as $pid => $pqty_not_approve) {
                            $itemsNotApprove[$pid] = isset($items_reg[$pid]) ? $items_reg[$pid] : array();
        ...
        //-------------------------------------
        $shoppingCartSubDetailFields = isset($this->configUi[$jenisMasterTrans]['shoppingCartSubDetailFields'][$step_number]) ? $this->configUi[$jenisMasterTrans]['shoppingCartSubDetailFields'][$step_number] : array();
        if (sizeof($shoppingCartSubDetailFields) > 0) {
            $this->load->model("Mdls/MdlProdukPerSerialNumber");
            $psn = New MdlProdukPerSerialNumber();
            $psn->addFilter("transaksi_id='$indTransksiID'");
...
//                arrPrintWebs($source_new);
                foreach ($source_new as $ii => $sdata) {
                    ...
                    if (in_array($key_data, $items[$key_data_id])) {
                        $anu_flip = array_flip($items[$key_data_id]);
        ...
        //-------------------------------------
        $addMainSource = isset($this->configUi[$jenisMasterTrans]["addMainSource"][$step_number]) ? $this->configUi[$jenisMasterTrans]["addMainSource"][$step_number] : array();
        $addMainSourceField = isset($this->configUi[$jenisMasterTrans]["addMainSource"][$step_number]["fields"]) ? $this->configUi[$jenisMasterTrans]["addMainSource"][$step_number]["fields"] : array();
        $addMainSourceEdit = isset($this->configUi[$jenisMasterTrans]["addMainSource"][$step_number]["editableFields"]) ? $this->configUi[$jenisMasterTrans]["addMainSource"][$step_number]["editableFields"] : array();
//arrPrintPink($arrSubDetailDataKolom);
        //-------------------------------------
        ...
        $data = array(
            ...
            //--------------------
            "arrDiskonLabel" => isset($arrDiskonLabel) ? $arrDiskonLabel : array(),
            "arrDiskonData" => isset($arrDiskonData) ? $arrDiskonData : array(),
            "diskonTitle" => "Diskon $supplierNama ",
