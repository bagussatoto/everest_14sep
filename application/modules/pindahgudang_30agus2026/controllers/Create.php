...
//        echo ($time_line);
        ...
        $defaultPihak = isset($configUi[$this->jenisTr]['defaultPihak']) ? $configUi[$this->jenisTr]['defaultPihak'] : array();
...
//                arrPrint($selectorProcessor);
        ...
        //-----------------------------------
        if (sizeof($defaultPihak) > 0) {
            if (isset($defaultPihak["enabled"]) && ($defaultPihak["enabled"] == true)) {
                $mdlNamePihak = $defaultPihak["mdlName"];
                $this->load->model("Mdls/$mdlNamePihak");
                $mdp = New $mdlNamePihak();
                if (isset($defaultPihak["filter"])) {
                    makeFilter($defaultPihak["filter"], $_SESSION[$cCode]["main"], $mdp);
                }
                $mdpTmp = $mdp->lookupAll()->result();
                showLast_query("biru");
                if (sizeof($mdpTmp) == 1) {
                    foreach ($defaultPihak["gate"] as $key => $val) {
                        $_SESSION[$cCode]["main"][$key] = isset($mdpTmp[0]->$val) ? $mdpTmp[0]->$val : "";
                    }
                }
            }
        }
        ...
        $scriptBottom = "";
        $scriptBottom .= "<script> top.$('#historyList').load('$link_historyList'); </script>";
        //endregion


        switch ($this->jenisTr){
            case "1587":
                $link_pihak = MODUL_PATH . "_processPihak/select/$jenisTr/MdlGudang?id=9";
                $scriptBottom = "<script>
                        top.$('#result').load('$link_pihak')
                    </script>";
                break;
            case "1687":
//                $link_pihak = MODUL_PATH . "_processPihak/select/$jenisTr/MdlGudang?id=9";
...
//                    </script>";
        ...
        $data = array(
            ...
            //-----------------------
            "pihakLabelNote" => isset($this->configUi[$jenisTr]["pihakLabelNote"]) ? $this->configUi[$jenisTr]["pihakLabelNote"] : "",
            "pihakLabel2Note" => isset($this->configUi[$jenisTr]["pihakMainLabelNote"]) ? $this->configUi[$jenisTr]["pihakMainLabelNote"] : "",
    ...
    public function preview()
    {
        ...
        $data = array(
            ...
            "configUi" => $this->configUi,
            "notFormatedFields" => isset($this->configUi[$this->jenisTr]['shoppingCartNotFormatedFields'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartNotFormatedFields'][1] : array(),
        );
...
//            unset($_SESSION[$cCode]["main"]["bookingNumber"]);
            if (!isset($_SESSION[$cCode]["main"]["bookingNumber"])) {
                $msg = "Nomer Booking transaksi baru belum terdaftar. Silahkan referesh halaman ini. code: " . __LINE__;
                mati_disini($msg);
            }
...
//                mati_disini("TEST EDIT...");
            ...
            if (isset($this->configCore[$this->jenisTr]['preProcessor'][$jenisTrTarget]['detail'])) {
                ...
                $this->load->helper("he_value_builder");
                fillValues_he_value_builder($this->jenisTr, 1, 1, $this->configCoreJenis, $this->configUiJenis, $ppnFactor);
    ...
    public function swapFrom()
    {
        ...
        if (isset($_POST['trID']) && sizeof($_POST['trID']) > 0) {
            ...
            if (sizeof($tmpTr) > 0) {
                ...
                $swappedKeys = isset($this->configUi[$jenisTr]['swappedKeys']) ? $this->configUi[$jenisTr]['swappedKeys'] : array();
                $allowMultiSelect = isset($this->configUi[$jenisTr]['requestCode']['allowMultiSelect']) ? $this->configUi[$jenisTr]['requestCode']['allowMultiSelect'] : false;
                // arrPrint($swappedKeys);
                //                matiHEre($jenisTr);
                $oldMain = "";
                if (sizeof($tmpReg) > 0 && sizeof($swappedKeys) > 0) {
                    $oldMain = unserialize(base64_decode($tmpReg[0]->main));
                ...
                // $targetProc = MODUL_PATH . $this->configUi[$jenisTr]['itemSwapper'] . "/$jenisTr/?items=" . base64_encode(serialize($items)) . "&main=" . base64_encode(serialize($oldMain)) . "&trs=" . base64_encode(serialize($trIDs));
                $targetProc = MODUL_PATH . $this->configUi[$jenisTr]['itemSwapper'] . "/$jenisTr/?items=" . base64_encode(serialize($items)) . "&main=" . base64_encode(serialize($oldMain)) . "&trs=" . base64_encode(serialize($trIDs));
                ...
                // echo "top.$('#_shopingCart').load('$targetProc');";
                echo "</script>";
            }
        ...
        elseif (isset($_POST['prdID']) && sizeof($_POST['prdID']) > 0) {
            ...
            if (sizeof($tmpTr) > 0) {
                ...
                $swappedKeys = isset($this->configUi[$origJenisTr]['swappedKeys']) ? $this->configUi[$origJenisTr]['swappedKeys'] : array();
                $allowMultiSelect = isset($this->configUi[$origJenisTr]['requestCode']['allowMultiSelect']) ? $this->configUi[$origJenisTr]['requestCode']['allowMultiSelect'] : false;
                $oldMain = "";
                if (sizeof($tmpReg) > 0 && sizeof($swappedKeys) > 0) {
                    $oldMain = unserialize(base64_decode($tmpReg[0]->main));
                    foreach ($tmpReg as $rowr) {
                ...
                $targetProc = MODUL_PATH . $this->configUi[$origJenisTr]['itemSwapper'] . "/$origJenisTr/?items=" . base64_encode(serialize($items)) . "&main=" . base64_encode(serialize($oldMain)) . "&trs=" . base64_encode(serialize($trIDs));
