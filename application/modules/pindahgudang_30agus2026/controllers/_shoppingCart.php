            ...
            if (isset($_SESSION[$cCode]['items'])) {
                ...
                foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                    ...
                    $tmp = array(//                        "id"     => $iSpec['id'],
                        ...
                        "premi" => isset($iSpec['premi']) ? $iSpec['premi'] : 0,
                        "premi_percent" => isset($iSpec['premi_percent']) ? $iSpec['premi_percent'] : 0,
                        "barcode" => isset($iSpec['barcode']) ? $iSpec['barcode'] : "",
                    );
                    ...
                    $tmp["editTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "pindahgudang/", $iSpec['handler']) : '0') . "/" . $editHandlerMethod . "/" . $this->jenisTr . "?id=" . $id . "&newQty=";
                    $tmp["removeTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "pindahgudang/", $iSpec['handler']) : '0') . "/remove/" . $this->jenisTr . "?id=" . $id;
...
//            cekHijau($pernyataanNote);
        ...
        //-----------------------------
        $shoppingCartInfo = isset($this->configUi[$this->jenisTr]['shoppingCartInfo']) ? $this->configUi[$this->jenisTr]['shoppingCartInfo'] : array();
        if (isset($shoppingCartInfo) && (sizeof($shoppingCartInfo) > 0)) {
            $label_replacer = $shoppingCartInfo["replacer"];
            $label_info = $shoppingCartInfo["label_info"];
            foreach ($label_replacer as $rep) {
                $label_info = str_replace($rep, $_SESSION[$cCode]["main"][$rep], $label_info);
            }
        }
        else {
            $label_info = NULL;
        ...
        $shoppingCartFieldsLink = isset($this->configUi[$this->jenisTr]['shoppingCartFieldsLink'][1]) ? $this->configUi[$this->jenisTr]['shoppingCartFieldsLink'][1] : array();
        if (sizeof($shoppingCartFieldsLink) > 0) {
            foreach ($shoppingCartFieldsLink as $key_link => $key_spec) {
                $anu = "";
                foreach ($key_spec as $aa => $bb) {
                    if ($anu == "") {
                        $anu = $aa . "=" . makeValue($bb, $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], 0);
                    }
                    else {
                        $anu .= "&" . $aa . "=" . makeValue($bb, $_SESSION[$cCode]['main'], $_SESSION[$cCode]['main'], 0);
                    }
                }
                $shoppingCartFieldsLinkAdd[$key_link] = MODUL_PATH . get_class($this) . "/modalInfoIntransit/?$anu";
            }
        }
...
//            "sumRows2" => $sumRows2,
            ...
            "shoppingCartFieldsLink" => isset($shoppingCartFieldsLinkAdd) ? $shoppingCartFieldsLinkAdd : array(),
        );
        ...
        if (isset($_SESSION[$cCode]['items'])) {
            foreach ($_SESSION[$cCode]['items'] as $id => $item) {
                if (isset($lockerConfig['enabled']) && $lockerConfig['enabled'] == true) {
                    ...
                    $c = new $mdlName();
                    $array_hold_sebelumnya = $c->cekLoker($pre__cabang_id, $id, "hold", $this->session->login['id'], "0", $pre__gudang_id, $this->session->login['toko_id']);
                    $where = array(
                    ...
                    $c = new $mdlName();
                    $array_active_sebelumnya = $c->cekLoker($pre__cabang_id, $id, "active", "0", "0", $pre__gudang_id, $this->session->login['toko_id']);
...
//        );
    ...
    //--------------------------------------
    public function modalInfoIntransit($produk_id = "")
    {
        ...
        $tmp_2s = infoIntransit($kiriman);


        echo $this->buildProdukView($tmp_2s);
    }

    public function buildProdukView($data, $config = [])
    {

        $output = infoIntransitView($data);
    ...
    public function kembalikanStok()
    {
        ...
        $arrKiriman = array(
            "gudang_id" => $_GET["gudang_id"],
            "cabang_id" => $_GET["cabang_id"],
            "tabel_id" => $_GET["tabel_id"],
            "produk_id" => $_GET["produk_id"],
        );
        // region exec kembalikan stok

        $this->db->trans_start();

        $exec = kembalikanStokIntransit($arrKiriman);

        $this->db->trans_complete() or die("Gagal saat berusaha  commit transaction!");
        // endregion exec kembalikan stok

        // region link kembali
        $link_back = MODUL_PATH . get_class($this) . "/modalInfoIntransit/?cabang_id=$cabangID&gudang_id=$gudangID&id=$produkID";
        $back = "top.BootstrapDialog.closeAll();
                            top.BootstrapDialog.show(
                                   {
                                       title:'$produkNama',
                                       message: $('<div></div>').load('$link_back'),
        ...
        echo "<script>";
        echo " $back";
        echo "top.$('#shopping_cart').load('" . MODUL_PATH . get_class($this) . "/viewCart/" . $this->jenisTr . "?epreketek=yes');";
        echo "</script>";
        // endregion link kembali
