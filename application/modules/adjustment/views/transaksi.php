...
//cekBiru($pihakExternLabel);
        ...
        $p->addTags(
            array(
                ...
                "selector_caller4" => isset($selectorCaller4) ? $selectorCaller4 : "",
                ...
                "selector_label4" => isset($selectorLabel4) ? $selectorLabel4 : "",
...
//                $undoneLinkIndex = MODUL_PATH . "Transaksi/index/" . $this->uri->segment(4) . "?step=$tStep";
        ...
        if (sizeof($arrayOnProgress) > 0) {
            ...
            $strOnprog .= "<table id='arrayOnProgress' class='table datatables stripe compact nowarp order-column table-condensed table-bordered no-padding' style='border:solid red 0px;'>";
            $strOnprog .= "<thead>";
            ...
            $strOnprog .= "<script>
                    $(document).ready( function(){
                        var table = $('#arrayOnProgress').DataTable({
                            dom: 'lBfrtip',
                            fixedHeader: true,
                            lengthMenu: [ [10, 20, 50, 100, -1], [10, 20, 50, 100, 'All'] ],
                            pageLength: -1,
...
//arrPrintPink($subItemLabels);
//arrPrint($subAdvanceItems2);
...
//                        cekLime($keyAdd);
                    $colspanX = sizeof($itemLabels2) > 1 ? sizeof($itemLabels2) : sizeof($itemLabels);
                    echo "<tr line=" . __LINE__ . ">";
                    if(isset($shopingcartAddDpp[$keyAdd])){
//                    arrPrint($shopingcartAddDpp[$k]);
                        $colspan_i = sizeof($itemLabels) - 2;
                        foreach($shopingcartAddDpp[$keyAdd] as $ik =>$ik_label){
                            if(isset($main[$ik])){
                                $colspan_r = "1";
