...
//                    $advanceAmountItemsLabel = isset($shoppingCartAdvanceAmountValue[$key_result][$stepNumber]) ? $shoppingCartAdvanceAmountValue[$key_result][$stepNumber] : array();
        ...
        //----------------------------------------------
//arrPrintWebs($shoppingCartAdvanceSubItemsLabel);
        ...
        if (isset($_SESSION[$cCode])) {
            ...
            if (isset($_SESSION[$cCode]['items'])) {
                ...
                foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                    ...
                    $tmp["editTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "adjustment/", $iSpec['handler']) : '0') . "/" . $editHandlerMethod . "/" . $this->jenisTr . "?id=" . $id . "&newQty=";
                    $tmp["removeTarget"] = base_url() . (isset($iSpec['handler']) ? str_replace("Selectors/", "adjustment/", $iSpec['handler']) : '0') . "/remove/" . $this->jenisTr . "?id=" . $id;
...
//arrPrintWebs($shoppingCartAdvanceNumType);
        //==selector for pairedItem
        if (sizeof($pairedItem) > 0) {
            if (isset($pairedItem['enabled']) && ($pairedItem['enabled'] == true)) {
                $mdlName = $pairedItem['mdlName'];
