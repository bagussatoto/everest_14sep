...
//            $content .= ("<form name='fSelect'>");
            ...
            $content .= ("<th class='text-uppercase text-muted text-center'>No.</th>");
...
//            $content .= ("</form>");
        ...
        $content = "";
        if (isset($conten_null)) {
            $content .= "<div id='conten_null'><div class='text-center'>loading ... ... <div style='margin-left:49%;margin-top: 10px;'>$loader_indikator</div></div></div>";
            $content .= "<script>$('#conten_null').load('$conten_null');</script>";
        }
...
// cekLime($thisPage);
    ...
    case "showData":
        ...
        // arrPrint($_GET);
        $view_methode = $this->uri->segment(5);
        $link_history = MODUL_PATH . "History/viewHistory/$jenisTr";
        $content = "";

        $content .= "<div class='box box-solid box-success'>";
        $content .= "<div class='box-header text-uppercase'>";
        $content .= "<h4 class='box-title'>Riwayat transaksi: $trName</h4> <span class='meta' style='color:#ffffff;'>$limit_view</span>";
        $content .= "<div class='box-tools pull-right'>";
        if (!isset($view_methode)) {
            $content .= "<button type='button' class='btn btn-box-tool' title='melihat seluruh history' onclick=\"location.href='$link_history'\"><i class='fa fa-list'></i> klik untuk melihat history $trName</button>&nbsp;&nbsp;";
        }
        // $content .= "<button type='button' class='btn btn-box-tool' data-widget='collapse'><i class='fa fa-minus'></i></button>";


        $content .= "<a href='#link_sumber_transaksi'><i class='fa fa-list'></i> klik history lainnya</a>";
         ...
         * body isi
         * --------------------------------------*/
        $content .= "<div class='sini box-body'>";
        if (sizeof($arrayHistory) > 0) {
//            $content .= "<form name='fSelect'>";
            $content .= "<div class='table-responsive data_history_$jenisTr' sstyle='background-color: cornsilk;'>";
        ...
        if (isset($arrHistoryLink) && (sizeof($arrHistoryLink) > 0)) {
            $content .= "<div id='link_sumber_transaksi'>";
            foreach ($arrHistoryLink as $xx => $xxLink) {
                $content .= "<span style='font-style: italic;font-size:15px;'>(*) $xxLink</span><br>";
            }
            $content .= "</div>";
        }
                        ...
                        var table = $('#data_history_$jenisTr').DataTable({
                            ...
                            buttons: [
                                        { extend: 'print', footer: true },
                                        {
                                            text: 'Download Excel',
                                            action: function (e, dt, node, config) {
                                                fnExcelReport('data_history_$jenisTr');
                                            }
                                        },
                                          $custom_button
                                    ],
