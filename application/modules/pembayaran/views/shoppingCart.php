...
if (isset($items)) {
    ...
    $trJenisVal = isset($transaksi_jenis) ? $transaksi_jenis : (isset($jenisTr) ? $jenisTr : (isset($jenisTransaksi) ? $jenisTransaksi : "489"));
    $targetSaveFakturUrlGlobal = MODUL_PATH . "Create/saveFaktur/" . $trJenisVal;
    echo "<script>// START OF COMPLETE REPEATED LOGIC
        window.simpanEfakturSaja = function() {
            ...
            var saveUrl = '" . $targetSaveFakturUrlGlobal . "';
            saveUrl += '?dateFaktur=' + encodeURIComponent(dateFaktur) + '&eFaktur=' + encodeURIComponent(eFaktur) + '&skip_faktur=' + skip_faktur;
            ...
            var doSave = function() {
                ...
                console.log('MELAKUKAN AJAX GET KE:', saveUrl);
                $.ajax({
                    url: saveUrl,
                    type: 'GET',
                    dataType: 'html',
                    ...
                    error: function(jqXHR, textStatus, errorThrown) {
                        alert('AJAX Call Error: ' + textStatus + ' - ' + errorThrown);
                    }
                });
            };
        ...
        \$(document).off('blur.efakturCheck change.efakturCheck', '#eFaktur, [name=\"eFaktur\"]').on('blur.efakturCheck change.efakturCheck', '#eFaktur, [name=\"eFaktur\"]', function() {
            var \$input = \$(this);
            var eFakturVal = \$.trim(\$input.val());
            var skipChecked = (\$('#skip_faktur').length > 0 && \$('#skip_faktur').prop('checked')) || (\$('[name=\"skip_faktur\"]').length > 0 && \$('[name=\"skip_faktur\"]').prop('checked'));

            if (skipChecked || eFakturVal.length < 10) {
                return;
            }

            var checkUrl = '" . base_url() . $this->modul . "/Create/checkDuplicateEfakturAjax?eFaktur=' + encodeURIComponent(eFakturVal);
            ...
            \$.ajax({
                ...
                success: function(resp) {
                    if (resp && resp.is_duplicate) {
                        \$input.css('border-color', '#d9534f').val('');

                        var htmlContent = '<div style=\"text-align: left; background: #fff5f5; border: 1px solid #ffcdd2; padding: 14px; border-radius: 8px; margin-top: 10px; font-size: 13px; color: #333; line-height: 1.5;\">' +
                            '<div style=\"margin-bottom: 10px; font-weight: bold; color: #c62828;\">🛑 Nomor E-Faktur (\'' + eFakturVal + '\') sudah terdaftar pada sistem!</div>' +
                            '<table style=\"width: 100%; border-collapse: collapse; margin-bottom: 10px; font-size: 13px;\">' +
                            '  <tr><td style=\"padding: 3px 0; width: 140px; color: #555;\">• Digunakan Pada</td><td style=\"font-weight: bold;\">: ' + (resp.nomer || '-') + '</td></tr>' +
                            '  <tr><td style=\"padding: 3px 0; color: #555;\">• Di-entry Oleh</td><td style=\"font-weight: bold;\">: ' + (resp.oleh || '-') + '</td></tr>' +
                            '  <tr><td style=\"padding: 3px 0; color: #555;\">• Waktu Input</td><td style=\"font-weight: bold;\">: ' + (resp.dtime || '-') + '</td></tr>' +
                            '</table>' +
                            '<div style=\"font-size: 12px; color: #666; border-top: 1px dashed #ffcdd2; padding-top: 8px; margin-top: 8px;\">' +
                            '<i>Satu Nomor E-Faktur tidak boleh digunakan pada transaksi lain. Kolom E-Faktur telah dikosongkan kembali.</i>' +
                            '</div>' +
                            '</div>';

                        if (typeof top !== 'undefined' && typeof top.swal === 'function') {
                            top.swal({
                                type: 'warning',
                                title: 'Duplikasi Nomor E-Faktur Terdeteksi',
                                html: htmlContent,
                                confirmButtonColor: '#d33',
                                confirmButtonText: 'Saya Paham / Isi Ulang'
                            }, function() {
...
// END OF COMPLETE REPEATED LOGIC
    ...
    if (isset($dtp_config['enabled']) && $dtp_config['enabled']) {
        $dtp_factor = isset($main['pajak_ditanggung_pemerintah']) ? $main['pajak_ditanggung_pemerintah'] : 0;
        $dtp_percent = $dtp_factor * 100;
        $is_dtp_active = isset($main['dtp_active']) ? (bool)$main['dtp_active'] : ($dtp_factor > 0);

        $dtpChecked = ($is_dtp_active) ? 'selected' : '';
        $notDtpChecked = (!$is_dtp_active) ? 'selected' : '';
        $inputDisplay = ($is_dtp_active) ? 'block' : 'none';

        echo "<div class='row' style='margin: 10px 0;'>";
        echo "<div class='col-md-6' style='background: #fff4e5; border: 1px solid #f9a825; border-radius: 8px; padding: 15px;'>";
        echo "  <div style='color: #a66300; font-weight: bold; margin-bottom: 10px;'><i class='fa fa-cog'></i> Opsi Insentif Pajak (DTP)</div>";
        echo "  <div class='row'>";
        echo "    <div class='col-md-6' style='padding-top: 7px;'>Status DTP:</div>";
        echo "    <div class='col-md-6'>";
        echo "      <select class='form-control' style='border: 2px solid #333; font-weight: bold;' onchange=\"if(this.value == '0') { $('#result').load('" . MODUL_PATH . "_processRecordDtp/select/$transaksi_jenis?ppn_dtp=0&rate=0'); } else { $('#dtp_rate_container').slideDown(); }\">";
        echo "        <option value='0' $notDtpChecked>Tanpa DTP (Normal)</option>";
        echo "        <option value='1' $dtpChecked>Pakai Insentif Pajak (DTP)</option>";
        echo "      </select>";
        echo "    </div>";
        echo "  </div>";

        echo "  <div id='dtp_rate_container' style='margin-top: 15px; display: $inputDisplay; border-top: 1px dashed #f9a825; padding-top: 15px;'>";
        echo "    <div class='row'>";
        echo "      <div class='col-md-6' style='padding-top: 7px;'>Besaran DTP (%):</div>";
        echo "      <div class='col-md-4'>";
        echo "        <input type='number' id='input_dtp_rate' class='form-control' value='$dtp_percent' min='0' max='100' step='0.01' onclick='this.select();' style='border: 2px solid #333; font-weight: bold;'>";
        echo "      </div>";
        echo "      <div class='col-md-2 no-padding'>";
        echo "        <button class='btn btn-warning btn-sm btn-block' style='height: 34px;' onclick=\"$('#result').load('" . MODUL_PATH . "_processRecordDtp/select/$transaksi_jenis?ppn_dtp=1&rate=' + $('#input_dtp_rate').val());\">SET</button>";
        echo "      </div>";
        echo "    </div>";

        // Add Nominal Input
        $dpp_pengganti = isset($main['dpp_pengganti']) ? $main['dpp_pengganti'] : 0;
        $dtp_nominal = $dpp_pengganti * ($dtp_percent / 100);

        echo "    <div class='row' style='margin-top: 10px;'>";
        echo "      <div class='col-md-6' style='padding-top: 7px;'>Nominal DTP (Rp):</div>";
        echo "      <div class='col-md-4'>";
        echo "        <input type='text' id='input_dtp_nominal' class='form-control' value='" . number_format($dtp_nominal, 0, ',', '.') . "' onclick='this.select();' style='border: 2px solid #333; font-weight: bold;'>";
        echo "      </div>";
        echo "    </div>";

        echo "    <script>
                    $(document).ready(function() {
                        var dpp_pengganti = $dpp_pengganti;
                        
                        $('#input_dtp_rate').on('input', function() {
                            var rate = parseFloat($(this).val()) || 0;
                            var nominal = dpp_pengganti * (rate / 100);
                            $('#input_dtp_nominal').val(new Intl.NumberFormat('id-ID').format(Math.round(nominal)));
                        });

                        $('#input_dtp_nominal').on('input', function() {
                            var rawValue = $(this).val().replace(/\./g, '').replace(/,/g, '.');
                            var nominal = parseFloat(rawValue) || 0;
                            var rate = (nominal / dpp_pengganti) * 100;
                            if (rate > 100) rate = 100;
                            $('#input_dtp_rate').val(rate.toFixed(2));
                            
                            // Re-format currency on input if needed, or wait for blur
                        }).on('blur', function() {
                            var rawValue = $(this).val().replace(/\./g, '').replace(/,/g, '.');
                            var nominal = parseFloat(rawValue) || 0;
                            $(this).val(new Intl.NumberFormat('id-ID').format(Math.round(nominal)));
                        });
                    });
                  </script>";
        echo "  </div>";
        ...
        echo "    <i class='fa fa-warning'></i> Sertakan keterangan \"PPN DITANGGUNG PEMERINTAH EKSEKUSI PER-XX\" pada E-Faktur Kode 07.";
        echo "  </div>";
        echo "</div>";
        echo "</div>";
    }
                        ...
                        $listProduk .= "
                                    <div class='input-group border-cekx'>
                                        ...
                                        <span class='input-group-addon'>
                                            <input name='arrItems4' it4pid='$pids' it4key='$pkey' id='det_$pids$pkey' class='checkDetails' $checked onclick=\" \" type='checkbox' title='centang box untuk edit'>
                                        </span>
                                    </div>
                              ";
                        $listProduk .= "</div>";
...
//                         swal({type: 'warning',title: 'Upss..',html: 'Cek pada nilai barang, bila sudah sesuai silahkan cukup berikan tic, bila belum sesuai silahkan dikoreksi.'});
        ...
        if (isset($sumRows) && sizeof($sumRows) > 0) {
            ...
            foreach ($sumRows as $key => $rowConfig) {
                // Handling new array format from config
                if (is_array($rowConfig)) {
                    $label = $rowConfig['label'];
                    $isTaxSummary = isset($rowConfig['is_tax_summary']) ? $rowConfig['is_tax_summary'] : false;
                    $isDtp = isset($rowConfig['is_dtp']) ? $rowConfig['is_dtp'] : false;
                    $isTotal = isset($rowConfig['is_total']) ? $rowConfig['is_total'] : false;
                }
                else {
                    $label = $rowConfig;
                    $isTaxSummary = false;
                ...
                if ($isTaxSummary) {
                    $viewSumRow .= "<div class='row' style='padding: 10px 0; border-top: 1px solid #eee; margin-top: 10px;'>";
                    $viewSumRow .= "<div class='col-md-12'><h4 class='text-blue' style='margin-bottom: 15px; font-weight: bold;'>Rincian Pembayaran Pajak</h4></div>";
                    $viewSumRow .= "</div>";
                }

                if ($isDtp) {
                    $viewSumRow .= "<div class='row' style='padding: 6px;'>";
                    $viewSumRow .= "<div class='col-md-7 col-lg-7'>";
                    $viewSumRow .= "<div class='text-right valign-t text-uppercase' style='color: #d32f2f; font-weight: bold;'>$label</div>";
                    $viewSumRow .= "</div>";
                    $viewSumRow .= "<div class='col-md-5 col-lg-5 no-padding'>";
                    $viewSumRow .= "<input type='text' id='$key' class='form-control text-right' readonly value='" . niceDecimal($val) . "' style='color: #d32f2f; font-weight: bold; background: white;'>";
                    $viewSumRow .= "</div>";
                    $viewSumRow .= "</div>";
                    continue; // Skip the default rendering
                }

                $rowStyle = $isTotal ? "background: #e3f2fd; padding: 10px; border: 2px solid #2196f3; border-radius: 4px; margin-top: 10px;" : "padding: 6px;";
                $labelStyle = $isTotal ? "font-weight: bold; font-size: 15px; padding-top: 8px;" : "valign-t text-uppercase";
                $inputStyle = $isTotal ? "font-weight: bold; font-size: 16px; color: #1565c0; border: 1px solid #90caf9;" : "";

                $customeElement = "";
                $customeFixedNote = "";
                if ($showItems) {
                    $viewSumRow .= "<div sumRows class='row' style='$rowStyle'>";

                    $viewSumRow .= "<div sumRows class='col-md-7 col-lg-7'>";
                    $viewSumRow .= "<div sumRows line='" . __LINE__ . "'  colspan='" . $colspanDiv . "' class='text-right $bottom_borderless $labelStyle'>$label</div>";
                    $viewSumRow .= "</div>";
                    ...
                    $viewSumRow .= "<input type='text' id='$key' class='form-control text-right' readonly value='" . niceDecimal($val) . "' style='$inputStyle' >";
                    $viewSumRow .= "</div>";
...
//                if(isset($elementConfigMutasi[$k]))
//                $viewAddRows .= "";
...
// END OF COMPLETE REPEATED LOGIC
            if (isset($showFormulirFaktur) && ($showFormulirFaktur == true)) {
                ...
                if (sizeof($shopingCartFakturInsentifParam) > 0) {

                    // Define the blocks to render (Buyer and Government)
                    $fakturBlocks = array();
                    if (sizeof($formulirFaktur) > 0) {
                        $fakturBlocks[] = array(
                            "data" => $formulirFaktur,
                            "param" => $shopingCartFakturParam,
                            "title" => "<i class='fa fa-user'></i> BAGIAN PEMBELI (Pajak Masukan kode 010)",
                            "panel_class" => "panel-info",
                            "gate" => $shopingCartFakturParam['gateTarget']
                        );
                    }
                    if (isset($formulirFakturInsentif) && sizeof($formulirFakturInsentif) > 0) {
                        $fakturBlocks[] = array(
                            "data" => $formulirFakturInsentif,
                            "param" => $shopingCartFakturInsentifParam,
                            "title" => "<i class='fa fa-university'></i> BAGIAN PEMERINTAH (Insentif Pajak kode 07)",
                            "panel_class" => "panel-warning",
                            "gate" => $shopingCartFakturInsentifParam['gateTarget']
                        );
                    }

                    $countItems = sizeof($items);
                    foreach ($fakturBlocks as $bIdx => $block) {
                        $currentData = $block['data'];
                        $currentParam = $block['param'];
                        $blockTitle = $block['title'];
                        $panelClass = $block['panel_class'];
                        $blockGate = $block['gate'];
                        $isClonable = ($blockGate == "items6_sum");
                        $linkFaktur = MODUL_PATH . $currentParam['editProcess'] . "/" . $transaksi_jenis . "/";

                        $isBlockPredefined = false;
                        if (sizeof($currentData) > 0) {
                            foreach ($currentData as $checkSpec) {
                                if (isset($checkSpec['is_predefined_faktur']) && intval($checkSpec['is_predefined_faktur']) == 1) {
                                    $isBlockPredefined = true;
                                    break;
                                }
                            }
                        }
                        ...
                        $headerBadge = "";
                        if ($isBlockPredefined) {
                            $headerBadge = "<span class='pull-right label label-success' style='font-size: 12px; padding: 4px 10px; font-weight: bold;'><i class='fa fa-check-circle'></i> Faktur Sudah Diterima</span>";
                        }

// START OF COMPLETE REPEATED LOGIC
                        // ini blok/kotak PPN MASUKAN (KODE 010 atau 070)
                        $faktur .= "<div class='panel $panelClass' style='margin-top: 25px; margin-bottom: 25px; border-width: 2px; box-shadow: 0 2px 5px rgba(0,0,0,0.08);'>";
                        $faktur .= "<div class='panel-heading' style='font-weight: bold;'>$blockTitle $headerBadge</div>";
                        $faktur .= "<table class='table' style='margin-bottom: 0;'>";
                        $faktur .= "<tr class='bg-primary'>";
                        foreach ($currentParam["fields"] as $ff => $ff_abels) {
                            $faktur .= "<th>$ff_abels</th>";
                        }
                        $faktur .= "</tr>";

                        foreach ($currentData as $ctt => $fSpec) {

                            // ini formulir/baris input faktur ppn masukan
                            $faktur .= "<tr>";
                            foreach ($currentParam["fields"] as $fff => $f_labels) {
                                $labels = "";
                                $btn_formulir = "";
                                $btn_formulir_delete = "";

                                if (isset($currentParam["editableFields"][$fff])) {
                                    $inputType = $currentParam["editableFields"][$fff];
                                    // Maintain existing IDs for first block (Buyer) for script compatibility, otherwise use unique IDs
                                    if ($blockGate == "items6_sum" && $ctt == 0) {
                                        $inputID = $fff;
                                        $tdID = "td_" . $fff . "_" . $ctt;
                                    }
                                    else {
                                        $inputID = $blockGate . "_" . $fff . "_" . $ctt;
                                        $tdID = "td_" . $blockGate . "_" . $fff . "_" . $ctt;
                                    }
                                    ...
                                    if ($currentParam["editableFields"][$fff] == "checkbox") {
                                        $classinputType = "";
                                        ...
                                        $checked = isset($fSpec[$fff]) && $fSpec[$fff] == "true" ? $vals : "";
                                         ...
                                         $value = ($ctt == 0) ? "<input type='$inputType' id='$inputID' class='$classinputType' name='$fff' onclick='this.select()' value='$defValues' $checked $disabledCheckbox onblur=\"eksekutor_$blockGate(this.$vals,this.name,$ctt)\">" : "";
                                        ...
                                        $checked = "";
                                         ...
                                         $value = "<input type='$inputType' id='$inputID' class='$classinputType' name='$fff' onclick='this.select()' value='$defValues' $checked $readonlyAttr onblur=\"eksekutor_$blockGate(this.$vals,this.name,$ctt)\">";
                                         if ($fff == 'eFaktur') {
                                    ...
                                    $tdID = ($blockGate == "items6_sum") ? "td_" . $fff . "_" . $ctt : "td_" . $blockGate . "_" . $fff . "_" . $ctt;
                                    $value = formatField($fff, $fSpec[$fff]);
                                }

                                $faktur .= "<td id='$tdID'>$value <span class='text-danger text-bold text-blink'> $labels </span>";
                                $faktur .= $btn_formulir;
                                $faktur .= $btn_formulir_delete;
                                $faktur .= "</td>";

                                if (is_numeric($fSpec[$fff])) {
                                    if (!isset($sub_total_bawah[$blockGate][$fff])) {
                                        $sub_total_bawah[$blockGate][$fff] = 0;
                                    }
                                    $sub_total_bawah[$blockGate][$fff] += $fSpec[$fff];
                                }
                            }
                            $faktur .= "</tr>";
                        ...
                        $faktur .= "</div>";
                        $faktur .= "<script>
                                function eksekutor_$blockGate(nilai,nama,ctt) {
...
// START OF COMPLETE REPEATED LOGIC
                $faktur .= "<script>
                                    ...
                                    if (typeof swal === 'function') {
                                        swal({
                                            ...
                                            if (isConfirm) {
                                                ...
                                                var $theForm = $('#f1').length > 0 ? $('#f1') : ($('#form1').length > 0 ? $('#form1') : $('form').first());
                                                if ($theForm.length > 0) {
                                                    if (typeof top !== 'undefined' && typeof top.open_holdon === 'function') {
                                                        top.open_holdon();
                                                    } else if (typeof open_holdon === 'function') {
                                                    ...
                                                    var targetSaveFakturUrl = '<?php echo base_url() . $this->modul . "/Create/saveFaktur/" . $this->jenisTr; ?>' + '?dateFaktur=' + encodeURIComponent(dateFaktur || '') + '&eFaktur=' + encodeURIComponent(eFaktur || '') + '&skip_faktur=' + (skip_faktur ? 'true' : 'false');
                                                    $theForm.attr('action', targetSaveFakturUrl).submit();
                                                } else {
                                                    alert('Error: Form transaksi tidak ditemukan.');
                                                }
                                            }
                                        });
                                    } else {
                                ...
                                if(skip_faktur == false && dateFaktur == '' && eFaktur == '' && konfirmasi_cek == true){
                                    $('#td_dateFaktur_0').append('<r>Isikan tanggal e-faktur</r>');
                                    $('#td_eFaktur_0').append('<r>Isikan e-faktur</r>');
                                    $('#dateFaktur').css('border-color', 'red');
                                    $('#eFaktur').css('border-color', 'red');
                                    $('#konfirmasi_cek').prop('disabled', true).prop('checked', false);
...
// START OF COMPLETE REPEATED LOGIC
        ...
        if ($faktur != "") {
            $addColspanFaktur = isset($colSpanWadah) ? $colSpanWadah : 12;
            echo "<tr id='tr_faktur_container'><td colspan='" . $addColspanFaktur . "' style='padding: 0;'>";
        echo $faktur;
            echo "</td></tr>";
        }
        echo $tableBawah;
...
//                    }
        ...
        if ($jenisTr == "483") {
            $nilai_entry = $main["tagihan_bayar_after_credit_card"];
            echo "<script>
        ...
        switch ($jenisTr) {
            ...
            case "1477":
                if ($optionPembayaran == 1) {
                    echo "<script>
                        top.document.getElementById('nilai_entry').disabled=false;
                        top.document.getElementById('nilai_dipakai_credit_card').disabled=true;
                        </script>";
                }
                elseif ($optionPembayaran == 2) {
                    echo "<script>
                        top.document.getElementById('nilai_entry').disabled=true;
                        top.document.getElementById('nilai_dipakai_credit_card').disabled=false;
                    </script>";
                }
                else {
                    echo "<script>
            ...
            case "477":
                if ($optionPembayaran == 1) {
                    echo "<script>
                        top.document.getElementById('dibayar_kas').disabled=false;
                        top.document.getElementById('nilai_dipakai_credit_card').disabled=true;
                        </script>";
                }
                elseif ($optionPembayaran == 2) {
                    echo "<script>
                        top.document.getElementById('dibayar_kas').disabled=true;
                        top.document.getElementById('nilai_dipakai_credit_card').disabled=false;
                    </script>";
                }
                else {
