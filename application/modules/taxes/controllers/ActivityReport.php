...
class ActivityReport extends Modul_Controller
{
    ...
    public function viewMonthly()
    {
        ...
        // }
        $selectedCabang = "transaksi.cabang_id='" . $this->session->login['cabang_id'] . "'";
        $currentState = strlen($this->uri->segment(5)) > 0 ? $this->uri->segment(5) : $this->configUi[$this->jenisTr]['steps'][1]['target'];
        $this->load->model("MdlTransaksi");
        $tr = new MdlTransaksi();
        ...
        $tmp = $tr->lookupJoined();
               // $this->debugShowLastQuery("pink");
        //        $this->debugArrPrint($tmp);
        // matiHEre();
        $masterID = array();
        $masmedID = array();
        foreach ($tmp as $df) {
        ...
        if (strlen($idList) > 3) {
            ...
            foreach ($registries as $registry) {
                ...
                //                $master_id = $listedSeller[]
                // cekMerah("$trDtime *** $trDtime_M");
                $regs = blobDecode($registry->items);
                // $this->debugArrPrint($regs);
                // cekHijau($trDtime);
                // $this->debugArrPrint($registry);
        ...
        //$this->debugArrPrint($recaps);
        //$this->debugArrPrint($reacpList);
        ...
        if (placeCanMakeTrans($this->session->login['membership'], $this->session->login['cabang_id'], $this->session->login['gudang_id'], $this->jenisTr)) {
            ...
            $addLink = array(
                "link" => $targetUrl,
                "label" => "<span class='glyphicon glyphicon-plus'></span> create new " . $this->configUi[$this->jenisTr]["steps"][1]['label'],
            );
        }
        else {
            $addLink = null;
        ...
        $data = array(
            "mode" => "recap",
            "title" => $this->configUi[$this->jenisTr]['label'] . " report",
            ...
            "trName" => $this->configUi[$this->jenisTr]["label"],
            ...
            "subPage" =>MODUL_PATH . $this->uri->segment(2) .  "/viewDaily/" . $this->jenisTr,
            "historyPage" => MODUL_PATH . "History/viewHistory/" . $this->jenisTr . "/$stID" . "?stID=" . $stID,
            "stepNames" => $stepNames,
    ...
    public function viewWeekly()
    {
        ...
        $currentState = strlen($this->uri->segment(4)) > 0 ? $this->uri->segment(4) : $this->configUi[$this->jenisTr]['steps'][1]['target'];
        $this->load->model("MdlTransaksi");
        $tr = new MdlTransaksi();
        //        $tr->addFilter("transaksi.cabang_id='" . $this->session->login['cabang_id'] . "'");
        $tr->addFilter($selectedCabang);
        ...
        $data = array(
            "jenisTr" => $this->jenisTr,
            "mode" => "recap",
            "title" => $this->configUi[$this->jenisTr]['label'] . " reports",
    ...
    public function viewDaily()
    {
        ...
        // $this->debugArrPrint($trIdDtimes_);
        $trIds = array_keys($trIds_);
        $idList = implode(",", $trIds);
        if (strlen($idList) > 3) {
            ...
            $tr_2->setFilters(array());
            $tr_2->addFilter("transaksi_id in '(" . $idList . ")'");
            $tr_2->setJointSelectFields("items,transaksi_id");//string
            // $tr_2->addFilter("param ='items'");
        ...
        $data = array(
            ...
            "identifierLabels" => $this->config->item("heTransaksi_report_identifiers"),
            "thisPage" =>MODUL_PATH . $this->uri->segment(2) . "/" . $this->uri->segment(3) ."/".$this->jenisTr,
            "historyPage" => MODUL_PATH . "History/viewHistory/" . $this->jenisTr . "/" . $selectedStep,
            "stepNames" => $stepNames,
            "defaultStep" => $defaultStep,
            "selectedStep" => $selectedStep,
            "addLink" => $addLink,
    ...
    public function viewMySettlement()
    {
        ...
        $trHeaders = isset($this->configUi[$jenisTr]['settlementHistoryFields']) ? $this->configUi[$jenisTr]['settlementHistoryFields'] : array(
            "dtime" => "time",
            "nomer" => "receipt number",
            "jenis_label" => "activity",
            //            "transaksi_nilai" => "orig. value",
        ...
        $tr->addFilter("gudang_id='" . $this->session->login['gudang_id'] . "'");
        $tr->addFilter("jenis_master='" . $this->jenisTr . "'");
        // $tr->addFilter("transaksi.oleh_id='" . $this->session->login['id'] . "'");//ini sengaja di off untuk test liahat settlement beas
        ...
        // $tmpHist0 = $tr0->lookupRegistries_joined()->result();
        if(!in_array("TableIn_mater_values",$pairRegistries)){
            $pairRegistries = $pairRegistries+array('TableIn_mater_values');
        }
        $selectKolom = implode(",", $pairRegistries) . ", transaksi_id";
        $tr0->setJointSelectFields($selectKolom);
        $tmpHist0 = $tr0->lookupDataRegistries_joined();
        ...
        //endregion
        //$this->debugArrPrint($trItems);
// $this->debugArrPrint($trIDs);
        $accounts = array();
        $rels = array();
        $accountRecaps = array();
        ...
        $thisTrName = isset($row->jenis_label) ? $row->jenis_label : $this->configUi[$jenisTr]["label"];
// $this->debugArrPrint($historyFields);
        $data = array(
            "mode" => $this->uri->segment(3),
            "isMobile" => $isMob,
            "jenisTr" => $jenisTr,
            "subJenisTr" => strlen($this->uri->segment(5)) > 0 ? $this->uri->segment(5) : $jenisTr,
            //            "trName"               => $this->configUi[$jenisTr]["label"],
            "trName" => $thisTrName,
            "errMsg" => $this->session->errMsg,
            "title" => (isset($subCodes) && isset($currentState) ? $subCodes[$currentState] : $this->jenisTrName) . " (" . $this->session->login['nama'] . ")",
            ...
            "addParams" => isset($_GET['addParams']) ? $_GET['addParams'] : null,
            "currentState" => isset($currentState) ? $currentState : "all states",
            // "alternateLink" =>MODUL_PATH . $this->uri->segment(1) . "/viewIncomplete/" . $this->uri->segment(3),
    ...
    public function viewHistory2()
    {
        ...
        $steps = $this->configLayout[$this->jenisTr]['historicalReport'][$defaultStep]['tabs'];
        // $this->debugArrPrint($steps);
        foreach ($steps as $num => $nSpec) {
            $stepNames[$nSpec['target']] = $nSpec['label'];
        }
        // $this->debugArrPrint($stepNames);
    ...
    public function viewMonth()
    {
        ...
        // $this->debugArrPrint($fieldJenis);
        // $this->debugArrPrint($reportingNetts['fields']);
        // $this->debugArrPrint($fieldToshows);
        // $this->debugArrPrint($fieldAttr);
        // $this->debugArrPrint($fields);
        // $this->debugArrPrint($steps);
        if ($this->session->login['cabang_id'] > 0) {
            $selectedCabang = "transaksi.cabang_id='" . $this->session->login['cabang_id'] . "'";
        }
        ...
        // $this->debugArrPrint($tmps);
        // $this->debugArrPrint($lurusDatas);
        // mati_disini();
        ...
        // endregion return
        // $this->debugArrPrint($rTmps);
        // $this->debugArrPrint($retDatas);
        ...
        */
        ...
        if (sizeof($compDatas) > 0) {
            ...
            foreach ($compDatas as $trJenis => $items) {
                ...
                foreach ($items as $transaksi_id => $item) {
                    ...
                    // $compSpecs = array();
                    // foreach ($steps as $num => $nSpec) {
                    //     // $this->debugArrPrint($nSpec);
                    //     $nJenis = $nSpec['jenis'];
                    //     if ($nJenis == $trJenis) {
                    //     // if (($nJenis == $trJenis) && ($trJenis == "582spd") || ($trJenis == "982")) {
                    //         if($nJenis == "582spd"){
                    //             $spdSpecs = $specs;
                    //             // $this->debugArrPrint($compSpecs);
                    //             $this->debugArrPrint($rSpecs);
                    //             cekHere();
                    //         }
                    //         else{
                    //             $compSpecs = $specs;
        ...
        // $this->debugArrPrint($rSpecs);
        // $this->debugArrPrint($spdSpecs);
        // $this->debugArrPrint($bodies);
        // $this->debugArrPrint($footers);
        // cekHijau($sumNetto);
        // mati_disini();
    ...
    public function viewHistory()
    {
        ...
        // $this->debugArrPrint($historicalReports['fields']);
        // $this->debugArrPrint($fieldToshows);
        // $this->debugArrPrint($fieldAttr);
        // $this->debugArrPrint($fields);
        // $this->debugArrPrint($steps);
    ...
    public function viewSales()
    {
        ...
        // $this->debugArrPrint($fieldToshows);
        // $this->debugArrPrint($fieldAttr);
        // $this->debugArrPrint($fieldAttrHeader);
        // $this->debugArrPrint($fields);
        // $this->debugArrPrint($steps);
        ...
        // );
        // $db2->where($condites);
        $rp->setCondites(array("subject_id" => $this->session->login['id'], "th" => $year, "bl" => $month));
        $srScr = $rp->lookupSalesMonthly()->result();
        // $this->debugArrPrint($srScr);
    ...
    public function viewPreSalesAll()
    {
        ...
        // $this->debugArrPrint($srcSm);
        // $this->debugArrPrint($slFields);
        // $this->debugArrPrint($sellerIds);
        // $this->debugArrPrint($sMans);
        // $this->debugArrPrint($sellers);
        ...
        // endregion sales
        // $this->debugArrPrint($srScr);
        // $this->debugArrPrint($cancelScr);
        // $this->debugArrPrint($sFields);
        // $this->debugArrPrint($cbFields);
        // $this->debugArrPrint($pFieldSumrows);
        $koloms = array_unique(array_merge($cbFields, $sFields, $pFields));
        $subjects = array();
        $cabangs = array();
        $sumSubjects = array();

        // $this->debugArrPrint($dSources);
        // $this->debugArrPrint($koloms);
        //region pre So
        foreach ($srScr as $dSources) {
        ...
        // $this->debugArrPrint($sumSubject);
        // $this->debugArrPrint($sumObject_1);
        // $this->debugArrPrint($sumObject);
        // $this->debugArrPrint($sumCabang);
        // $this->debugArrPrint($sumCabang_2);
        // $this->debugArrPrint($sumObjectCancel);
        // $this->debugArrPrint($sumObject_2);
        // $this->debugArrPrint($sumObject);
        // region headerCabang
        ...
        //   "cabangs" => $cabangs,
        // );
        // $this->debugArrPrint($cabangs);
        // $this->debugArrPrint($sumCabang);
        // $this->debugArrPrint($sumCabangCancel);
        // $this->debugArrPrint($sumSubject);
        // matiHere(__METHOD__);
        $compDatas = $srScr;
        // $this->debugArrPrint($compDatas);
    ...
    public function viewPurchasingSpAll()
    {
        ...
        // $this->debugArrPrint($reportingNetts);
        // $this->debugArrPrint($this->config->item("report"));
        // mati_disini();
        // $bulan = "$year-$month";
        // $bulan_f = formatTanggal($bulan, 'Y F');
        $data = array(
    ...
    public function viewInvoice()
    {
        ...
        // $this->debugArrPrint($reportingNetts['mdlFields']);
        $confReportCabang = $reportingSumCabang = $this->config->item('report')['penjualan_cabang'];
        $confReportSubject = $reportingSumSubject = $reportingSumSeller = $this->config->item('report')['penjualan_seller'];
//        $this->debugArrPrint($historyFields);
        ...
        $tr = New MdlTransaksi();
        $this->db->select(array("id", "indexing_registry"));
...
//            arrPrintPink($arrTransID_return_reg);
            ...
            if (sizeof($tmpReg) > 0) {
                foreach ($tmpReg as $rSpec) {
                    $val_decode = blobDecode($rSpec->values);
...
//                $this->debugArrPrint($id_hist);
                ...
                $rawIdsHis = blobDecode($row->ids_his);
                $counterIds_his = array();
                if (is_array($rawIdsHis)) {
                    if (isset($rawIdsHis[1]['counters'])) {
                        $counterIds_his = blobDecode($rawIdsHis[1]['counters']);
                    } else {
                        foreach ($rawIdsHis as $stepItem) {
                            if (is_array($stepItem) && isset($stepItem['counters'])) {
                                $counterIds_his = blobDecode($stepItem['counters']);
                                break;
                            }
                        }
                    }
                ...
                // $this->debugArrPrint($counters);
                // cekHijau($row->id);
                // $this->debugArrPrint($counterIds_his);
...
//                        foreach ($tmpReg as $regRow) {
//                            $param = $regRow->param;
//                            // $this->debugArrPrint(blobDecode($regRow->values));
...
//                        cekHere($val_return);
            ...
            // $this->debugArrPrint($sumValue);
        }
        // $this->debugShowLastQuery("lime");

//         $this->debugArrPrint($sumValueManual);
//         $this->debugArrPrint($sumValue);
        // $this->debugArrPrint($srcHi);
    ...
    public function viewSalesRealization()
    {
        ...
        // $rp->setLimit(1);
        $srScr = $rp->lookupPreSalesMonthly()->result();
        // $this->debugShowLastQuery('merah');
        $cancelScr = $rp->lookupPreSalesCanceledMonthly()->result();
        // $this->debugShowLastQuery('kuning');
        $saleScr = $rp->lookupSalesMonthly()->result();
        // $this->debugShowLastQuery('hijau');

        // $this->debugArrPrint($srScr);
        // $this->debugArrPrint($cancelScr);
        // $this->debugArrPrint($saleScr);
        ...
        // $this->debugArrPrint($srScr1);
        // $this->debugArrPrint($cancelScr1);
        // $this->debugArrPrint($saleScr1);
        //
        // matiHere();
        $koloms = array_unique(array_merge($cbFields, $sFields, $pFields));
...
//        $this->debugArrPrint($valueGate = $fieldSelected["valueGate"]);
...
//        $this->debugArrPrint($dtimeArray);
        $diferDays = array();
        foreach ($tmpProce as $mainID => $mainValuesday) {
//            $this->debugArrPrint($mainValuesday);
            $strtTime = $mainValuesday["582so"]["dtime"];
//            cekLime($strtTime);
...
//        $this->debugArrPrint($tempEmployee);
//$this->debugArrPrint($regMain);
//$this->debugArrPrint($regMain);
        //endregion
        ...
        foreach ($mainValues as $idx => $temp) {
            // $this->debugArrPrint($temp);
            // $this->debugArrPrint($diferDays[$idx]);
            // $this->debugArrPrint($dtimeArray[$idx]);
...
// $this->debugArrPrint($mainValuese);
...
//            $this->debugArrPrint($baseData);
        }
//$this->debugArrPrint($regIDs);
//        matiHere();
        //region lookup registry
        $tr->setFilters(array());
        $tr->addFilter("id in '(" . implode(",", array_filter($regIDs)) . ")'");
...
//$this->debugArrPrint($sumSubject);
//$this->debugArrPrint($sumSubjects);
        $data = array(
    ...
    public function viewPembelian()
    {
        ...
        // $this->debugArrPrint($src_pembelians);
        // $this->debugArrPrint($src_pembelian_returns);
    ...
    public function viewDetile()
    {
        ...
        // $this->debugShowLastQuery("kuning");
        // $this->debugArrPrint($trRegMainId);
        // $this->debugArrPrint($srcReg);
        // $this->debugArrPrint($srcRegMain);
        // $this->debugArrPrint($trJenisId);
    ...
    public function viewOutstandingSalesMonthly()
    {
        ...
        $selectedTrans = array(
            "582so" => "sales order", "582spd" => "packing list", "982" => "return", "1982" => "fullfill", "pending" => "out standing"
        );
        ...
        if (sizeof($pmp) > 0) {
            foreach ($pmp as $pmp_0) {
                ...
                // $this->debugArrPrint($pIndexingMain);
                ...
                // $this->debugArrPrint($indexingMain);
            }
            // arrprint($strOld);
        ...
        if (sizeof($pMasterID) > 0) {
            ...
            // $this->debugArrPrint($listedSeller);
        }
        $m->setFilters(array());
        $m->setParam("id");
        ...
        //endregion
        // $this->debugArrPrint($pRecaplistAll);
        ...
        if (sizeof($masterID) > 0) {
            ...
            foreach ($tempMaster as $tempMaster_0) {
                // $this->debugArrPrint($tempMaster_0);
                $listedSeller[$tempMaster_0['id']] = $tempMaster_0['oleh_id'];
                $listedSellerLabel[$tempMaster_0['oleh_id']] = $tempMaster_0['oleh_nama'];
        ...
        foreach ($recaplistAll as $sID => $sidData) {
            ...
            // $this->debugArrPrint($sidData);
        }
        // $this->debugArrPrint($recaplistAll);
        //endregion
        ...
        //endregion
        // $this->debugArrPrint(sizeof($listedCustomer));
    ...
    public function viewSalesOrderMonthly()
    {
        ...
        // $this->debugArrPrint($prevIDS);

        // $this->debugArrPrint($indListPrev);
        ...
        if (sizeof($reg) > 0) {
            ...
            if (sizeof($fnReg) > 0) {
                foreach ($fnReg as $idsFn) {
                    // $this->debugArrPrint($regEntries[$idsFn]);
                    ...
                    // $this->debugArrPrint($regEntries[$idx]);
                }
            }
        }
// $this->debugArrPrint($test);
// matiHEre();
        foreach ($tmp as $row) {
            $sellID = $listedCustomer[$row->id_master];
            $trDtime = $trIdDts[$row->id]["dtime"];
...
// matiHere();
        if (sizeof($prevIDS) > 0) {
            ...
            // }
            //         $this->debugArrPrint($idsRelPrev);
            // cekLime($this->db->last_query());
            // $this->debugArrPrint($prevData);
        }
// $this->debugArrPrint($prevOutsanding);
        $months = array();
    ...
    public function viewSalesOrderMonthlyExport()
    {
        ...
        if (sizeof($tmp) > 0) {
            ...
            if (sizeof($getReg) > 0) {
                ...
                $updateReg = $tr->lookupRegistries()->result();
                $dataToUpdate = array();
                ...
                // $this->debugArrPrint($dataToUpdate);
                // matiHEre();
                $tr->setFilters(array());
                $this->beginSafeTransaction();
                if (sizeof($dataToUpdate) > 0) {
                    foreach ($dataToUpdate as $trid => $tridData) {
                        ...
                        $updated = $tr->updateData(array("id" => $trid), array(
                        ...
                        if (!$updated) {
                            ...
                            $this->failMutationResponse("Failed to update tr next-state!");
                            return;
                        }
                        cekHitam($this->db->last_query());
                    }
                ...
                // matiHEre();
                if (!$this->finalizeSafeTransaction("Gagal saat berusaha  commit transaction!")) {
                    return;
                }
                cekBiru($this->db->last_query());
                // $this->debugArrPrint($updateReg);
            }
            if (sizeof($masterID) > 0) {
    ...
    public function viewdetail_def()
    {
        ...
        // $this->debugArrPrint($this->uri->rsegment_array());
        // matiHere();
        $header = array("dtime" => "date", "nomer" => "receipt", "harga" => "amount", "harga_disc" => " amount reject", "nett" => "netto");
        $dataTmp = array(
            "mode" => "recapDetil",
            "title" => $this->configUi[$this->jenisTr]['label'] . " report",
            "thisPage" => base_url() . get_class($this) . "/" . $this->uri->segment(2) . "/" . $this->jenisTr,
    ...
    public function viewdetail_OLD()
    {
        ...
        $dataTmp = array(
            "mode" => "recapDetil",
            "title" => $this->configUi[$jn]['label'] . $customer . " report",
            "sub_title" => "detil " . $this->configUi[$jn]['label'] . " by customer $customer",
            "thisPage" => base_url() . get_class($this) . "/" . $this->uri->segment(2) . "/" . $this->jenisTr,
            "subTitle" => "monthly",
            "items" => $data,
            "subtotal" => $arrAll,
    ...
    public function viewdetailPrev()
    {
        ...
        $dataTmp = array(
            "mode" => "recapDetil",
            "title" => $this->configUi[$this->jenisTr]['label'] . $customer . " report",
            "sub_title" => "detil " . $this->configUi[$this->jenisTr]['label'] . " by customer $customer",
            "thisPage" => base_url() . get_class($this) . "/" . $this->uri->segment(2) . "/" . $this->jenisTr,
            "subTitle" => "monthly",
            "items" => $data,
            "subtotal" => $arrAll,
...
// $this->debugArrPrint($cabangAlias);
        $permanentFilter = "oleh_id";
        $currYear = isset($_GET['year']) ? $_GET['year'] : date("Y");
        $prevYear = $currYear - 1;
        // cekLime($prevYear);
...
// $this->debugArrPrint($rekValues);
        ...
        // $this->debugArrPrint($listedRekID);
        //endregion
        // $this->debugArrPrint($tempDataReturn);
...
// $this->debugArrPrint($listedCustomer);
        //region lihat registry main
        foreach ($tmp as $row) {
...
//                        }
        ...
        //endregion
// $this->debugArrPrint($sumValidateValuesRek);
        //return penjualan
        $r->setFilters(array());
        if ($this->session->login['cabang_id'] > 0) {
            // $filter = array(
    ...
    public function viewPurchaseOrderMonthly()
    {
        ...
        switch ($modeItem) {
            case "produk":
                ...
                $listTrJenis = array("466", "467", "967", "1967");
                $kolom_nilai = "harga";
            ...
            case "produkImport":
                ...
                $listTrJenis = array("460a", "460", "960a", "1960a");
            ...
            case "supplies":
                ...
                $listTrJenis = array("461r", "461", "961", "1961");
                $kolom_nilai = "harga";
...
//             $this->debugArrPrint($tmp_0);
        ...
        if (sizeof($masterID) > 0) {
            ...
            $m->setInParam($masterID);
            $m->setTableName("transaksi");
            $m->setFields(array("id", "oleh_id", "oleh_nama", "customers_id", "customers_nama", "suppliers_id", "suppliers_nama"));
            $tempMaster = $m->lookUpAll();
            // $this->debugArrPrint($tempMaster);
            // matiHEre();
            $listedSeller = array();
...
//                arrprint($sumTimes);
        ...
        asort($listedCustomerLabel);
//         $this->debugArrPrint($listedCustomerLabel);
...
//        arrPrintPink($arrAll);
        ...
        $dataTmp = array(
            "mode" => "recapDetil",
            "title" => $this->configUi[$jn]['label'] . $customer . " report",
            "sub_title" => "detil " . $this->configUi[$jn]['label'] . " by vendor $customer",
            "thisPage" => base_url() . get_class($this) . "/" . $this->uri->segment(2) . "/" . $this->jenisTr,
            "subTitle" => "monthly",
            "items" => $data,
            "subtotal" => $arrAll,
...
// $this->debugArrPrint($listedCustomer);
    ...
    public function viewPembelians()
    {
        ...
        // $this->debugShowLastQuery("kuning");
        // $this->debugArrPrint($src_vendor_incative);
...
//        $this->debugShowLastQuery("lime");
        ...
        // $this->debugArrPrint($prevIDS);
//        $this->debugArrPrint($arrPenjualan_lainya);
//        cekLime("total=" . sizeof($tmp) . " pejualan=" . sizeof($arrPenjualan_idMaster) . " lain=" . sizeof($arrPenjualan_lainya));
//        $this->debugArrPrint($arrPenjualan_idMaster); // ini transaksiID => id_spo
//        $this->debugArrPrint($arrPenjualan_lainya); // ini transaksiID => id registry

        // $this->debugArrPrint($indListPrev);
        if (sizeof($arrPenjualan_lainya) > 0) {
            $regReferensi = $tr->lookupBaseRegistries($arrPenjualan_lainya)->result();
//            $this->debugShowLastQuery("biru");
//            $this->debugArrPrint($regReferensi);
            foreach ($regReferensi as $item) {
                $regId = $item->id; // id registry main
                $regTrId = $item->transaksi_id; // id transaksi 982 atau 1982

                $regValues = blobDecode($item->values);
...
//                $this->debugArrPrint($regReferensiId_SPD);
                $tr->setFilters(array());
                $this->db->select(array("id", "oleh_id", "oleh_nama", "id_master"));
...
//            $this->debugShowLastQuery("merah");
        ...
        // $this->debugArrPrint($listRegMain);
        // // $this->debugArrPrint($listedSeller);
        ...
        // $m->setFields(array("transaksi_id", "param", "values"));
        // $m->setTableName("transaksi_registry");
        $reg = $tr->lookupBaseRegistries($regIDS)->result();
        // $this->debugShowLastQuery("orange");
        // $this->debugArrPrint($reg);
        // matiHere(__LINE__);
        $regEntries = array();
        // $this->debugArrPrint($reg);
...
//        $this->debugArrPrint($recaplistAll_0['65']);
//mati_disini();
...
// $this->debugArrPrint($recaplistAll_0);
// $this->debugArrPrint($recaplistAll);
//         matiHere();
        //region outstanding per bulan
        // $netPending = array();
...
//        $this->debugArrPrint($addParam);
//        $this->debugArrPrint($addParam_00);
            ...
            switch ($jn) {
                ...
                case "582spd":
                    if (sizeof($arrMasterID) > 0) {
                        $tr = new MdlTransaksi();
                        $this->db->select(array("id", "id_master", "id_top"));
                        $tr->setFilters(array());
                ...
                case "1982":
                    ...
                    $tr->addFilter("param='main'");
                    $tReg = $tr->lookupRegistries()->result();
                    $this->debugShowLastQuery("kuning");
                    foreach ($tReg as $spec) {
                        $valuesDecode = blobDecode($spec->values);
//                            $this->debugArrPrint($valuesDecode);
                        if ($jn == "982") {
...
//                                $this->debugArrPrint($arr_id);
                                foreach ($arr_id as $iid) {
                                    if (isset($ref_SPD_reverse[$iid])) {
//                                        $this->debugArrPrint($ref_SPD_reverse[$iid]);
...
//            $this->debugArrPrint($arrMasterID_result);
            foreach ($tmp as $tmp0) {
//                $this->debugArrPrint($tmp0);
                if ($jn == "982") {
                    $id_cek = $tmp0->id;
                }
                else {
...
//$this->debugArrPrint($arrNoIndexReg_by_seller);
        $this->debugArrPrint($ids_by_seller);
...
//                $mong->setParam("transaksi_id");
////                $mong->setInParam($arrNoIndexReg);
//                $mong->setInParam($arrNoIndexReg_by_seller);
//                $mong->setFields(array("id", "transaksi_id", "param", "values"));
//                $mong->setTableName("transaksi_registry");
//                $tReg = $mong->lookUpAll();
                $tr = new MdlTransaksi();
...
//            $this->debugArrPrint($tmp0);
        ...
        $dataTmp = array(
            "mode" => "recapDetil",
//            "title" => $this->configUi[$jn]['label'] . $seller_nama . " report",
//            "sub_title" => "detail " . $this->configUi[$jn]['label'] . " by salesman $seller_nama",
