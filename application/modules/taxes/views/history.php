...
$searching .= "<div class='input-group input-group-sm' style='position: relative;'>";
$searching .= "<input type='text' id='global_search_input' name='search' value='$getSearch' onclick=\"this.select();\" class='form-control' placeholder='Search by customer or PIC name' style='padding-right: 25px;'>";
$searching .= "<span id='clear_global_search' style='position: absolute; right: 45px; top: 50%; transform: translateY(-50%); z-index: 10; cursor: pointer; color: #999; font-size: 16px; font-weight: bold; display: none;'>&times;</span>";
...
$(document).ready(function() {
    var searchInput = $('#global_search_input');
    var clearBtn = $('#clear_global_search');
...
//            $content .= ("</form>");
            ...
            // Render bootstrap pager server-side
            $pagerHtml = "";
            if (isset($jmlData) && isset($limit) && $limit > 0) {
                $numPages = ceil($jmlData / $limit);
                if ($numPages > 1) {
                    $page = (isset($_GET['page']) && $_GET['page'] > 0) ? intval($_GET['page']) : 1;
                    $pagerHtml .= "<div class='text-center' style='margin-top: 15px; margin-bottom: 15px;'>";
                    $pagerHtml .= "<ul class='pagination pagination-sm no-margin'>";

                    // Prev button
                    if ($page > 1) {
                        $qParams = $_GET;
                        $qParams['page'] = $page - 1;
                        $prevUrl = $thisPage . "?" . http_build_query($qParams);
                        $pagerHtml .= "<li><a href='$prevUrl'>&laquo; Prev</a></li>";
                    } else {
                        $pagerHtml .= "<li class='disabled'><span>&laquo; Prev</span></li>";
                    }

                    // Page numbers (show max 7 pages around current page)
                    $startPage = max(1, $page - 3);
                    $endPage = min($numPages, $page + 3);

                    if ($startPage > 1) {
                        $qParams = $_GET;
                        $qParams['page'] = 1;
                        $url = $thisPage . "?" . http_build_query($qParams);
                        $pagerHtml .= "<li><a href='$url'>1</a></li>";
                        if ($startPage > 2) {
                    ...
                    for ($p = $startPage; $p <= $endPage; $p++) {
                        ...
                        $url = $thisPage . "?" . http_build_query($qParams);
                        if ($p == $page) {
                            $pagerHtml .= "<li $activeClass><span>$p</span></li>";
                        } else {
                            $pagerHtml .= "<li $activeClass><a href='$url'>$p</a></li>";
                        }
                    }
                    ...
                    if ($endPage < $numPages) {
                        ...
                        $qParams = $_GET;
                        $qParams['page'] = $numPages;
                        $url = $thisPage . "?" . http_build_query($qParams);
                        $pagerHtml .= "<li><a href='$url'>$numPages</a></li>";
                    }
                    ...
                    if ($page < $numPages) {
                        $qParams = $_GET;
                        $qParams['page'] = $page + 1;
                        $nextUrl = $thisPage . "?" . http_build_query($qParams);
                        $pagerHtml .= "<li><a href='$nextUrl'>Next &raquo;</a></li>";
                    } else {
                        $pagerHtml .= "<li class='disabled'><span>Next &raquo;</span></li>";
                    }

                    $pagerHtml .= "</ul>";
                    $pagerHtml .= "<div class='text-muted' style='margin-top: 5px; font-size: 11px;'>Showing page $page of $numPages (Total: $jmlData records)</div>";
                    $pagerHtml .= "</div>";
                }
            }
            $content .= $pagerHtml;
        ...
        $strBg = "";
        if (isset($bgTransaksiColor) && (sizeof($bgTransaksiColor) > 0)) {
            $strBg .= "<span style='font-size: 15px;'>Keterangan warna: </span>";
            foreach ($bgTransaksiColor as $clr => $clr_label) {
                $strBg .= "<span class='btn' style='background-color: $clr;'>&nbsp;&nbsp;&nbsp;&nbsp;  </span> $clr_label &nbsp;&nbsp;&nbsp;";
            }
        }
        $dateShiftScript = "
        <script>
        $(document).ready(function() {
            $('#select_year').on('change', function() {
                ...
                if (selectedYear) {
                    ...
                    var d1 = selectedYear + '-01-01';
                    var d2 = selectedYear + '-12-31';
                    if (selectedYear === currentYear) {
                        var month = String(today.getMonth() + 1).padStart(2, '0');
                        var day = String(today.getDate()).padStart(2, '0');
                        d2 = currentYear + '-' + month + '-' + day;
                    }
                    $('#date1').val(d1);
                    $('#date2').val(d2);
                }
            ...
            $('#date1').on('change', function() {
                var date1Val = $(this).val();
                var date2El = $('#date2');
                if (date1Val) {
                    var year1 = date1Val.substring(0, 4);
                    var date2Val = date2El.val();
                    if (date2Val) {
                        var year2 = date2Val.substring(0, 4);
                        if (year1 !== year2) {
                            date2El.val(year1 + date2Val.substring(4));
                            $('#select_year').val(year1);
                        }
                    }
                }
            });
            ...
            $('#date2').on('change', function() {
                var date2Val = $(this).val();
                var date1El = $('#date1');
                if (date2Val) {
                    var year2 = date2Val.substring(0, 4);
                    var date1Val = date1El.val();
                    if (date1Val) {
                        var year1 = date1Val.substring(0, 4);
                        if (year1 !== year2) {
                            date1El.val(year2 + date1Val.substring(4));
                            $('#select_year').val(year2);
                        }
                    }
                }
            });
        ...
        // Generate list tahun untuk dropdown
        $startYear = 2020;
        $endYear = intval(date('Y'));
        $activeYear = substr($filters['date2'], 0, 4);
        $yearOptions = "";
        for ($y = $endYear; $y >= $startYear; $y--) {
            $selected = ($y == $activeYear) ? "selected" : "";
...
// cekLime($thisPage);
        $p->addTags(array(
            ...
            "content" => $content . $dateShiftScript . (isset($swalScript) ? $swalScript : ""),
            "alert_message" => isset($alertMessage) ? $alertMessage : "",
    ...
    case "showData":
         ...
         * --------------------------------------*/
        $content .= "<script>
        function loadHistoryTab(url) {
            var hl = $('#historyList');
            if (hl.length > 0) {
                hl.css('position', 'relative').append(\"<div id='overlay_history' style='position:absolute;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);z-index:999;display:flex;align-items:center;justify-content:center;color:#ffffff;font-size:16px;font-weight:bold;'><div id='text_history'><i class='fa fa-spinner fa-spin'></i> Loading Content...</div></div>\");
                hl.load(url);
            } else {
                location.href = url;
            }
...
// START OF COMPLETE REPEATED LOGIC
                $varUrl = $steps[$step]['target'] == $currentState ? "javascript:void(0);" : "loadHistoryTab('$linkStep');";
...
//            $content .= "<form name='fSelect'>";
        ...
        $dt_dom = "lBfrtip";
        if (isset($_GET['limit'])) {
            $link_history_full = base_url() . $this->uri->segment(1) . "/History/viewHistory/" . $jenisTr;
            $content .= "<div class='callout callout-warning text-center' style='margin-top:20px; border: 2px solid #c87f0a; background-color: #fcf8e3 !important; color: #8a6d3b !important;'>";
            $content .= "<h4 style='font-weight: bold;'><i class='icon fa fa-info-circle'></i> Informasi</h4>";
            $content .= "<p style='font-size:16px; margin-bottom:15px;'>Tabel ini dibatasi hanya menampilkan riwayat data terbaru. Untuk pencarian history yang lebih lengkap, silahkan klik tombol berikut:</p>";
            $content .= "<button type='button' class='btn btn-lg' style='background:#00a65a; color:#fff; font-weight:bold; box-shadow: 0 3px 6px rgba(0,0,0,0.16); padding: 10px 25px;' title='melihat seluruh history' onclick=\"location.href='$link_history_full'\"><i class='fa fa-list'></i> Tampilkan History Full</button>";
            $content .= "</div>";
            $dt_dom = "t";
        }
