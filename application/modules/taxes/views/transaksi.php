    ...
    $(document).ready( function () {
        if( $('table#arrayOnProgressHistory') ){
            ...
            arrayOnProgressHistory = $('table#arrayOnProgressHistory').DataTable( {
                ...
                ajax: {
                    type: 'GET',
                    url: '" . base_url() . $this->uri->segment(1) . "/viewHistoryApi/$jenisTr/" . $steps[$stepper]['target'] . "?xxx=1&debuger=0',
                    datatype: 'json',
                    data: function (data) {
...
//            pageResize: true,
    ...
    case "createForm":
        ...
        if (sizeof($arrayOnProgress) > 0 || sizeof($arrayOnProgress2) > 0) {
            ...
            if (sizeof($arrayOnProgress) > 0) {
                ...
                if (sizeof($arrayOnProgress) > 0) {
                    ...
                    $strOnprog .= "<a class='btn btn-default' href='" . MODUL_PATH . $this->uri->segment(2) . "/viewIncomplete/" . $jenisTr . "'><span class='glyphicon glyphicon-time'></span> complete list ...</a>";
                    $strOnprog .= "</div class='text-right'>";
                    $strOnprog .= "</div class='panel-body'>";
        ...
        if (sizeof($uploadConfig) > 0) {
            ...
            $uploadData .= "<input type='file' name='fileExcel' class='form-control' accept='.xls,.xlsx,.csv' onchange=\"if(this.files && this.files[0] && this.files[0].size > 5242880){swal('File terlalu besar (maks 5MB)'); this.value='';}\">";
...
//        }
        ...
        if (sizeof($steps) > 1) {
            $strOnprog .= "<ul class='nav nav-tabs'>";
            foreach ($steps as $tStep => $stepData) {
                $badgeStepCount = isset($stepCounts[$tStep]) ? intval($stepCounts[$tStep]) : (isset($arrayOnprogressGroup[$tStep]) ? sizeof($arrayOnprogressGroup[$tStep]) : 0);
                $isiBadge = $badgeStepCount > 0 ? "<span class='badge bg-red'>" . $badgeStepCount . "</span>" : "";
                $actives = $tStep == $stepper ? "active" : "";
                $trSelesai = count($steps) == $tStep ? "SELESAI<br>" : "";
                    ...
                    $(document).ready( function(){
                        var table = $('#arrayOnProgress_step_$stepper').DataTable({
                            ...
                            initComplete: function(settings, json) {
                                ...
                                // Unbind default DataTables search (which causes typing lag on large datasets)
                                searchInput.unbind();

                                var doSearch = function(searchTerm) {
                                    api.search(searchTerm).draw();
                                    var recordsFiltered = api.page.info().recordsDisplay;
                                    if (recordsFiltered === 0 && searchTerm !== '') {
                                        var urlHistory = '" . MODUL_PATH . "History/viewHistory/" . $this->uri->segment(4) . "';
                                        var urlFilterKiri = '" . MODUL_PATH . "Transaksi/" . $this->uri->segment(3) . "/" . $this->uri->segment(4) . "?step=' + '$stepper';

                                        window.triggerTypoSearch_$stepper = function() {
                                            var newVal = $('#swalTypoInput').val();
                                            var input = $(api.table().container()).find('.dataTables_filter input');
                                            try {
                                                if (typeof swal.close === 'function') { swal.close(); } else if (typeof swal.closeModal === 'function') { swal.closeModal(); } else { $('.swal2-container').remove(); $('body').removeClass('swal2-shown'); }
                                            } catch(e) {}
                                            if(input.length > 0) { input.val(newVal); }

                                            // Delay to allow swal close animation
                                            setTimeout(function(){
                                                doSearch(newVal);
                                            }, 400);
                                        };
                                        setTimeout(function(){
                                            var inp = document.getElementById('swalTypoInput');
                                            if(inp){ inp.focus(); inp.select(); }
                                            var btn = document.getElementById('btnSwalTypo');
                                            if(btn) {
                                                $(btn).off('click').on('click', function(e) {
                                                    e.preventDefault();
                                                    window.triggerTypoSearch_$stepper();
                                                });
                                            }
                                        }, 400);
                                          window.checkTypoInput_$stepper = function(e) {
                                              var btn = document.getElementById('btnSwalTypo');
                                              var input = document.getElementById('swalTypoInput');
                                              if (!btn || !input) return;
                                              var orig = input.getAttribute('data-orig');
                                              if (input.value.trim() !== orig && input.value.trim() !== '') {
                                                  btn.disabled = false;
                                        ...
                                        swal({
                                            ...
                                            html: 'Data untuk pencarian <b>' + searchTerm + '</b> tidak ada di tabel ini.<br><br>' +
                                                  '<div style=\"background: #f9f9f9; padding: 10px; border-radius: 5px; margin-bottom: 15px; border: 1px solid #ddd;\">' +
                                                  '<label style=\"font-size:12px; font-weight:normal;\">Perbaiki Typo untuk tabel ini:</label>' +
                                                  '<input type=\"text\" id=\"swalTypoInput\" class=\"form-control\" data-orig=\"' + searchTerm.replace(/\"/g, '&quot;') + '\" value=\"' + searchTerm.replace(/\"/g, '&quot;') + '\" onkeyup=\"window.checkTypoInput_$stepper(event);\" style=\"margin-bottom: 10px; text-align: center;\">' +
                                                  '<button type=\"button\" id=\"btnSwalTypo\" class=\"btn btn-default btn-block\" disabled=\"disabled\" style=\"border:1px solid #aaa; color:#333;\"><i class=\"fa fa-refresh\"></i> Cari Ulang Lokal</button>' +
                                                  '</div>Atau pilih aksi pencarian global:',
                                            type: 'warning',
                                            showCancelButton: true,
                                            confirmButtonText: '<i class=\"fa fa-history\"></i> Seluruh Riwayat',
                                            cancelButtonText: '<i class=\"fa fa-search\"></i> Filter Kiri (Global)',
                                            confirmButtonColor: '#3085d6',
                                            cancelButtonColor: '#ff9800',
                                        }).then(function(confirm) {
                                            if (confirm) {
                                                window.open(urlHistory + '?search=' + encodeURIComponent(searchTerm), 'taxesHistoryTab');
                                            }
                                        }, function(dismiss) {
                                            if (dismiss === 'cancel') {
                                                $('#search_ky').val(searchTerm);
                                                $('#month').val('');
                                                kirimFilter(urlFilterKiri, 'undoneList', 'search_ky');
                                ...
                                searchInput.on('keyup', function(e) {
                                    ...
                                    if(e.which == 13) {
                                        // On ENTER: Search immediately and show SweetAlert if empty
                                        doSearch(searchTerm);
                                    } else {
                                        // On typing: debounce search by 800ms and show SweetAlert if empty
...
//        }
        ...
        $strOnprog .= "<style type='text/css'>
            .bg-highlight {
                background-color: #28a745 !important; /* Hijau misalnya */
        ...
        $strOnprog .= "<div class=\"box-header with-border text-red\">";
        $strOnprog .= "<h4 class=\"box-title text-uppercase blink\">$onprogressTitle</h4>";
//        $strOnprog .= "<span class=\"btn btn-sm btn-warning pull-right\" onclick=\"viewundoneList()\"><i class='fa fa-refresh'></i> reload</h4>";
        $strOnprog .= "<span class=\"btn btn-sm btn-warning pull-right\" 
            onclick=\"if($('#undoneList>div>div.box-body')){ $('#undoneList>div>div.box-body').append(`<div id='overlay'><div id='text'>Loading Content...</div></div>`); document.getElementById('overlay').style.display = 'block'; $('#undoneList').load('$reloadLink'); top.$('#floating-box').removeClass('show'); }else{ top.$('#floating-box').removeClass('show'); console.log('tidak ada container undoneList'); }\">
            <i class='fa fa-refresh'></i> reload</h4>";
        $strOnprog .= "</div>";
        ...
        if (sizeof($steps) > 1) {
            ...
            foreach ($steps as $tStep => $stepData) {
                $badgeStepCount = isset($stepCounts[$tStep]) ? intval($stepCounts[$tStep]) : (isset($arrayOnprogressGroup[$tStep]) ? sizeof($arrayOnprogressGroup[$tStep]) : 0);
                $isiBadge = $badgeStepCount > 0 ? "<span class='badge bg-red' id='tab_badge_step_$tStep'>" . $badgeStepCount . "</span>" : "<span class='badge bg-red' id='tab_badge_step_$tStep' style='display:none;'>0</span>";
                $actives = $tStep == $stepper ? "active" : "";
                $trSelesai = count($steps) == $tStep ? "SELESAI<br>" : "";
                $filterData = ($getDate != NULL) ? "&date=$getDate" : "";

                $cssSelesai = count($steps) == $tStep ? "style='padding-top: 0;padding-bottom: 0;'" : "";
                $strOnprog .= "<li class='$actives'>";
                $undoneLinkIndex = MODUL_PATH . $this->uri->segment(2) . "/" . $this->uri->segment(3) . "/" . $this->uri->segment(4) . "?step=$tStep" . $filterData;
                $varUrl = $tStep == $stepper ? "javascript:void(0);" : "if($('#undoneList>div>div.box-body')){ $('#undoneList>div>div.box-body').append(`<div id='overlay'><div id='text'>Loading Content...</div></div>`); document.getElementById('overlay').style.display = 'block'; $('#undoneList').load('$undoneLinkIndex'); top.$('#floating-box').removeClass('show'); }else{ top.$('#floating-box').removeClass('show'); console.log('tidak ada container undoneList'); }";
                $strOnprog .= "<a $cssSelesai class='nav-link btn' onclick=\"$varUrl\">";
                $strOnprog .= "<span class='text-uppercase text-bold'>$trSelesai" . $stepData['label'] . "</span>  $isiBadge </a>";
                $strOnprog .= "</li>";
            }
        ...
        // cekHere("$stepper");
        // arrPrintKuning($arrayOnProgress[$stepper]);
        // if (isset($arrayOnprogressGroup[$stepper]) && sizeof($arrayOnprogressGroup[$stepper]) > 0) {
        if (isset($arrayOnProgress[$stepper]) && sizeof($arrayOnProgress[$stepper]) > 0) {
            // $arrayOnProgress = $arrayOnprogressGroup[$stepper];
            $arrayOnProgressData = $arrayOnProgress[$stepper];
            $arrayOnprogressMarking = (isset($arrayOnprogressGroupPartialMark[$stepper]) && (sizeof($arrayOnprogressGroupPartialMark[$stepper]) > 0)) ? $arrayOnprogressGroupPartialMark[$stepper] : "";

        }
        else {
            $arrayOnProgressData = array();
            $arrayOnprogressMarking = array();
        }

        $item_data = $items = $arrayOnProgressData;
        $itemJml = count($items);
//arrPrint($item_data);
        if ($itemJml > 0 && !($stepper == 1 && $jenisTr == "110") && $stepper != "pre_gunggungan") {

            /** ---------------------------------------
             * limit untuk tertua maupun termuda
             * ------------------------------------------*/
            $limit = 100;
            ...
            // arrPrintHijau($segmenTua);

            $segmenMuda = $segmen;
            $segmenMuda[5] = $limit;
            $segmenMuda[6] = "desc";
            // arrPrintHijau($semens);
            ksort($segmenAsli);
            ksort($segmen);
            ksort($segmenTua);
            ksort($segmenMuda);
            $segmenUrlTua = implode("/", $segmenTua);
            $segmenUrlMuda = implode("/", $segmenMuda);
            $segmenUrl = implode("/", $segmen);
            $segmenUrlAsli = implode("/", $segmenAsli);
            // arrPrintPink($segmenUrl);
            $gets = $_GET;
            unset($gets['page']);
            $xtraHref = http_build_query($gets);
            $kw = isset($gets['kw']) ? $gets['kw'] : "";
            unset($gets['kw']);
            $xtraHrefClear = http_build_query($gets);
            $getDateMonth = isset($gets['date']) ? $gets['date'] : "";
            unset($gets['date']);
            $xtraHref = http_build_query($gets);
            $xtraHrefClear = http_build_query($gets);
            ...
            // ----------------------------sorting-----------------------------------------------------------------------------
            $btn .= "&nbsp;";
            $btn .= "<ul class='pagination modal-1'>";
            $link_tertua = base_url() . $segmenUrlTua . "?$xtraHref";
            $link_termuda = base_url() . $segmenUrlMuda . "?$xtraHref";
            $link_reset = base_url() . $segmenUrl . "?$xtraHref";
            if ($urutanDb == "asc") {
                $btn .= "<li><span class='current'>$limit Tertua</span></li>";
            }
            else {
                $btn .= "<li><a href='javascript:void(0);' onclick=\"loadPaging('$link_tertua','undoneList')\">$limit Tertua</a></li>";
            }
            // -----------------------------------------------
            if ($urutanDb == "desc") {
                $btn .= "<li><span class='current'>$limit Termuda</span></li>";
            }
            else {
                $btn .= "<li><a href='javascript:void(0);' onclick=\"loadPaging('$link_termuda','undoneList')\">$limit Termuda</a></li>";
            }
            $btn .= "<li><a href='javascript:void(0);' onclick=\"loadPaging('$link_reset','undoneList')\">All</a></li>";
            $btn .= "</ul>";
            // ---------------------------searching------------------------------------------------------------------------------
            $btn .= "&nbsp;";
            $link_quey = base_url() . $segmenUrlAsli . "?$xtraHref";
            $link_quey_clear = base_url() . $segmenUrlAsli . "?$xtraHrefClear";
//cekHere($link_quey);
//cekHitam($link_quey_clear);
            $btn .= "<ul class='pagination modal-1'>";
            $btn .= "<li><input id='month' type='month' name='month' value='$getDateMonth' placeholder='pilih bulan/tahun' onchange=\"loadPaging('$link_quey&date='+this.value, 'undoneList');\"></li>";
            $btn .= "<li><input id='search_ky' type='text' name='kw' value='$kw' placeholder='Tulis kata kunci' onkeydown=\"\"></li>";
            $btn .= "<li><a href='javascript:void(0);' onclick=\"kirimFilter('$link_quey','undoneList','search_ky');\">Terapkan Filter</a></li>";
            $btn .= "<li><a href='javascript:void(0);' onclick=\"loadPaging('$link_quey_clear','undoneList');\">Clear Filter</a></li>";
            $btn .= "</ul>";
//            cekHere("[$entry_faktur_masal]");
            if ($entry_faktur_masal == true) {

                $btn .= "<ul class='pagination modal-1' style='margin-left: 10px;'>";
                $btn .= "<li><a id='buka_modal_masal' href='javascript:void(0);' onclick=\"buka_modal_masal();\" style='border-radius:6px;'>Entry faktur masal</a></li>";
                $btn .= "</ul>";
            }

            $btn .= "<script>
                function kirimFilter(link,id_target, id_sumber) {

                  let kw = encodeURI($('#' + id_sumber).val());
                  let url = link + '&kw=' + kw;
                  console.log('kw:', url);
                  
                  loadPaging(url,id_target);
                }
            </script>";
            // ---------------------------------------------------------------------------------------------------------

            $strOnprog .= "<div>$btnPage $btn</div>";
            ...
            if (sizeof($arrayProgressLabels) > 0) {
                $strOnprog .= "<th class=''>No.</th>";
                // $strOnprog .= "<th class=''><input type='checkbox' id='pilih_all_tax'> Select</th>";
                foreach ($arrayProgressLabels as $key => $label) {
                    $strOnprog .= "<th class=''>";
                    if (is_array($label)) {
                        $strOnprog .= isset($label['label']) ? $label['label'] : "-";
            ...
            elseif ($gPage > 1) {
                $no = $jmlPerhalaman * ($gPage - 1);
            }

            // foreach ($arrayOnProgress as $key => $val) {
            foreach ($item_data as $key => $val) {
                //----------------------
                $trid = $val["transaksi_id"];
                //----------------------
//                $background_color = isset($arrayOnprogressMarking[$key]['style']) ? $arrayOnprogressMarking[$key]['style'] : "";
                $background_color = isset($val['row_style']) ? $val['row_style'] : (isset($arrayOnprogressGunggungMark[$key]['style']) ? $arrayOnprogressGunggungMark[$key]['style'] : "");
//                arrPrintCyan($val);
                $no++;
                $strOnprog .= "<tr id='line_$trid' line=" . __LINE__ . " style='$background_color;'>";
                $strOnprog .= "<td>$no</td>";
                if (sizeof($arrayProgressLabels) > 0) {
                    foreach ($arrayProgressLabels as $key => $label) {
                        $strOnprog .= "<td>";
            ...
            $strOnprog .= "<script>
                    ...
                    $(document).ready( function(){
                        var table = $('#arrayOnProgress_step_$stepper').DataTable({
                            ...
                            pageLength: -1,
                            paging: false,
                            oLanguage: {'sSearch' :'Search hanya pada halaman ini:'},
                            buttons: [],
//                            buttons: [
//                                        'copy', 'csv', 'excel', 'pdf', 'print'
//                                    ],
            ...
            $link_prev = base_url() . "taxes/FollowUp/followupPrePreviewMasal/111/0/2/1";
            $strOnprog .= "<script>
                $(document).ready(function() {
                    $('#pilih_all_tax').on('change', function() {
                        let isChecked = $(this).is(':checked');
                        $('.tic_tax').prop('checked', isChecked);
                                                
                    });
                    
                    function updateButtonState() {
                        let checkedCount = $('.tic_tax:checked').length;
        
                        if (checkedCount > 1) {
                            $('#buka_modal_masal').addClass('bg-highlight');
                        } else {
                            $('#buka_modal_masal').removeClass('bg-highlight');
                        }
                    }
                
                    // Saat checkbox diubah
                    $(document).on('change', '.tic_tax', function() {
                        updateButtonState();
                    });
                
                    // Juga panggil saat tombol \"Pilih Semua\" diklik
                    $('#pilih_all_tax').on('change', function() {
                        updateButtonState();
                    });
                     ...
                     function buka_modal_masal() {
                        let selected = [];
                    
                        $('.tic_tax:checked').each(function() {
                            selected.push($(this).val()); // atau bisa pakai data-attribute seperti $(this).data('id')
                        });
                    
                        if (selected.length <= 1) {
                            swal('Perhatian', 'Pilih minimal 2 transaksi untuk bisa mengunakan fitur ini.', 'warning');
                            return;
                        }
                    
                        // Kirim data ke modal, atau lakukan sesuatu
                        console.log('Data terpilih:', selected);
                    
                        // Contoh: Buka modal
                        // $('#modal_masal').modal('show');       
                        top.$('#result').load('$link_prev?id=' + selected);                                     
                    }
                    
            </script>";
        }
        else {
            if (($stepper == 1 && $jenisTr == "110") || $stepper == "pre_gunggungan") {
                $undoneLinkIndex = MODUL_PATH . $this->uri->segment(2) . "/viewGunggunganIndex/" . $this->uri->segment(4) . "?step=1&debuger=0";
                $getDateMonth = isset($_GET['date']) ? $_GET['date'] : "";
                $initAllCount = isset($allStepOneCount) ? intval($allStepOneCount) : 0;
                $initNpwpCount = isset($npwpStepOneCount) ? intval($npwpStepOneCount) : 0;
                $initGunggungCount = isset($nonNpwpCount) ? intval($nonNpwpCount) : 0;
                $displayNpwpCount = ($initNpwpCount > 0) ? $initNpwpCount : "-";
                $displayGunggungCount = ($initGunggungCount > 0) ? $initGunggungCount : "-";
                $tableId = "table_step_1_unified";

                // Toolbar Filter (Lv1 Tombol Filter + Periode Bulan & Lv2 Filter Cabang)
                $strOnprog .= "
                <div class='well well-sm' style='background:#fcfcfc; border:1px solid #e3e3e3; margin-bottom:12px; padding:10px 15px;'>
                    <div class='row'>
                        <div class='col-md-7 col-sm-12' style='padding-top:2px;'>
                            <span style='font-size:12px; font-weight:bold; color:#555; margin-right:8px;'><i class='fa fa-filter'></i> Filter:</span>
                            <div class='btn-group' id='unified_filter_buttons' role='group'>
                                <button type='button' class='btn btn-primary btn-sm' id='btn_filter_all' onclick=\"setStep1Filter('all');\">
                                    <i class='fa fa-list'></i> <b>SEMUA</b> <span class='badge' style='background:#ffffff; color:#337ab7; margin-left:4px;' id='badge_count_all'>$initAllCount</span>
                                </button>
                                <button type='button' class='btn btn-default btn-sm' id='btn_filter_npwp' onclick=\"setStep1Filter('npwp');\">
                                    <i class='fa fa-id-card-o'></i> <b>NPWP</b> <span class='badge bg-green' style='margin-left:4px;' id='badge_count_npwp'>$displayNpwpCount</span>
                                </button>
                                <button type='button' class='btn btn-default btn-sm' id='btn_filter_gunggungan' onclick=\"setStep1Filter('gunggungan');\">
                                    <i class='fa fa-cubes'></i> <b>GUNGGUNGAN</b> <span class='badge bg-yellow' style='margin-left:4px;' id='badge_count_gunggungan'>$displayGunggungCount</span>
                                </button>
                            </div>
                        </div>
                        <div class='col-md-5 col-sm-12 text-right'>
                            <div class='form-inline'>
                                <div class='form-group'>
                                    <label for='step1_month' style='font-size:12px; margin-right:5px; color:#555;'><i class='fa fa-calendar'></i> Periode:</label>
                                    <input id='step1_month' type='month' class='form-control input-sm' value='$getDateMonth' placeholder='Pilih bulan/tahun' onchange=\"reloadStep1Table();\" style='width:160px; display:inline-block;'>
                                </div>
                                <button type='button' class='btn btn-default btn-sm' onclick=\"$('#step1_month').val(''); reloadStep1Table();\" title='Clear Periode'>
                                    <i class='fa fa-times'></i> Clear
                                </button>
                            </div>
                        </div>
                    </div>
                    <div class='row' style='margin-top:8px; padding-top:8px; border-top:1px dashed #e0e0e0;'>
                        <div class='col-md-12' style='padding-top:2px;'>
                            <span style='font-size:12px; font-weight:bold; color:#555; margin-right:8px;'><i class='fa fa-building-o'></i> Filter Cabang:</span>
                            <div class='btn-group' id='unified_filter_cabang_buttons' role='group'>
                                <button type='button' class='btn btn-primary btn-xs' id='btn_cabang_all' onclick=\"setStep1Cabang('all');\">
                                    <b>SEMUA CABANG</b> <span class='badge' style='background:#ffffff; color:#337ab7; margin-left:3px;' id='badge_cabang_all'>-</span>
                                </button>
                                <button type='button' class='btn btn-default btn-xs' id='btn_cabang_m1' onclick=\"setStep1Cabang('-1');\">
                                    <b>001 - PUSAT (DC)</b> <span class='badge bg-navy' style='margin-left:3px;' id='badge_cabang_m1'>-</span>
                                </button>
                                <button type='button' class='btn btn-default btn-xs' id='btn_cabang_1' onclick=\"setStep1Cabang('1');\">
                                    <b>101 - CABANG PINANG</b> <span class='badge bg-teal' style='margin-left:3px;' id='badge_cabang_1'>-</span>
                                </button>
                                <button type='button' class='btn btn-default btn-xs' id='btn_cabang_33' onclick=\"setStep1Cabang('33');\">
                                    <b>102 - SURABAYA</b> <span class='badge bg-purple' style='margin-left:3px;' id='badge_cabang_33'>-</span>
                                </button>
                                <button type='button' class='btn btn-default btn-xs' id='btn_cabang_34' onclick=\"setStep1Cabang('34');\">
                                    <b>103 - GADING SERPONG</b> <span class='badge bg-orange' style='margin-left:3px;' id='badge_cabang_34'>-</span>
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                // Hitung index kolom yang default hidden (bisa dimunculkan via tombol ColVis 'Kolom')
                // Catatan: 'cabang2_nama' (Cabang) ditampilkan default, 'cabang2_kode' (Kode Cabang) hidden default
                $colIdx = 1;
                $hiddenColIndexes = array();
                $targetHidden = array(
                    'cabang2_kode',
                    'jenis_pajak',
                    'projectName',
                ...
                if (sizeof($arrayProgressLabels) > 0) {
                    foreach ($arrayProgressLabels as $kCol => $lCol) {
                        if (in_array($kCol, $targetHidden)) {
                            $hiddenColIndexes[] = $colIdx;
                        }
                        $colIdx++;
                    }
                }
                $jsonHiddenCols = json_encode($hiddenColIndexes);
                // END OF COMPLETE REPEATED LOGIC

                // Tabel Container
                $strOnprog .= "<div class='table-responsive'>";
                $strOnprog .= "<table id='$tableId' class='table dataTable compact table-hover' style='width:100%;'>";
                $strOnprog .= "<thead>";
                $strOnprog .= "<tr class='text-capitalize'>";
                if (sizeof($arrayProgressLabels) > 0) {
                    $strOnprog .= "<th class='text-capitalize'>No.</th>";
                    foreach ($arrayProgressLabels as $key => $label) {
                        $strOnprog .= "<th class='text-capitalize'>";
                        if (is_array($label)) {
                            $strOnprog .= isset($label['label']) ? $label['label'] : "-";
                        }
                        else {
                ...
                $strOnprog .= "</tr>";
                $strOnprog .= "</thead>";
                $strOnprog .= "<tbody></tbody>";
                $strOnprog .= "</table>";
                $strOnprog .= "</div>";

                $strOnprog .= "<style>
                    #table_step_1_unified_processing {
                ...
                $strOnprog .= "<script>
                                    ...
                                    $('#undoneList>div>div.box-body').append(`<div id='overlay'><div id='text'>Loading Content...</div></div>`);
                                    document.getElementById('overlay').style.display = 'block';

                    var activeFilterPajak = 'all';
                    var activeFilterCabang = 'all';
                    window.step1CountBadge = null;
                    window.step1CountCabang = null;

                    function updateBadgesDisplay() {
                        if (!window.step1CountCabang || !window.step1CountBadge) return;

                        var pKey = activeFilterPajak || 'all';
                        var cKey = activeFilterCabang || 'all';
                        var cc = window.step1CountCabang;
                        var cb = window.step1CountBadge;

                        // 1. Update Lv1 Badges (SEMUA, NPWP, GUNGGUNGAN) berdasarkan filter cabang aktif
                        if (cKey !== 'all' && cc[cKey]) {
                            $('#badge_count_all').text(cc[cKey].all);
                            $('#badge_count_npwp').text(cc[cKey].npwp);
                            $('#badge_count_gunggungan').text(cc[cKey].gunggungan);
                        } else {
                            $('#badge_count_all').text(cb.all);
                            $('#badge_count_npwp').text(cb.npwp);
                            $('#badge_count_gunggungan').text(cb.gunggungan);
                        }

                        // Tab badge di header selalu menampilkan total keseluruhan yang belum diproses
                        var tabBadge = $('#tab_badge_step_1');
                        if (tabBadge.length) {
                            tabBadge.text(cb.all);
                            if (parseInt(cb.all, 10) > 0) {
                                tabBadge.show();
                            } else {
                                tabBadge.hide();
                            }
                        ...
                        // 2. Update Lv2 Badges (Cabang) berdasarkan filter pajak aktif (all / npwp / gunggungan)
                        var valAll = (cb[pKey] !== undefined) ? cb[pKey] : 0;
                        var valM1 = (cc['-1'] && cc['-1'][pKey] !== undefined) ? cc['-1'][pKey] : 0;
                        var val1 = (cc['1'] && cc['1'][pKey] !== undefined) ? cc['1'][pKey] : 0;
                        var val33 = (cc['33'] && cc['33'][pKey] !== undefined) ? cc['33'][pKey] : 0;
                        var val34 = (cc['34'] && cc['34'][pKey] !== undefined) ? cc['34'][pKey] : 0;

                        $('#badge_cabang_all').text(valAll);
                        $('#badge_cabang_m1').text(valM1);
                        $('#badge_cabang_1').text(val1);
                        $('#badge_cabang_33').text(val33);
                        $('#badge_cabang_34').text(val34);
                    }

                    function setStep1Filter(filterType) {
                        activeFilterPajak = filterType;
                        $('#cross_search_alert').remove();
                        $('#unified_filter_buttons .btn').removeClass('btn-primary active').addClass('btn-default');
                        $('#btn_filter_' + filterType).removeClass('btn-default').addClass('btn-primary active');
                        updateBadgesDisplay();
                        reloadStep1Table();
                    }

                    function setStep1Cabang(cabangId) {
                        activeFilterCabang = String(cabangId);
                        $('#unified_filter_cabang_buttons .btn').removeClass('btn-primary active').addClass('btn-default');
                        var btnId = (cabangId === '-1') ? 'btn_cabang_m1' : (cabangId === 'all' ? 'btn_cabang_all' : 'btn_cabang_' + cabangId);
                        $('#' + btnId).removeClass('btn-default').addClass('btn-primary active');
                        updateBadgesDisplay();
                        reloadStep1Table();
                    }

                    function reloadStep1Table() {
                        if (typeof tableStep1 !== 'undefined') {
                            top.$('#undoneList>div>div.box-body').append(`<div id='overlay'><div id='text'>Loading Content...</div></div>`);
                            top.document.getElementById('overlay').style.display = 'block';
                            tableStep1.ajax.reload(function() {
                                top.document.getElementById('overlay').style.display = 'none';
                            });
                        }
                    }

                    var tableStep1 = $('#' + '$tableId').DataTable({
                        dom: 'lBpfrtip',
                        initComplete: function(settings, json) {
                            var api = this.api();
                            var searchInput = $(api.table().container()).find('.dataTables_filter input');
                            ...
                            // Unbind default DataTables search to fix typing lag
                            searchInput.unbind();
                        ...
                        ajax: {
                            ...
                            type: 'POST',
                            data: function(d) {
                                d.filter_pajak = activeFilterPajak;
                                d.filter_cabang = activeFilterCabang;
                                d.date = $('#step1_month').val();
                            },
                            dataSrc: function(json) {
                                window.lastCrossSearchMatch = (json && json.crossSearchMatch) ? json.crossSearchMatch : null;
                                if (json && json.countBadge) {
                                    window.step1CountBadge = json.countBadge;
                                }
                                if (json && json.countCabang) {
                                    window.step1CountCabang = json.countCabang;
                                }
                                updateBadgesDisplay();
                        ...
                        buttons: [
                            ...
                            {
                                text: \"<i class='fa fa-check-square'></i> check all\",
                                action: function (e, dt, node, config) {
                                    var items_check = top.$('.items_check');
                                    if (items_check.length > 10) {
                                        swal('memproses lebih dari 10 item akan membutuhkan banyak waktu, tetap lanjutkan?')
                                        .then(function(result) {
                                            if (result) {
                                                swal('Processing..!', '', 'info');
                                                swal.enableLoading();
                                                var total = items_check.length;
                                                var prog = 0;
                                                setTimeout(function() {
                                                    jQuery.each(items_check, function(a, b) {
                                                        prog++;
                                                        if (!$(b).is(':checked')) {
                                                            $(b).trigger('click');
                                                        }
                                ...
                                render: function (data, type, row, meta) {
                                    ...
                                    if (data.dataType) {
                                        if (data.value && data.value.link) {
                                            return \"<span class='text-bold text-uppercase text-link' onclick=\\\"openClassModal('\" + data.value.fieldValue + \"')\\\">\" + data.value.value + \"</span>\";
                                        }
                                        else {
                                            var vClass = (data.value && data.value.class) ? data.value.class : '';
                                            var vStyle = (data.value && data.value.style) ? data.value.style : '';
                                            var vVal = (data.value && data.value.value) ? data.value.value : '';
                                            return '<span ' + vClass + ' ' + vStyle + '>' + vVal + '</span>';
                                        }
                                    }
                                    else {
                                        if (data.key === 'action') {
                                            if (!data.is_npwp) {
                                                var checked = (data.checked == 1) ? 'checked' : '';
                                                return \"<div style='text-align:center;'><input class='items_check' \" + checked + \" onclick=\\\"checkBoxPost(this,'\" + data.link + \"')\\\" type='checkbox'></div>\";
                                            }
                                            else {
                                                return data.value;
                                            }
                        ...
                        rowCallback: function(row, data, index) {
                            ...
                            if (Array.isArray(data)) {
                                for (var i = 0; i < data.length; i++) {
                                    if (data[i] && data[i].key === 'action') {
                                        isNpwp = (data[i].is_npwp == 1 || data[i].is_npwp === true);
                                        break;
                                    }
                                }
                            }
                            if (!isNpwp) {
                                $(row).css('background-color', '#fffcf0').attr('title', 'Transaksi Non-NPWP (Dapat digunggung atau diproses individual)');
                            } else {
                                $(row).css('background-color', '').attr('title', 'Transaksi NPWP (Diproses individual E-Faktur)');
                            }

                            if (top.document.getElementById('overlay_floating')) {
                                top.document.getElementById('overlay_floating').style.display = 'none';
                            }
                            if (top.document.getElementById('overlay')) {
                                top.document.getElementById('overlay').style.display = 'none';
                            }
                            debouncedFunction();
                        },
                        ...
                        drawCallback: function(settings) {
                            var api = this.api();
                            var recordsFiltered = settings._iRecordsDisplay;
                            var searchTerm = api.search();

                            if (recordsFiltered === 0 && searchTerm !== '') {
                                var urlHistory = '" . MODUL_PATH . "History/viewHistory/110';
                                var safeSearchTerm = $('<div>').text(searchTerm).html();

                                if (window.lastCrossSearchMatch && window.lastCrossSearchMatch.count > 0) {
                                    var m = window.lastCrossSearchMatch;
                                    var title = 'Data Ditemukan di Kategori Lain';
                                    var htmlMsg = '';
                                    var btnText = '';
                                    var onConfirm = function() {};

                                    if (m.type === 'pajak') {
                                        title = 'Data Ditemukan di Kategori Lain';
                                        htmlMsg = 'Pencarian <b>\"' + safeSearchTerm + '\"</b> tidak ditemukan di kategori <b>' + m.currentFilterLabel + '</b>.<br><br>' +
                                                  'Namun ditemukan <b>' + m.count + ' data</b> di kategori <b>' + m.targetFilterLabel + '</b>.';
                                        btnText = '<i class=\"fa fa-arrow-right\"></i> Beralih ke ' + m.targetFilterLabel + ' (' + m.count + ')';
                                        onConfirm = function() {
                                            setStep1Filter(m.targetFilter);
                                        };
                                    } else if (m.type === 'cabang') {
                                        title = 'Data Ditemukan di Cabang Lain';
                                        htmlMsg = 'Pencarian <b>\"' + safeSearchTerm + '\"</b> tidak ditemukan di cabang <b>' + m.currentCabangNama + '</b>.<br><br>' +
                                                  'Namun ditemukan <b>' + m.count + ' data</b> di cabang <b>' + m.targetCabangNama + '</b>.';
                                        btnText = '<i class=\"fa fa-arrow-right\"></i> Beralih ke ' + m.targetCabangNama + ' (' + m.count + ')';
                                        onConfirm = function() {
                                            setStep1Cabang(m.targetCabang);
                                        };
                                    } else if (m.type === 'both') {
                                        title = 'Data Ditemukan di Cabang & Kategori Lain';
                                        htmlMsg = 'Pencarian <b>\"' + safeSearchTerm + '\"</b> tidak ditemukan di cabang <b>' + m.currentCabangNama + '</b> (' + m.currentFilterLabel + ').<br><br>' +
                                                  'Namun ditemukan <b>' + m.count + ' data</b> di cabang <b>' + m.targetCabangNama + '</b> (Kategori <b>' + m.targetFilterLabel + '</b>).';
                                        btnText = '<i class=\"fa fa-arrow-right\"></i> Beralih ke ' + m.targetCabangNama + ' (' + m.targetFilterLabel + ')';
                                        onConfirm = function() {
                                            setStep1Cabang(m.targetCabang);
                                            setStep1Filter(m.targetFilter);
                                        };
                                    ...
                                    swal({
                                        title: 'Data Tidak Ditemukan',
                                        html: 'Data untuk pencarian <b>\"' + safeSearchTerm + '\"</b> tidak ada di tabel ini.<br><br>Atau buka riwayat transaksi:',
                                        type: 'warning',
                                        showCancelButton: true,
                                        confirmButtonText: '<i class=\"fa fa-history\"></i> Seluruh Riwayat',
                                        cancelButtonText: '<i class=\"fa fa-times\"></i> Tutup',
                        ...
                        // END OF COMPLETE REPEATED LOGIC
                                        ...
                                        function checkSessionData() {
                                            var monthVal = $('#step1_month').val();
                                            var statikUrl = '" . MODUL_PATH . "FollowUp/previewGunggung/110?debuger=0' + ((monthVal && monthVal !== '') ? '&date=' + monthVal : '');
                                            $.get(statikUrl, function(apiResponse){
                                                var res = JSON.parse(apiResponse);
                                                if (res.hasData && res.data.dpp*1 > 0) {
                                                    //inputan E-Faktur
                                                    var strInputFaktur = '<table class=\"table dataTable compact table-bordered\">'
                                                        strInputFaktur += '<thead>'
                                                        strInputFaktur += '<tr bgcolor=\"#f5f5f5\">'
                                                        strInputFaktur += '<th class=\"text-muted\" style=\"font-weight:bold;\" align=\"right\" width=\"5\">no.</th>'
                                                        strInputFaktur += '<th class=\"text-muted\" style=\"font-weight:bold;\">JML INV</th>'
                                                        strInputFaktur += '<th class=\"text-muted\" style=\"font-weight:bold;\">DPP</th>'
                                                        strInputFaktur += '<th class=\"text-muted\" style=\"font-weight:bold;\">PPN</th>'
                                                        strInputFaktur += '<th class=\"text-muted\" style=\"font-weight:bold;\">DPP+PPN</th>'
                                                        strInputFaktur += '<th class=\"text-muted\" style=\"font-weight:bold;\">Tgl faktur </th>'
                                                        strInputFaktur += '<th class=\"text-muted\" style=\"font-weight:bold;\">nomer faktur</th>'
                                                        strInputFaktur += '</tr>'
                                                        strInputFaktur += '</thead>'
                                                        strInputFaktur += '<tbody>'
                                                        strInputFaktur += '<tr>'
                                                        strInputFaktur += '<td>1</td>'
                                                        strInputFaktur += '<td class=\"text-right text-bold\" id=\"td_efaktur_source\"><input disabled size=\"1\" type=\"text\" class=\"form-control text-center no-border no-shadow\" value=\"'+addCommas( (res.data.nota).toFixed(0) )+'\"></td>'
                                                        strInputFaktur += '<td class=\"text-right text-bold\" id=\"td_total_ui\"><input disabled size=\"7\" type=\"text\" class=\"form-control text-center no-border no-shadow\" value=\"'+addCommas( (res.data.dpp).toFixed(0) )+'\"></td>'
                                                        strInputFaktur += '<td class=\"text-right text-bold\" id=\"td_new_grand_ppn\"><input disabled size=\"7\" type=\"text\" class=\"form-control text-center no-border no-shadow\" value=\"'+addCommas( (res.data.ppn).toFixed(0) )+'\"></td>'
                                                        strInputFaktur += '<td class=\"text-right text-bold\" id=\"td_new_grand_ppn\"><input disabled size=\"7\" type=\"text\" class=\"form-control text-center no-border no-shadow\" value=\"'+addCommas( (res.data.dpp_nppn).toFixed(0) )+'\"></td>'
                                                        strInputFaktur += '<td id=\"td_dateFaktur\"> <input type=\"date\" id=\"dateFaktur\" class=\"form-efaktur form-control\" name=\"dateFaktur\" onclick=\"this.select()\" value=\"'+res.data.dateFaktur+'\" onblur=\"\"></td>'
                                                        strInputFaktur += '<td id=\"td_eFaktur\">    <input type=\"text\" id=\"eFaktur\" class=\"form-efaktur form-control\" name=\"eFaktur\" onclick=\"this.select()\" value=\"'+res.data.eFaktur+'\" onblur=\"\"></td>'
                                                        strInputFaktur += '</tr>'
                                                        strInputFaktur += '</tbody>'
                                                        strInputFaktur += '</table>'

//                                                    $('.floating-box-static-bottom').html(strInputFaktur);

                                                    var buttonNext = '<span class=\"btn btn-md btn-info text-bold\" onclick=\"checkNext(this);\"> LANJUTKAN </span>';

                                                    $('.floating-box-footer').html(buttonNext);
                                                    $('.floating-box-body > .isi-body').html(strInputFaktur);
                                                    $('#floating-box').addClass('show');
                                                    onEFakturEvent();
                                                }
                                                else {
                                                    $('#floating-box').removeClass('show');
                                                    $('.floating-box-body > .isi-body').html('')
                                                }
                                                top.$('#overlay_floating').css('display', 'none');
                                            });
                                        }

                                        const debouncedFunction = debounce(() => {
                                            checkSessionData();
                                            console.log('eksekusi dari debounce karena idle 0.7 detik');
                                        }, 700);

                                        function onEFakturEvent(){
                                            top.$('.form-efaktur').off();
                                            top.$('.form-efaktur').bind('keyup change', delay_v2(function(){
                                                var thisKey = $(this).attr('name');
                                                var thisVal = $(this).val();
                                                console.log('key: '+thisKey+' || value: '+thisVal);

                                                addTaxInfo(thisKey,thisVal);
                                            },700))
                                        }
                                        ...
                                        function checkNext(e){
                                            ...
                                            if(dateFaktur==''){
                                                er++;
                                                errText += '- Tanggal E-Faktur harap di isi.<br>'
                                            }
                                            if(eFaktur==''){
                                                er++;
                                                errText += '- E-Faktur harap di isi.'
                                            }
                                            if(er>0){
                                                top.swal('Info', errText , 'warning');
                                            }
                                            else{
                                                
                                                top.swal({
                                                  title: 'konfirmasi',
                                                  html: 'apakah anda ingin melanjutkan entry faktur?',
                                                  type: 'warning',
                                                ...
                                                .then((confirm) => {
                                                      if (confirm) {
                                                top.open_holdon();
                                                var urlFollowUpMulti = '" . MODUL_PATH . "FollowUp/doFollowupMulti/110/2/1';
                                                var ajaxUrlFollowUpMulti = urlFollowUpMulti + '?ajax=1&_ts=' + new Date().getTime();
                                                top.$.ajax({
                                                    url: ajaxUrlFollowUpMulti,
                                                    method: 'GET',
                                                    dataType: 'json',
                                                    cache: false,
                                                    timeout: 120000
                                                }).done(function(resp){
                                                    console.log('urlFollowUpMulti: ' + urlFollowUpMulti);
                                                    console.log(resp);
                                                    top.close_holdon();
                                                    if (resp && (resp.success === 1 || resp.success === true || resp.success === '1')) {
                                                top.location.reload();
                                                    } else {
                                                        var msg = (resp && resp.message) ? resp.message : 'Controller belum mengembalikan status sukses.';
                                                        var raw = (resp && resp.raw) ? '<br><small>' + resp.raw + '</small>' : '';
                                                        top.swal('Info', msg + raw, 'warning');
                                                    }
                                                }).fail(function(xhr, status){
                                                    top.close_holdon();
                                                    var respText = (xhr && xhr.responseText) ? xhr.responseText : '';
                                                    var detail = respText ? respText.substring(0, 500) : status;
                                                    top.swal('Error', 'Request doFollowupMulti gagal. ' + detail, 'error');
                                                });
                                                      }
                                                });
                                        ...
                                        function clearSesGunggung(){
                                            swal('nota akan dihapus dari pilihan di gunggung, yakin lanjutkan?<br><r>kamu bisa pilih lagi dari daftar input E-Faktur</r>')
                                            .then((result) => {
                                              if(result) {
                                                var statikUrl = '" . MODUL_PATH . "FollowUp/clearSesGunggung/110?debuger=0'
                                                $.get(statikUrl, function(apiResponse){
                                                    swal('Nota di Clear', '', 'info');
                                                    setTimeout(function(){
                                                        swal.close();
                                                    }, 700)
                                        ...
                                        function addTaxInfo(key,value){
                                            var statikUrl = '" . MODUL_PATH . "FollowUp/addTaxInfo/110?key='+key+'&value='+value+'&debuger=0'
                                            $.get(statikUrl, function(apiResponse){
                                                console.log( apiResponse );
                                                //debouncedFunction();
                                            });
                                        ...
                                        // START OF COMPLETE REPEATED LOGIC
                                        window.onRetakeLockSuccess = function() {
                                            if (window.lastLockedCheckbox && window.lastLockedCheckbox.elem) {
                                                var chk = window.lastLockedCheckbox.elem;
                                                var url = window.lastLockedCheckbox.url;
                                                $(chk).prop('checked', true);
                                                checkBoxPost(chk, url);
                                            } else if (typeof reloadStep1Table === 'function') {
                                                reloadStep1Table();
                                            }
                                        };
                                        if (typeof top !== 'undefined') {
                                            top.onRetakeLockSuccess = window.onRetakeLockSuccess;
                                        }

                                        window.handleRetakeLock = function(btn) {
                                            if (!btn) return;
                                            var btnObj = $(btn);
                                            var url = btnObj.attr('data-url');
                                            if (!url) return;

                                            btnObj.text('⏳ Memproses...').prop('disabled', true).css('opacity', '0.7');
                                            var sep = (url.indexOf('?') > -1) ? '&' : '?';
                                            var reqUrl = url + sep + 'ajax=1';
                                            ...
                                            $.ajax({
                                                ...
                                                success: function(resp) {
                                                    try {
                                                        if (top && top.swal && top.swal.close) {
                                                            top.swal.close();
                                                        } else if (typeof swal !== 'undefined' && swal.close) {
                                                            swal.close();
                                                        }
                                                        if (top && top.$) {
                                                            top.$('.swal2-container, .sweet-alert, .sweet-overlay').remove();
                                                            top.$('body').removeClass('swal2-shown stop-scrolling modal-open');
                                                        }
                                                    } catch(err) {}

                                                    var msg = (resp && resp.message) ? resp.message : 'Kunci transaksi berhasil diambil alih. Melanjutkan proses...';
                                                    if (top && top.swal) {
                                                        top.swal({
                                                            title: 'Berhasil!',
                                                            text: msg,
                                                    ...
                                                    if (typeof window.onRetakeLockSuccess === 'function') {
                                                        window.onRetakeLockSuccess();
                                                    } else if (top && typeof top.onRetakeLockSuccess === 'function') {
                                                        top.onRetakeLockSuccess();
                                                    } else if (typeof reloadStep1Table === 'function') {
                                                        reloadStep1Table();
                                                    }
                                                },
                                                error: function(xhr, status, error) {
                                                    btnObj.text('⚠️ Ambil Alih Transaksi (Petugas Idle)').prop('disabled', false).css('opacity', '1');
                                                    var errMsg = 'Gagal mengambil alih transaksi. Silakan coba lagi.';
                                                    try {
                                                        var jsonErr = JSON.parse(xhr.responseText);
                                                        if (jsonErr && jsonErr.message) errMsg = jsonErr.message;
                                                    } catch(e) {}
                                                    if (top && top.swal) {
                                                        top.swal('Error', errMsg, 'error');
                                                    } else {
                                                        alert(errMsg);
                                                    }
                                                }
                                        ...
                                        if (typeof top !== 'undefined') {
                                            top.handleRetakeLock = window.handleRetakeLock;
                                        }

                                        // Attach global event delegation on top.document and current document
                                        try {
                                            if (typeof top !== 'undefined' && top && top.$) {
                                                top.$(top.document).off('click.retakeLock', '#btn_force_retake_lock, .btn_force_retake_lock').on('click.retakeLock', '#btn_force_retake_lock, .btn_force_retake_lock', function(e){
                                                    e.preventDefault();
                                                    e.stopPropagation();
                                                    window.handleRetakeLock(this);
                                                });
                                                top.$(top.document).off('click.lockedClose', '#btn_locked_modal_close, .btn_locked_modal_close').on('click.lockedClose', '#btn_locked_modal_close, .btn_locked_modal_close', function(e){
                                                    e.preventDefault();
                                                    e.stopPropagation();
                                                    if(top && top.swal && top.swal.close){
                                                        top.swal.close();
                                        ...
                                        $(document).off('click.retakeLock', '#btn_force_retake_lock, .btn_force_retake_lock').on('click.retakeLock', '#btn_force_retake_lock, .btn_force_retake_lock', function(e){
                                            e.preventDefault();
                                            e.stopPropagation();
                                            window.handleRetakeLock(this);
                                        });
                                        $(document).off('click.lockedClose', '#btn_locked_modal_close, .btn_locked_modal_close').on('click.lockedClose', '#btn_locked_modal_close, .btn_locked_modal_close', function(e){
                                            e.preventDefault();
                                            e.stopPropagation();
                                            if(top && top.swal && top.swal.close){
                                                top.swal.close();
                                        ...
                                        function checkBoxPost(a,b){
                                            window.lastLockedCheckbox = { elem: a, url: b };
                                            var state = $(a).is(':checked');
                                            $.get(b+'?state='+state, function(prePreview){
                                                var cek = null;
                                                try {
                                                    cek = (typeof prePreview === 'object') ? prePreview : JSON.parse(prePreview);
                                                } catch(e) {
                                                    console.error('Failed to parse prePreview', e, prePreview);
                                                }

                                                if(cek && (cek.hasData == 1 || cek.hasData === true)){
                                                    debouncedFunction();
                                                    top.$('#overlay_floating').css('display', 'block');
                                                    console.log('memanggil debounce');
                                                }
                                                else{
                                                    $(a).prop('checked', false);
                                                    if(cek && cek.locked && cek.html){
                                                        top.swal({
                                                            title: '',
                                                            html: cek.html,
                                                            showConfirmButton: false,
                                                        ...
                                                        var modalTitle = (cek && cek.locked) ? 'Transaksi Terkunci' : 'Error';
                                                        var modalReason = (cek && cek.reason) ? cek.reason : 'Transaksi tidak dapat diproses atau sedang dikunci.';
                                                        top.swal({
                                                            title: modalTitle,
                                                            html: modalReason,
                                                            type: 'warning',
                                        ...
                                        function openClassModal(noinv){
                                            top.showModal('" . MODUL_PATH . "Transaksi/viewResume/110/'+noinv,'view resume for ' + noinv)
                                        }
    ...
    case "viewGunggunganIndex":
        cekHere("[viewGunggunganIndex]");
        $str = "";
        $arrBlacklist = array(
            "no",
        );
        ...
        if ($itemJml > 0) {
            ...
            // ---------------------------searching------------------------------------------------------------------------------
            $btn .= "&nbsp;";
            $link_quey = base_url() . $segmenUrlAsli . "?$xtraHref" . $filterData;
            $btn .= "<form class='pagination modal-1' method='get' action='$link_quey' target='undoneList'>";
            $btn .= "<ul class='pagination modal-1'>";
            $btn .= "<li><input id='month' type='month' name='month' value='$getDateMonth' placeholder='pilih bukan/tahun' onchange=\"loadPaging('$link_quey&date='+this.value, 'undoneList');\"></li>";
            $btn .= "<li><input id='search_ky' type='text' name='kw'></li>";
            $btn .= "<li><input type='button' onclick=\"kirimFilter('$link_quey','undoneList','search_ky');\" value='Terapkan Filter'></li>";
            $btn .= "<li><a href='javascript:void(0);' onclick=\"kirimFilter('$link_quey','undoneList','search_ky');\">Terapkan Filter</a></li>";
            $btn .= "</ul>";
            $btn .= "</form>";
            $btn .= "<script>
...
//                        $val = isset($detailValues[$id][$key]) ? $detailValues[$id][$key] : $subVal;
                        ...
                        if (($allowEdit == true) && (in_array($key, $editableFields))) {
                            ...
                            if (is_numeric($val)) {
                                if (!array_key_exists($key, $editableFieldsType)) {
                                    $val += 0;
                            ...
                            if (array_key_exists($key, $editableFieldsType)) {
                                cekOrange("HAHAHAHA");
                                $inputType = $editableFieldsType[$key];
//                                $addEvent = "  onblur=\"top.$('#result').load('$updateItemFieldTarget?ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL&id=$id&key=$key&val='+removeCommas(this.value))\" ";
                                $addEvent = "  onblur=\"top.$('#result').load('$updateItemFieldTarget?ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL&id=$id&key=$key&val='+this.value)\" ";
...
//                    }
                ...
                if ((isset($itemLabels3)) && (sizeof($itemLabels3) > 1)) {
...
//                        echo $mainValues[$key];
                        ...
                        if (in_array($key, $extEditableFields)) {
                            $defValue = (0 + $val);
                            echo "<input type=number class='form-control text-right' name='$key' step='1000' value='" . ($defValue) . "' min='0' max='" . ($defValue) . "' 
                            onkeyup=\"if(parseInt(this.value)>$defValue || parseInt(this.value)<0){this.value='$defValue';}\">";
                        }
                        else {
                            echo formatField($key, $val);
                        }
...
//                            echo $mainValues[$key . "_tax"];
                            ...
                            if (in_array($key, $extEditableFields)) {
                                $defValue = (0 + $val);
                                echo "<input type=number class='form-control text-right' name='$key" . "_tax" . "' step=1000 value='" . ($defValue) . "' min='0' max='" . ($defValue) . "' 
                                onkeyup=\"if (parseInt(this.value) > $defValue || parseInt(this.value)<0) {this.value= '$defValue';}\">";
                            }
                            else {
                ...
                if (isset($addMainSourceField) && sizeof($addMainSourceField) > 0) {
                    $faktur = "";
                    $faktur .= "<div class='table-responsive'>";
                    $faktur .= "<table class='table table-bordered table-condensed'>";
                    $faktur .= "<tr bgcolor='#f5f5f5'>";
                    $faktur .= "<th class='text-muted' style='font-weight:bold;' align='right' width='5'>no.</th>";
                    $countItems = sizeof($items);
                    if ($countItems == 1) {
                        foreach ($addMainSourceFieldHidden as $vall) {
                            if (array_key_exists($vall, $addMainSourceField)) {
                                unset($addMainSourceField[$vall]);
                            }
                        }
                    }
                    foreach ($addMainSourceField as $key => $label) {
                        $faktur .= "<th class='text-muted' style='font-weight:bold;'>";
                        $faktur .= $label;
                        $faktur .= "</th>";
                    }
                    $faktur .= "</tr>";
                    ...
                    if ($pakai_ini == 1) {
                        ...
                        if (sizeof($formulirFaktur) > 0) {
                            $countItems = sizeof($items);
                            if ($countItems == 1) {
                                unset($addMainSourceEdit["dpp_final"]);
                                foreach ($addMainSourceFieldHidden as $vall) {
                                    if (array_key_exists($vall, $addMainSourceField)) {
                                        unset($addMainSourceField[$vall]);
                                    }
...
//                                arrPrintHitam($fSpec);
                                $faktur .= "<tr>";
                                $faktur .= "<td>$nom</td>";
                                foreach ($addMainSourceField as $fff => $f_labels) {
                                    $linkFaktur = MODUL_PATH . $shopingCartFakturTarget . "/";
                                    $labels = "";
                                    $btn_formulir = "";
                                    $btn_formulir_delete = "";
                                    if (isset($addMainSourceEdit[$fff])) {
                                        $inputType = $addMainSourceEdit[$fff];
                                        $defValues = isset($fSpec[$fff]) ? $fSpec[$fff] : "";
                                        if ($addMainSourceEdit[$fff] == "checkbox") {
                                            $classinputType = "";
                                            $labels = ($ctt == 0) ? "" : "";
                                            $vals = "checked";
                                            ...
                                            $vals = "value";
                                            $checked = "";
                                            $addEvent = " onblur=\"document.getElementById('result').src='$updateMainSourceTarget$ctt?gunggunganMode=$gunggunganModeTransaksi&ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL&id=$fff&val='+this.value\" ";
                                            $value = "<input type='$inputType' id='$fff' class='$classinputType' name='$fff' 
                                                onclick='this.select()' value='$defValues' $checked $addEvent>";
                                        }
                                    }
                                    else {
                            ...
                            foreach ($addMainSourceField as $kol => $alias) {
                                if (isset($addMainSourceEdit[$kol])) {
                                    $inputType = $addMainSourceEdit[$kol];
                                    $val = isset($mainValues[$kol]) ? $mainValues[$kol] : "";
                                    $addEvent = " onblur=\"document.getElementById('result').src='$updateMainSourceTarget?gunggunganMode=$gunggunganModeTransaksi&ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL&id=$kol&&val='+this.value\" ";
                                    $strVal = "<input type=$inputType name='$kol' class='form-control text-left' value='$val' onclick='this.select()' $addEvent>";
                                    $tdOpt = "style='margin:0px;padding:0px;$addStyle' ";
                                }
                                else {
...
//                }
            ...
            //untuk checkbox default desabled
            $pakai = 1;
            if ($pakai == 1) {
                if (isset($approvalSpec['targetUrl']) != "" && $approvalSpec['targetUrl'] != "") {
                    echo "<div class='form-group'>";
                    echo "<input type='checkbox' id='allow_next' onclick=\"allow_follow();\"> ";
                    echo "<label class='text-bold text-danger'><h4>Tic disini untuk melanjutkan</h4>";
                    echo "</label>";
                    echo "</div class='col-md-6'>";
                }
            }
...
//                        echo "&nbsp;&nbsp;";
                    ...
                    if (isset($approvalSpec['targetUrl']) != "" && $approvalSpec['targetUrl'] != "") {
                        ...
                        echo "<button button disabled id='approve_btn'type='button' class='btn btn-success margin' style='border:1px #008800 solid;color:#ffffff;' 
                        onclick=\"if(confirm('" . $approvalSpec['warning'] . "')==1){this.disabled=true;document.getElementById('f1').action='" . $approvalSpec['targetUrl'] . "';
                        document.getElementById('f1').submit();top.open_holdon();}\"><span class='glyphicon glyphicon-ok'></span> " . $approvalSpec['label'] . "</button>";
                    }
                    else {
                        echo "&nbsp;";
                    }
                ...
                $btn_ff = "<script>
                            function allow_follow() {
                              if(document.getElementById('allow_next').checked) {
                                  document.getElementById('approve_btn').removeAttribute('disabled');
                              }
                              else{
                                  document.getElementById('approve_btn').setAttribute('disabled','disabled');
                              }
...
//                }
        ...
        echo "<script>  $('.modal-dialog').removeClass('modal-lg').addClass('modal-xl');top.close_holdon();</script>";
...
//                        cekHere($label." - ".$key." - ".$val);
            ...
            if (isset($items2) && sizeof($items2) > 0) {
                ...
                if (defined('ENVIRONMENT') && ENVIRONMENT !== 'production') { arrPrint($sumRows2); }
...
//            $strOnprogFooter = "<a class='btn btn-default' href='" . base_url() . $this->uri->segment(1) . "/viewIncomplete/" . $jenisTr . "'><span class='glyphicon glyphicon-time'></span> complete list ...</a>";
        ...
        if (defined('ENVIRONMENT') && ENVIRONMENT !== 'production') { arrPrint($btnLabel); }
        //endregion
...
//cekHere("cek subitem");
    ...
    case "followupPreviewMasal":

        echo "<div id='followupPreview' sstyle='border:1px solid blue;'>";
        if (isset($msgWarning) && sizeof($msgWarning)) {
            $msgWarnings = $msgWarning;
            echo "<div class='alert alert-danger text-center'>";
            foreach ($msgWarnings as $msgSpec) {
...
//        if (isset($items) && sizeof($items) > 0) {
            echo "<form id='f1' name='f1' method='post' target='result'>";
            echo "<div class='table-responsive bborder-cek'>";

            $no = 0;
            if (isset($items) && sizeof($items) > 0) {
                echo "<table class='table table-bordered table-condensed bborder-cek' style='background:#ffffff;'>";
                echo "<thead>";
                echo "<tr bgcolor='#f0f0f0'>";
                echo "<th class='text-muted' style='font-weight:bold;' align='right' width='5'>no.</th>";
                foreach ($itemLabels as $key => $label) {
                                ...
                                foreach ($addMainSourceField as $fff => $f_labels) {
                                    ...
                                    if (isset($addMainSourceEdit[$fff])) {
                                        ...
                                        if ($addMainSourceEdit[$fff] == "checkbox") {
                                            ...
                                            $checked = isset($fSpec[$fff]) && $fSpec[$fff] == "true" ? $vals : "";
                                            $value = ($ctt == 0) ? "" : "";
                                            $addGm = "gunggunganMode=$gunggunganMode";
                                            $btn_formulir = (($ctt == 0) && ($countItems > 1)) ? "<button class='btn btn-warning' onclick=\"$('#result').load('$cloneFormulirFaktur?$addGm');\"><span class='glyphicon glyphicon-plus'></span> Tambah Formulir Faktur</button>" : "";
                                            $btn_formulir_delete = ($ctt > 0) ? "<button class='btn btn-danger' onclick=\"$('#result').load('$cloneFormulirFakturDelete/$ctt?$addGm');\"><span class='glyphicon glyphicon-trash'></span></button>" : "";
                                        }
                                        else {
                                            $classinputType = "form-control ";
                                            $labels = "";
                                            ...
                                            $checked = "";
                                            $addGm = "gunggunganMode=$gunggunganMode";
                                            $addEvent = " onblur=\"document.getElementById('result').src='$updateMainSourceTarget$ctt?ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL&$addGm&id=$fff&val='+this.value\" ";
                                            $value = "<input type='$inputType' id='$fff' class='$classinputType' name='$fff' 
                                                onclick='this.select()' value='$defValues' $checked $addEvent>";
                                        }
                                    }
...
//                        echo "&nbsp;&nbsp;";
                    ...
                    if (isset($approvalSpec['targetUrl']) != "" && $approvalSpec['targetUrl'] != "") {
                        ...
                        echo "<button button disabled id='approve_btn'type='button' class='btn btn-success margin' style='border:1px #008800 solid;color:#ffffff;' onclick=\"if(confirm('" . $approvalSpec['warning'] . "')==1){this.disabled=true;document.getElementById('f1').action='" . $approvalSpec['targetUrl'] . "';document.getElementById('f1').submit();top.open_holdon();}\"><span class='glyphicon glyphicon-ok'></span> " . $approvalSpec['label'] . "</button>";
                    }
                    else {
                        echo "&nbsp;";
                    }
