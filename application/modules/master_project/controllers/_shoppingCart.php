...
//        // Sanitasi Sesi Items: Otomatis buang phantom item (tanpa nama/handler valid) setiap kali shopping cart dipanggil/refreshed
...
//        $editableFields = isset($this->configUi[$this->jenisTr]['shoppingCartEditableFields'][$stepNumber]) ? (isset($_SESSION[$cCode]["mode"]["edit"]) ? array() : $this->configUi[$this->jenisTr]['shoppingCartEditableFields'][$stepNumber]) : array();
        ...
        $editableFields3 = isset($this->configUi[$this->jenisTr]['shoppingCartEditableFields3'][$stepNumber]) ? $this->configUi[$this->jenisTr]['shoppingCartEditableFields3'][$stepNumber] : array();
...
//arrPrintKuning($editableFields);
        ...
        //tambahan untuk handling biaya project
        $pairedItemEnabled2 = isset($this->configUi[$this->jenisTr]['shoppingCartPairedItem2']['enabled']) && $this->configUi[$this->jenisTr]['shoppingCartPairedItem2']['enabled'] == true ? true : false;
        $pairedItem2 = isset($this->configUi[$this->jenisTr]['shoppingCartPairedItem2']) ? $this->configUi[$this->jenisTr]['shoppingCartPairedItem2'] : array();
        $pairedItemTarget2 = isset($this->configUi[$this->jenisTr]['shoppingCartPairedItem2']['targetGateName']) ? $this->configUi[$this->jenisTr]['shoppingCartPairedItem2']['targetGateName'] : "items2";
        ...
        $avoidRemove = isset($this->configUi[$this->jenisTr]['shoppingCartAvoidRemove']) ? $this->configUi[$this->jenisTr]["shoppingCartAvoidRemoveAll_items"] : (isset($_SESSION[$cCode]["mode"]["edit"]) ? true : false);
        ...
        $fieldSrcs2 = isset($this->configUi[$this->jenisTr]['shoppingCartFieldSrc2']) ? $this->configUi[$this->jenisTr]['shoppingCartFieldSrc2'] : array();
        ...
        $editHandlesConfig = isset($this->configUi[$this->jenisTr]['selectorProcessor2']) ? $this->configUi[$this->jenisTr]['selectorProcessor2'] : array();
        //----------------------------------------------
        $isSettlement = isset($this->configUi[$this->jenisTr]['isSettlement']) ? $this->configUi[$this->jenisTr]['isSettlement'] : false;
        //----------------------------------------------
        // mati_disini(__LINE__." ".$avoidRemove);
            ...
            if (isset($_SESSION[$cCode]['items'])) {
                ...
                foreach ($_SESSION[$cCode]['items'] as $id => $iSpec) {
                    ...
                    $tmp = array(//                        "id"     => $iSpec['id'],
                        ...
                        "progress_nama" => isset($iSpec['progress_nama']) ? $iSpec['progress_nama'] : "",
                        "projectName" => isset($iSpec['projectName']) ? $iSpec['projectName'] : "",
                        "note" => isset($iSpec['note']) ? $iSpec['note'] : "",
                    );
...
//                    $tmp["editTarget"] = base_url() . (isset($iSpec['handler']) ? $iSpec['handler'] : '0') . "/" . $this->jenisTr . "?id=" . $id . "&newQty=";
                    $tmp["removeTarget"] = base_url() . (isset($iSpec['handler']) ? $iSpec['handler'] : '0') . "/remove/" . $this->jenisTr . "?id=" . $id;

                    $tmp['nama'] = isset($iSpec['projectName']) ? $iSpec['projectName'] : "";
                    $tmp['name'] = isset($iSpec['projectName']) ? $iSpec['projectName'] : "";
                    $tmp['note'] = isset($iSpec['note']) ? $iSpec['note'] : "";
...
//                        "produk_kode" => isset($iSpec['produk_kode']) ? $iSpec['produk_kode'] : isset($iSpec['kode']) ? $iSpec['kode'] : "",
                        ...
                        "hpp_supplier" => isset($iSpec['hpp_supplier']) ? $iSpec['hpp_supplier'] : (isset($iSpec['jual']) ? $iSpec['jual'] : 0),
                        "harga_budget" => isset($iSpec['harga_budget']) ? $iSpec['harga_budget'] : (isset($iSpec['harga']) ? $iSpec['harga'] : 0),
                    ...
                    if (sizeof($itemNumLabels2) > 0) {

                        foreach ($itemNumLabels2 as $key => $label) {
                            $tmp[$key] = isset($iSpec[$key]) ? $iSpec[$key] : 0;
                            //  if (!isset($main[$key])) {
                            //      $main[$key] = 0;
                            //  }
                    ...
                    if (sizeof($fieldSrcs2) > 0) {
                        foreach ($fieldSrcs2 as $key => $label) {
                            $tmp[$key] = isset($iSpec[$key]) ? $iSpec[$key] : 0;
                        }
                    }
                    ...
                    if (isset($editHandlerMethod2)) {
                        if (isset($iSpec['handler'])) {
                            $tmp["editTarget"] = MODUL_PATH . $editHandlesConfig . "/" . $editHandlerMethod2 . "" . $this->jenisTr . "?id=" . $iSpec['id'] . "&newQty=";
                    ...
                    if (isset($_SESSION[$cCode]['extractedSubItems'][$iSpec['id']])) {
                        foreach ($_SESSION[$cCode]['extractedSubItems'][$iSpec['id']] as $exSpec) {
                            if ($iSpec['id'] == $exSpec['produk_id']) {
                                if ($exSpec['dikirim_qty'] > 0) {
                                    $tmp["removeTarget"] = "";
                                }
                                $tmp["dikirim_qty"] = $exSpec['dikirim_qty'];
                                $tmp["order_qty"] = $exSpec['qty'];
                            }
                            else {
                                $tmp["dikirim_qty"] = 0;
                                $tmp["order_qty"] = isset($exSpec['qty']) ? $exSpec['qty'] : 0;
                            }
                        }
                    }
            ...
            if (isset($_SESSION[$cCode]['items3_sum'])) {
                ...
                foreach ($_SESSION[$cCode]['items3_sum'] as $iSpec) {
                    ...
                    $tmp = array(
                        ...
                        "anggaran" => isset($iSpec['anggaran']) ? $iSpec['anggaran'] : 0,
                        "sub_anggaran" => isset($iSpec['sub_anggaran']) ? $iSpec['sub_anggaran'] : 0,
                        "harga_budget" => isset($iSpec['harga_budget']) ? $iSpec['harga_budget'] : (isset($iSpec['harga']) ? $iSpec['harga'] : 0),
                    );
                    if ($noteEnabled) {
                        $tmp['note'] = isset($iSpec['note']) ? $iSpec['note'] : "";
                    ...
                    if (sizeof($itemNumLabels3) > 0) {
                        foreach ($itemNumLabels3 as $key => $label) {
                            $tmp[$key] = isset($iSpec[$key]) ? $iSpec[$key] : 0;
                            if (!isset($main[$key])) {
                                $main[$key] = 0;
                            }
...
//        arrprint($items3);
        ...
        if (isset($this->configUi[$this->jenisTr]['shoppingCartHideSubamount2']) && $this->configUi[$this->jenisTr]['shoppingCartHideSubamount2'][$stepNumber] == true) {
...
onfocus='this.select()' onkeyup=\"$keyupStr if(parseFloat(removeCommas(this.value))>0){this.value=(this.value)}else{this.value=0}\" onfocus=\"$keyupStr\"
onblur=\"$blurStr\"
onmouseout=\"$blurStr\"
>";
...
//            cekHijau($pernyataanNote);
        ...
        $termin_label = array(
            ...
            "progress" => "NILAI TERMIN (%) dari TOTAL PROJECT",
            "persen" => "PROJECT SELESAI (%)",
        );
        $linkAddTerminSetting = MODUL_PATH . get_class($this) . "/recordTermin/" . $this->jenisTr . "";
        //=================== TERMIN ====================
        ...
        $data = array(
            ...
            "items2_selector" => isset($this->configUi[$this->jenisTr]["selectorItems2"]) ? MODUL_PATH . $this->configUi[$this->jenisTr]["selectorItems2"] . "/" . $this->jenisTr : "",
            "items2_processor" => isset($this->configUi[$this->jenisTr]["selectorProcessorItems2"]) ? MODUL_PATH . $this->configUi[$this->jenisTr]["selectorProcessorItems2"] . "/" . $this->jenisTr : "",
            //            "itemLabels"=>$itemLabels,
...
//            "sumRows2" => $sumRows2,
            ...
            "editableFields3"      => $editableFields3,
            //            "applets"        => $applets,
            "elements" => $elements,
            ...
            "switchMaterialBiaya" => MODUL_PATH . "_shoppingCart/setTipeProject/" . $this->jenisTr,
            //https://demo.mayagrahakencana.com/everest_7jul/master_project/_shoppingCart/recordFieldInput/588/non_biaya?val=0
            "checkTaxes" => isset($_SESSION[$cCode]['main']['selectedType_konsumen']) ? $_SESSION[$cCode]['main']['selectedType_konsumen'] : "",


            //termin setting
            "settingTermin" => isset($_SESSION[$cCode]['main']['pengaturan_termin']) ? $_SESSION[$cCode]['main']['pengaturan_termin'] * 1 : 0,
//            "settingTermin" => 1,
            "new_dp" => isset($_SESSION[$cCode]['NEW']['dp']) ? $_SESSION[$cCode]['NEW']['dp'] : array(),
            "ref_kontrak" => isset($_SESSION[$cCode]['items7']) ? $_SESSION[$cCode]['items7'] : array(),
            "dp" => isset($_SESSION[$cCode]['items4']) ? $_SESSION[$cCode]['items4'] : array(),
            "new_termin" => isset($_SESSION[$cCode]['NEW']['termin']) ? $_SESSION[$cCode]['NEW']['termin'] : array(),
            "termin" => isset($_SESSION[$cCode]['items3']) ? $_SESSION[$cCode]['items3'] : array(),
            "new_garansi" => isset($_SESSION[$cCode]['NEW']['garansi']) ? $_SESSION[$cCode]['NEW']['garansi'] : array(),
            "garansi" => isset($_SESSION[$cCode]['items5']) ? $_SESSION[$cCode]['items5'] : array(),
            "nilai_total_project" => isset($_SESSION[$cCode]['main']['harga_nppn']) ? $_SESSION[$cCode]['main']['harga_nppn'] : 0,
            "nilai_total_project_last" => isset($_SESSION[$cCode]['main']['harga_nppn_last']) ? $_SESSION[$cCode]['main']['harga_nppn_last'] : 0,
            "termin_label" => $termin_label,
            "linkAddTerminSetting" => $linkAddTerminSetting,
            "set_harga_jual_project" => isset($_SESSION[$cCode]['main']['harga_jual_project']) ? $_SESSION[$cCode]['main']['harga_jual_project'] : 0,
...
//                                $selItemsSubTarget[$row->$srcKey] = $sub_gate;
                            }
                        }
                    }
                }
                ...
                $data['pairedPairSub'] = $sub_gate;

            }
        }
        if (count($pairedItem2) > 0) {
            if (isset($pairedItem2['enabled']) && ($pairedItem2['enabled'] == true)) {
                $sub_gate = $pairedItem2['sub_gate'];
                $mdlName = $pairedItem2['mdlName'];
                $srcKey = isset($pairedItem2['srcKey']) ? $pairedItem2['srcKey'] : "";
                $srcLabel = isset($pairedItem2['srcLabel']) ? $pairedItem2['srcLabel'] : array();

                $this->load->model("Mdls/$mdlName");
                $pro = new $mdlName();

                if (isset($pairedItem2['mdlFilter']) && (sizeof($pairedItem2['mdlFilter']) > 0)) {
                    foreach ($pairedItem2['mdlFilter'] as $filter) {
                        $pro->addFilter($filter);
                    }
                }
                    ...
                    foreach ($tmp as $row) {
                        if (sizeof($srcLabel) > 0) {
                            foreach ($srcLabel as $label) {
                                ...
                                $barcode = isset($row->barcode) ? $row->barcode : "--";
//                                $selItems[$row->$srcKey] = $row->$label . " " . $kode;
                                $tmpBiaya[$row->$srcKey] = array(
                                    "id" => $row->id,
                                    "nama" => $row->nama,
                                    "coa_code" => $row->coa_code,
//                                    "pairedPairSub"=>$sub_gate,
                                );
                            }
                        }
                    }
                ...
                //                arrPrint($tmp);
                $data['itemBiaya'] = $tmpBiaya;
                $data['itemBiaya_subgate'] = $sub_gate;
//                $data['selItems'] = $selItems;
//                $data['selItemsKode'] = $selItemsKode;
//                $data['selItemsFolder'] = $selItemsFolder;
//                $data['selItemsKeterangan'] = $selItemsKeterangan;
...
//                "dtime" => "date",
        ...
        if ($shopingCartReload) {
            ...
            if (isset($_GET['stop1']) && $_GET['stop1'] == 1) {
                ...
                echo "<script>";
                echo "setTimeout(function(){top.$('div#shopping_cart').load('" . MODUL_PATH . "_shoppingCart/viewCart/" . $this->jenisTr . "?selID=0&stop1=1');}, 700) ";
                echo "</script>";
            }
        }
...
//        );
    ...
    public function recordAddRow()
    {
        ...
        //add ppn jika harga_jual_project
        if($rowName=="harga_jual_project"){
            $_SESSION[$cCode]["main"][$rowName."_ppn"] = ($val*1.11) - $val;

            $_SESSION[$cCode]['main']['diskon_pembulatan'] = 0;
            $_SESSION[$cCode]['main']['harga_bruto2'] = $_SESSION[$cCode]['add_rows'][$rowName];
            $_SESSION[$cCode]['main']['harga_items2'] = $_SESSION[$cCode]['main']['harga_bruto2']-$_SESSION[$cCode]['main']['diskon_pembulatan'];
            $_SESSION[$cCode]['main']['ppn_items2'] = ($_SESSION[$cCode]['main']['harga_bruto2']-$_SESSION[$cCode]['main']['diskon_pembulatan'])*(11/100);

            $harga_hpp_material = $_SESSION[$cCode]["main"]['harga_anggran_material']*1;
            $harga_hpp_biaya = $_SESSION[$cCode]["main"]['harga_anggaran_biaya']*1;

            $harga_hpp_total = $_SESSION[$cCode]["main"]['total_anggaran']*1;
            $harga_ang_total = $_SESSION[$cCode]["main"]['harga_non_ppn']*1;

            $_SESSION[$cCode]["main"]['perkiraan_rl_hpp'] = $_SESSION[$cCode]["main"]["harga_jual_project"] - ($_SESSION[$cCode]["main"]['harga_anggran_material']+$_SESSION[$cCode]["main"]['harga_anggaran_biaya']);
//        $_SESSION[$cCode]["main"]['perkiraan_rl_hpp'] = 2222222222222;
            $_SESSION[$cCode]["main"]['perkiraan_rl_ang'] = $_SESSION[$cCode]["main"]["harga_jual_project"] - $_SESSION[$cCode]["main"]['harga_non_ppn'];
//        $_SESSION[$cCode]["main"]['perkiraan_rl_ang'] = 444444444444;
...
//        );
    ...
    public function recordPairedItem()
    {
        ...
        $stepNum = $_SESSION[$cCode]['main']['stepNumber'] > 0 ? $_SESSION[$cCode]['main']['stepNumber'] : 1;
        $sub_target = isset($_GET["sub_target"]) ? $_GET["sub_target"] : matiHere("gagal menentukan posisi komposisi target tidak terbaca! Silahkan hubungi admin untuk melakukan pengecekan.");

        if($_GET["sub_target"] == "biaya"){
            //biaya details
        ...
        if (!isset($_SESSION[$cCode]["items_komposisi"])) {
            $_SESSION[$cCode]["items_komposisi"] = array();
        }
        ...
        switch ($_GET["sub_target"]) {
            case "produk":
                if (isset($_SESSION[$cCode]["items2_sum"][$val])) {
                    $_SESSION[$cCode]["items2_sum"][$val] = null;
                    unset($_SESSION[$cCode]["items2_sum"][$val]);
                }
                $switchTargetGate = "items2_sum";
                break;
            case "biaya":
                $switchTargetGate = "items3_sum";
                break;
            default:
                matiHere(__LINE__);
                break;
...
//        unset($_SESSION[$cCode]["items_komposisi"]);
//        matiHere();
...
//        matiHere(__LINE__);
        ...
        if (sizeof($tmpB) > 0) {
            foreach ($tmpB as $row) {
                ...
                if (!array_key_exists($val, $_SESSION[$cCode][$switchTargetGate])) {
                    $tmp = array(
                        ...
                        "harga" => 0,
                        "subtotal" => 0,
                        "allow_autopurchase" => isset($row->allow_autopurchase) ? (int)$row->allow_autopurchase : 0,
                    );
                    if (sizeof($priceConfig2) > 0) {
                        ...
                        $this->db->where_in("jenis_value", $priceConfig2['label']);
                        $tmpH = $h->lookupAll($val)->result();

                        if (sizeof($tmpH) > 0) {
                            foreach ($tmpH as $hSpec) {
                                foreach ($priceConfig2['key_label'] as $key => $valz) {
                                    if ($key == $hSpec->jenis_value) {
                                        $tmp[$valz] = isset($hSpec->nilai) ? ($hSpec->nilai + 0) : 0;
                                    }
                                }
                            }
                        }
...
//                                lgShowAlertBiru($_SESSION['errMsg']);
                            }
                        }
                    }

                    if ($switchTargetGate == "items2_sum") {
                        $this->load->helper("he_project");
                        $faHpp = getHppProdukProject($val);
                        if ($faHpp > 0) {
                            $tmp['hpp_supplier'] = $faHpp;
                            $tmp['hpp'] = $faHpp;
                        }
                    }
                    ...
                    if ($sub_target == "produk") {
                        $_SESSION[$cCode]["items2_sum"][$row->id] = $tmp;
                    }
                    else {
                        $_SESSION[$cCode]["items3_sum"][$row->id] = $tmp;
                    }
                    $_SESSION[$cCode]["items2"][$iID][$sub_target][$row->id] = $tmp;
                    $_SESSION[$cCode]["items_komposisi"][$iID][$sub_target][$row->id] = $tmp;
                    if($row->jenis == "item_komposit"){
                        $mdlName = "MdlProdukKompositKomposisi";
                        $this->load->model("Mdls/" . $mdlName);
                        $ik = new $mdlName();
                        $ik->addFilter("produk_id='$val'");
                        $ik->addFilter("status='1'");
                        $tmpH = $ik->lookupAll()->result();
                        if(!empty($tmpH)){
                            $tmpIK = array();
                            $hargaJual = array();
                            foreach($tmpH as $iRow){
                                $tmpIK[$iRow->produk_id][$iRow->produk_dasar_id] = (array)$iRow;
                                if(!isset($hargaJual[$iRow->produk_id])){
                                    $hargaJual[$iRow->produk_id] = 0;
                                }
                                $hargaJual[$iRow->produk_id] += $iRow->jml * $iRow->jual;
                            }
                            $_SESSION[$cCode]["items2"][$iID][$sub_target][$row->id]['harga'] = $hargaJual[$row->id];
                            $_SESSION[$cCode]["items2"][$iID][$sub_target][$row->id]['subtotal'] = $hargaJual[$row->id];
                            $_SESSION[$cCode]["items2_sum"][$row->id]['harga'] = $hargaJual[$row->id];
                            $_SESSION[$cCode]["items2_sum"][$row->id]['subtotal'] = $hargaJual[$row->id];
                            $_SESSION[$cCode]["items2"][$iID]['komposit_komposisi'] = $tmpIK;
                            $_SESSION[$cCode]["items_komposisi"][$iID]['komposit_komposisi'] = $tmp;
                        }
                    }
                    //endregion
                }
                    ...
                    if (sizeof($itemNumLabels) > 0) {
                        ...
                        foreach ($itemNumLabels as $key => $label) {
                            if (isset($_GET[$key]) && strlen(trim($_GET[$key])) > 0) {
                                $newValue = str_replace(',', '', $_GET[$key]);
                                $tmp[$key] = $newValue;
        ...
        if($_GET["sub_target"] == "biaya"){
            $mainDataBiaya = $_SESSION[$cCode][$switchTargetGate];
            //details biaya jasa

            foreach($mainDataBiaya as $idBiaya => $dataBiaya){
                $jml_main = $dataBiaya['jml'];
                $subTotalDetails = 0;
                $_SESSION[$cCode]["items3_sum"][$idBiaya]['anggaran'] = $subTotalDetails;
                if(isset($arrByDetails[$idBiaya])){
                    foreach($arrByDetails[$idBiaya] as $idDetil => $dataDetBiaya){
                        $subTotalDetails += $dataDetBiaya['harga']*$dataDetBiaya['jml'];
                    }
                    $_SESSION[$cCode]["items3_sum"][$idBiaya]['anggaran'] = $subTotalDetails;
                    $_SESSION[$cCode]["items2"][0]['biaya'][$idBiaya]['anggaran'] = $subTotalDetails;
                }
                else{
                    unset($_SESSION[$cCode]["items3_sum"][$idBiaya]);
                    unset($_SESSION[$cCode]["items2"][$iID][$sub_target][$idBiaya]);
                    unset($_SESSION[$cCode]["items_komposisi"][$iID][$sub_target][$idBiaya]);
                    matiHere("Biaya Ini belum memiliki details Biaya, silahkan update pada biaya project agar biaya ini mendapatkan nilai anggaran yang sesuai.");
//                    matiHere("material ini tidak memiliki nilai anggaran, silahkan update nilai anggaran ");
                }
            }
        }
//matiHere(__LINE__);
        if($_GET["sub_target"] == "produk"){
            $mainDataMaterial = $_SESSION[$cCode][$switchTargetGate];

            foreach($mainDataMaterial as $idMaterial => $dataMaterial){
                // Cek apakah produk ini memiliki status allow_autopurchase = 1
                $isAllowAutopurchase = false;
                if (isset($dataMaterial['allow_autopurchase']) && $dataMaterial['allow_autopurchase'] == 1) {
                    $isAllowAutopurchase = true;
                } else {
                    $chkProd = $this->db->query("SELECT allow_autopurchase FROM produk WHERE id = ? LIMIT 1", array($idMaterial))->row_array();
                    if (!empty($chkProd) && isset($chkProd['allow_autopurchase']) && $chkProd['allow_autopurchase'] == 1) {
                        $isAllowAutopurchase = true;
                        $_SESSION[$cCode][$switchTargetGate][$idMaterial]['allow_autopurchase'] = 1;
                    }
                }

                // Deteksi nilai hpp_supplier
                $hppSupp = isset($dataMaterial['hpp_supplier']) ? (float)$dataMaterial['hpp_supplier'] : 0.0;
                if ($hppSupp == 0) {
                    if ($isAllowAutopurchase) {
                        // Barang Autopurchase FG diizinkan memiliki estimasi HPP = 0 saat inisialisasi Step 1
                        if (!isset($_SESSION[$cCode][$switchTargetGate][$idMaterial]['hpp_supplier'])) {
                            $_SESSION[$cCode][$switchTargetGate][$idMaterial]['hpp_supplier'] = 0;
                        }
                        if (!isset($_SESSION[$cCode][$switchTargetGate][$idMaterial]['hpp'])) {
                            $_SESSION[$cCode][$switchTargetGate][$idMaterial]['hpp'] = 0;
                        }
                    } else {
                        $nama_materialnya = isset($_SESSION[$cCode][$switchTargetGate][$idMaterial]['nama']) ? $_SESSION[$cCode][$switchTargetGate][$idMaterial]['nama'] : "";
                        $nama_esc = htmlspecialchars($nama_materialnya, ENT_QUOTES);
                        unset($_SESSION[$cCode][$switchTargetGate][$idMaterial]);

                        // Ambil daftar PIC berwenang (Tim Data & Purchasing) secara dinamis dari database
                        $picQuery = $this->db->query("SELECT nama, membership FROM per_employee WHERE status = '1' AND trash = '0' ORDER BY id ASC")->result_array();
                        $picData = array();
                        $picPurchasing = array();
                        foreach ($picQuery as $pRow) {
                            $mems = !empty($pRow['membership']) ? @unserialize(base64_decode($pRow['membership'])) : array();
                            if (is_array($mems)) {
                                if (in_array('c_data', $mems)) {
                                    $picData[] = $pRow['nama'];
                                }
                                if (in_array('c_purchasing', $mems)) {
                                    $picPurchasing[] = $pRow['nama'];
                                }
                            }
                        }
                        $strPicData = !empty($picData) ? implode(', ', array_unique($picData)) : 'Admin Data Master';
                        $strPicPurchasing = !empty($picPurchasing) ? implode(', ', array_unique($picPurchasing)) : 'Tim Purchasing';

                        $htmlMsg = "<div style='text-align:left;font-family:Segoe UI,Roboto,sans-serif;font-size:13px;color:#334155;line-height:1.5;margin-top:5px;'>"
                            . "<div style='background-color:#fef2f2;border-left:4px solid #ef4444;padding:10px 14px;border-radius:4px;margin-bottom:12px;'>"
                            . "<div style='font-size:13px;font-weight:bold;color:#991b1b;'>HPP Material Belum Terdaftar (Rp 0)</div>"
                            . "<div style='font-size:12px;color:#b91c1c;margin-top:2px;'>Material <b>" . $nama_esc . "</b> belum memiliki riwayat harga modal/penerimaan di Gudang Project.</div>"
                            . "</div>"
                            . "<div style='font-size:12px;font-weight:bold;color:#475569;margin-bottom:8px;text-transform:uppercase;letter-spacing:0.5px;'>Solusi & Langkah Tindakan:</div>"
                            . "<div style='background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;padding:10px 12px;margin-bottom:8px;'>"
                            . "<div style='font-weight:bold;color:#0f172a;font-size:13px;'><span style='background:#2563eb;color:#fff;padding:2px 7px;border-radius:10px;font-size:11px;margin-right:6px;'>1</span> Opsi A: Pengadaan Otomatis (Autopurchase FG)</div>"
                            . "<div style='color:#475569;font-size:12px;margin-top:4px;padding-left:24px;'>Buka menu <b>Master Data &raquo; Master Produk</b>, lalu aktifkan centang <span style='background:#e0f2fe;color:#0369a1;padding:2px 6px;border-radius:4px;font-weight:bold;'>Ijinkan Autopurchase</span> pada produk ini agar PO pengadaan diterbitkan otomatis oleh Tim Purchasing di Step 3.</div>"
                            . "</div>"
                            . "<div style='background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;padding:10px 12px;margin-bottom:10px;'>"
                            . "<div style='font-weight:bold;color:#0f172a;font-size:13px;'><span style='background:#64748b;color:#fff;padding:2px 7px;border-radius:10px;font-size:11px;margin-right:6px;'>2</span> Opsi B: Menggunakan Stok Gudang Reguler</div>"
                            . "<div style='color:#475569;font-size:12px;margin-top:4px;padding-left:24px;'>Pastikan material sudah memiliki transaksi <b>Penerimaan / Pembelian / Mutasi Stok</b> ke <b>Gudang Project</b> agar harga modalnya terbentuk di sistem.</div>"
                            . "</div>"
                            . "<div style='background:#eff6ff;border:1px dashed #93c5fd;border-radius:6px;padding:10px 12px;'>"
                            . "<div style='font-size:11px;font-weight:bold;color:#1e40af;text-transform:uppercase;margin-bottom:4px;'><i class='fa fa-user-circle'></i> Kontak Petugas Berwenang (Hak Akses Edit Produk):</div>"
                            . "<div style='font-size:12px;color:#1e3a8a;line-height:1.4;'>"
                            . "<div>&bull; <b>Tim Data Master:</b> " . htmlspecialchars($strPicData, ENT_QUOTES) . "</div>"
                            . "<div style='margin-top:2px;'>&bull; <b>Tim Purchasing:</b> " . htmlspecialchars($strPicPurchasing, ENT_QUOTES) . "</div>"
                            . "</div>"
                            . "</div>"
                            . "</div>";

                        $alerts = array(
                            "title" => "Perhatian: Validasi HPP Material",
                            "html"  => $htmlMsg,
                            "type"  => "warning",
                            "width" => "540px",
                        );
...
//        );
        ...
        if(isset($_GET["r"])){
        echo "<script>";
            echo "top.$('#shopping_cart').load('" . base_url() . $this->modul . "/" . "_shoppingCart/viewCart/" . $this->jenisTr . "?r=1&epreketek=yes&populate=1');";
        echo "</script>";
        }
...
//        );
    ...
    public function recordImage()
    {
        ...
        if ($files['error'] == 0) {
            ...
            $cUrl_result = upload_document($files);

            $_SESSION[$cCode]['items'][$iID][$colName] = $cUrl_result->full_url;
        }
        $addQS = "";

        $json_cUrl_result = json_encode($cUrl_result);
...
//        );
        ...
        echo "<script>";
        echo "console.log($json_cUrl_result);";
        echo "top.$('#shopping_cart').load('" . base_url() . $this->modul . "/" . "_shoppingCart/viewCart/" . $this->jenisTr . "?epreketek=yes&populate=1');";
        echo "</script>";
...
//        );
        $id = 0;
        $stepNum = isset($_SESSION[$this->cCode]['tableIn_master']['step_number']) ? $_SESSION[$this->cCode]['tableIn_master']['step_number'] : 1;
        $initMasterValues = heInitMasterValues_he_cart($this->jenisTr, $stepNum, $this->configUiJenis);
        $vg->buildValue($this->jenisTr, $id, $initMasterValues, $this->modul);
    ...
    public function remove()
    {
        ...
        if (isset($_SESSION[$cCode]['items3_sum'][$id])) {
            $_SESSION[$cCode]['items3_sum'][$id] = null;
            unset($_SESSION[$cCode]['items3_sum'][$id]);
        }

        //tambahan
        if (isset($_SESSION[$cCode]['items2'][0]["produk"][$id])) {
            $_SESSION[$cCode]['items2'][0]["produk"][$id] = null;
            unset($_SESSION[$cCode]['items2'][0]["produk"][$id]);
        }
        if (isset($_SESSION[$cCode]['items2'][0]["biaya"][$id])) {
            $_SESSION[$cCode]['items2'][0]["biaya"][$id] = null;
            unset($_SESSION[$cCode]['items2'][0]["biaya"][$id]);
        }
        if (isset($_SESSION[$cCode]['items_komposisi'][0]["produk"][$id])) {
            $_SESSION[$cCode]['items_komposisi'][0]["produk"][$id] = null;
            unset($_SESSION[$cCode]['items_komposisi'][0]["produk"][$id]);
        }
        if (isset($_SESSION[$cCode]['items_komposisi'][0]["biaya"][$id])) {
            $_SESSION[$cCode]['items_komposisi'][0]["biaya"][$id] = null;
            unset($_SESSION[$cCode]['items_komposisi'][0]["biaya"][$id]);
        }
        if (isset($_SESSION[$cCode]['tableIn_detail_values2'][$id])) {
            $_SESSION[$cCode]['tableIn_detail_values2'][$id] = null;
            unset($_SESSION[$cCode]['tableIn_detail_values2'][$id]);
        }
        if (isset($_SESSION[$cCode]['tableIn_sub_detail'][$id])) {
            $_SESSION[$cCode]['tableIn_sub_detail'][$id] = null;
            unset($_SESSION[$cCode]['tableIn_sub_detail'][$id]);
        }
    ...
    public function recordTermin()
    {

        $cCode = $this->cCode;
        $id = $iID = $_GET['iid'];
        $tabelName = $this->uri->segment(5);
        $val = rawurldecode($_GET['val']);
        $valCol = isset($_GET['valCol']) ? $_GET['valCol'] : null;
        $valValue = isset($_GET['valValue']) ? rawurldecode($_GET['valValue']) : null;
        $ppnFactor = $_SESSION[$cCode]['main']['ppnFactor'] > 0 ? $_SESSION[$cCode]['main']['ppnFactor'] : 1;
        $stepNum = $_SESSION[$cCode]['main']['stepNumber'] > 0 ? $_SESSION[$cCode]['main']['stepNumber'] : 1;
        $initMasterValues = heInitMasterValues_he_cart($this->jenisTr, $stepNum, $this->configUiJenis);

        if($_SESSION[$cCode]['main']['harga_jual_project']*1<=0){

            $balikans = array(
                "status"  => "error",
//                "message" => "silahkan menyusun RAB untuk mengetahui harga project",
                "message" => "silahkan mengisi harga project",
                "data"    => array(
                    "nilai" => "0"
                )
            );
            echo json_encode($balikans);

            // echo "<script>";
            // echo "top.swal('RAB Belum di tentukan', 'silahkan menyusun RAB untuk mengetahui harga project.', 'warning');";
            // echo "</script>";
            die();
        }

        $total_project = $_SESSION[$cCode]['main']['harga_jual_project']*1 > 0 ? $_SESSION[$cCode]['main']['harga_jual_project']*1.11 : matiHere('silahkan menyusun RAB untuk mengetahui harga project.');

        switch ($tabelName) {
            case "termin":
                if(empty($_GET['nama'])){
                    matiHere('nama termin harus di isi');
                }

                if(empty($_GET['progress'])){
                    matiHere('progress termin harus di isi');
                }

                if(empty($_GET['persen'])){
                    matiHere('persen dimintakan harus di isi');
                }

                if(empty($_GET['harga'])){
                    matiHere('harga termin harus di isi');
                }

                // $per_termin = $total_project * ($_GET['persen'] / 100);
                $per_termin = isset($_GET['harga']) ? (float)str_replace(',', '', $_GET['harga']) : 0;
                $per_penyelesaian = $_GET['persen']*1;
                $prev = 0;
                if(!empty($_SESSION[$cCode]['items3'])){
                    foreach($_SESSION[$cCode]['items3'] as $dTrm){
                        $prev += $dTrm['harga']*1;
                    }
                }
                if(!empty($_SESSION[$cCode]['items4'])){
                    foreach($_SESSION[$cCode]['items4'] as $dTrm){
                        $prev += $dTrm['harga']*1;
                    }
                }
                if(!empty($_SESSION[$cCode]['items5'])){
                    foreach($_SESSION[$cCode]['items5'] as $dTrm){
                        $prev += $dTrm['harga']*1;
                    }
                }

                $total_termin = (int)($prev + $per_termin);
                $belum_termin = $total_project - $total_termin;
                $total_project_f = formatField_he_format("harga", $total_project);

                // if ($belum_termin > 100 || $belum_termin < -100) {

                if ($belum_termin < -100) {
                    echo "<script>";
                    echo "top.swal('STOP', 'Anda tidak bisa membuat termin lebih besar dari nilai Project...!!!<br>(" . $total_project . ") Total yang Anda termin (" . $total_termin .  ") ', 'warning');";
                    echo "</script>";
                    die();
                }
                else{
                    $countPrev = count($_SESSION[$cCode]['items3']);
                    $_SESSION[$cCode]['items3'][] = array(
                        "urut"          => $countPrev+1,
                        "harga_project" => $total_project,
                        "nama"          => $_GET['nama'],
                        "progress"      => $_GET['progress'],
                        "persen"        => $_GET['persen'],
                        "harga"         => isset($_GET['harga']) ? (float)str_replace(',', '', $_GET['harga']) : $per_termin,
                );
                }

                $_SESSION[$cCode]['main']['harga_nppn_last'] = $total_project;

                $total_termin_only = 0;
                $total_termin_only_inclusif = 0;
                if (!empty($_SESSION[$cCode]['items3'])) {
                    foreach ($_SESSION[$cCode]['items3'] as $dTrm) {
                        $total_termin_only += $dTrm['harga'] * 1;
                        $total_termin_only_inclusif += $dTrm['harga'] / (1 + ($ppnFactor/100) );
                    }
                }

                $_SESSION[$cCode]['main']['harga_termin'] = $total_termin_only_inclusif;
                $_SESSION[$cCode]['main']['harga_nppn_termin'] = $total_termin_only;

                break;
            case "dp":
                $dp_num = (float)str_replace(',', '', $_GET['total_dp']);
                $belumConfirm = isset($_GET['confirm_dp']) && $_GET['confirm_dp'] == 'on' ? false : true;
                $defaultSetting_persen_dp_project = 10;
                $dp_persen = isset($_GET['dp']) ? $_GET['dp']*1 : 0;
                if($dp_persen<$defaultSetting_persen_dp_project && $belumConfirm){
                    echo "<script>
                                top.swal('peringatan', 'DP tidak boleh dibawah $defaultSetting_persen_dp_project%, jika ini bukan kesalahan, centang cek box di bawah dan confirmasi kembali.', 'warning');
                                top.$('#confirmDP').removeClass('hidden');
                            </script>";
                    die();
                }
                $dp_ppn = $dp_num * 0.11 / 1.11;
                $_SESSION[$cCode]['items4'][0] = array(
                    "harga_project" => $total_project,
                    "persen"        => $_GET['dp'],
                    // "harga"         => $total_project * ($_GET['dp'] / 100),
                    "harga"         => $dp_num,
                    "keterangan_dp" => trim($_GET['keterangan_dp']),
                );
                $_SESSION[$cCode]['main']['harga_nppn_last'] = $total_project;
                $_SESSION[$cCode]['main']['uang_muka_project'] = $dp_num - $dp_ppn;
                $_SESSION[$cCode]['main']['ppn_uang_muka_project'] = $dp_ppn;
                $_SESSION[$cCode]['main']['uang_muka_project_nppn'] = $dp_num;

                unset($_SESSION[$cCode]['items3']);
                break;
            case "garansi":
                $retensi = (float)str_replace(',', '', $_GET['total_garansi']);
                $ppn_retensi = $retensi * 0.11 / 1.11;
                $_SESSION[$cCode]['items5'][0] = array(
                    "harga_project"      => $total_project,
                    "persen"             => $_GET['garansi'],
                    // "harga"              => $total_project * ($_GET['garansi'] / 100),
                    "harga"              => $retensi,
                    "tgl_akhir_garansi"  => isDate($_GET['tgl_akhir_garansi']) ? $_GET['tgl_akhir_garansi'] : "-",
                    "keterangan_garansi" => trim($_GET['keterangan_garansi']),
                );

                $_SESSION[$cCode]['main']['retensi_project'] = $retensi-$ppn_retensi;
                $_SESSION[$cCode]['main']['ppn_retensi_project'] = $ppn_retensi;
                $_SESSION[$cCode]['main']['retensi_project_nppn'] = $retensi;
                $_SESSION[$cCode]['main']['harga_nppn_last'] = $total_project;
                $_SESSION[$cCode]['main']['piutang_retensi'] = $retensi;
                $_SESSION[$cCode]['main']['projectHarga'] = $total_project*1 > 0 ? $total_project*1 : 0;
                $_SESSION[$cCode]['main']['tarifGaransi'] = $_GET['garansi']*1 > 0 ? $_GET['garansi']*1 : 0;
                unset($_SESSION[$cCode]['items3']);
                break;
            case "ref":
                // cekHijau($_GET);
                ...
                $balikans = "";
                $upload_ref_kontraks = [];
                if (!empty($_FILES['upload_ref_kontrak']['name'])) {
                    $config['upload_path'] = './uploads/ref_kontrak/';
                    $config['allowed_types'] = 'pdf|doc|docx|jpg|jpeg|png';
                    $config['max_size'] = 2048; // 2 MB
                    $config['file_name'] = 'ref_kontrak_' . time();

                    $type = $_FILES['upload_ref_kontrak']['type'];
                    ...
                    // }
                    //
                    // if (!$this->upload->do_upload('upload_ref_kontrak')) {
                    ...
                    switch($arrType[$type]){
                        case "doc":
                            $balikans = upload_document($_FILES['upload_ref_kontrak']);
                            break;
                        case "img":
                    $balikans = upload_image($_FILES['upload_ref_kontrak']);
                        break;
                    }

                    if($balikans){
                        $file_datas = $balikans->data[0];
                        $upload_ref_kontraks["file_nama"] = $file_datas->file_name;
                        $upload_ref_kontraks["file_type"] = $file_datas->file_type;
                        $upload_ref_kontraks["full_url"] = $balikans->full_url;
                    }
                    else{
                        matiHere(__LINE__ . "<br>" . base64_encode(json_encode($balikans)) . " | Type: " . $type);
                    }

//arrPrint($_FILES);
//                    matiHere(json_encode($balikans));
                    // arrPrintKuning($balikans);

                }

                // matiHere(__LINE__);
                $_SESSION[$cCode]['items7'][0] = array(
                    "upload_ref_kontraks" => $upload_ref_kontraks,
                    "upload_ref_kontrak"  => $upload_ref_kontraks["file_nama"],
                    "ref_kontrak"         => $_POST['ref_kontrak'],
                    "tgl_kontrak"         => $_POST['tgl_kontrak'],
                    "tgl_mulai_project"   => $_POST['tgl_mulai_project'],
                    "tgl_akhir_project"   => $_POST['tgl_akhir_project'],
                    "notes_project"       => $_POST['notes_project'],

                );
                $_SESSION[$cCode]['main']['harga_nppn_last'] = $total_project;

                // arrPrintWebs($_SESSION[$cCode]['items7']);
                // matiHere(__LINE__);
                break;
            case "ref_get":
                cekHijau($_GET);
                ...
                $_SESSION[$cCode]['items7'][0] = array(
                    // "upload_ref_kontrak" => "",
                    "ref_kontrak"       => $_REQUEST['ref_kontrak'],
                    "tgl_kontrak"       => $_GET['tgl_kontrak'],
                    "tgl_mulai_project" => $_GET['tgl_mulai_project'],
                    "tgl_akhir_project" => $_GET['tgl_akhir_project'],
                    "notes_project"     => $_GET['notes_project'],
                );
                $_SESSION[$cCode]['main']['harga_nppn_last'] = $total_project;

                break;
            case "pengaturan":
                $_SESSION[$cCode]['main']['pengaturan_termin'] = $val;
                $initMasterValues['pengaturan_termin'] = $val;

                unset($_SESSION[$cCode]['items3']);
                unset($_SESSION[$cCode]['items4']);
                unset($_SESSION[$cCode]['items5']);
                unset($_SESSION[$cCode]['items7']);
        ...
        //TERMIN
        if(!empty($items3)){
            foreach($items3 as $k => $tSpec){
                $total_harga_termin += $tSpec['harga']*1;
            }
        }

        //DP
        if(!empty($items4)){
            foreach($items4 as $k => $tSpec){
                $total_harga_termin += $tSpec['harga']*1;
            }
        }
        ...
        //GARANSI
        if(!empty($items5)){
            foreach($items5 as $k => $tSpec){
                $total_harga_termin += $tSpec['harga']*1;
            }
        }

        $_SESSION[$cCode]['main']['total_harga_termin'] = $total_harga_termin*1;
    ...
    public function removeTermin()
    {
        ...
        switch($jenis){
            case "termin":
                unset($_SESSION[$cCode]['items3'][$key]);
                break;
            case "garansi":
                unset($_SESSION[$cCode]['items5'][$key]);
                break;
            case "dp":
                unset($_SESSION[$cCode]['items4'][$key]);
                break;
            case "ref":
                unset($_SESSION[$cCode]['items7'][$key]);
                break;
        }
    }

    public function recalculation_termin(){

//        matiHere(__LINE__);
        $cCode = $this->cCode;

        $total_project = isset($_SESSION[$cCode]['main']['harga_jual_project']) && $_SESSION[$cCode]['main']['harga_jual_project']*1 > 0 ? $_SESSION[$cCode]['main']['harga_jual_project']*1.11 : 0;
        $total_project_last = isset($_SESSION[$cCode]['main']['harga_nppn_last']) && $_SESSION[$cCode]['main']['harga_nppn_last']*1 > 0 ? $_SESSION[$cCode]['main']['harga_nppn_last']*1 : 0;
        $total_harga_termin = isset($_SESSION[$cCode]['main']['total_harga_termin']) && $_SESSION[$cCode]['main']['total_harga_termin']*1 > 0 ? $_SESSION[$cCode]['main']['total_harga_termin']*1 : 0;

//        cekMerah($total_project_last);
//        cekMerah($total_project);
//        cekMerah($total_harga_termin);
//        matiHere();
        $reset = true;
        if($total_project_last*1 != $total_project*1 || $total_project*1 != $total_harga_termin){
            $_SESSION[$cCode]['main']['harga_nppn_last'] = $total_project;
            $white_list = array("items3", "items4", "items5");
            $injector = array("harga_project","persen","harga");
            $new_harga_termin = 0;
            foreach($white_list as $gate){
                if(isset($_SESSION[$cCode][$gate]) && is_array($_SESSION[$cCode][$gate])){
                    $target = $_SESSION[$cCode][$gate];
                    if($reset){
                        foreach($target as $ky =>$cSpec){
                            unset($_SESSION[$cCode][$gate][$ky]);
                        }
                    }
                    else{
                        foreach ($target as $ky => $cSpec) {
                            $per = isset($cSpec['persen']) ? $cSpec['persen'] : 0;
                            $_SESSION[$cCode][$gate][$ky]['harga_project'] = $total_project;
                            $_SESSION[$cCode][$gate][$ky]['harga'] = $total_project * ($per / 100);
                            $new_harga_termin += $total_project * ($per / 100);
                        }
                    }
                }
            }
            $_SESSION[$cCode]['main']['total_harga_termin'] = $new_harga_termin;
        }

//        matiHere($total_project);
    ...
    public function sessionTotalCheck(){


    }

    public function setTipeProject(){
        ...
        if(isset($_SESSION[$cCode]["main"]['non_biaya']) && $_SESSION[$cCode]["main"]['non_biaya'] == 1){
            $_SESSION[$cCode]["items3_sum"] = array();
            $_SESSION[$cCode]["items_komposisi"][0]['biaya'] = array();
            $_SESSION[$cCode]["items2"][0]['biaya'] = array();
        }

        if(isset($_SESSION[$cCode]["main"]['non_material']) && $_SESSION[$cCode]["main"]['non_material'] == 1){
            $_SESSION[$cCode]["items2_sum"] = array();
            $_SESSION[$cCode]["tableIn_sub_detail"] = array();
            $_SESSION[$cCode]["tableIn_detail_values2_sum"] = array();
            $_SESSION[$cCode]["items_komposisi"][0]['produk'] = array();
            $_SESSION[$cCode]["items2"][0]['produk'] = array();
        }
