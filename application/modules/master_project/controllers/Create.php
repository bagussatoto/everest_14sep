...
class Create extends Modul_Controller
{
    ...
    public function index()
    {
        ...
        $this->jenisTrName = isset($this->configUiJenis['steps'][1]['label']) ? $this->configUiJenis['steps'][1]['label'] : "unnamed";
        $this->allSteps = isset($this->configUiJenis['steps']) ? $this->configUiJenis['steps'] : array();
        if (isset($this->configUiJenis['template'])) {
            $this->template = base_url() . "template/" . $this->configUiJenis['template'] . ".html";
        }
        else {
            $this->template = "";
        ...
        if (isset($_SESSION[$cCode]['mode']) && array_key_exists("edit", $_SESSION[$cCode]['mode'])) {
            $prevStepNum = $_SESSION[$cCode]['mode']["stepNum"];//penanda edit edit dari executor doPreEdit
            $selectedID = $_SESSION[$cCode]['mode']['edit'];//penanda edit edit dari executor doPreEdit
            $title = "";
            $subTitle = "Edit " . $this->configUi[$jenisTr]["steps"][$prevStepNum]['label'];
            $editableFields = array(
                "selector_key" => "disabled",//close gak boleh diganti project
                "pihak_key" => "disabled",//close gak boleh diganti customer
                "selector_caller2_key" => "",//open
            );
            //            cekHitam("biruuu");
        }
    ...
    public function preview()
    {
                    ...
                    foreach ($aSpec as $iSpec) {
                        if (isset($iSpec['id'])) {
                            ...
                            $tmp = array(
                                ...
                                "hpp_supplier" => isset($iSpec['hpp_supplier']) ? $iSpec['hpp_supplier'] : "",
            ...
            if (isset($_SESSION[$cCode]['items2_sum'])) {
                foreach ($_SESSION[$cCode]['items2_sum'] as $iSpec) {
                    $tmp = array(
                        ...
                        "kode" => isset($iSpec['kode']) ? $iSpec['kode'] : "n/a",
                        "harga" => (isset($iSpec['harga_ori']) && strlen($iSpec['harga_ori']) > 0) ? $iSpec['harga_ori'] : (isset($iSpec['harga']) ? $iSpec['harga'] : "0"),
                        "harga_ori" => isset($iSpec['harga_ori']) ? $iSpec['harga_ori'] : "",
                        ...
                        "hpp_supplier" => (isset($iSpec['hpp_supplier']) && (float)str_replace(',', '', $iSpec['hpp_supplier']) > 0) ? $iSpec['hpp_supplier'] : ((isset($iSpec['hpp']) && (float)str_replace(',', '', $iSpec['hpp']) > 0) ? $iSpec['hpp'] : ((isset($iSpec['hrg_hpp']) && (float)str_replace(',', '', $iSpec['hrg_hpp']) > 0) ? $iSpec['hrg_hpp'] : ((isset($iSpec['nilai']) && (float)str_replace(',', '', $iSpec['nilai']) > 0) ? $iSpec['nilai'] : (isset($iSpec['anggaran']) ? $iSpec['anggaran'] : 0)))),
                        "harga_budget" => (isset($iSpec['harga_budget']) && (float)str_replace(',', '', $iSpec['harga_budget']) > 0) ? $iSpec['harga_budget'] : ((isset($iSpec['hpp_supplier']) && (float)str_replace(',', '', $iSpec['hpp_supplier']) > 0) ? $iSpec['hpp_supplier'] : ((isset($iSpec['hpp']) && (float)str_replace(',', '', $iSpec['hpp']) > 0) ? $iSpec['hpp'] : ((isset($iSpec['hrg_hpp']) && (float)str_replace(',', '', $iSpec['hrg_hpp']) > 0) ? $iSpec['hrg_hpp'] : ((isset($iSpec['nilai']) && (float)str_replace(',', '', $iSpec['nilai']) > 0) ? $iSpec['nilai'] : (isset($iSpec['anggaran']) ? $iSpec['anggaran'] : 0))))),
                    );
                    if (sizeof($itemNumLabels) > 0) {
                        foreach ($itemNumLabels as $key => $label) {
                            $tmp[$key] = isset($iSpec[$key]) ? $iSpec[$key] : 0;
            ...
            if (isset($_SESSION[$cCode]['items3_sum'])) {
                foreach ($_SESSION[$cCode]['items3_sum'] as $iSpec) {
                    $tmp = array(
                        ...
                        "harga" => isset($iSpec['harga']) ? $iSpec['harga'] : "0",
                        "referensi" => isset($iSpec['pihakName']) ? $iSpec['pihakName'] : "",
                        ...
                        "stok_center" => isset($iSpec['stok_center']) ? $iSpec['stok_center'] : "0",
                        "sisa" => isset($iSpec['stok']) ? $iSpec['stok'] - $iSpec['jml'] : "",
                        "anggaran" => isset($iSpec['anggaran']) ? $iSpec['anggaran'] : "",
                        "hpp_supplier" => (isset($iSpec['anggaran']) && (float)str_replace(',', '', $iSpec['anggaran']) > 0) ? $iSpec['anggaran'] : ((isset($iSpec['hrg_hpp']) && (float)str_replace(',', '', $iSpec['hrg_hpp']) > 0) ? $iSpec['hrg_hpp'] : ((isset($iSpec['hpp']) && (float)str_replace(',', '', $iSpec['hpp']) > 0) ? $iSpec['hpp'] : (isset($iSpec['hpp_supplier']) ? $iSpec['hpp_supplier'] : 0))),
                        "harga_budget" => (isset($iSpec['harga_budget']) && (float)str_replace(',', '', $iSpec['harga_budget']) > 0) ? $iSpec['harga_budget'] : ((isset($iSpec['anggaran']) && (float)str_replace(',', '', $iSpec['anggaran']) > 0) ? $iSpec['anggaran'] : ((isset($iSpec['hrg_hpp']) && (float)str_replace(',', '', $iSpec['hrg_hpp']) > 0) ? $iSpec['hrg_hpp'] : ((isset($iSpec['hpp']) && (float)str_replace(',', '', $iSpec['hpp']) > 0) ? $iSpec['hpp'] : (isset($iSpec['hpp_supplier']) ? $iSpec['hpp_supplier'] : 0)))),
                    );
                    if (sizeof($itemNumLabels) > 0) {
                        foreach ($itemNumLabels as $key => $label) {
                            $tmp[$key] = isset($iSpec[$key]) ? $iSpec[$key] : 0;
        ...
        if (empty($_SESSION[$cCode]['main']['projectName'])) {
            // Hentikan eksekusi jika projectName tidak ada atau kosong
            echo "<div class='container-fluid'>
                <div class='row text-center text-bold text-red'>
                <h2>NAMA PROJECT TIDAK BOLEH KOSONG</h2>
                    </div>
                </div>";
            die();
        }

        if ( empty($_SESSION[$cCode]['items2'][0]['biaya']) && empty($_SESSION[$cCode]['main']['non_biaya']) ) {
            // Hentikan eksekusi jika projectName tidak ada atau kosong
            echo "<div class='container-fluid'>
                <div class='row text-center text-bold text-red'>
                <h2>BIAYA TIDAK BOLEH KOSONG</h2>
                    </div>
                </div>";
            die();
        }

        if ( empty($_SESSION[$cCode]['items2'][0]['produk']) && empty($_SESSION[$cCode]['main']['non_material']) ) {
            // Hentikan eksekusi jika projectName tidak ada atau kosong
            echo "<div class='container-fluid'>
                <div class='row text-center text-bold text-red'>
                <h2>MATERIAL TIDAK BOLEH KOSONG</h2>
                    </div>
                </div>";
            die();
        }
        ...
        $calc_total_hpp = 0;
        $calc_total_budget = 0;
        $calc_total_rincian = 0;

        $source_items2 = !empty($items2_sum) ? $items2_sum : (!empty($items2) ? $items2 : array());
        if (!empty($source_items2)) {
            foreach ($source_items2 as $it2) {
                $jml2 = isset($it2['jml']) ? (float)str_replace(',', '', $it2['jml']) : 1;
                $hpp2 = (isset($it2['hpp_supplier']) && (float)str_replace(',', '', $it2['hpp_supplier']) > 0) ? (float)str_replace(',', '', $it2['hpp_supplier']) : ((isset($it2['hpp']) && (float)str_replace(',', '', $it2['hpp']) > 0) ? (float)str_replace(',', '', $it2['hpp']) : ((isset($it2['hrg_hpp']) && (float)str_replace(',', '', $it2['hrg_hpp']) > 0) ? (float)str_replace(',', '', $it2['hrg_hpp']) : ((isset($it2['nilai']) && (float)str_replace(',', '', $it2['nilai']) > 0) ? (float)str_replace(',', '', $it2['nilai']) : (isset($it2['anggaran']) ? (float)str_replace(',', '', $it2['anggaran']) : 0))));
                $bgt2 = (isset($it2['harga_budget']) && (float)str_replace(',', '', $it2['harga_budget']) > 0) ? (float)str_replace(',', '', $it2['harga_budget']) : $hpp2;
                $hrg2 = (isset($it2['subtotal']) && (float)str_replace(',', '', $it2['subtotal']) > 0) ? (float)str_replace(',', '', $it2['subtotal']) : (isset($it2['harga']) ? (float)str_replace(',', '', $it2['harga']) * $jml2 : 0);
                $calc_total_hpp += ($hpp2 * $jml2);
                $calc_total_budget += ($bgt2 * $jml2);
                $calc_total_rincian += $hrg2;
            }
        }

        $source_items3 = !empty($items3_sum) ? $items3_sum : (!empty($items3) ? $items3 : array());
        if (!empty($source_items3)) {
            foreach ($source_items3 as $it3) {
                $jml3 = isset($it3['jml']) ? (float)str_replace(',', '', $it3['jml']) : 1;
                $hpp3 = (isset($it3['anggaran']) && (float)str_replace(',', '', $it3['anggaran']) > 0) ? (float)str_replace(',', '', $it3['anggaran']) : ((isset($it3['hrg_hpp']) && (float)str_replace(',', '', $it3['hrg_hpp']) > 0) ? (float)str_replace(',', '', $it3['hrg_hpp']) : ((isset($it3['hpp']) && (float)str_replace(',', '', $it3['hpp']) > 0) ? (float)str_replace(',', '', $it3['hpp']) : (isset($it3['hpp_supplier']) ? (float)$it3['hpp_supplier'] : 0)));
                $bgt3 = (isset($it3['harga_budget']) && (float)str_replace(',', '', $it3['harga_budget']) > 0) ? (float)str_replace(',', '', $it3['harga_budget']) : $hpp3;
                $hrg3 = (isset($it3['subtotal']) && (float)str_replace(',', '', $it3['subtotal']) > 0) ? (float)str_replace(',', '', $it3['subtotal']) : (isset($it3['harga']) ? (float)str_replace(',', '', $it3['harga']) * $jml3 : 0);
                $calc_total_hpp += ($hpp3 * $jml3);
                $calc_total_budget += ($bgt3 * $jml3);
                $calc_total_rincian += $hrg3;
            }
        }

        $main['total_anggaran'] = $calc_total_hpp;
        $main['total_budget'] = $calc_total_budget;
        $main['harga_non_ppn'] = $calc_total_rincian;
        $mainAddValues['total_anggaran'] = $calc_total_hpp;
        $mainAddValues['total_budget'] = $calc_total_budget;
        $mainAddValues['harga_non_ppn'] = $calc_total_rincian;

        if (!isset($main['harga_jual_project']) || (float)str_replace(',', '', $main['harga_jual_project']) <= 0) {
            $main['harga_jual_project'] = $calc_total_rincian;
        }
        $hJualProj = (float)str_replace(',', '', $main['harga_jual_project']);
        $main['harga_jual_project'] = $hJualProj;
        $main['harga_jual_project_ppn'] = $hJualProj * 0.11;
        $main['harga_jual_project_nppn'] = $hJualProj * 1.11;
        $main['perkiraan_rl_hpp'] = $hJualProj - $main['total_anggaran'];
        $main['perkiraan_rl_ang'] = $hJualProj - $main['harga_non_ppn'];

        $mainAddValues['harga_jual_project'] = $main['harga_jual_project'];
        $mainAddValues['harga_jual_project_ppn'] = $main['harga_jual_project_ppn'];
        $mainAddValues['harga_jual_project_nppn'] = $main['harga_jual_project_nppn'];
        $mainAddValues['perkiraan_rl_hpp'] = $main['perkiraan_rl_hpp'];
        $mainAddValues['perkiraan_rl_ang'] = $main['perkiraan_rl_ang'];
        ...
        if (isset($configUi[$this->jenisTr]['shoppingCartHideSubamount3']) && $configUi[$this->jenisTr]['shoppingCartHideSubamount3'][$stepNumber] == true) {
        ...
        $pairedItemTarget = isset($configUi[$this->jenisTr]['shoppingCartPairedItem']['targetGateName']) ? $configUi[$this->jenisTr]['shoppingCartPairedItem']['targetGateName'] : "items2_sum";

        //cekHitam($this->configUi[$jenisTr]["template"]);
        ...
        $data = array(
            ...
            "items2_ses" => isset($_SESSION[$cCode]['items2']) ? $_SESSION[$cCode]['items2'] : array(),
            "items3" => $items3_sum,
            "itemsSrc" => $itemsSrc,
    ...
    public function save()
    {
        ...
        // 1. Validasi Indikasi Transaksi Ganda (Session Check) - Early Validation Pattern
        if (isset($_SESSION[$cCode]['is_processing']) && $_SESSION[$cCode]['is_processing'] === true) {
            die("Transaksi sedang diproses. Mohon tunggu dan jangan klik tombol simpan dua kali.");
        }

        // 2. Validasi field wajib isi untuk project
        if ($this->jenisTr == "588") {
            $projectName = "";
            if (isset($_SESSION[$cCode]['main']['projectName'])) {
                $projectName = trim($_SESSION[$cCode]['main']['projectName']);
            } else {
                if (isset($_SESSION[$cCode]['items']) && is_array($_SESSION[$cCode]['items'])) {
                    foreach ($_SESSION[$cCode]['items'] as $item) {
                        if (isset($item['projectName']) && trim($item['projectName']) != "") {
                            $projectName = trim($item['projectName']);
                            break;
                        } elseif (isset($item['nama']) && trim($item['nama']) != "") {
                            $projectName = trim($item['nama']);
                            break;
                        }
                    }
                }
            ...
            if ($projectName == "") {
                die("Gagal menyimpan: Nama Project tidak boleh kosong. Jika Anda baru saja mengetik nama, mohon klik di luar kotak teks (auto-save) dan tunggu sesaat sebelum menekan tombol Simpan.");
            } else {
                $_SESSION[$cCode]['main']['projectName'] = $projectName;
            }

            // Validasi duplikasi project (Early Validation Pattern)
            $cabangId = my_cabang_id();
            $customerId = isset($_SESSION[$cCode]['main']['customer_id']) ? $_SESSION[$cCode]['main']['customer_id'] : (isset($_SESSION[$cCode]['main']['pihakID']) ? $_SESSION[$cCode]['main']['pihakID'] : 0);
            $spek = isset($_SESSION[$cCode]['main']['spek']) ? trim($_SESSION[$cCode]['main']['spek']) : (isset($_SESSION[$cCode]['main']['spesifikasi']) ? trim($_SESSION[$cCode]['main']['spesifikasi']) : "");
            $projectId = isset($_SESSION[$cCode]['main']['projectID']) ? $_SESSION[$cCode]['main']['projectID'] : (isset($_SESSION[$cCode]['main']['project_id']) ? $_SESSION[$cCode]['main']['project_id'] : 0);
            $excludeId = $projectId;

            if ($cabangId > 0 && $customerId > 0 && $spek != "") {
                $this->load->model("Mdls/MdlProdukProject");
                $mdlPr = new MdlProdukProject();
                $dup = $mdlPr->checkDuplicateProject($cabangId, $customerId, $spek, $excludeId);
                if (!empty($dup)) {
                    unset($_SESSION[$cCode]['is_processing']);
                    $dupMsg = "Gagal menyimpan: Project dengan Konsumen dan Spesifikasi/PO ('" . htmlspecialchars($spek) . "') sudah terdaftar pada Project: '" . htmlspecialchars($dup->nama) . "' (ID: " . $dup->id . "). Silahkan gunakan project yang sudah ada.";
                    die(lgShowAlert($dupMsg));
                }
            }
        }

        // Tandai bahwa sesi ini sedang diproses untuk mencegah request paralel (Race Condition)
        $_SESSION[$cCode]['is_processing'] = true;
        ...
        if (isset($_SESSION[$cCode])) {
            ...
            if (isset($this->configCore[$this->jenisTr]['writeSubDetail'][$jenisTrTarget]) && $this->configCore[$this->jenisTr]['writeSubDetail'][$jenisTrTarget] == true) {
                ...
                if (sizeof($_SESSION[$cCode]['tableIn_sub_detail']) > 0) {
                    $addSubValues = array(
                        "sub_step_number" => 1,
                        "sub_step_current" => 1,
                        "sub_step_avail" => sizeof($this->configUi[$this->jenisTr]['steps']),
                    ...
                    foreach ($_SESSION[$cCode]['tableIn_sub_detail'] as $id => $dSpec) {
                        foreach ($addSubValues as $key => $val) {
                            $_SESSION[$cCode]['tableIn_sub_detail'][$id][$key] = $val;
                        }
                    }
                    ...
                    if (isset($_SESSION[$cCode]['tableIn_sub_detail']) && sizeof($_SESSION[$cCode]['tableIn_sub_detail']) > 0) {
                        ...
                        $insertIDs = array();
                        $insertDeIDs = array();
                        foreach ($_SESSION[$cCode]['tableIn_sub_detail'] as $dSpec) {
                            ...
                            if ($insertDetailID < 1) {
                                ...
                                $insertIDs[] = $insertDetailID;
                                $insertDeIDs[$insertID][] = $insertDetailID;
                                $mongoList['sub_detail'][] = $insertDetailID;
                            }
                            if ($epID != 999) {
            ...
            $pakai_ini = 0;// dicoba dulu tanpa cli
            if ($pakai_ini == 1) {
...
//            arrPrintKuning("PENULISAN KE REGISTRY");
//            arrPrintKuning($baseRegistries['items']);
//            arrPrintKuning($_SESSION[$cCode]['items3']);
//            arrPrintKuning($_SESSION[$cCode]['items4']);
//            arrPrintKuning($_SESSION[$cCode]['items5']);
//            arrPrintKuning($_SESSION[$cCode]['items7']);
//matiHere(__LINE__);
...
//            mati_disini("LINE: " . __LINE__ . " under maintenance, tunggu beberapa saat lagi yaa.., TRID: $insertID");
    ...
    public function recordColumn()
    {
        ...
        $id = isset($_GET['selID']) ? $_GET['selID'] : (isset($_GET['id']) ? $_GET['id'] : 0);
        ...
        $initMasterValues = heInitMasterValues_he_cart($this->jenisTr, 1, $this->configUiJenis);
        //        arrPrintWebs($initMasterValues);
        //        arrPrintPink($initMasterValuess);
        //        $initMasterValuess['test2'] = "100000000";
        //        $initMasterValuessss = $initMasterValues + $initMasterValuess;
        //        arrPrint($initMasterValuessss);
    ...
    public function editPreview()
    {
        ...
        $stepNumber = isset($_SESSION[$cCode]['mode']['stepNum']) ? $_SESSION[$cCode]['mode']['stepNum'] : (isset($_SESSION[$cCode]['tableIn_master']['step_number']) ? $_SESSION[$cCode]['tableIn_master']['step_number'] : 1);
            ...
            if (isset($_SESSION[$cCode]['items3_sum'])) {
                foreach ($_SESSION[$cCode]['items3_sum'] as $iSpec) {
                    $tmp = array(
                        ...
                        "sisa" => isset($iSpec['stok']) ? $iSpec['stok'] - $iSpec['jml'] : "",
                        "anggaran" => isset($iSpec['anggaran']) ? $iSpec['anggaran'] : "",
                        "hpp_supplier" => (isset($iSpec['hpp_supplier']) && (float)str_replace(',', '', $iSpec['hpp_supplier']) > 0) ? $iSpec['hpp_supplier'] : ((isset($iSpec['anggaran']) && (float)str_replace(',', '', $iSpec['anggaran']) > 0) ? $iSpec['anggaran'] : ((isset($iSpec['hrg_hpp']) && (float)str_replace(',', '', $iSpec['hrg_hpp']) > 0) ? $iSpec['hrg_hpp'] : ((isset($iSpec['hpp']) && (float)str_replace(',', '', $iSpec['hpp']) > 0) ? $iSpec['hpp'] : 0))),
                        "harga_budget" => (isset($iSpec['harga_budget']) && (float)str_replace(',', '', $iSpec['harga_budget']) > 0) ? $iSpec['harga_budget'] : ((isset($iSpec['anggaran']) && (float)str_replace(',', '', $iSpec['anggaran']) > 0) ? $iSpec['anggaran'] : ((isset($iSpec['hrg_hpp']) && (float)str_replace(',', '', $iSpec['hrg_hpp']) > 0) ? $iSpec['hrg_hpp'] : ((isset($iSpec['hpp']) && (float)str_replace(',', '', $iSpec['hpp']) > 0) ? $iSpec['hpp'] : (isset($iSpec['hpp_supplier']) ? $iSpec['hpp_supplier'] : 0)))),
                    );
                    if (sizeof($itemNumLabels) > 0) {
                        foreach ($itemNumLabels as $key => $label) {
        ...
        $buttonLabel = "Edit " . $this->configUi[$jenisTr]["steps"][$stepNumber]['actionLabel'];
        if (sizeof($this->configUi[$this->jenisTr]['steps']) > 1) {
            $saveWarning = "clicking <strong>$buttonLabel</strong> button will make transaction state to: <strong class='badge bg-grey text-white'>" . $this->configUi[$this->jenisTr]['steps'][$stepNumber]['stateLabel'] . "</strong>";
            ...
            $saveWarning = "clicking <strong>$buttonLabel</strong> button will instantly make transaction state to <strong class='text-blue'>" . $this->configUi[$this->jenisTr]['steps'][$stepNumber]['stateLabel'] . "</strong>";
        ...
        $data = array(
            ...
            "title" => $this->configUi[$jenisTr]["label"],
            "subTitle" => $this->configUi[$jenisTr]["steps"][$stepNumber]['label'],
            "jenisTr" => $jenisTr,
    ...
    public function doEdit__OLD()
    {
        ...
        if (isset($_SESSION[$cCode])) {
            ...
            if (isset($_SESSION[$cCode]["mode"]["stepNum"]) && $_SESSION[$cCode]["mode"]["stepNum"] == 1) {
                ...
                if (isset($_SESSION[$cCode]['tableIn_detail']) && sizeof($_SESSION[$cCode]['tableIn_detail']) > 0) {
                    ...
                    foreach ($_SESSION[$cCode]['tableIn_detail'] as $dSpec) {
                        ...
                        cekUngu(__LINE__ . " ::: " . $this->db->last_query() . " [" . $this->db->affected_rows() . "]");
                        $insertIDs[] = $insertDetailID;
                ...
                if (isset($this->configCore[$this->jenisTr]['writeSubDetail'][$jenisTrTarget]) && $this->configCore[$this->jenisTr]['writeSubDetail'][$jenisTrTarget] == true) {
                    ...
                    if (isset($_SESSION[$cCode]['tableIn_sub_detail']) && sizeof($_SESSION[$cCode]['tableIn_sub_detail']) > 0) {
                        ...
                        $registrySubDetail = array();
                        $insertIDs = array();
                        $insertDeIDs = array();
                        $masterID = $_SESSION[$cCode]['mode']['edit'];
                        $gate_update = "extractedSubItems";
                        $tbl_detail = "sub_detail";
                        foreach ($_SESSION[$cCode]['tableIn_sub_detail'] as $iID => $dSpec) {
                            ...
                            if (isset($_SESSION[$cCode][$gate_update])) {
                                if (array_key_exists($iID, $_SESSION[$cCode][$gate_update])) {
                                    $registrySubDetail[$iID] = $_SESSION[$cCode]['items2_sum'][$iID];
                                    $itemFulfilledJml = 0;
                                    foreach ($_SESSION[$cCode][$gate_update][$iID] as $triID => $triSpec) {
                                        ...
                                        if ($triSpec['valid_qty'] == $dSpec['produk_ord_jml']) {
                                            $newValidQty = $dSpec['produk_ord_jml'];
                                        }
                                        else {
                                            $newValidQty = $dSpec['produk_ord_jml'];
                                        }
                                    ...
                                    //insert baru transaksi_data_items
                                    ceklime($iID);
                                    $registrySubDetail[$iID] = $_SESSION[$cCode]['items2_sum'][$iID];
                                    // cekHitam("prev items");
                                    // arrPrint($_SESSION[$cCode][$gate_update]);
                                    // matiHere("add items " . $iID);
                                    $trMasterId = $_SESSION[$cCode]["mode"]["edit"];
                                    $addSubValues = array(
                                        "sub_step_number" => 1,
                                        "sub_step_current" => 1,
                                        "sub_step_avail" => sizeof($this->configUi[$this->jenisTr]['steps']),
                                    ...
                                    if (isset($_SESSION[$cCode]['tableIn_sub_detail'][$iID]) && sizeof($_SESSION[$cCode]['tableIn_sub_detail'][$iID]) > 0) {
                                        $tr = new MdlTransaksi();
                                        $insertDetailID = $tr->writeDetailSubEntries($trMasterId, $_SESSION[$cCode]['tableIn_sub_detail'][$iID]);
                                        if ($insertDetailID < 1) {
                                            die("Gagal saat berusaha write transaction detail entry pada " . __FILE__ . " baris " . __LINE__);
                                        }
                                        else {
                                            $insertIDs[] = $insertDetailID;
                                            // $insertDeIDs[$trMasterId][] = $insertDetailID;
                                        }
                                        cekUngu($this->db->last_query());
                                    }
                                    else {
                        ...
                        //update
                        if (sizeof($registrySubDetail) > 0) {
                            arrPrint($registrySubDetail);
                            $tr = new MdlTransaksi();
                            ...
                            $udpateRegistry = array(
                                "items2_sum" => $registrySubDetail,
                            );
                            $tr->updateDataRegistry($where, $udpateRegistry) or die(lgShowError("Ada kesalahan", "Gagal saat berusaha  update  params registries"));
                            cekLime($this->db->last_query());
                        }
                        // rombonga upte main transki
                        if (sizeof($insertIDs) > 0) {
                            $tr->addFilter("id=$masterID");
                            $prevMainTransaksi = $tr->lookupMainTransaksi()->result();
                            $indexing_subDetail = blobDecode($prevMainTransaksi[0]->indexing_sub_details);
                            $newSubRegIndexing = blobEncode(array_merge($indexing_subDetail, $insertIDs));
                            $this->db->query("UPDATE transaksi SET indexing_sub_details = '$newSubRegIndexing' WHERE id=$masterID");
                            // cekMerah($this->db->last_query());
                            // arrPrint($indexing_subDetail);
                            // arrPrint($insertIDs);
                            // arrPrint($newSubRegIndexing);
                            // cekHitam($this->db->last_query());
...
//mati_disini("SETOP EDIT STEP 1");
            }
            else {
                cekHitam(":: EDITING STEP 2");
                $stepNum = $_SESSION[$cCode]['mode']['stepNum'];
                $jenisTrTarget = $this->configUi[$this->jenisTr]["steps"][$stepNum]["target"];

                if (isset($this->configUi[$this->jenisTr]['steps'][$stepNum])) {
                    $nextProp = array(
                        "num" => $stepNum,
                        "code" => $this->configUi[$this->jenisTr]['steps'][$stepNum]['target'],
                        "label" => $this->configUi[$this->jenisTr]['steps'][$stepNum]['label'],
                        "groupID" => $this->configUi[$this->jenisTr]['steps'][$stepNum]['userGroup'],
                    );
                }
                else {
                    $nextProp = array(
                ...
                if (isset($this->configCore[$this->jenisTr]['writeSubDetail'][$jenisTrTarget]) && $this->configCore[$this->jenisTr]['writeSubDetail'][$jenisTrTarget] == true) {
                    ...
                    if (isset($_SESSION[$cCode]['tableIn_sub_detail']) && sizeof($_SESSION[$cCode]['tableIn_sub_detail']) > 0) {
                        ...
                        foreach ($_SESSION[$cCode]['tableIn_sub_detail'] as $iID => $dSpec) {
                            ...
                            echo "<script>top.writeProgress('" . strtoupper($dSpec['produk_nama']) . "');</script>";
                            if (isset($_SESSION[$cCode][$gate_update])) {
//                                arrPrintPink($_SESSION[$cCode][$gate_update]);
                                if (array_key_exists($iID, $_SESSION[$cCode][$gate_update])) {
                                    $registrySubDetail[$iID] = $_SESSION[$cCode]['items2_sum'][$iID];
                                    $itemFulfilledJml = 0;
                                    foreach ($_SESSION[$cCode][$gate_update][$iID] as $triID => $triSpec) {
                                        ...
                                        //----------------------------------------------------------
                                        if ($triSpec['valid_qty'] == $dSpec['produk_ord_jml']) {
                                            cekKuning("[extracted] valid_qty == produk_ord_jml [tableIn_sub_detail]");
                                            $newValidQty = $dSpec['produk_ord_jml'];
                                            $newProdukOrdJml = $dSpec['produk_ord_jml'] + $triSpec['dikirim_qty'];
                                        }
                                        else {
                                            cekOrange("[extracted] valid_qty != produk_ord_jml [tableIn_sub_detail]");
                                            $newValidQty = $dSpec['produk_ord_jml'];
                                            $newProdukOrdJml = $dSpec['produk_ord_jml'] + $triSpec['dikirim_qty'];
                                        }
                                        cekUngu("new produk ord jml => $newProdukOrdJml");

                                        $itemFulfilledJml += $newValidQty;
                                        $updateContents = array(
                                            "valid_qty" => $newValidQty,
                                            "produk_ord_jml" => $newProdukOrdJml,
                                        );
                                        if ($newValidQty < 1) {
                                            $childPrevRepaclers = array(
                                                "next_substep_code" => "",
                        ...
                        if (sizeof($insertIDs) > 0) {
                            $tr->addFilter("id=$masterID");
                            $prevMainTransaksi = $tr->lookupMainTransaksi()->result();
                            $indexing_subDetail = isset($prevMainTransaksi[0]->indexing_sub_details) ? blobDecode($prevMainTransaksi[0]->indexing_sub_details) : array();
                            $newSubRegIndexing = blobEncode(array_merge($indexing_subDetail, $insertIDs));
                            $this->db->query("UPDATE transaksi SET indexing_sub_details = '$newSubRegIndexing' WHERE id=$masterID");
                            // cekMerah($this->db->last_query());
                            // arrPrint($indexing_subDetail);
                ...
                $dataSign = array(
                    "nomer" => $_SESSION[$cCode]['main']['nomer'],
                    "step_number" => $_SESSION[$cCode]['mode']['stepNum'],
                    "step_code" => $this->jenisTr,
                    "step_name" => $this->configUi[$this->jenisTr]['steps'][$stepNum]['label'],
                    "group_code" => $this->configUi[$this->jenisTr]['steps'][$stepNum]['userGroup'],
                    "oleh_id" => $this->session->login['id'],
                    "oleh_nama" => $this->session->login['nama'],
                    "keterangan" => $this->configUi[$this->jenisTr]['steps'][$stepNum]['label'] . " oleh " . $this->session->login['nama'],
                    "transaksi_id" => $_SESSION[$cCode]["mode"]["edit"],
                    "deskripsi" => "edit transaksi",
                    "cabang_id" => $this->session->login['cabang_id'],
                    "cabang_nama" => $this->session->login['cabang_nama'],
                    "current_transaksi_id" => $_SESSION[$cCode]["mode"]["edit"],
                    "current_nomer" => $_SESSION[$cCode]['main']['nomer'],
                );
                $dwsign = $tr->writeSignature($_SESSION[$cCode]["mode"]["edit"], $dataSign) or die("Failed to write signature");
                cekBiru($this->db->last_query());
            }
...
//            mati_disini("EDIT transaction under maintenance");
    ...
    public function doEdit()
    {
        ...
        if ($this->jenisTr == "588") {
            ...
            if (isset($_SESSION[$cCode]['main']['projectName'])) {
                ...
                if (isset($_SESSION[$cCode]['items']) && is_array($_SESSION[$cCode]['items']) && sizeof($_SESSION[$cCode]['items']) > 0) {
                    // Project name always sits at the first index of the items array
                    $first_item = reset($_SESSION[$cCode]['items']);
                    if (isset($first_item['projectName'])) {
                        $projectName = trim($first_item['projectName']);
                    } elseif (isset($first_item['nama'])) {
                        $projectName = trim($first_item['nama']);
                    }
                }
            }
            if ($projectName == "") {
        ...
        if (isset($_SESSION[$cCode])) {
            ...
            // Sanitasi array items: bersihkan item phantom (tanpa nama/handler valid)
...
//                            "id_master" => $insertID,
            ...
            //mati_disini("SIMULASI EDIT TRANSAKSI SUKSES <BR>transaction under maintenance<BR>HANYA TRANSAKSI SIMULASI, BELUM TERSIMPAN");
