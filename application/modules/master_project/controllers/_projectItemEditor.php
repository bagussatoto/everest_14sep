                    ...
                    if (sizeof($tmpC) > 0) {
                        ...
                        foreach ($tmpC as $row) {
                            ...
                            if ($jml_nambah > $jml_now) {
                                $msgStok = addslashes("Stok $nama tidak cukup (dibutuhkan $jml_diperlukan, penambahan $jml_nambah, stok tersedia $jml_now)");
                                echo "<script>
                                if (typeof top.close_holdon === 'function') top.close_holdon();
                                if (typeof top.HoldOn !== 'undefined') top.HoldOn.close();
                                if (typeof top.swal === 'function') {
                                    top.swal('Stok Tidak Cukup', '$msgStok', 'warning');
                                } else {
                                    alert('$msgStok');
                                }
                                </script>";
...
//                                 matiHEre();
                    ...
                    if (!isset($tmp['harga_budget'])) {
                        $tmp['harga_budget'] = isset($tmp['harga']) ? $tmp['harga'] : 0;
                    }
...
//                                 matiHEre();
                        ...
                        if ($target_session == "items2_sum") {
                            $this->load->helper("he_project");
                            $faHpp = getHppProdukProject($id);
                            if ($faHpp > 0) {
...
//                                    matiHere(__LINE__ . "<br> hrg_beli: $hrg_beli || hrg_jual: $hrg_jual || nilai_confirm: $nilai_confirm" );
                                    ...
                                    if (($hrg_jual < $hrg_beli) && (!isset($_GET['confirm']))) {
                                        if (($hrg_jual * 1) != ($nilai_confirm * 1)) {
                                            ...
                                            echo "
                                            top.swal({
                                              title: 'HARGA JUAL < HPP REALISASI',
                                              html: 'Harga Jual Project <r>(" . number_format($hrg_jual) . ")</r> lebih kecil dari HPP Realisasi <r>(" . number_format($hrg_beli) . ")</r>. <br>Jika dilanjutkan, Anda mengalami kerugian pada item ini..<br><br>Apakah Anda yakin ingin melanjutkan?',
                                              type: 'warning',
                                              showConfirmButton: true,
                                              showCancelButton: true
                                            }).then((confirm) => {
                                                  if (confirm) {
                                                        top.swal('Anda memilih untuk melanjutkan Harga Jual lebih kecil dari HPP Realisasi');
...
//            arrprint($datas);
        ...
        $diskon_pembulatan = 0;
        if(!isset($_SESSION[$cCode]['main']['diskon_pembulatan'])){
            $_SESSION[$cCode]['main']['diskon_pembulatan'] = 0;
        }
        else{
            $diskon_pembulatan = $_SESSION[$cCode]['main']['diskon_pembulatan'];
        }

//        matiHere($diskon_pembulatan);

        $_SESSION[$cCode]['main']['diskon_pembulatan'] = $diskon_pembulatan;
//        $_SESSION[$cCode]['main']['harga_bruto2'] = $harga_total;
//        $_SESSION[$cCode]['main']['harga_items2'] = $harga_total-$diskon_pembulatan;
        /**
         * ini dipindah ke configCore supaya menjalankan valueBuilders
         * kalau disni jika tidak ada edit, maka gerbang tidak tercipta
         *ketika dijalankan akan diblocking karena gerbang nilai project 0
         *
         */
//        $_SESSION[$cCode]['main']['harga_items3'] = $harga_total-$diskon_pembulatan;
//        $_SESSION[$cCode]['main']['ppn_items3'] = ($harga_total-$diskon_pembulatan)*(11/100);
        //================================= INI dipidah lihat configcore valuebuilder =============================================
        ...
        $_SESSION[$cCode]['main']['diskon_pembulatan'] = 0;
        $_SESSION[$cCode]['main']['harga_bruto2']   = $_SESSION[$cCode]["main"]["harga_jual_project"];
        $_SESSION[$cCode]['main']['harga_items2']   =  $_SESSION[$cCode]['main']['harga_bruto2']  - $_SESSION[$cCode]['main']['diskon_pembulatan'];
        $_SESSION[$cCode]['main']['harga_non_ppn']  =  $_SESSION[$cCode]["main"]["harga_bruto2"]  - $_SESSION[$cCode]['main']['diskon_pembulatan'];
