...
class Transaksi extends Modul_Controller
{
    ...
    public function validate()
    {
        ...
        if (empty($items3) && !$allow_empty_termin) {
            ...
            $selisih_termin = abs($total_harga_termin - $total_project);
            $toleransi_termin = 5; // Toleransi Rp 5 untuk selisih pembulatan/sen persentase termin

            if (
                $pengaturan_termin
                && $selisih_termin > $toleransi_termin
                && !$allow_inchoate_termin
            ) {
                // Debug Information
                $debug  = "<div class='meta bg-yellow'>pengaturan_termin: {$pengaturan_termin}</div>";
                $debug .= "<div class='meta bg-yellow'>Selisih: " . number_format($selisih_termin, 2) . " (Toleransi: {$toleransi_termin})</div>";
    ...
    public function viewUndoneItems()
    {
        ...
        $activeProject = array();
        $activeProjectById = array();
        if (!empty($tmpPrj)) {
            foreach ($tmpPrj as $pk => $rowPrj) {
                if (isset($rowPrj->id) && $rowPrj->id > 0) {
                    $activeProjectById[$rowPrj->id] = $rowPrj;
                }
                if (!empty($rowPrj->project_start_nomer)) {
                    $activeProject[$rowPrj->project_start_nomer] = $rowPrj;
                }
                if (!empty($rowPrj->quot_nomer)) {
                    $activeProject[$rowPrj->quot_nomer] = $rowPrj;
                }
                if (!empty($rowPrj->t1_closing_transaksi_nomer)) {
                    $activeProject[$rowPrj->t1_closing_transaksi_nomer] = $rowPrj;
                }
                if (!empty($rowPrj->transaksi_no)) {
...
//                $linkRencana = MODUL_PATH . "Printing/viewReceiptReg/$jenisTr/".$row->nomer;
                ...
                $curPrj = null;
                if (isset($activeProject[$row->nomer])) {
                    $curPrj = $activeProject[$row->nomer];
                }
                elseif (isset($row->project_id) && isset($activeProjectById[$row->project_id])) {
                    $curPrj = $activeProjectById[$row->project_id];
                }

                if ($curPrj != null && $curPrj->project_start == 1) {
                    $link = MODUL_PATH . "MasterData/index/";
                    $newLink = $link . $curPrj->id;
                    ...
                    if ($curPrj != null && $curPrj->lock == 1 && $curPrj->quot_status == 0) {
                        $tmp['rencana'] = "";
                    }
                    else {
                        if ($curPrj != null && $curPrj->lock == 1 && $curPrj->quot_status == 1) {
                            $tmp['rencana'] = "<div class='btn-group btn-group-sm' style='display: inline-flex; gap: 4px;'>";
                            $tmp['rencana'] .= "<button type='button' class='btn bg-olive' title='Simple Quotation' data-toggle='tooltip' onclick=\"top.popBig('$linkRencana')\"><i class='fa fa-print'></i> Simple</button>";
                            $tmp['rencana'] .= "<button type='button' class='btn bg-yellow' title='Detail Quotation' data-toggle='tooltip' onclick=\"top.popBig('$linkRencanaDetail')\"><i class='fa fa-print'></i> Details</button>";
                            $tmp['rencana'] .= "</div>";
                        }
                        else {
                            $tmp['rencana'] = "";
                ...
                if ($allowFollowup) {
                    ...
                    if ($curPrj != null && $curPrj->project_start == 1) {
                        ...
                        if ($nextStepNum == 5) {
                            // Sudah closing tahap 1, menuju tahap akhir (FINAL)
                            $prjId = $curPrj != null ? $curPrj->id : 0;
                            $tmp['action'] .= "<button id='prj_id_" . $prjId . "' type='button' class='btn btn-sm btn-info' title='Project Bisa Dilanjutkan untuk closing tahap akhir (final)' data-toggle='tooltip'><i class='fa fa-check-square'></i> PROGRESS FINAL </button>";
                            $tmp['action'] .= "<button type='button' class='btn btn-sm btn-danger' title='turn this entry into $stepLabel' data-toggle='tooltip' onclick=\"top.open_holdon();$followupLink\"><i class='glyphicon glyphicon-check'></i> <b>(FINAL) SERAH TERIMA AKHIR PEKERJAAN</b></button>";
                        }
                        elseif ($curPrj->persen_progress >= 99) {
                            //progress > 99% atau = 99%
                            //belum tahap 1
                            $tmp['action'] .= "<button 
                                                id='prj_id_" . $curPrj->id . "' 
                                                type='button' 
                            ...
                            if (isset($curPrj->edit) && $curPrj->edit) {
                                ...
                                $tmp['action'] .= "<button line='" . __LINE__ . "' 
                                                    ...
                                                    <i class='glyphicon glyphicon-check'></i> <b>(TAHAP 1) SERAH TERIMA SEMENTARA</b>
                                                    </button>";
                            }
                        }
                        else {
                            //progress di bawah 99%
                            $persentase = 0 + $curPrj->persen_progress * 1;
                            $tmp['action'] .= "<button line='" . __LINE__ . "' id='prj_id_" . $curPrj->id . "' type='button' class='btn btn-sm btn-danger' title='Project Bisa Dilanjutkan setelah Progress 100%...' data-toggle='tooltip'><i class='fa fa-gear fa-spin'></i> On Progress " . number_format($curPrj->persen_progress * 1) . "% </button>";
                            if ($persentase * 1 > 0) {
                                $tmp['action'] .= "
                                    <script>
                                        var p =  $persentase || 1, g = Math.round(255*(p/100)), r = 255-g;
                                        $('#prj_id_" . $curPrj->id . "').css('background',\"rgb(\"+r+\",\"+g+\",0)\");
                                    </script>
                                ";
                            }
                            if ($allowCancel) {
                                $tmp['action'] .= "<div class='input-group'>";
                                $tmp['action'] .= "<a class='btn btn-primary btn-block'
                                                        title='turn this entry into $stepLabel' 
                                                        href='javascript:void(0)' 
                                                        onClick=\"top.open_holdon();$followupLink\">" . $actionLabel . "
                                                        </a>";
                                if ($allowJoin) {
                        ...
                        if ($curPrj != null && $curPrj->lock == 1 && $curPrj->quot_status == 0) {
                            $tmp['action'] = "<div class='btn-group-sm btn-group-vertical'>";
                            $tmp['action'] .= "<button type='button' class='btn btn-sm btn-primary' title='turn this entry into $stepLabel' data-toggle='tooltip' onclick=\"top.open_holdon();$followupLink\"><i class='fa fa-check'></i> " . $actionLabel . "</button>";
                            $editBomLink = "" . MODUL_PATH . "MasterData/editBom/" . $curPrj->id;
                            $tmp['action'] .= "<button type='button' class='btn btn-sm btn-warning' title='cancel $stepLabel' data-toggle='tooltip' onclick=\"if(confirm('mengaktifkan mode BOM, akan mengunci transaksi ini hingga BOM selesai di Edit dan disimpan kembali. Yakin untuk melakukan ini...??')){window.location.href='$editBomLink'}\"><i class='fa fa-pencil'></i> Edit BOM</button>";
                            if ($allowJoin) {
                                $tmp['action'] .= "<span class='input-group-addon'>";
                                $tmp['action'] .= "<a title='process many items at once'
                                                        href='" . MODUL_PATH . "Transaksi/viewIncomplete/" . $this->jenisTr . "/$currentStepNum'>
                                                        <i class='fa fa-dedent'></i>
                                                        </a>";
                                $tmp['action'] .= "</span>";
                            ...
                            if ($curPrj != null && $curPrj->lock == 1 && $curPrj->quot_status == 1) {
                                if (isset($curPrj->edit) && $curPrj->edit == 1) {
                                    $link = MODUL_PATH . "MasterData/index/";
                                    $newLink = $link . $curPrj->id;
                                    $tmp['action'] = "<div class='btn-group-sm btn-group-vertical'>";
                                    $tmp['action'] .= "<a class='btn btn-sm btn-warning'
                                                            title='Project masih dalam proses edit RAB'
                                                            data-toggle='tooltip'
                                                            href='javascript:void(0);'
                                                            onclick=\"top.open_holdon();window.location.href='$newLink'\">
                                                            <i class='fa fa-hourglass fa-spin'></i> PROSES EDIT RAB
                                                            </a>";
                                    $tmp['action'] .= "</div>";
                                }
                                else {
                                    $tmp['action'] = "<div class='btn-group-sm btn-group-vertical'>";
                                    $tmp['action'] .= "<button type='button' class='btn btn-sm btn-info' title='turn this entry into $stepLabel' data-toggle='tooltip' onclick=\"top.open_holdon();$followupLink\"><i class='fa fa-check'></i> APPROVE & START PROJECT</button>";
                                    if ($allowJoin) {
                                        $tmp['action'] .= "<span class='input-group-addon'>";
                                        $tmp['action'] .= "<a title='process many items at once'
                                                                href='" . MODUL_PATH . "Transaksi/viewIncomplete/" . $this->jenisTr . "/$currentStepNum'>
                                ...
                                if ($nextStepNum == 3) {
                                    $link = MODUL_PATH . "MasterData/index/";
                                    $newLink = $curPrj != null ? $link . $curPrj->id : "javascript:void(0);";
                                    $tmp['action'] = "<div class='btn-group-sm btn-group-vertical'>";
                                    $tmp['action'] .= "<a class='btn btn-sm btn-warning'
                                                            title='RAB Belum Disimpan. Silahkan Buat / Kelola BOM terlebih dahulu'
                                                            data-toggle='tooltip' 
                                                            href='javascript:void(0);' 
                                                            onclick=\"top.open_holdon();window.location.href='$newLink'\">
                                                            <i class='fa fa-cogs'></i> CREATE BOM / SUSUN RAB
                                                            </a>";
                                    $tmp['action'] .= "</div>";
                                }
                                else {
...
//                arrPrintPink($sSpec);
        ...
        if (array_intersect($allowedView, $mems)) {
            ...
            if (sizeof($tmp) > 0) {
                ...
                //region Description
                $mutasiLinks = array();
                $mutasiPihakID = isset($main['pihakID']) ? $main['pihakID'] : 0;
                $mutasiCabangID = isset($main['cabangID']) ? $main['cabangID'] : 0;
                foreach ($accountChildsLinks as $rekNama => $rekLink) {
                    $mutasiLinks[$rekNama] = $rekLink . "/" . $mutasiPihakID . "?o=" . $mutasiCabangID . "&trID=$transaksiID";
                }
                //endregion
...
//            "mode" => $this->uri->segment(3),
    ...
    public function index()
    {
        ...
        ";
            ...
            $scriptBottom = "";
             $scriptBottom .= "<script>
             var viewundoneList = window.viewundoneList = top.viewundoneList = function () {
                         top.$('#undoneList').html(\"<div class='row' style='margin-top: 14px; padding-bottom: 14px;'> <span class='col-lg-12 text-center'><img width='5%' src='$baseUrl/assets/images/loading_16_p.gif'></span></div>\");
                     setTimeout(function () {
                                      if (top.$('#undoneList').find('img').length > 0) {
                                          top.$('#undoneList').load('$link_undoneList', function(){viewundoneList})
             ...
             var undoneTambahanProject = window.undoneTambahanProject = top.undoneTambahanProject = function () {
            ...
            var viewhistoryList = window.viewhistoryList = top.viewhistoryList = function () {
                top.$('#historyList').html(\"<div class='row' style='margin-top: 14px; padding-bottom: 14px;'> <span class='col-lg-12 text-center'><img width='5%' src='$baseUrl/assets/images/loading_16_p.gif'></span> </div>\");
                setTimeout(function () {
                      if (top.$('#historyList').find('img').length > 0) {
                          top.$('#historyList').load('$link_historyList', function(){viewhistoryList})
             ...
             var viewundoneListApprove = window.viewundoneListApprove = top.viewundoneListApprove = function () {
        ...
        ";
            ...
            $scriptBottom .= "<script>
             ...
             var qcProject = window.qcProject = top.qcProject = function () {
...
//        $this->db->limit(10);
            ...
            if (!empty($activeProjectIds)) {
                ...
                $arrTaskTmb = $this->db->get("project_tasklist_tambahan")->result();
        ...
        // PURPOSE   : Fast pre-calculation of dashboard summary statistics
        // COMPLIANCE: ISO 27001 & PHP 5.6 Compatibility
        // LOG_EXPIRE: 2026-11-27
        // [/AGENT_LOG]
        $todayStr = date('Y-m-d');
        $project_stats = array('total' => 0, 'completed' => 0, 'inProgress' => 0, 'delayed' => 0, 'overallProgress' => 0);
        $pTotalProg = 0;
        $pCounted = 0;
        if (!empty($tmpDataProject)) {
            $project_stats['total'] = count($tmpDataProject);
            foreach ($tmpDataProject as $p) {
                $pProg = isset($p->persen_progress) ? (float)$p->persen_progress : 0;
                $pTotalProg += min(100, max(0, $pProg));
                $pCounted++;
                if ($pProg === 100.0) {
                    $project_stats['completed']++;
                } elseif ($pProg > 0) {
                    $project_stats['inProgress']++;
                } else {
                    $project_stats['delayed']++;
                }
            }
            $project_stats['overallProgress'] = $pCounted > 0 ? (int)round($pTotalProg / $pCounted) : 0;
        }

        $tasklist_stats = array('total' => 0, 'completed' => 0, 'inProgress' => 0, 'delayed' => 0, 'pending' => 0, 'deleted' => 0, 'overallProgress' => 0);
        $tTotalProg = 0;
        $tCounted = 0;
        if (!empty($arrTask)) {
            $tasklist_stats['total'] = count($arrTask);
            foreach ($arrTask as $t) {
                $prog = isset($t->progress_percent) ? (float)$t->progress_percent : 0;
                $trash = isset($t->trash) ? (int)$t->trash : 0;
                $dEnd = (!empty($t->dtime_end) && $t->dtime_end != '0000-00-00') ? substr($t->dtime_end, 0, 10) : '';
                $isOverdue = (!empty($dEnd) && $dEnd < $todayStr && $prog < 100);

                if ($trash == 1) {
                    $tasklist_stats['deleted']++;
                } else {
                    $tTotalProg += $prog;
                    $tCounted++;
                    if ($prog == 100) {
                        $tasklist_stats['completed']++;
                    } elseif ($prog > 0) {
                        $tasklist_stats['inProgress']++;
                    } else {
                        $tasklist_stats['pending']++;
                    }
                    if ($isOverdue) {
                        $tasklist_stats['delayed']++;
                    }
                }
            }
            $tasklist_stats['overallProgress'] = $tCounted > 0 ? (int)round($tTotalProg / $tCounted) : 0;
        }
...
//            $this->db->where("closing_status", 0);
        ...
        foreach ($tmpkl as $k => $locRow) {
            ...
            $locRow->nama = isset($produk_name[$locRow->produk_id]) ? $produk_name[$locRow->produk_id] : (isset($locRow->nama) && $locRow->nama !== '' ? $locRow->nama : '-');
        ...
        if (count($tmp) > 0) {
            ...
            foreach ($tmp as $trmp_0) {
                ...
                foreach ($viewFields as $key => $label) {
                    $newLink = $link . $prID;
                    $newLinkTimeline = $linkTimeline . $prID;
...
//                "formatNota" => "stepCode|employee_id,.stepCode,.employee_id,stepCode|employee_id,.month,.years",
     ...
     * [AGENT_LOG]
     * ROLE      : Software Engineer Agent
     * PURPOSE   : Server-side DataTables AJAX endpoint for project and tasklist drilldown modal
     * COMPLIANCE: ISO 27001 (RBAC Security), PHP 5.6 Compatibility
     * LOG_EXPIRE: 2026-11-27
     * [/AGENT_LOG]
     */
    public function drilldownAjax()
    {
        session_write_close();
        ...
        $cabangId = isset($this->session->login['cabang_id']) ? $this->session->login['cabang_id'] : 0;
        $myId = isset($this->session->login['id']) ? $this->session->login['id'] : 0;

        $category = isset($_REQUEST['category']) ? $_REQUEST['category'] : 'tasklist';
        $type = isset($_REQUEST['type']) ? $_REQUEST['type'] : 'all';
        $tahun = isset($_REQUEST['tahun']) ? trim($_REQUEST['tahun']) : '';
        $includeBatal = (isset($_REQUEST['include_batal']) && $_REQUEST['include_batal'] == '1') ? 1 : 0;
        ...
        if ($category === 'project') {
            $whereParts = array("closing_status = 0", "status = '1'", "trash = 0");
            $bindParams = array();

            if (!$isManagement) {
                $qMy = $this->db->query("SELECT DISTINCT produk_id FROM project_tasklist WHERE employee_id = ? AND status = 1 AND trash = 0
                                         UNION
                                         SELECT DISTINCT produk_id FROM project_tasklist_tambahan WHERE employee_id = ? AND status = 1 AND trash = 0", array($myId, $myId))->result();
                $myPrjIds = array();
                foreach ($qMy as $r) {
                    if (!empty($r->produk_id)) $myPrjIds[] = (int)$r->produk_id;
                }
                if (!empty($myPrjIds)) {
                    $whereParts[] = "id IN (" . implode(",", $myPrjIds) . ")";
                } else {
                    $whereParts[] = "id = -99999";
                }
            } else {
                if (!in_array("c_holding", $mems)) {
                    $whereParts[] = "cabang_id = ?";
                    $bindParams[] = $cabangId;
                }
            }

            if ($type === 'completed') {
                $whereParts[] = "persen_progress = 100";
            } elseif ($type === 'inprogress') {
                $whereParts[] = "persen_progress > 0 AND persen_progress != 100";
            } elseif ($type === 'delayed') {
                $whereParts[] = "persen_progress = 0";
            }

            if (!empty($tahun)) {
                $thn = (int)$tahun;
                $whereParts[] = "(YEAR(start_dtime) = ? OR YEAR(end_dtime) = ? OR (start_dtime IS NULL AND ? = " . (int)date('Y') . "))";
                $bindParams[] = $thn;
                $bindParams[] = $thn;
                $bindParams[] = $thn;
            }

            $whereBaseSql = implode(" AND ", $whereParts);
            $sqlTotal = "SELECT COUNT(*) as cnt FROM project_produk WHERE " . $whereBaseSql;
            $qTotal = $this->db->query($sqlTotal, $bindParams)->row();
            $totalRecords = $qTotal ? (int)$qTotal->cnt : 0;

            $whereFilterParts = $whereParts;
            $bindFilterParams = $bindParams;
            if (!empty($searchValue)) {
                $whereFilterParts[] = "(nama LIKE ? OR customer_nama LIKE ? OR spek LIKE ?)";
                $likeVal = "%" . $searchValue . "%";
                $bindFilterParams[] = $likeVal;
                $bindFilterParams[] = $likeVal;
                $bindFilterParams[] = $likeVal;
            }

            $whereFilterSql = implode(" AND ", $whereFilterParts);
            $sqlFiltered = "SELECT COUNT(*) as cnt FROM project_produk WHERE " . $whereFilterSql;
            $qFiltered = $this->db->query($sqlFiltered, $bindFilterParams)->row();
            $filteredRecords = $qFiltered ? (int)$qFiltered->cnt : 0;

            $sqlData = "SELECT id, nama, customer_nama, harga, persen_progress, end_dtime, start_dtime, spek 
                        FROM project_produk 
                        WHERE " . $whereFilterSql . " 
                        ORDER BY id DESC 
                        LIMIT " . (int)$start . ", " . (int)$length;
            $resData = $this->db->query($sqlData, $bindFilterParams)->result();

            $dataRows = array();
            $no = $start + 1;
            $baseModul = MODUL_PATH . "MasterData/index/";

            foreach ($resData as $p) {
                $rawProg = (float)$p->persen_progress;
                $prg = min(100, max(0, $rawProg));
                $badgeColor = $prg >= 80 ? 'label-success' : ($prg >= 50 ? 'label-warning' : 'label-danger');
                $deadlineStr = !empty($p->end_dtime) ? substr($p->end_dtime, 0, 10) : '-';
                $isOverdue = (!empty($p->end_dtime) && substr($p->end_dtime, 0, 10) < $today && $rawProg < 100);
                $overdueBadge = $isOverdue ? ' <span class="label label-danger" style="font-size: 10px;">Lewat Deadline</span>' : '';
                $link = $baseModul . $p->id;

                $dataRows[] = array(
                    "no" => $no++,
                    "nama" => "<strong>" . htmlspecialchars($p->nama) . "</strong>",
                    "customer_nama" => htmlspecialchars(!empty($p->customer_nama) ? $p->customer_nama : '-'),
                    "harga" => number_format((float)$p->harga),
                    "progress" => '<span class="label ' . $badgeColor . '" style="font-size: 12px; padding: 3px 8px;">' . $prg . '%</span>',
                    "deadline" => $deadlineStr . $overdueBadge,
                    "aksi" => '<a href="' . $link . '" class="btn btn-xs btn-primary" target="_blank"><i class="fa fa-folder-open"></i> Buka</a>'
                );
            }
            ...
            // Tasklist Drilldown
            $wherePrj = array("closing_status = 0", "status = '1'", "trash = 0");
            $bindPrj = array();
            if (!$isManagement) {
                // Personal
            } else {
            ...
            $sqlPrj = "SELECT id FROM project_produk WHERE " . implode(" AND ", $wherePrj);
            $resPrj = $this->db->query($sqlPrj, $bindPrj)->result();
            $prjIds = array();
            foreach ($resPrj as $rp) {
                $prjIds[] = (int)$rp->id;
            }
            ...
            $inPrj = implode(",", $prjIds);

            $subquery = "
                SELECT id, no_spk, no_pre_spk, nama, produk_id, produk_nama, employee_id, employee_nama, CAST(progress_percent AS DECIMAL(10,2)) as prog, dtime_start, dtime_end, dtime, trash, status, nilai, cabang_id 
                FROM project_tasklist WHERE produk_id IN ($inPrj)
                UNION ALL
                SELECT id, no_spk, no_pre_spk, nama, produk_id, produk_nama, employee_id, employee_nama, CAST(progress_percent AS DECIMAL(10,2)) as prog, dtime_start, dtime_end, dtime, trash, status, nilai, cabang_id 
                FROM project_tasklist_tambahan WHERE produk_id IN ($inPrj)
            ";
            ...
            if ($type === 'completed') {
                $whereParts[] = "trash = 0 AND prog = 100";
            } elseif ($type === 'inprogress') {
                $whereParts[] = "trash = 0 AND prog > 0 AND prog < 100";
            } elseif ($type === 'delayed') {
                $whereParts[] = "trash = 0 AND prog < 100 AND dtime_end IS NOT NULL AND dtime_end != '0000-00-00' AND dtime_end < ?";
                $bindParams[] = $today;
            } elseif ($type === 'pending') {
                $whereParts[] = "trash = 0 AND prog = 0";
            } elseif ($type === 'deleted') {
                $whereParts[] = "trash = 1";
            } else {
                if (!$includeBatal) {
            ...
            if (!empty($tahun)) {
                $whereParts[] = "YEAR(dtime) = ?";
                $bindParams[] = (int)$tahun;
            }

            $whereBaseSql = implode(" AND ", $whereParts);
            $sqlTotal = "SELECT COUNT(*) as cnt FROM (" . $subquery . ") as t WHERE " . $whereBaseSql;
            $qTotal = $this->db->query($sqlTotal, $bindParams)->row();
            $totalRecords = $qTotal ? (int)$qTotal->cnt : 0;
            ...
            $bindFilterParams = $bindParams;
            if (!empty($searchValue)) {
                $whereFilterParts[] = "(no_spk LIKE ? OR no_pre_spk LIKE ? OR nama LIKE ? OR produk_nama LIKE ? OR employee_nama LIKE ? OR nilai LIKE ?)";
                $likeVal = "%" . $searchValue . "%";
                for ($i = 0; $i < 6; $i++) {
                    $bindFilterParams[] = $likeVal;
                }
            ...
            $whereFilterSql = implode(" AND ", $whereFilterParts);
            $sqlFiltered = "SELECT COUNT(*) as cnt FROM (" . $subquery . ") as t WHERE " . $whereFilterSql;
            $qFiltered = $this->db->query($sqlFiltered, $bindFilterParams)->row();
            $filteredRecords = $qFiltered ? (int)$qFiltered->cnt : 0;

            $sqlData = "SELECT * FROM (" . $subquery . ") as t WHERE " . $whereFilterSql . " ORDER BY id DESC LIMIT " . (int)$start . ", " . (int)$length;
            $resData = $this->db->query($sqlData, $bindFilterParams)->result();

            $dataRows = array();
            $no = $start + 1;

            foreach ($resData as $t) {
                $prg = (float)$t->prog;
                $badgeColor = $prg >= 100 ? 'label-success' : ($prg > 0 ? 'label-warning' : 'label-default');
                $spkNo = !empty($t->no_spk) ? $t->no_spk : (!empty($t->no_pre_spk) ? $t->no_pre_spk : '-');
                $deadlineStr = !empty($t->dtime_end) && $t->dtime_end != '0000-00-00' ? substr($t->dtime_end, 0, 10) : '-';
                $isOverdue = (!empty($t->dtime_end) && $t->dtime_end != '0000-00-00' && substr($t->dtime_end, 0, 10) < $today && $prg < 100);
                $overdueBadge = $isOverdue ? ' <span class="label label-danger" style="font-size: 10px;">Terlambat</span>' : '';
                $deletedBadge = $t->trash == 1 ? ' <span class="label label-danger" style="font-size: 10px;">Batal</span>' : '';
                $tugasNama = (!empty($t->produk_nama) ? $t->produk_nama : '') . (!empty($t->nama) && $t->nama !== $t->produk_nama ? ' - ' . $t->nama : '');

                $dataRows[] = array(
                    "no" => $no++,
                    "no_spk" => "<strong>" . htmlspecialchars($spkNo) . "</strong>" . $deletedBadge,
                    "tugas" => htmlspecialchars(!empty($tugasNama) ? $tugasNama : '-'),
                    "petugas" => '<i class="fa fa-user text-muted"></i> ' . htmlspecialchars(!empty($t->employee_nama) ? $t->employee_nama : '-'),
                    "progress" => '<span class="label ' . $badgeColor . '" style="font-size: 12px; padding: 3px 8px;">' . $prg . '%</span>',
                    "tenggat" => $deadlineStr . $overdueBadge,
                    "keterangan" => htmlspecialchars(!empty($t->nilai) ? $t->nilai : '-')
                );
            }
