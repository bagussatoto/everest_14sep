    ...
    case "viewUndoneItemsIndex":
        ...
        if (sizeof($steps) > 1) {
            ...
            foreach ($steps as $tStep => $stepData) {
                ...
                $cssSelesai = count($steps) == $tStep ? "style='padding-top: 0;padding-bottom: 0;'" : "";
                $strOnprog .= "<li class='$actives'>";
                //                $strOnprog .= "<a $cssSelesai class='nav-link btn'
                //                        onclick=\"location.href='" . MODUL_PATH . "Pembelian/index/" . $this->uri->segment(4) . "?step=$tStep';\">";
                //                $undoneLinkIndex = MODUL_PATH . "Transaksi/index/" . $this->uri->segment(4) . "?step=$tStep";
                $undoneLinkIndex = MODUL_PATH . $this->uri->segment(2) . "/" . $this->uri->segment(3) . "/" . $this->uri->segment(4) . "?step=$tStep";
                $strOnprog .= "<a $cssSelesai class='nav-link btn' 
                        onclick=\"$('#undoneList').load('$undoneLinkIndex');\">";
                            ...
                            foreach ($itemLabels2 as $key2 => $label2) {
                                ...
                                if ($allowEdit == true && in_array($key2, $editableFields2)) {
                                    ...
                                    if (is_numeric($val2)) {
                                        ...
                                        $addEvent = "";
                                        if (!$allowIncrement) {
                                            $addEvent = " oninput=\"if(parseInt(removeCommas(this.value))<1 || parseInt(removeCommas(this.value))>$maxVal2){this.value='" . number_format($maxVal2) . "';}\" onkeyup=\"this.value=addCommas(this.value);\" onblur=\"top.$('#result').load('$updateSubItemFieldTarget?id=$id&key=$key&val='+removeCommas(this.value))\" ";
                                        }
                                        else {
                                            // cekHitam("allowincrement");
                                            $addEvent = " onkeyup=\"this.value=addCommas(this.value);\"  onblur=\"top.$('#result').load('$updateSubItemFieldTarget?ravPrev=$rawPrevURL&rawBuilderURL=$rawBuilderURL&id=$id&key=$key2&val='+removeCommas(this.value))\" ";
                                        }
                                    ...
                                    //                            cekMerah(":: $key NOT editable ::");
                                    $strVal2 = formatField($key2, $iSpec2[$key2]);
                ...
                foreach ($detil_item_rslt_label2 as $jn => $jndata) {
                    foreach ($jndata as $yy => $rrr) {
                        ...
                        foreach ($datas[$jn] as $datas_0) {
                            $rt++;
                            $var = formatField("harga", $datas_0[$yy]);
                            if ($hasil == "") {
                                $hasil .= "$var";
                            }
                            else {
                ...
                foreach ($totalJn as $jnKey => $jnVAlues) {
                    ...
                    foreach ($jnVAlues as $rkey => $rValue) {
                        if ($rkey == "harga") {
                            ...
                            $valSUB = formatField("harga", $rValue);
                        }
