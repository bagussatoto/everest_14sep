...
//    // Sanitasi Tampilan: buang phantom item (tanpa nama/handler valid) sebelum merender tabel
        ...
        if (isset($itemLabels)) {
            if (sizeof($itemLabels) && (is_array($itemLabels)) && $showItems) {
                ...
                foreach ($itemLabels as $key => $label) {
                    echo "<th line='".__LINE__."' style='width:1%;white-space: nowrap;' class='text-muted text-center'>";
                    echo $label;
                    echo "</th>";
                }
                        ...
                        case "text":
                            if (is_numeric($fieldVal)) {
                                ...
                                if ($key == "nama") {
                                    ...
                                    if (isset($iSpec['allow_autopurchase']) && $iSpec['allow_autopurchase'] == 1) {
                                        $isAutopurchase = true;
                                    } else if (isset($iSpec['id']) && $iSpec['id'] > 0) {
                                        $chkId = (int)$iSpec['id'];
                                        $cCode = isset($cCode) ? $cCode : (isset($this->cCode) ? $this->cCode : "_TR_" . $this->uri->segment(4));
                                        if (isset($_SESSION[$cCode]['items2_sum'][$chkId]['allow_autopurchase']) && $_SESSION[$cCode]['items2_sum'][$chkId]['allow_autopurchase'] == 1) {
                                            $isAutopurchase = true;
                                        } else {
                                            $ci =& get_instance();
                                            $qP = $ci->db->query("SELECT allow_autopurchase FROM produk WHERE id = ? LIMIT 1", array($chkId))->row_array();
                                            if (!empty($qP) && isset($qP['allow_autopurchase']) && $qP['allow_autopurchase'] == 1) {
                                                $isAutopurchase = true;
                                                $_SESSION[$cCode]['items2_sum'][$chkId]['allow_autopurchase'] = 1;
                                            }
                                        }
                                    }
                                    if ($isAutopurchase) {
                                        $extraBadge = " <span class='label label-primary' style='background-color:#0284c7; font-size:10px; padding:2px 6px; border-radius:3px; vertical-align:middle; letter-spacing:0.3px; display:inline-block; margin-left:5px;' title='Material Pengadaan Otomatis (PO Step 3)'><i class='fa fa-bolt'></i> AUTOPURCHASE</span>";
                                    }
                                }

                                if ($key == "satuan") {
                                    if (empty($fieldVal) || $fieldVal == "n/a" || $fieldVal == "-") {
                                        if (isset($iSpec['id']) && $iSpec['id'] > 0) {
                                            $chkId = (int)$iSpec['id'];
                                            $ci =& get_instance();
                                            $qSat = $ci->db->query("SELECT s.nama AS satuan_nama FROM produk p LEFT JOIN satuan s ON p.satuan_id = s.id WHERE p.id = ? LIMIT 1", array($chkId))->row_array();
                                            if (!empty($qSat['satuan_nama'])) {
                                                $fieldVal = $qSat['satuan_nama'];
                                            } else {
                                                $fieldVal = "unit";
                                            }
                                        } else {
                                ...
                                if (strlen($fieldVal) > 10) {
                                    echo "<span keyid=$keyID noid=$noID id=$colID class='' style='color:$color;border:0px;'>" . formatField_he_format($key, $fieldVal) . $extraBadge . "</span>";
                                }
                                else {
                                    echo "<span keyid=$keyID noid=$noID id=$colID class='form-control' style='color:$color;border:0px;'>" . formatField_he_format($key, $fieldVal) . $extraBadge . "</span>";
                                }
                            }
                            break;
                    }
                        ...
                        case "text":
                            if (is_numeric($fieldVal)) {
                                ...
                                if (strlen($fieldVal) > 10) {
                                    ...
                                    echo "<span line='".__LINE__."' key='$key' keyid=$keyID noid=$noID id=$keyNoID fieldVal='$fieldVal' class='form-control' style='color:$color;border:0px;'>" . formatField_he_format($key, $fieldVal) . "</span>";
                                }
                            }
                            break;
                    }
...
//                    }
        ...
        if (sizeof($addRows) > 0) {
            $totalJenisMaterial = (isset($items2) && is_array($items2)) ? sizeof($items2) : 0;
            $totalQtyMaterial = 0;
            if ($totalJenisMaterial > 0) {
                foreach ($items2 as $itSpec) {
                    $totalQtyMaterial += isset($itSpec['jml']) ? ($itSpec['jml'] + 0) : 0;
                }
            }

            $totalJenisBiaya = (isset($items3) && is_array($items3)) ? sizeof($items3) : 0;
            $totalQtyBiaya = 0;
            if ($totalJenisBiaya > 0) {
                foreach ($items3 as $itSpec3) {
                    $totalQtyBiaya += isset($itSpec3['jml']) ? ($itSpec3['jml'] + 0) : 0;
                }
            }

            echo "<tr class='bg-grey-01'>";
            echo "<td colspan='" . sizeof($itemLabels) . "' class='text-right bottom-borderless valign-m text-uppercase text-bold' style='color:#334155;'>JUMLAH ITEM MATERIAL</td>";
            echo "<td colspan='2' class='text-right'>";
            echo "<input type='text' id='total_item_material' class='form-control text-right' readonly style='font-size:15px; font-weight:bold; color:#1e293b; background:#f8fafc;' value='" . number_format($totalJenisMaterial) . " ITEM' >";
            echo "</td>";
            echo "</tr>";

            echo "<tr class='bg-grey-01'>";
            echo "<td colspan='" . sizeof($itemLabels) . "' class='text-right bottom-borderless valign-m text-uppercase text-bold' style='color:#334155;'>TOTAL QTY MATERIAL</td>";
            echo "<td colspan='2' class='text-right'>";
            echo "<input type='text' id='total_qty_material' class='form-control text-right' readonly style='font-size:15px; font-weight:bold; color:#0284c7; background:#f8fafc;' value='" . niceDecimal($totalQtyMaterial) . "' >";
            echo "</td>";
            echo "</tr>";

            if ($totalJenisBiaya > 0) {
                echo "<tr class='bg-grey-01'>";
                echo "<td colspan='" . sizeof($itemLabels) . "' class='text-right bottom-borderless valign-m text-uppercase text-bold' style='color:#334155;'>JUMLAH ITEM BIAYA</td>";
                echo "<td colspan='2' class='text-right'>";
                echo "<input type='text' id='total_item_biaya' class='form-control text-right' readonly style='font-size:15px; font-weight:bold; color:#1e293b; background:#f8fafc;' value='" . number_format($totalJenisBiaya) . " BIAYA (" . niceDecimal($totalQtyBiaya) . " QTY)' >";
                echo "</td>";
                echo "</tr>";
            }
        ...
        if($settingTermin==1){
            ...
            $ppn_jual_project = $set_harga_jual_project*0.11;
            $nppn_jual_project = $set_harga_jual_project*1.11;
...
//            $total_project = $nilai_total_project*1;
            ...
            if($nppn_jual_project*1 != $total_project_last*1){

//                matiHere("nppn_jual_project: $nppn_jual_project VS total_project_last: $total_project_last");
                echo "<script>";
            ...
            //=============================SETTING HARGA JUAL PROJECT (HARGA INI BERBEDA DARI HARGA PENYUSUN BOM)====================================
            echo "<form id='table_harga_jual_project_form' method='post' enctype='multipart/form-data' name='table_harga_jual_project' target='result' action='$linkAddTerminSetting/harga_jual_project'>";
            echo "<table  id='table_harga_jual_project' width='100%' class='table dataTable compact table-bordered table-hover-color-red table-striped'>";
            echo "<caption class='text-bold'>
                    1# HARGA JUAL PROJECT
                    <div class='meta'> harga jual di-isi sesuai nilai project, bukan berdasarkan RAB.</div>
                </caption>";
            echo "<thead>";
            echo "<tr class='bg-gray'>";
            echo "<th>SETTING HARGA JUAL PROJECT</th>";
            echo "<th>PPN (11%)</th>";
            echo "<th>HARGA JUAL + PPN</th>";
            echo "</tr>";
            echo "</thead>";

            echo "<tbody>";

            if( isset($set_harga_jual_project) && $set_harga_jual_project*1 > 0 ){
                echo "<tr class='text-bold'>";

                    echo "<td>" . number_format($set_harga_jual_project) . "</td>";
                    echo "<td>" . number_format($ppn_jual_project) . "</td>";
                    echo "<td>" . number_format($nppn_jual_project) . "</td>";
                echo "</tr>";
            }
            ...
            echo "<caption class='text-bold'>
                    2# INFORMASI KONTRAK PROJECT PIHAK KE-3
                    <div class='meta'> isikan informasi project yang diperoleh dari pihak ke-3</div>
                </caption>";
            echo "<thead>";
            echo "<tr class='bg-gray'>";
            ...
            if( !empty($ref_kontrak) ){
                ...
                foreach($ref_kontrak as $k => $val){
                    ...
                    $nilai_total_dp = isset($val['total']) ? $val['total']*1 : 0;
                    $nilai_persen_dp = isset($val['nilai']) ? $val['nilai']*1 : 0;
                }
                echo "</tr>";
            }
            else{
            ...
            //=============================SETTING DP====================================
            echo "<form id='table_dp_form' name='table_dp_form' target='result' data-action='$linkAddTerminSetting/dp' action='$linkAddTerminSetting/dp'>";
            echo "<table  id='table_dp' width='100%' class='table dataTable compact table-bordered table-hover-color-red table-striped'>";
            echo "<caption class='text-bold'>
                    3# SETTING DP
                    <div class='meta'> uang muka minimal adalah 10% jika dibawah 10%, perlu otorisasi atasan pada tahap selanjutnya.</div>
                </caption>";
            echo "<thead>";
            echo "<tr class='bg-gray'>";
            ...
            if( !empty($dp) ){
                echo "<tr class='text-bold'>";
                foreach($dp as $k => $val){
                    echo "<td>". number_format($val['persen'],2)."%</td>";
                ...
                $rupiah_DP_project = round($nppn_jual_project*($defaultSetting_persen_DP_Project/100),2);

                echo "<tr>";

                echo "<td>
                        <input total_project='$nppn_jual_project' onclick=\"$(this).select();\" id='input_dp' name='dp' class='form-control' value='$defaultSetting_persen_DP_Project'>
                      </td>";
                echo "<td>
                        <input total_project='$nppn_jual_project' onclick=\"$(this).select();\" id='input_total_dp' name='total_dp' class='form-control text-right' value='$rupiah_DP_project'>
                      </td>";
                echo "<td>
                        <input id='input_keterangan_dp' name='keterangan_dp' class='form-control text-right' type='text'>
                      </td>";
                echo "<td>
                        <div id='confirmDP' class='hidden'>harap centang jika DP dibawah ketentuan.</br><r>(ketentuan saat ini adalah $defaultSetting_persen_DP_Project%)</r><input type='checkbox' name='confirm_dp' id='cekConfirmDP'></div>
                        <span onclick=\"$('#table_dp_form').submit()\" id='btn_table_dp_form' class='btn btn-sm btn-success'>
                ...
                echo "<script>
                        ...
                        toggleFormElements('table_termin_form', false);
                        var enables_form = false;
                        var surl = $('#table_dp_form').data('action'); // URL asli

                        $('#cekConfirmDP').on('change', delay_v2(function(){
                            console.log('check berubah');
                            if( $(this).is(':checked') ){
                                var newUrl = surl.includes('?') ? surl + '&confirm_dp=1' : surl + '?confirm_dp=1';
                                $('#table_dp_form').attr('action', newUrl);
                            }
                            else{
                                $('#table_dp_form').attr('action', surl);
                            }
                        }, 500));
...
//=============================SETTING GARANSI/RETENSI====================================
            ...
            echo "<caption class='text-bold'>
                    4# SETTING GARANSI / RETENSI
                    <div class='meta'> garansi yang disepakati (optional)</div>
                </caption>";
            ...
            if(!empty($garansi)){
                ...
                echo "<tr>";
                echo "<td><input total_project='$nppn_jual_project' onclick=\"$(this).select();\" id='input_garansi' name='garansi' class='form-control' value='0'></td>";
                echo "<td><input total_project='$nppn_jual_project' onclick=\"$(this).select();\" id='input_total_garansi' name='total_garansi' class='form-control text-right' value='0'></td>";
            ...
            echo "<caption class='text-bold'>
                    5# SETTING TERMIN
                    <div class='meta'> isikan termin ber-urutan, misal Termin 1, Termin 2, dst.</div>
                </caption>";
            echo "<thead>";
            echo "<tr class='bg-gray'>";
            echo "<th>No</th>";
                ...
                foreach($termin as $key => $row){
                    ...
                    if(!empty($termin_label)){
                        ...
                        foreach($termin_label as $k => $tLabel){
                            ...
                            $val_termin = in_array($k, $number_key) ? number_format($row[$k],2) : $row[$k] ;
                            echo "<td class='$class $k'>".$val_termin."</td>";
                        }
            ...
            $nilaiB = floor($nppn_jual_project*1);
            ...
            if( $nilaiA < $nilaiB ){
                ...
                if(!empty($termin_label)){
                    ...
                    foreach($termin_label as $k => $tLabel){
                        ...
                        echo "<td><input $attr style='$style' onclick='this.select()' total_project='$nppn_jual_project' id='new_termin_$k' name='$k' placeholder='Termin ke-$last_termin' class='text-center form-control $class' value=''></td>";
                    }
                    echo "<td><input onclick='this.select()' total_project='$nppn_jual_project' id='new_termin_total' name='harga' class='form-control text-right' value=''></td>";
                    echo "<td>
            ...
            $sisa_rupiah = $nppn_jual_project-($nilai_total_garansi+$nilai_total_dp+$total);
            ...
            if($sisa_persen*1 > 0){
                ...
                echo "<tr class='sisa_termin' style='font-size: 1.5em;'>";
                echo "<th colspan='3' class='text-right   bg-green'><span  class='text-bold'>TOTAL TERMIN</span></th>";
                echo "<th class='sisa_persen text-center  bg-green'><span class='text-bold'>".number_format($progress)."%</span></th>";
                echo "<th class='sisa_rupiah text-right   bg-green'><span  class='text-bold'>".number_format($total)."</span></th>";
                echo "<th class='sisa_rupiah text-right   bg-green'>&nbsp;</th>"; //kolom kosong
                echo "</tr>";
            }

            if(!empty($termin)){
                echo "<tr>";
                echo "<th colspan='4' class='text-right'>NILAI TERMIN <r>(<span class='live_persen_total_termin'>".number_format($progress,2)."</span>%)</r></th>";
                echo "<th class='live_termin text-right'>".number_format($total_pengerjaan)."</th>";
                echo "<th>&nbsp;</th>";
                echo "</tr>";

                echo "<tr>";
                echo "<th colspan='4' class='text-right'>NILAI DP <r>(".number_format($nilai_persen_dp,2)."%)</r></th>";
                echo "<th class='text-right'>".number_format($nilai_total_dp)."</th>";
                echo "<th>&nbsp;</th>";
                echo "</tr>";

                echo "<tr>";
                echo "<th colspan='4' class='text-right'>NILAI GARANSI <r>(".number_format($nilai_persen_garansi)."%)</r></th>";
                ...
                echo "<th colspan='4' class='text-right fa-2x'>TOTAL <span class='live_persen_total$persen_color'>(".number_format($progress+$nilai_persen_garansi+$nilai_persen_dp)."%)</span></th>";
        ...
        echo "<script>
            $(document).ready(function () {
                ...
                function bindPair(inputPersenId, inputTotalId) {
                    ...
                    inputPersen.off();
                    inputPersen.on('input', delay_v2(function () {
                        let persen = parseFloat($(this).val());
                        if (isNaN(persen)) persen = 0;
                        if (inputPersenId === 'input_dp' && persen < 10) {
                            swal('warning', 'DP minimal 10%</br>dibawah minimal yang ditetapkan harus melalui persetujuan (spv,mgr,owner)','warning');
                            showError(inputPersen, 'DP tidak boleh kurang dari 10%');
                        } 
                        else {
                            if (inputPersenId === 'input_dp') clearError(inputPersen);
                        }
                        if (persen>100) {
                    ...
                    inputTotal.off();
                    inputTotal.on('input', delay_v2(function () {
                ...
                function jalankanAjax(check) {
                    ...
                    .done(function (response) {
                        ...
                        if (check === 1 && resp.data?.nilai == 0) {
                            swal('<b>Harga Jual Project</b> Belum Ditetapkan, silahkan isikan Harga Project pada form yang telah disediakan.', resp.message, 'warning');
                        } 
                        else {
                            console.warn('untic');
                        }
...
?>
<script>
// Real-time instant calculation untuk Summary (Jumlah Item & Total Qty) tanpa reload shopping cart
function recalcSummaryTotals() {
    var countMat = 0;
    var totalQtyMat = 0;
    var $matInputs = $("input[id^='jml_2_']").length ? $("input[id^='jml_2_']") : (typeof top !== 'undefined' && top.$ ? top.$("input[id^='jml_2_']") : $());
    $matInputs.each(function() {
        countMat++;
        var rawVal = $(this).val();
        var cleanVal = (typeof removeCommas === 'function') ? removeCommas(String(rawVal)) : String(rawVal).replace(/,/g, '');
        totalQtyMat += (parseFloat(cleanVal) || 0);
    });

    var formattedQtyMat = (typeof addCommas === 'function') ? addCommas(totalQtyMat) : totalQtyMat.toLocaleString('en-US');
    var $elItemMat = $("#total_item_material").length ? $("#total_item_material") : (typeof top !== 'undefined' && top.$ ? top.$("#total_item_material") : $());
    var $elQtyMat = $("#total_qty_material").length ? $("#total_qty_material") : (typeof top !== 'undefined' && top.$ ? top.$("#total_qty_material") : $());
    $elItemMat.val(countMat + " ITEM");
    $elQtyMat.val(formattedQtyMat);

    var countBiaya = 0;
    var totalQtyBiaya = 0;
    var $biayaInputs = $("input[id^='jml_biaya_']").length ? $("input[id^='jml_biaya_']") : (typeof top !== 'undefined' && top.$ ? top.$("input[id^='jml_biaya_']") : $());
    if (!$biayaInputs.length) {
        $biayaInputs = $("input[id^='jml_3_']").length ? $("input[id^='jml_3_']") : (typeof top !== 'undefined' && top.$ ? top.$("input[id^='jml_3_']") : $());
    }
    $biayaInputs.each(function() {
        countBiaya++;
        var rawVal = $(this).val();
        var cleanVal = (typeof removeCommas === 'function') ? removeCommas(String(rawVal)) : String(rawVal).replace(/,/g, '');
        totalQtyBiaya += (parseFloat(cleanVal) || 0);
    });

    var $elBiaya = $("#total_item_biaya").length ? $("#total_item_biaya") : (typeof top !== 'undefined' && top.$ ? top.$("#total_item_biaya") : $());
    if ($elBiaya.length) {
        var formattedQtyBiaya = (typeof addCommas === 'function') ? addCommas(totalQtyBiaya) : totalQtyBiaya.toLocaleString('en-US');
        $elBiaya.val(countBiaya + " BIAYA (" + formattedQtyBiaya + " QTY)");
    }
}

$(document).off('input keyup change', "input[id^='jml_2_'], input[id^='jml_biaya_'], input[id^='jml_3_']").on('input keyup change', "input[id^='jml_2_'], input[id^='jml_biaya_'], input[id^='jml_3_']", function() {
    recalcSummaryTotals();
});
if (typeof top !== 'undefined' && top.$) {
    top.$(document).off('input keyup change', "input[id^='jml_2_'], input[id^='jml_biaya_'], input[id^='jml_3_']").on('input keyup change', "input[id^='jml_2_'], input[id^='jml_biaya_'], input[id^='jml_3_']", function() {
        recalcSummaryTotals();
    });
}
</script>
