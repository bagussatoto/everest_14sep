        ...
        if (sizeof($rowValidatorRules) > 0) {
            ...
            foreach ($rowValidatorRules as $field => $label) {
                if (!isset($_SESSION[$cCode]['main'][$field])) {
                    $errMsgs[] = "$label wajib diisi. code: " . __LINE__;
                    $errRows[] = $field;
                }
                elseif($_SESSION[$cCode]['main'][$field] == NULL){
                    $errMsgs[] = "$label wajib diisi. code: " . __LINE__ . " || " . $_SESSION[$cCode]['main'][$field];
                    $errRows[] = $field;
                }
            }
...
//cekHere($tmp1[0]->cabang_id);
//cekHere($tmp1[0]->reference_jenis);
        ...
        $reference_nomer = $tmp1[0]->reference_nomer;
        $reference_jenis = $tmp1[0]->reference_jenis;
        $reference_jenis_label = isset(arrCodeAliasing($tmp1[0]->cabang_id)[$reference_jenis]) ? arrCodeAliasing($tmp1[0]->cabang_id)[$reference_jenis] : arrCodeAliasing(CB_ID_PUSAT)[$reference_jenis];
        ...
        if (sizeof($signArray) > 0) {
            ...
            //---
            $signStr .= "<td class='text-muted'>reference transaction</td>";
            $signStr .= "<td class='text-muted'>reference number</td>";
            //---
            ...
            foreach ($signArray as $k => $sSpec) {
                ...
                //---
                $signStr .= "<td>";
                $signStr .= $reference_jenis_label;
                $signStr .= "</td>";
                $signStr .= "<td>";
                $signStr .= (formatField_he_format("nomer_nolink", $reference_nomer));
                $signStr .= "</td>";
                //---
...
// matiHEre("hoop".__LINE__);
    ...
    public function index()
    {
        ...
        // region redirect ke create transaksi bila hanya ada 1 hak akses create

        $link_redirect = customAccessJenis_he_access_right($this->session->login['id'], $jenisTr, $this->configUiJenis);
        if ($link_redirect != NULL) {
//            topRedirect($link_redirect);
        }
...
//arrPrintPink($this->session->login['membership']);
        if (placeCanMakeTrans_he_menu($this->session->login['membership'], $this->session->login['cabang_id'], $this->session->login['gudang_id'], $this->jenisTr, $this->configUiJenis)) {
            //        if (in_array($this->configUi[$jenisTr]["steps"][1]['userGroup'], $this->session->login['membership'])) {
            $createIndexes = (null != $this->config->item("transaksi_createIndex")) ? $this->config->item("transaksi_createIndex") : array();
            $isDisableMakeTrans = isset($this->configUi[$this->jenisTr]['isDisableMakeTrans']) ? $this->configUi[$this->jenisTr]['isDisableMakeTrans'] : false;
