        ...
        if ($shoppingCartAdvanceItems == true) {
            ...
            foreach ($items as $iSpec) {
                if ($showItems) {
                    ...
                    $iID = $iSpec['id'];
                    $sub_key = $iSpec['pph'];
                    $itemLabels = isset($shoppingCartAdvanceItemsLabel[$iID]) ? $shoppingCartAdvanceItemsLabel[$iID] : array();
...
//arrPrintPink($itemLabels);
                    ...
                    //-- header
                    if (isset($itemLabels)) {
                        if (sizeof($itemLabels) && (is_array($itemLabels))) {
                    ...
                    if (sizeof($subItemLabels) > 0) {
                        if (isset($subItemLabels)) {
                            if (sizeof($subItemLabels) && (is_array($subItemLabels))) {
                                ...
                                echo "<td>";
//                                echo "<a class='text-green btn' title='add sub item' data-toggle='tooltip'
//                                        onclick=\"document.getElementById('result').src='$linkSubAdd';\"
//                                        >";
...
//                                echo "</a>";
                                ...
                                foreach ($subAdvanceItems2 as $subNo => $iiSpec) {
                                    ...
                                    foreach ($subItemLabels as $key => $label) {
                                        ...
                                        echo "<td style='width:1%;white-space: nowrap;' class='text-muted text-center'>";
...
//                echo "<script> setTimeout( function(){ top.$('.select2').selectpicker(); console.log('dari shopingcart') }, 500 ); </script>";
            ...
            //region items3
            if (sizeof($itemLabels3) && (is_array($itemLabels3))) {
...
//                arrPrint($addRowHiddens[$k]);
        ...
        if ($showLastStock == 1) {
            $str_stok = "<div class='table-responsive no-padding no-border' style='margin-top:20px;'>";
            $str_stok .= "<span style='font-weight:bold;font-size:13px;text-align:left;'>Daftar Stok:  </span>";
            $str_stok .= "<span style='font-size:13px;text-align:left;'><button class='btn bbtn-danger btn-sm' style='background-color:red;'></button> Stok kurang</span>,  ";
            $str_stok .= "<span style='font-size:13px;text-align:left;'><button class='btn bbtn-warning btn-sm' style='background-color:yellow;'></button> Stok sama dengan pembatalan</span>,  ";
            $str_stok .= "<span style='font-size:13px;text-align:left;'><button class='btn bbtn-success btn-sm' style='background-color:limegreen;'></button> Stok lebih dari pembatalan</span>";
            $str_stok .= "<table class='table table-condensed no-padding table-bordered no-margin'>";
            if (isset($showLastStockFields)) {
                if (sizeof($showLastStockFields) && (is_array($showLastStockFields))) {
                    $str_stok .= "<tr class='bg-grey-2 text-uppercase'>";
                    $str_stok .= "<th style='width:1%;' class='text-muted text-center'>";
                    $str_stok .= "NO";
                    $str_stok .= "</th>";
                    foreach ($showLastStockFields as $key => $label) {
                        $str_stok .= "<th style='width:1%;white-space: nowrap;' class='text-muted text-center'>";
                        $str_stok .= $label;
                        $str_stok .= "</th>";
                    }
                    $str_stok .= "</tr>";
            ...
            foreach ($items as $iSpec) {
                $i_jml = $iSpec["jml"];
                $i_stock = $iSpec["last_stock_avail"];
                if ($i_jml > $i_stock) {
                    $bgColor = "background-color:red;";
                    $stokKurang[$iSpec["id"]] = $iSpec["id"];
                }
                elseif ($i_jml == $i_stock) {
                    $bgColor = "background-color:yellow;";
                }
                else {
                    $bgColor = "background-color:limegreen;";
                }

                $no++;
                $str_stok .= "<tr style='$bgColor'>";
                $str_stok .= "<td style='vertical-align:middle; width:1%' class='text-center'>";
                $str_stok .= $no;
                $str_stok .= "</td>";
                foreach ($showLastStockFields as $key => $label) {
                    $str_stok .= "<td>";
                    $str_stok .= formatField_he_format($key, $iSpec[$key]);
                    $str_stok .= "</td>";
                }
                $str_stok .= "</tr bgcolor=$bgColor>";
            }
        ...
        if ($allowed_koreksi_serial == true) {
//                cekHitam("serial bisa pilih....");
//            arrPrintCyan($sourceAllDetailDataProduk);
            if (isset($arrDaftarSerialKoreksi) && (sizeof($arrDaftarSerialKoreksi) > 0)) {
                echo "<div class='alert aalert-info  text-left' style='margin-top: 10px;font-size: 15px;'>";
                echo "<span class='text-uppercase text-bold'>$sourceAllDetailDataProdukLabel</span><br>";
                $str = "";
                foreach ($arrDaftarSerialKoreksi as $pid => $pSpec) {
                    $pNama = $itemsProduk[$pid]["nama"];
                    $pKode = $itemsProduk[$pid]["produk_kode"];
                    $itemsProdukFlip = array_flip($itemsProduk[$pid]);
                    $str .= "<div cclass='direct-chat-msg'>";
                    $str .= "<div cclass='direct-chat-info clearfix'>";
                    $str .= "<label class='text-uppercase' style='vertical-align:middle;font-size:15px;'> ($pid) ($pKode)  $pNama</label>";
                    $str .= "</div class='direct-chat-info clearfix'>";

                    $str .= "<div class='ddirect-chat-text ooverflow-h text-uppercase table table-responsive nno-padding nno-border'>";
                    $str .= "<table rrules='all' style='bborder:1px solid black;width:100%;' class='table table-responsive nno-padding nno-border'>";
                    $str .= "<tr>";
                    foreach ($arrDaftarSerialKoreksi[$pid] as $pSku => $sSpec) {
                        $pSkuNama = isset($itemsProdukFlip[$pSku]) ? $itemsProdukFlip[$pSku] : "";
                        $str .= "<th>$pSkuNama $pSku</th>";
                    }
                    $str .= "</tr>";
                    $str .= "<tr>";
                    foreach ($arrDaftarSerialKoreksi[$pid] as $pSku => $sSpec) {
                        $str .= "<td style='vertical-align:top;padding:0 20px 0 0;'>";
                        if (sizeof($sSpec) > 0) {
                            $num = 0;
                            foreach ($sSpec as $subSpec) {
                                $num++;
                                $serial = $subSpec["serial"];
                                $bgcolor = "";
                                if (in_array($serial, $diffSerialDataProduk[$pid][$pSku])) {
                                    $bgcolor = "background-color:red;color:#000000;";
                                }
                                $link = MODUL_PATH . "$koreksi_link?pid=$pid&sku=$pSku&old=$serial&new=";
                                $str .= "<div>";
                                $str .= "<input type='text' name='serial' value='$serial' class='form-control text-left col-md-3'
                                                style='$bgcolor'
                                                onclick='this.select()'
                                                onblur=\"$('#result').load('$link'+this.value);\"
                                            >";
                                $str .= "</div>";
                            }
                        }
                    ...
                    $str .= "</tr>";
                    $str .= "</table>";
                    $str .= "</div class='direct-chat-text overflow-h text-uppercase'>";
            ...
            if (isset($diffSerialData) && (sizeof($diffSerialData) > 0)) {
                echo "<div class='alert alert-danger text-left' style='margin-top: 10px;font-size: 15px;'>";
                echo "<span class='text-uppercase text-bold'>$diffSerialLabel</span><br>";
                $no = 0;
                $pakai_ini = 0;
                if ($pakai_ini == 1) {
                    foreach ($diffSerialData as $serial) {
                        $no++;
                        echo "<span class='text-uppercase'>$no. </span>";
                        echo "<span class=''> $serial</span><br>";
                    }
                }
                else {
                    if (isset($diffSerialDataProduk) && (sizeof($diffSerialDataProduk) > 0)) {
                        $str = "";
                        foreach ($diffSerialDataProduk as $pid => $pSpec) {
                            $pNama = $itemsProduk[$pid]["nama"];
                            $pKode = $itemsProduk[$pid]["produk_kode"];
                            $itemsProdukFlip = array_flip($itemsProduk[$pid]);
                            $str .= "<div class='direct-chat-msg'>";
                            ...
                            $str .= "<div class='ddirect-chat-text overflow-h text-uppercase'>";
                            $str .= "<table rrules='all' style='bborder:1px solid black;width:100%;'>";
                            $str .= "<tr>";
                            foreach ($diffSerialDataProduk[$pid] as $pSku => $sSpec) {
                                $pSkuNama = isset($itemsProdukFlip[$pSku]) ? $itemsProdukFlip[$pSku] : "";
                                $str .= "<th>$pSkuNama $pSku</th>";
                            }
                            $str .= "</tr>";
                            ...
                            foreach ($diffSerialDataProduk[$pid] as $pSku => $sSpec) {
                                ...
                                if (sizeof($sSpec) > 0) {
                                    ...
                                    foreach ($sSpec as $subSpec) {
                                        $num++;
                                        $serial = $subSpec;
                                        $link_move_serial = $diffSerialLink . "?pid=$pid&serial=$serial";
                                        $str .= "<a href='javascript:void(0)' title='klik mutasi terakhir' data-toggle='tooltip' data-placement='right' 
                                onclick=\"top.BootstrapDialog.show(
                                   {
                                       title:'mutasi terakhir serial " . $serial . "',
                                       message: " . '$' . "('<div></div>').load('" . $link_move_serial . "'),
                                        ...
                                        \" >";
                                        $str .= "<div>$num.<span class='ttext-bold'>$serial </span><span class='fa fa-qrcode'></a></span> </div>";
                                    }
                                }
                                $str .= "</td>";
                            }
                ...
                echo "</div>";
                echo "<script>
                document.getElementById('btnProcess').setAttribute('disabled', true);
                </script>";
            }
        }
...
//            echo "<div class='panel panel-info col-md-12'>";
            foreach ($previewJurnal['jurnal'] as $cabangID => $subItems) {
                if (sizeof($subItems) > 0) {
                    ...
                    echo "<tr class='footerValidate' style='font-size: 15px;font-weight: bold;'>";
                    $totNum = 0;
                    foreach ($headersJurnal as $key => $label) {
                        $totNum++;
                        echo "<td class='total_$totNum'>";
                        if (isset($total[$cabangID][$key])) {
//                            if($totNum==2){
//                                echo formatField($key, $total[$cabangID][$key]+2 );
//                            }
//                            else{
...
//            echo "</div>";
            ...
            echo "
            <script>
                if( top.$('#tic_lanjut').is(':checked') ){
                    if( top.validateTotals() ){
                        console.log('balance langsung enable button :: from shoppingcart');
                        $('#btnProcess').removeAttr('disabled');
                    }
                    else{
                        top.swal({
                            title: 'PERHATIAN<br><r>TRANSAKSI GAGAL</r>', 
                            html: 'mohon ulangi transaksi Anda, karena ada gangguan komunikasi dengan server, transaksi ini kami bantu hapus.', 
                            type: 'warning',
                            confirmButtonText: 'Ulangi Transaksi!',
                            allowOutsideClick: false
                        })
                        .then((confirm) => {
                              if (confirm) {
                                    top.getData('" . base_url() . "pembatalan/_shoppingCart/reset/9912', 'result', 1);
                              }
                        });
                        
                        top.$('#tic_lanjut').prop('checked', false);
                    }
                }
            </script>
            ";
        ...
        if (isset($stokKurang) && (sizeof($stokKurang) > 0)) {
            echo "<script>
                if(top.document.getElementById('btnProcess')){
                    top.$('#btnProcess').prop('disabled', true);
                }
