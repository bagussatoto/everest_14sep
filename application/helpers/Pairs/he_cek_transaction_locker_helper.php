...
<!--
    <div style='display: flex; flex-direction: column; gap: 8px;'>
        <button type='button' onclick=\"" . htmlspecialchars($batalOnclick, ENT_QUOTES, 'UTF-8') . "\" style='display: block; width: 100%; padding: 12px 16px; background-color: #374151; color: #ffffff; border: none; border-radius: 8px; font-weight: 600; font-size: 14px; text-align: center; box-sizing: border-box; cursor: pointer;'>
            ↺ Kembali
        </button>
    </div>
    -->
...
<script>
    function closeLockedModalCascade() {
        ...
        if (typeof BootstrapDialog !== 'undefined' && typeof BootstrapDialog.closeAll === 'function') {
            try { BootstrapDialog.closeAll(); } catch(e){}
        }
        var targets = [window.top, window.parent, window];
        for (var i = 0; i < targets.length; i++) {
            if (targets[i] && targets[i].$) {
                targets[i].$('.sweet-alert, .sweet-overlay, .swal2-container, .modal-backdrop, .holdon-overlay, .holdon, #holdon-overlay, .loading-overlay').hide().remove();
                targets[i].$('body').removeClass('stop-scrolling modal-open swal2-shown holdon-loading');
            }
        }
    }
    function closeLockedModalNow() {
        closeLockedModalCascade();
    }
    if (typeof top !== 'undefined') { top.closeLockedModalCascade = closeLockedModalCascade; }
    window.closeLockedModalCascade = closeLockedModalCascade;
