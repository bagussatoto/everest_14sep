...
class MdlProduk extends MdlMother
{
    ...
    protected $fields = array(
        ...
        "allow_autopurchase" => array(
            "label" => "ijinkan autopurchase",
            "type" => "int", "length" => "24", "kolom" => "allow_autopurchase",
            "inputType" => "combo",
            "checbox" => true,
            "checbox_fungsi" => "checkboxUpdAutopurchase",
            "dataSource" => array(0 => "no", 1 => "yes"),
            "defaultValue" => 0,
        ),
        "kapasitas" => array(
    ...
    protected $listedFields = array(
        ...
        "allow_autopurchase" => "ijinkan autopurchase",
...
//                "id" => "indoor_id_1",
    ...
    /**
     * Kategori alasan penonaktifan khusus produk/persediaan
     *
     * @return array
     */
    ...
    {
        return array(
            "Discontinue Pabrik / End of Life (EOL)",
            "Ganti Kemasan / Ganti Seri Baru",
            "Barang Rusak Permanen / Afkir",
            "Produk Lambat Laku / Dead Stock",
            "Larangan Edar / Kebijakan Regulasi",
    ...
    /**
     * Pemeriksaan dependensi bisnis produk (sisa stok fisik, locker hold, & SO aktif) sebelum dinonaktifkan
     *
     * @param int|string $produkId
     * @return array
     */
    public function checkDeactivationWarning($produkId)
    {
        $warnings = array();
        $hasWarning = false;

        // 1. Cek Sisa Saldo Stok Fisik
        $stokFisik = 0;
        if ($this->db->table_exists('_rek_pembantu_produk_cache')) {
            $this->db->select('SUM(debet_akhir) - SUM(kredit_akhir) as total_stok');
            $this->db->from('_rek_pembantu_produk_cache');
            $this->db->where('extern_id', $produkId);
            $this->db->where('periode', 'forever');
            $qStok = $this->db->get();
            if ($qStok && $qStok->num_rows() > 0) {
                $rowStok = $qStok->row();
                $stokFisik = isset($rowStok->total_stok) ? floatval($rowStok->total_stok) : 0;
            }
        } elseif ($this->db->table_exists('stok')) {
        ...
        if ($stokFisik > 0) {
            $hasWarning = true;
            $strStok = number_format($stokFisik, 0, ',', '.');
            $warnings[] = "Produk masih memiliki saldo stok fisik sebanyak " . $strStok . " unit di gudang. Penonaktifan memblokir transaksi penjualan/pembelian baru, namun nilai persediaan tetap tercatat dalam buku besar.";
        }

        // 2. Cek Reservasi / Lock Stok Aktif
        if ($this->db->table_exists('locker_stocks')) {
            $this->db->select('SUM(jumlah) as total_hold');
            $this->db->from('locker_stocks');
            $this->db->where('state', 'hold');
            if ($this->db->field_exists('produk_id', 'locker_stocks')) {
                $this->db->where('produk_id', $produkId);
            } elseif ($this->db->field_exists('item_id', 'locker_stocks')) {
                $this->db->where('item_id', $produkId);
            }
            $qHold = $this->db->get();
            if ($qHold && $qHold->num_rows() > 0) {
                $rowHold = $qHold->row();
                $holdQty = isset($rowHold->total_hold) ? floatval($rowHold->total_hold) : 0;
                if ($holdQty > 0) {
                    $hasWarning = true;
                    $warnings[] = "Terdapat " . number_format($holdQty, 0, ',', '.') . " unit stok yang sedang terkunci (hold) dalam antrean checkout/transaksi kasir.";
                }
            }
        }

        // 3. Cek Pesanan Penjualan (SO) Aktif yang Memuat Produk Ini
        if ($this->db->table_exists('transaksi_details') && $this->db->table_exists('transaksi')) {
            $this->db->select('COUNT(DISTINCT transaksi_details.transaksi_id) as total_so');
            $this->db->from('transaksi_details');
            $this->db->join('transaksi', 'transaksi.id = transaksi_details.transaksi_id');
            $this->db->where('transaksi_details.produk_id', $produkId);
            $this->db->where('transaksi.jenis', 'so');
            ...
            if ($qSo && $qSo->num_rows() > 0) {
                ...
                if (isset($rowSo->total_so) && intval($rowSo->total_so) > 0) {
                    $hasWarning = true;
                    $warnings[] = "Terdapat " . intval($rowSo->total_so) . " Pesanan Penjualan (SO) aktif yang memuat produk ini.";
                }
            }
        }
