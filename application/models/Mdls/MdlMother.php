...
//         cekBiru($criteria);
    ...
    public function detectPrimarySearchColumn()
    {
        $candidateCols = array("nama", "name", "nomer", "nomor", "kode");
        if (isset($this->listedFieldsSelectItem) && is_array($this->listedFieldsSelectItem) && sizeof($this->listedFieldsSelectItem) > 0) {
            foreach ($candidateCols as $cCol) {
                foreach ($this->listedFieldsSelectItem as $kCol => $vCol) {
                    $val = is_string($vCol) ? $vCol : "";
                    $key = is_string($kCol) ? $kCol : "";
                    if ($val === $cCol || $key === $cCol) {
                        return $val !== "" ? $val : $key;
                    }
                    if ($val !== "" && substr($val, -(strlen($cCol) + 1)) === "." . $cCol) {
                        return $val;
                    }
                }
            }
            ...
            foreach ($this->listedFieldsSelectItem as $kCol => $vCol) {
                $colName = is_numeric($kCol) ? $vCol : $kCol;
                if ($colName != "id" && substr($colName, -3) != ".id") {
                    return $colName;
                }
            }
        }

        return isset($this->sortBy['kolom']) ? $this->sortBy['kolom'] : "id";
    }
    ...
    public function lookupByKeyword($key)
    {
        ...
        if (strlen($cleanKey) >= 3) {
            $this->createSmartSearch($cleanKey, $this->listedFieldsSelectItem);

            $primaryCol = $this->detectPrimarySearchColumn();
            if ($primaryCol != "" && $primaryCol != "id" && substr($primaryCol, -3) != ".id") {
                $escapedLike = $this->db->escape_like_str($cleanKey);
                $escapedExact = $this->db->escape($cleanKey);

                $caseOrder = "CASE "
                    . "WHEN " . $primaryCol . " = " . $escapedExact . " THEN 1 "
                    . "WHEN " . $primaryCol . " LIKE '" . $escapedLike . " %' THEN 2 "
                    . "WHEN " . $primaryCol . " LIKE '" . $escapedLike . "%' THEN 3 "
                    . "ELSE 4 END ASC, "
                    . "CHAR_LENGTH(" . $primaryCol . ") ASC";

                $this->db->order_by($caseOrder, '', FALSE);
            }
        }
    ...
    function make_query()
    {
        ...
        // Pastikan kolom status dan trash selalu terpilih jika ada pada tabel database master
        if (!empty($this->tableName) && $this->db->table_exists($this->tableName)) {
            if ($this->db->field_exists('trash', $this->tableName) && !in_array('trash', $koloms)) {
                $koloms[] = 'trash';
            }
            if ($this->db->field_exists('status', $this->tableName) && !in_array('status', $koloms)) {
                $koloms[] = 'status';
            }
            $koloms = array_filter(array_unique($koloms));
        }
...
//            $this->db->trans_complete();//ini tidak boleh di mother membuat commit bocor**
    ...
    /**
     * Menghitung rekapitulasi jumlah data berdasarkan status secara generik
     * Mendukung tabel dengan kolom status & trash, maupun tabel dengan kolom status saja.
     *
     * @return array
     */
    ...
    {
        $tbl = $this->tableName;
        if (empty($tbl) || !$this->db->table_exists($tbl)) {
            return array();
        }

        $hasStatus = $this->db->field_exists('status', $tbl);
        if (!$hasStatus) {
            return array();
        }

        $hasTrash = $this->db->field_exists('trash', $tbl);

        if ($hasTrash) {
            $sql = "SELECT 
                COUNT(*) as count_semua,
                SUM(CASE WHEN status = '1' AND trash = '0' THEN 1 ELSE 0 END) as count_aktif,
                SUM(CASE WHEN status = '1' AND trash = '1' THEN 1 ELSE 0 END) as count_nonaktif,
                SUM(CASE WHEN status = '0' AND trash = '1' THEN 1 ELSE 0 END) as count_trash
            FROM " . $this->db->escape_identifiers($tbl);
        } else {
            $sql = "SELECT 
                COUNT(*) as count_semua,
                SUM(CASE WHEN status = '1' THEN 1 ELSE 0 END) as count_aktif,
                SUM(CASE WHEN status = '0' THEN 1 ELSE 0 END) as count_nonaktif,
                0 as count_trash
            FROM " . $this->db->escape_identifiers($tbl);
        }
    ...
    /**
     * Kategori alasan penonaktifan default (tata kelola data umum)
     * Dapat di-override oleh model spesifik.
     *
     * @return array
     */
    ...
    /**
     * Hook validasi dependensi bisnis sebelum penonaktifan data
     * Dapat di-override oleh model spesifik (Customer, Supplier, Produk, dll).
     *
     * @param int|string $id
     * @return array
     */
    public function checkDeactivationWarning($id)
    {
        return array(
            "safe" => true,
            "has_warning" => false,
