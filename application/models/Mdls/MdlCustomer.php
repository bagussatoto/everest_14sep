...
class MdlCustomer extends MdlMother
{
    ...
    protected $fields = array(
        ...
        "trash" => array(
            "label" => "trash",
            "type" => "int", "length" => "11", "kolom" => "trash",
            "inputType" => "hidden",
            "defaultValue" => 0,
        ),
        //        "ppn" => array(
 ...
 * ----------------------------------------------------------*/
    ...
    /**
     * Menghitung total saldo piutang berjalan per customer
     *
     * @param array|int $customerIds
     * @return array
     */
    public function getCustomerOutstandingAr($customerIds = array())
    {
        ...
        // 1. Cek dari tabel cache rekening pembantu customer jika ada
        if ($this->db->table_exists('_rek_pembantu_customer_cache')) {
            $this->db->select('extern_id, debet_akhir, kredit_akhir');
            $this->db->from('_rek_pembantu_customer_cache');
            $this->db->where_in('extern_id', $customerIds);
            $this->db->where('periode', 'forever');
            $queryCache = $this->db->get();
            if ($queryCache && $queryCache->num_rows() > 0) {
                foreach ($queryCache->result() as $row) {
                    $saldo = ($row->debet_akhir * 1) - ($row->kredit_akhir * 1);
                    if ($saldo > 0 && isset($result[$row->extern_id])) {
                        $result[$row->extern_id]['saldo_piutang'] += $saldo;
                    }
                }
            }
        }

        // 2. Cek dari transaksi_due_date untuk faktur jatuh tempo & nilai transaksi
        if ($this->db->table_exists('transaksi_due_date')) {
            $this->db->select('customers_id, transaksi_nilai, due_date, status');
            $this->db->from('transaksi_due_date');
            $this->db->where_in('customers_id', $customerIds);
            $this->db->where('status', '1');
            $queryDue = $this->db->get();
            if ($queryDue && $queryDue->num_rows() > 0) {
                $today = date('Y-m-d');
                foreach ($queryDue->result() as $row) {
                    if (isset($result[$row->customers_id])) {
                        $result[$row->customers_id]['faktur_count']++;
                        if ($row->due_date < $today) {
                            $result[$row->customers_id]['overdue_count']++;
                        }
                        if ($result[$row->customers_id]['saldo_piutang'] == 0 && ($row->transaksi_nilai * 1) > 0) {
                            $result[$row->customers_id]['saldo_piutang'] += ($row->transaksi_nilai * 1);
                        }
                    }
                }
            }
    ...
    /**
     * Memeriksa apakah data customer aman untuk dihapus ke trash
     *
     * @param int $customerId
     * @return bool
     */
    public function isCustomerDeletable($customerId)
    {
        $arInfo = $this->getCustomerOutstandingAr($customerId);
        if (isset($arInfo[$customerId]) && $arInfo[$customerId]['saldo_piutang'] > 0) {
            return false;
        }
    ...
    /**
     * Menghitung rekapitulasi jumlah data konsumen berdasarkan status
     * - Semua Data: status = '1' (aktif + nonaktif)
     * - Aktif: status = '1' AND trash = '0'
     * - Non-Aktif: status = '1' AND trash = '1'
     * - Trash: status = '0' AND trash = '1'
     *
     * @return array
     */
    public function getCustomerStatusCounts()
    {
        $sql = "SELECT 
            SUM(CASE WHEN status = ? THEN 1 ELSE 0 END) AS count_semua,
            SUM(CASE WHEN status = ? AND trash = ? THEN 1 ELSE 0 END) AS count_aktif,
            SUM(CASE WHEN status = ? AND trash = ? THEN 1 ELSE 0 END) AS count_nonaktif,
            SUM(CASE WHEN status = ? AND trash = ? THEN 1 ELSE 0 END) AS count_trash
            FROM " . $this->tableName;
        $binds = array('1', '1', '0', '1', '1', '0', '1');
        $query = $this->db->query($sql, $binds);
        $row = $query->row();
        return array(
            'semua' => isset($row->count_semua) ? intval($row->count_semua) : 0,
            'aktif' => isset($row->count_aktif) ? intval($row->count_aktif) : 0,
            'nonaktif' => isset($row->count_nonaktif) ? intval($row->count_nonaktif) : 0,
            'trash' => isset($row->count_trash) ? intval($row->count_trash) : 0,
        );
    }

    /**
     * Kategori alasan penonaktifan khusus konsumen
     *
     * @return array
     */
    ...
    {
        return array(
            "Kredit Macet / Overdue Jatuh Tempo",
            "Tutup Usaha / Pailit",
            "Pelanggaran Kontrak / Kebijakan Perusahaan",
            "Konsumen Tidak Aktif (> 12 Bulan)",
    ...
    /**
     * Pemeriksaan dependensi bisnis konsumen (piutang & SO aktif) sebelum dinonaktifkan
     *
     * @param int|string $customerId
     * @return array
     */
    public function checkDeactivationWarning($customerId)
    {
        $warnings = array();
        $hasWarning = false;

        // 1. Cek Saldo Piutang Berjalan
        $arInfo = $this->getCustomerOutstandingAr($customerId);
        if (isset($arInfo[$customerId]) && $arInfo[$customerId]['saldo_piutang'] > 0) {
            $hasWarning = true;
            $strAr = number_format($arInfo[$customerId]['saldo_piutang'], 0, ',', '.');
            $fakturText = $arInfo[$customerId]['faktur_count'] . " faktur";
            if ($arInfo[$customerId]['overdue_count'] > 0) {
                $fakturText .= ", " . $arInfo[$customerId]['overdue_count'] . " overdue";
            }
            $warnings[] = "Konsumen memiliki saldo piutang berjalan sebesar Rp " . $strAr . " (" . $fakturText . "). Penonaktifan memblokir pesanan baru, namun pelunasan piutang tetap dapat diproses.";
        }

        // 2. Cek Transaksi Sales Order (SO) Aktif
        if ($this->db->table_exists('transaksi')) {
            $this->db->select('COUNT(*) as total_so');
            $this->db->from('transaksi');
            $this->db->where('customers_id', $customerId);
            $this->db->where('jenis', 'so');
            $this->db->where('status', '1');
            $this->db->where('trash', '0');
            $qSo = $this->db->get();
            if ($qSo && $qSo->num_rows() > 0) {
                $rowSo = $qSo->row();
                if (isset($rowSo->total_so) && intval($rowSo->total_so) > 0) {
                    $hasWarning = true;
                    $warnings[] = "Terdapat " . intval($rowSo->total_so) . " Pesanan Penjualan (SO) aktif yang belum selesai diproses.";
                }
            }
        }
