...
//            ),
    ...
    protected function applyFetchDataProjectQuery()
    {
        $criteria = array();
        $criteria2 = "";
    ...
    {
        $this->applyFetchDataProjectQuery();
    ...
    public function fectDataProjectByReferenceNumbers($numbers)
    {
        $cleanNumbers = array();
        if (sizeof($numbers) > 0) {
        ...
        if (sizeof($cleanNumbers) == 0) {
            $this->db->where("id", "-99999");
            return $this->db->get($this->tableName);
        }

        $refNumbers = array_values($cleanNumbers);
        $this->db->group_start();
        $this->db->where_in("project_start_nomer", $refNumbers);
        $this->db->or_where_in("quot_nomer", $refNumbers);
        $this->db->or_where_in("t1_closing_transaksi_nomer", $refNumbers);
        $this->db->or_where_in("transaksi_no", $refNumbers);
        $this->db->group_end();
    ...
    public function getDataProject($projectid){
    ...
    public function checkDuplicateProject($cabangId, $customerId, $spek, $excludeId = 0)
    {
        ...
        $results = $this->db->query($sql, array($cabangId, $customerId, $spek))->result();
    ...
    {
        // Enforce branch isolation at the model level before inserting to DB
        $cabangId = isset($data['cabang_id']) ? $data['cabang_id'] : (isset($_SESSION['login']['cabang_id']) ? $_SESSION['login']['cabang_id'] : 0);
        $customerId = isset($data['customer_id']) ? $data['customer_id'] : 0;
        $spek = isset($data['spek']) ? $data['spek'] : '';

        if ($cabangId > 0 && $customerId > 0) {
            $duplicate = $this->checkDuplicateProject($cabangId, $customerId, $spek, 0);
            if ($duplicate) {
                die("Akses Ditolak: Anda mencoba menduplikasi Master Project yang sama pada Cabang ini. (Security Model Validation)");
            }
        }
