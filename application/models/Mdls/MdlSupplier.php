...
* ----------------------------------------------------------*/
    ...
    /**
     * Kategori alasan penonaktifan khusus supplier/pemasok
     *
     * @return array
     */
    ...
    {
        return array(
            "Sengketa Tagihan / Overdue AP",
            "Kualitas Pasokan Tidak Sesuai Standar (Disqualified)",
            "Pemasok Tutup Usaha / Pailit",
            "Pemasok Tidak Aktif (> 12 Bulan)",
            "Ganti Rekanan / Kontrak Berakhir",
    ...
    /**
     * Pemeriksaan dependensi bisnis supplier (hutang berjalan & PO aktif) sebelum dinonaktifkan
     *
     * @param int|string $supplierId
     * @return array
     */
    public function checkDeactivationWarning($supplierId)
    {
        $warnings = array();
        $hasWarning = false;

        // 1. Cek Hutang Dagang Berjalan (AP)
        if ($this->db->table_exists('transaksi')) {
            $this->db->select('COUNT(*) as total_faktur, SUM(transaksi_nilai) as total_hutang');
            $this->db->from('transaksi');
            $this->db->where('suppliers_id', $supplierId);
            $this->db->where('status', '1');
            $this->db->where('trash', '0');
            $this->db->where_in('jenis', array('pembelian', 'hutang', 'ap', 'tagihan'));
            $qAp = $this->db->get();
            if ($qAp && $qAp->num_rows() > 0) {
                $rowAp = $qAp->row();
                $totHutang = isset($rowAp->total_hutang) ? floatval($rowAp->total_hutang) : 0;
                $totFaktur = isset($rowAp->total_faktur) ? intval($rowAp->total_faktur) : 0;
                if ($totHutang > 0 || $totFaktur > 0) {
                    $hasWarning = true;
                    $strHutang = number_format($totHutang, 0, ',', '.');
                    $warnings[] = "Pemasok memiliki saldo tagihan/hutang berjalan sebesar Rp " . $strHutang . " (" . $totFaktur . " transaksi). Penonaktifan memblokir PO baru, namun pelunasan pembayaran tetap dapat diproses.";
                }
            }
        }

        // 2. Cek Purchase Order (PO) Aktif
        if ($this->db->table_exists('transaksi')) {
            $this->db->select('COUNT(*) as total_po');
            $this->db->from('transaksi');
            $this->db->where('suppliers_id', $supplierId);
            $this->db->where('jenis', 'po');
            $this->db->where('status', '1');
            $this->db->where('trash', '0');
            $qPo = $this->db->get();
            if ($qPo && $qPo->num_rows() > 0) {
                $rowPo = $qPo->row();
                if (isset($rowPo->total_po) && intval($rowPo->total_po) > 0) {
                    $hasWarning = true;
                    $warnings[] = "Terdapat " . intval($rowPo->total_po) . " Pesanan Pembelian (PO) aktif yang masih menunggu penyelesaian/penerimaan.";
                }
            }
        }
