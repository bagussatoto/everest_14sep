            ...
            foreach ($this->inParams as $cnt => $inSpec) {
                if (isset($inSpec['static']) && sizeof($inSpec['static']) > 0) {
                    if(isset($inSpec['static']['rejection']) && ($inSpec['static']['rejection'] == 1)){
                        ...
                        $cabangId = isset($dataBaru['cabang_id']) ? $dataBaru['cabang_id'] : 0;
                        $customerId = isset($dataBaru['customer_id']) ? $dataBaru['customer_id'] : 0;
                        $spek = isset($dataBaru['spek']) ? $dataBaru['spek'] : "";
                        $projId = isset($dataBaru['id']) ? $dataBaru['id'] : 0;

                        if ($projId > 0) {
                            $existingProject = $p->lookupById($projId)->row();
                        } elseif ($cabangId > 0 && $customerId > 0 && $spek != "") {
                            $existingProject = $p->checkDuplicateProject($cabangId, $customerId, $spek);
                        }

                        if (!empty($existingProject)) {
                            $p->updateData(array("id" => $existingProject->id), $dataBaru) or mati_disini("gagal memperbaharui data project. Segera hubungi admin.");
                        } else {
