 ...
 * Time: 11:09 AM
 */
class ComProjectSales extends MdlMother
{
        ...
        if (sizeof($this->inParams) > 0) {
            ...
            foreach ($this->inParams as $cnt => $inSpec) {
                if (isset($inSpec['static']) && sizeof($inSpec['static']) > 0) {
                    ...
                    $prev = $this->cekPreValue($inSpec['static']);
...
// arrPrint($array);
        ...
        switch ($array['methode']) {
            case "open":
                if (isset($array['transaksi_id']) && $array['transaksi_id'] > 0) {
                    $tr->addFilter("(transaksi_id='0' OR transaksi_id='" . $array['transaksi_id'] . "')");
                } else {
                    $tr->addFilter("transaksi_id='0'");
                }
                if (isset($array['cabang_id']) && $array['cabang_id'] > 0) {
                    $tr->addFilter("cabang_id='" . $array['cabang_id'] . "'");
                }
                break;
            case "update":
                $tr->addFilter("id='" . $array['id'] . "'");
